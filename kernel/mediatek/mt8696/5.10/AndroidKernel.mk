...
ifneq ($(filter $(TARGET_DEVICE),karat kara),)
...
$(GEN_KERNEL_BUILD_CONFIG): $(KERNEL_DIR)/scripts/gen_build_config.py $(KERNEL_DIR)/build.config.common $(build_config_file) $(KERNEL_CONFIG_FILE) $(LOCAL_PATH)/AndroidKernel.mk
	$(hide) python $< --kernel-defconfig $(KERNEL_DEFCONFIG) -m $(TARGET_BUILD_VARIANT) -o $@

MY_REL_KERNEL_OUT := $(REL_KERNEL_OUT)/../../../
$(TARGET_KERNEL_CONFIG): PRIVATE_KERNEL_OUT := $(MY_REL_KERNEL_OUT)
...
$(TARGET_KERNEL_CONFIG): kernel/mediatek/5.10/build/build.sh $(GEN_KERNEL_BUILD_CONFIG) $(KERNEL_MAKE_DEPENDENCIES)
	$(hide) mkdir -p $(dir $@)
	$(hide) CC_WRAPPER=$(PRIVATE_CC_WRAPPER) SKIP_MRPROPER=1 BUILD_CONFIG=$(PRIVATE_KERNEL_BUILD_CONFIG) OUT_DIR=$(PRIVATE_KERNEL_OUT) DIST_DIR=$(PRIVATE_DIST_DIR) POST_DEFCONFIG_CMDS="exit 0" ./kernel/mediatek/5.10/build/build.sh

$(warning ================== MY_REL_KERNEL_OUT =$(MY_REL_KERNEL_OUT))

#ifeq (yes,$(strip $(BUILD_KERNEL)))
.KATI_RESTAT: $(KERNEL_ZIMAGE_OUT)
$(KERNEL_ZIMAGE_OUT): PRIVATE_DIR := $(KERNEL_DIR)
$(KERNEL_ZIMAGE_OUT): PRIVATE_KERNEL_OUT := $(MY_REL_KERNEL_OUT)
...
$(KERNEL_ZIMAGE_OUT): $(TARGET_KERNEL_CONFIG) $(KERNEL_MAKE_DEPENDENCIES)
	$(hide) mkdir -p $(dir $@)
	$(hide) CC_WRAPPER=$(PRIVATE_CC_WRAPPER) SKIP_MRPROPER=1 BUILD_CONFIG=$(PRIVATE_KERNEL_BUILD_CONFIG) OUT_DIR=$(PRIVATE_KERNEL_OUT) DIST_DIR=$(PRIVATE_DIST_DIR) SKIP_DEFCONFIG=1 ./kernel/mediatek/5.10/build/build.sh
...
include device/mediatek/build/core_mt8696/build_dtbimage.mk

MTK_DTBOIMAGE_DTS := $(addsuffix .dts,$(addprefix $(KERNEL_DIR)/arch/$(KERNEL_TARGET_ARCH)/boot/dts/,$(PROJECT_DTB_NAMES)))
include device/mediatek/build/core_mt8696/build_dtboimage.mk
endif #karat

...
static int hdmi_coding_type_info(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
	uinfo->value.integer.min = 0;
	uinfo->value.integer.max = HDMI_AUDIO_CODING_TYPE_CXT;

	return 0;
}

static int hdmi_coding_type_get(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct hdmi_codec_priv *hcp = snd_soc_component_get_drvdata(component);

	memcpy(&ucontrol->value.integer.value[0],
	       &hcp->coding_type, sizeof(hcp->coding_type));

	return 0;
}

static int hdmi_coding_type_put(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	...
	memcpy(&hcp->coding_type, &ucontrol->value.integer.value[0],
		sizeof(hcp->coding_type));
				...
				struct snd_soc_dai *dai)
{
	...
	hp.cea.coding_type = hcp->coding_type;
...
struct snd_kcontrol_new hdmi_codec_controls[] = {
	{
		...
		.name  = "HDMI_CODEC_CODE_TYPE",
		.info  = hdmi_coding_type_info,
		.get   = hdmi_coding_type_get,
		.put   = hdmi_coding_type_put,
	},
	{

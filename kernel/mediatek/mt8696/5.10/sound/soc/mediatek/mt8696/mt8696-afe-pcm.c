	...
	if (reset_out_change) {
		...
		if (!afe->force_clk_on) {
			...
			mt8696_afe_disable_clk(afe,
				afe_priv->clocks[MT8696_CLK_APLL12_DIV3]);
			afe->etdm1_mclk_is_on = false;
			dev_info(dai->dev, "%s disable mclk after shutdown\n",
				__func__);
		}
	}
	...
	if (apply_out_change) {
		...
		if (afe->force_clk_on) {
			if ((afe->cached_hdmi_audio_format == format) &&
				...
				dev_info(dai->dev, "%s no need reset i2s clk if no change!\n",
					__func__);
			} else {
				mt8696_afe_disable_etdm(afe, etdm_set,
					SNDRV_PCM_STREAM_PLAYBACK);
				...
				afe->cached_hdmi_audio_channels = channels;
				afe->cached_hdmi_audio_bit_depth = bit_width;
				dev_info(dai->dev, "%s set audio inf before i2s clock reset!\n",
					__func__);
			}

			if (afe->suspend_toggle) {
				afe->need_hdmi_toggle = true;
				afe->suspend_toggle = false;
				dev_info(dai->dev, "%s i2s clock reset when suspend/resume!\n",
					__func__);
			}
		}
#endif
			  ...
			  struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			afe->hdmi_dac_timestamp = timespec64_to_ns(&ts);
...
static int mt8696_afe_runtime_suspend(struct device *dev)
{
	...
	dev_info(afe->dev, "%s suspend %d %d >>\n", __func__,
	...
	if (afe->force_clk_on)
		dev_info(afe->dev, "%s clock always on no need toggle again!\n",
			__func__);
	else
		mt8696_afe_enable_main_clk(afe);
	...
	if (afe->force_clk_on && afe->etdm1_mclk_is_on) {
		mt8696_afe_disable_etdm(afe, etdm_set,
			SNDRV_PCM_STREAM_PLAYBACK);
		mt8696_afe_disable_top_cg(afe,
			MT8696_TOP_CG_ETDM_OUT1);
		mt8696_afe_disable_clk(afe,
			afe_priv->clocks[MT8696_CLK_APLL12_DIV3]);
		afe->etdm1_mclk_is_on = false;
		dev_info(afe->dev, "%s close APLL1 for audio clock always on case!\n",
			__func__);
	}
	...
	dev_info(afe->dev, "%s suspend(%d) <<\n", __func__, afe->suspended);
...
static int mt8696_afe_runtime_resume(struct device *dev)
{
	...
	if (afe->force_clk_on) {
		afe->suspend_toggle = true;
		dev_info(afe->dev, "%s  trigger toggle when resume after suspend!\n",
			__func__);
	}
...
static int mt8696_afe_suspend(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	if (afe->suspended)
		return 0;
	dev_info(afe->dev, "%s suspend begins >>\n", __func__);

	/* do afe suspend */
	mt8696_afe_runtime_suspend(afe->dev);

	dev_info(afe->dev, "%s suspend ends <<\n", __func__);
	return 0;
}

static int mt8696_afe_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	dev_info(afe->dev, "%s resume begins >>\n", __func__);
	if (!afe->suspended)
		return 0;

	/* do afe resume */
	mt8696_afe_runtime_resume(afe->dev);

	dev_info(afe->dev, "%s resume ends <<\n", __func__);
	return 0;
}
				...
				struct device_node *np)
{
	...
	for (i = 0; i < ARRAY_SIZE(of_fe_table); i++) {
		...
		sret = snprintf(prop, sizeof(prop), "mediatek,%s-use-sram",
			 of_fe_table[i].name);
		if (sret < 0)
			dev_dbg(afe->dev, "%s snprintf error\n", __func__);
...
static int mt8696_afe_pcm_probe(struct snd_soc_component *platform)
{
...
static const struct dev_pm_ops mt8696_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt8696_afe_runtime_suspend,
			   mt8696_afe_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(mt8696_afe_suspend, mt8696_afe_resume)
};

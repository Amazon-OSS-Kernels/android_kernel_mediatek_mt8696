	...
	const char *name)
{
	...
	sret = snprintf(prop, sizeof(prop),
		PREFIX"%s-set-pll-clk-in-hw-params", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	codec_dai_data->set_pll_clk_in_hw_params =
		of_property_read_bool(np, prop);

	sret = snprintf(prop, sizeof(prop), PREFIX"%s-pll-id", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	...
	sret = snprintf(prop, sizeof(prop), PREFIX"%s-pll-src-id", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	ret = of_property_read_u32(np, prop, &val);
	if (ret == 0)
	...
	sret = snprintf(prop, sizeof(prop), PREFIX"%s-pll-freq-in", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	ret = of_property_read_u32(np, prop, &val);
	if (ret == 0)
	...
	sret = snprintf(prop, sizeof(prop), PREFIX"%s-pll-freq-out", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	ret = of_property_read_u32(np, prop, &val);
	if (ret == 0)
	...
	char *name)
{
	...
	sret = snprintf(prop, sizeof(prop), PREFIX"%s-audio-codec-num", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	...
	dai_link->codecs = devm_kzalloc(dev,
		...
		GFP_KERNEL);
	if (!dai_link->codecs) {
		dev_info(pr_dev, "dai_link->codecs is NULL\n");
		return;
	}
	...
	be->codec_dai_data = devm_kzalloc(dev,
		...
		GFP_KERNEL);
	if (!be->codec_dai_data) {
		dev_info(pr_dev, "be->codec_dai_data is NULL\n");
		return;
	}
	...
	for (i = 0; i < num_codecs; i++) {
		...
		if (of_str) {
			...
			if (idx < MAX_CODEC_CONF) {
				...
				if (codec_node)
					codec_conf->dlc.of_node = codec_node;
				else
					codec_conf->dlc.name =
						dai_link->codecs[i].name;
...
single_codec:
	// parse codec_of_node
	sret = snprintf(prop, sizeof(prop), PREFIX"%s-audio-codec", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	codec_node = of_parse_phandle(np, prop, 0);
	if (codec_node) {
	...
	// parse codec prefix
	sret = snprintf(prop, sizeof(prop), PREFIX"%s-audio-codec-prefix", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	of_property_read_string(np, prop, &of_str);
	if (of_str) {
		...
		if (idx < MAX_CODEC_CONF) {
			...
			if (codec_node)
				...
				codec_conf->dlc.name = dai_link->codecs->name;
	...
	// parse codec dai name
	sret = snprintf(prop, sizeof(prop), PREFIX"%s-codec-dai-name", name);
	if (sret < 0)
		dev_dbg(pr_dev, "%s snprintf error\n", __func__);
	of_property_read_string(np, prop, &of_str);
	if (of_str) {
		dai_link->codecs->dai_name = of_str;
				...
				struct device_node *np)
{
	...
	for (i = 0; i < ARRAY_SIZE(of_dai_links_fe); i++) {
		...
		// parse trigger order
		sret = snprintf(prop, sizeof(prop), PREFIX"%s-trigger-order",
			 of_dai_links_fe[i].name);
		if (sret < 0) {
			dev_dbg(card->dev, "%s snprintf error\n",
				__func__);
...
static int mt8696_sound1_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < dais_num; i++) {
		if (mt8696_sound1_dais[i].platforms->name)
			continue;
		...
		if ((id >= DAI_LINK_FE_AFE_BASE &&
			...
			mt8696_sound1_dais[i].platforms->of_node = afe_plat_node;
		}
	}

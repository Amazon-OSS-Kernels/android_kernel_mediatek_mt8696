...
__attribute__((weak)) unsigned int mtk_dramc_get_ddr_type(void)
{
	pr_info("mtk-dvfsrc:%s not ready!", __func__);
	return 0;
}

__attribute__((weak)) unsigned int mtk_dramc_get_data_rate(void)
{
	pr_info("mtk-dvfsrc;%s not ready!", __func__);
	return 0;
}
...
static const int mt8696_regs[] = {
	[DVFSRC_LEVEL] =	0x4A4,
	[DVFSRC_BASIC_CONTROL] = 0x4AC,
	[DVFSRC_SW_REQ] =	0x510,
	[DVFSRC_SW_REQ2] =	0x528,
	[DVFSRC_SW_BW] =	0x160,
	[DVFSRC_LAST] =		0x308,
	[DVFSRC_TARGET_LEVEL] = 0x520,
	[DVFSRC_TARGET_FORCE] = 0x4FC,
};
...
static void mtk_dvfsrc_get_property(struct platform_device *pdev, struct mtk_dvfsrc *dvfsrc)
{
	struct arm_smccc_res ares;
#ifndef CONFIG_MTK_DVFSRC_MET_MT8696
	...
	int ddr_hz = mtk_dramc_get_data_rate();
#endif

#ifdef CONFIG_MTK_DVFSRC_MET_MT8696
	dev_info(dvfsrc->dev, "ddr=[%d][%d]\n", ddr_type, ddr_hz);
	for (i = 0; i < 3U; i++)
		if (ddr_hz == mtk_dramc_get_steps_freq(i))
			boot_opp = i;
	dev_info(dvfsrc->dev, "boot_opp=[%d]\n", boot_opp);

	arm_smccc_smc(MTK_SIP_VCOREFS_CONTROL, MTK_SIP_DVFSRC_INIT,
				      true, boot_opp, 0, 0, 0, 0, &ares);
	dvfsrc->dvfsrc_enable = true;
#else
#ifdef DVFSRC_PROPERTY_ENABLE
	if (of_property_read_u32(np, "dvfsrc,bringup", &is_bringup))
		dev_info(dvfsrc->dev, "dvfsrc undefine is_bringup\n");

	if (of_property_read_u32(np, "dvfsrc_flag", &dvfsrc_flag))
		dev_info(dvfsrc->dev, "dvfsrc undefine dvfsrc_flag\n");

	if (of_property_read_u32(np, "dvfsrc_vmode", &dvfsrc_vmode))
		dev_info(dvfsrc->dev, "dvfsrc undefine dvfsrc_vmode\n");
...
static int mtk_dvfsrc_probe(struct platform_device *pdev)
{
	...
	mtk_dvfsrc_get_property(pdev, dvfsrc);
...
static const struct dvfsrc_opp dvfsrc_opp_mt8696_lp4[] = {
	{0, 0}, {0, 1},
};

static const struct dvfsrc_opp_desc dvfsrc_opp_mt8696_desc[] = {
	DVFSRC_OPP_DESC(dvfsrc_opp_mt8696_lp4),
};

static int mt8696_get_target_level(struct mtk_dvfsrc *dvfsrc)
{
	u32 rc_info;

	rc_info = dvfsrc_read(dvfsrc, DVFSRC_TARGET_LEVEL);
	return rc_info;
}

static int mt8696_get_current_level(struct mtk_dvfsrc *dvfsrc)
{
	u32 spm_dvfs_level;

	spm_dvfs_level = dvfsrc_read(dvfsrc, DVFSRC_LEVEL);
	if (spm_dvfs_level <= 1U)
		spm_dvfs_level = 0;
	else
		spm_dvfs_level = spm_dvfs_level - 1U;
	dev_dbg(dvfsrc->dev, "spm_dvfs_level=%d\n", spm_dvfs_level);
	return spm_dvfs_level;
}

static u32 mt8696_get_vcore_level(struct mtk_dvfsrc *dvfsrc)
{
	u32 vcore_level;

	vcore_level = (dvfsrc_read(dvfsrc, DVFSRC_LEVEL) >> 16) & 0x3;
	return vcore_level;
}


static void mt8696_set_dram_bw(struct mtk_dvfsrc *dvfsrc, u64 bw)
{
	dev_dbg(dvfsrc->dev, "%s\n", __func__);
}

static void mt8696_set_opp_level(struct mtk_dvfsrc *dvfsrc, u32 level)
{
	dev_dbg(dvfsrc->dev, "%s\n", __func__);
}

static void mt8696_set_vcore_level(struct mtk_dvfsrc *dvfsrc, u32 level)
{
	dev_dbg(dvfsrc->dev, "%s\n", __func__);
}

static void mt8696_set_dram_level(struct mtk_dvfsrc *dvfsrc, u32 level)
{
	spin_lock(&dvfsrc->req_lock);
	if (level >= 1U)
		level = 2;
	...
	dev_dbg(dvfsrc->dev, "set dram level=%d\n", level);
	dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 0, 0x1, 4);
	dvfsrc_write(dvfsrc, DVFSRC_SW_REQ, level);
	dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 1, 0x1, 4);
	spin_unlock(&dvfsrc->req_lock);
}

static u32 mt8696_get_dram_level(struct mtk_dvfsrc *dvfsrc)
{
	u32 dram_dvfs_level;

	dram_dvfs_level = dvfsrc_read(dvfsrc, DVFSRC_LEVEL);

	if (dram_dvfs_level <= 1U)
		dram_dvfs_level = 0;
	else
		dram_dvfs_level = dram_dvfs_level - 1U;

	dev_dbg(dvfsrc->dev, "dram_dvfs_level=%d\n", dram_dvfs_level);
	return dram_dvfs_level;
}

static int mt8696_wait_for_opp_level(struct mtk_dvfsrc *dvfsrc, u32 level)
{
	const struct dvfsrc_opp *target, *curr;
	int ret;
...
#ifdef DVFSRC_FORCE_OPP_SUPPORT
static void mt8696_set_force_opp_level(struct mtk_dvfsrc *dvfsrc, u32 level)
{
	int enable = 1;
	unsigned long flags;
	...
	if (enable) {
		dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 0, 0x1, 4);
		dvfsrc_write(dvfsrc, DVFSRC_TARGET_FORCE, level);
		dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 1, 0x1, 2);
		dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 1, 0x1, 4);
	} else {
		dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 0, 0x1, 4);
		dvfsrc_write(dvfsrc, DVFSRC_TARGET_FORCE, 0x0);
		dvfsrc_write(dvfsrc, DVFSRC_SW_REQ2, 0x1);
		dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 0, 0x1, 2);
		dvfsrc_rmw(dvfsrc, DVFSRC_BASIC_CONTROL, 1, 0x1, 4);
	}
	spin_unlock_irqrestore(&dvfsrc->force_lock, flags);
}
...
static const struct dvfsrc_soc_data mt8696_data = {
	.opps_desc = dvfsrc_opp_mt8696_desc,
	.num_opp_desc = ARRAY_SIZE(dvfsrc_opp_mt8696_desc),
	.regs = mt8696_regs,
	.get_target_level = mt8696_get_target_level,
	.get_current_level = mt8696_get_current_level,
	.get_vcore_level = mt8696_get_vcore_level,
	.set_dram_bw = mt8696_set_dram_bw,
	.set_dram_level = mt8696_set_dram_level,
	.get_dram_level = mt8696_get_dram_level,
	.set_opp_level = mt8696_set_opp_level,
	.set_vcore_level = mt8696_set_vcore_level,
#ifdef DVFSRC_FORCE_OPP_SUPPORT
	.set_force_opp_level = mt8696_set_force_opp_level,
#endif
	.wait_for_opp_level = mt8696_wait_for_opp_level,
	.wait_for_vcore_level = dvfsrc_wait_for_vcore_level,
	.wait_for_dram_level = dvfsrc_wait_for_dram_level
};
...
static const struct of_device_id mtk_dvfsrc_of_match[] = {
	{
		...
		.compatible = "mediatek,mt8696-dvfsrc",
		.data = &mt8696_data,
	}, {
		/* sentinel */
	},
...
late_initcall_sync(mtk_dvfsrc_init);

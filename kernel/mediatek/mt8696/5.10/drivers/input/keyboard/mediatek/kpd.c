...
{
	pr_info("Power Key generate, pressed=%ld\n", pressed);
...
static int mt_kpd_debugfs(void)
{
	...
	debugfs_create_u32("debug", 0600, kpd_droot, &kpd_klog_en);
	return 0;
}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	if (!kpd_input_dev) {
		pr_info("input allocate device fail.\n");
		return -ENOMEM;
	}
...
#ifdef CONFIG_PM_WAKELOCKS
	kpd_suspend_lock = wakeup_source_create("kpd wakelock");
#else

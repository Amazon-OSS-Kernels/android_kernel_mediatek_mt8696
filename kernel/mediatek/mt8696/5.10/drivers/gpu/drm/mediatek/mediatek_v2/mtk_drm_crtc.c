...
static void mtk_drm_crtc_attach_property(struct drm_crtc *crtc)
{
	...
	for (i = 0; i < CRTC_PROP_MAX; i++) {
		...
		crtc_prop = &(mtk_crtc_property[i]);
		if (!prop) {
			DDPINFO("%s(crtc-%d): attach property %s = %d\n",
			     ...
			     unsigned int pipe)
{
	...
	DDPINFO("%s(crtc-%d)+\n", __func__, pipe);
	DDPINFO("%s(crtc-%d): mtk_crtc->base = %x\n", __func__, pipe, mtk_crtc->base);
	DDPINFO("%s(crtc-%d): primary plane  = %x\n", __func__, pipe, primary);
	DDPINFO("%s(crtc-%d): curosr  plane  = %x\n", __func__, pipe, cursor);
			...
			const struct mtk_crtc_path_data *path_data)
{
	...
	for (i = 0; i < DDP_PATH_NR; i++) {
		...
		if ((pipe == 1) && (priv->side_config_regs != NULL))
			mtk_crtc->mutex[i] = mtk_disp_mutex_get(
				priv->mutex_dev, pipe, i);
		else
			mtk_crtc->mutex[i] = mtk_disp_mutex_get(
				priv->mutex_dev, pipe, pipe * DDP_PATH_NR + i);
	...
	enum mtk_ddp_comp_id comp_id)
{
	...
	if (mtk_crtc->dispsys_num > 1 &&
		...
		comp_id >= 0 && comp_id < DDP_COMPONENT_ID_MAX &&

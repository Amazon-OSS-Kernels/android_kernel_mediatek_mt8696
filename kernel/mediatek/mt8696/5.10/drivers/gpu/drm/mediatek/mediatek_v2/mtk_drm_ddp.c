...
#define MT8188_VDO0_DL_RELAY0_CFG_WD 0x240
#define MT8188_VDO0_DL_RELAY1_CFG_WD 0x244
#define MT8188_VDO0_DL_RELAY2_CFG_WD 0x248
#define MT8188_VDO0_DL_RELAY3_CFG_WD 0x24C
#define MT8188_VDO0_DL_RELAY4_CFG_WD 0x250
...
struct mtk_ddp {
	...
	struct mtk_disp_mutex side_mutex[10];
...
static const unsigned long mt8188_mutex_mod[DDP_COMPONENT_ID_MAX] = {
		...
		/* To do: Across subsys mutex setting */
		/* MT8188_MUTEX_MOD_DISP_VPP1_DL_RELAY0 */
		/* MT8188_MUTEX_MOD_DISP_VPP1_DL_RELAY1 */
			      ...
			      enum mtk_ddp_comp_id next)
{
	...
	DDPDBG("%s(crtc-%d): curr=%d, prev=%d next=%d, reg=%x\n",
				   ...
				   enum mtk_ddp_comp_id next)
{
	...
	DDPDBG("%s(crtc-%d): reg=%x, curr=%d, next=%d\n",
				   ...
				   struct mtk_disp_mutex *mutex)
{
	...
	ddp = mtk_crtc->use_second_mutex ?
		container_of(mutex, struct mtk_ddp, side_mutex[mutex->id]) :
	...
	curr_mutex = mtk_crtc->use_second_mutex ?
		&ddp->side_mutex[mutex->id] :
		&ddp->mutex[mutex->id];

	if (curr_mutex != mutex) {
		DDPAEE("%s(): invalid mutex[%d] 0x%x, should be 0x%x\n",
	...
	unsigned int id)
{
	...
	DDPDBG("%s(crtc-%d): id=%d\n", __func__, pipe, id);

	if (id >= 10)
		return ERR_PTR(-EINVAL);

	mutex = (pipe == 1) ? ddp->side_mutex : ddp->mutex;
	...
	mutex[id].claimed = true;

	DDPDBG("%s(crtc-%d): return 0x%x\n", __func__, pipe, &mutex[id]);
				...
				enum mtk_ddp_comp_id id)
{
	...
	DDPDBG("%s(crtc-%d): mutex of the component = %x (mod: %d)\n",
...
static int mtk_ddp_probe(struct platform_device *pdev)
{
	...
	DDPDBG("%s(): ddp->side_regs_pa = %x\n", __func__, ddp->side_regs_pa);
	DDPDBG("%s(): ddp->side_regs = %x\n", __func__, ddp->side_regs);

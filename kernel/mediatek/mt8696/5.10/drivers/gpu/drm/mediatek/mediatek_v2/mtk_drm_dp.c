...
static void mtk_dp_mn_overwrite(struct mtk_dp *mtk_dp, bool enable,
				u32 video_m, u32 video_n)
{
	if (enable) {
		mtk_dp_write(mtk_dp, MTK_DP_ENC0_P0_3008,
			video_m & VIDEO_M_CODE_SW_0_DP_ENC0_P0_MASK);
		mtk_dp_write(mtk_dp, MTK_DP_ENC0_P0_300C,
			((video_m >> 16) &
			VIDEO_M_CODE_SW_1_DP_ENC0_P0_MASK));

		mtk_dp_write(mtk_dp, MTK_DP_ENC0_P0_3044,
			video_n & VIDEO_N_CODE_0_DP_ENC0_P0_MASK);
		mtk_dp_write(mtk_dp, MTK_DP_ENC0_P0_3048,
			(video_n >> 16) & VIDEO_N_CODE_1_DP_ENC0_P0_MASK);
		mtk_dp_write(mtk_dp, MTK_DP_ENC0_P0_3050,
			video_n & VIDEO_N_CODE_MN_GEN_0_DP_ENC0_P0_MASK);
		mtk_dp_write(mtk_dp, MTK_DP_ENC0_P0_3054,
			(video_n >> 16) &
			VIDEO_N_CODE_MN_GEN_1_DP_ENC0_P0_MASK);

		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_3004,
			   (1 << VIDEO_M_CODE_SEL_DP_ENC0_P0_SHIFT),
...
static void mtk_dp_mn_set(struct mtk_dp *mtk_dp)
{
	u32 mvid = 0;
	u32 htt, htt_src;

	mtk_dp_mn_overwrite(mtk_dp, FALSE, mvid, MTK_DP_MSA_NVID);
	htt = mtk_dp->info.timings.htotal;
	...
	mvid = mtk_dp_read(mtk_dp, MTK_DP_ENC1_P0_33C8);
	mvid = mvid * htt / htt_src;
	mtk_dp_mn_overwrite(mtk_dp, TRUE, mvid, MTK_DP_MSA_NVID);
}
...
static bool mtk_dp_plug_state(struct mtk_dp *mtk_dp)
{
	return ((!!(mtk_dp_read(mtk_dp, MTK_DP_TRANS_P0_3414) &
		  HPD_DB_DP_TRANS_P0_MASK)) | mtk_dp->hpd_sts);
}
...
{
	mtk_dp_bulk_16bit_write(mtk_dp, MTK_DP_ENC1_P0_3200, data_bytes, 0x20);
}
			     ...
			     struct mtk_dp_sdp_packet *packet)
{
	...
	case MTK_DP_SDP_PPS3:
		...
		mtk_dp_update_bits(mtk_dp, mtk_dp_sdp_type_to_reg[packet->type] & 0xfffc,
					  ...
					  int swing_val, int preemphasis)
{
	...
	dev_dbg(mtk_dp->dev, "link training swing_val= 0x%x, preemphasis = 0x%x\n",
...
void mtk_dp_initialize_settings(struct mtk_dp *mtk_dp)
{
	...
	mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_304C, 1,
...
static int mtk_dp_get_calibration_data(struct mtk_dp *mtk_dp)
{
	...
	if (IS_ERR(cell)) {
		dev_err(dev, "Error: Failed to get nvmem cell dp_calibration_data\n");
		return PTR_ERR(cell);
	}
	...
	if (IS_ERR(buf) || ((len / sizeof(u32)) != 4)) {
		dev_err(dev, "Error: Failed to read nvmem_cell_read fail len %d\n",
			(len / sizeof(u32)));
		return PTR_ERR(buf);
	}

	if (mtk_dp_is_edp(mtk_dp)) {
		cal_data->glb_bias_trim = check_cal_data_valid(1, 0x1e, (buf[3] >> 27) & 0x1f, 0xf);
		cal_data->clktx_impse = check_cal_data_valid(1, 0xe, (buf[0] >> 9) & 0xf, 0x8);
		cal_data->ln0_tx_impsel_pmos = check_cal_data_valid(1, 0xe,
			(buf[2] >> 28) & 0xf, 0x8);
		cal_data->ln0_tx_impsel_nmos = check_cal_data_valid(1, 0xe,
			(buf[2] >> 24) & 0xf, 0x8);
		cal_data->ln1_tx_impsel_pmos = check_cal_data_valid(1, 0xe,
			(buf[2] >> 20) & 0xf, 0x8);
		cal_data->ln1_tx_impsel_nmos = check_cal_data_valid(1, 0xe,
			(buf[2] >> 16) & 0xf, 0x8);
		cal_data->ln2_tx_impsel_pmos = check_cal_data_valid(1, 0xe,
			(buf[2] >> 12) & 0xf, 0x8);
		cal_data->ln2_tx_impsel_nmos = check_cal_data_valid(1, 0xe,
			(buf[2] >> 8) & 0xf, 0x8);
		cal_data->ln3_tx_impsel_pmos = check_cal_data_valid(1, 0xe,
			(buf[2] >> 4) & 0xf, 0x8);
		cal_data->ln3_tx_impsel_nmos = check_cal_data_valid(1, 0xe, buf[2] & 0xf, 0x8);
	} else {
		cal_data->glb_bias_trim = check_cal_data_valid(1, 0x1e, (buf[0] >> 27) & 0x1f, 0xf);
		cal_data->clktx_impse = check_cal_data_valid(1, 0xe, (buf[0] >> 13) & 0xf, 0x8);
		cal_data->ln0_tx_impsel_pmos = check_cal_data_valid(1, 0xe,
			(buf[1] >> 28) & 0xf, 0x8);
		cal_data->ln0_tx_impsel_nmos = check_cal_data_valid(1, 0xe,
			(buf[1] >> 24) & 0xf, 0x8);
...
void mtk_dp_set_cal_data(struct mtk_dp *mtk_dp)
{
	...
	mtk_dp_update_bits(mtk_dp, DP_PHY_GLB_DPAUX_TX, cal_data->clktx_impse << 20,
		RG_CKM_PT0_CKTX_IMPSEL);
	mtk_dp_update_bits(mtk_dp, DP_PHY_GLB_BIAS_GEN_00, cal_data->glb_bias_trim << 16,
		RG_XTP_GLB_BIAS_INTR_CTRL);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_0, cal_data->ln0_tx_impsel_pmos << 12,
		RG_XTP_LN0_TX_IMPSEL_PMOS);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_0, cal_data->ln0_tx_impsel_nmos << 16,
		RG_XTP_LN0_TX_IMPSEL_NMOS);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_1, cal_data->ln1_tx_impsel_pmos << 12,
		RG_XTP_LN1_TX_IMPSEL_PMOS);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_1, cal_data->ln1_tx_impsel_nmos << 16,
		RG_XTP_LN1_TX_IMPSEL_NMOS);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_2, cal_data->ln2_tx_impsel_pmos << 12,
		RG_XTP_LN2_TX_IMPSEL_PMOS);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_2, cal_data->ln2_tx_impsel_nmos << 16,
		RG_XTP_LN2_TX_IMPSEL_NMOS);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_3, cal_data->ln3_tx_impsel_pmos << 12,
		RG_XTP_LN3_TX_IMPSEL_PMOS);
	mtk_dp_update_bits(mtk_dp, DP_PHY_LANE_TX_3, cal_data->ln3_tx_impsel_nmos << 16,
...
static void mtk_dp_power_enable(struct mtk_dp *mtk_dp)
{
	...
	mtk_dp_update_bits(mtk_dp, MTK_DP_TOP_RESET_AND_PROBE, SW_RST_B_PHYD,
			   SW_RST_B_PHYD);
	mtk_dp_write(mtk_dp, MTK_DP_TOP_PWR_STATE, DP_PWR_STATE_BANDGAP_TPLL);
}
					 ...
					 u32 sram_read_start)
{
	...
	sdp_down_cnt_init = (u32)(sram_read_start *
				mtk_dp->train_info.lane_count * 2 *
				mtk_dp->train_info.link_rate * 2700 * 8) /
				mtk_dp->info.timings.pix_rate_khz;

	if (mtk_dp->info.format == MTK_DP_COLOR_FORMAT_YUV_420)
		sdp_down_cnt_init = sdp_down_cnt_init / 2;
	switch (mtk_dp->train_info.lane_count) {
	case 1:
		sdp_down_cnt_init = (sdp_down_cnt_init > 0x1E) ?
			sdp_down_cnt_init : 0x1E;
		break;
	case 2:
		sdp_down_cnt_init = (sdp_down_cnt_init > 0x14) ?
			sdp_down_cnt_init : 0x14;
		break;
	case 4:
	default:
...
static void mtk_dp_sdp_set_down_cnt_init_in_hblank(struct mtk_dp *mtk_dp)
{
	...
	sdp_down_cnt_init = (u32)((mtk_dp->info.timings.htotal -
	...
	mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_3364, sdp_down_cnt_init,
			   SDP_DOWN_CNT_INIT_IN_HBLANK_DP_ENC1_P0_MASK);
}

static void mtk_dp_audio_sample_arrange(struct mtk_dp *mtk_dp, bool enable)
{
	u32 value = 0;
	...
	value = (mtk_dp->info.timings.htotal-mtk_dp->info.timings.vm.hactive) *
			mtk_dp->train_info.link_rate * 27 * 200 /
			mtk_dp->info.timings.pix_rate_khz;

	if (enable) {
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_3374, BIT(12), BIT(12));
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_3374, (u16)value, 0xFFF);
	} else {
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_3374, 0, BIT(12));
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_3374, 0, 0xFFF);
	}
...
static void mtk_dp_setup_tu(struct mtk_dp *mtk_dp)
{
	...
	mtk_dp_audio_sample_arrange(mtk_dp, TRUE);
...
{
	mtk_dp_update_bits(mtk_dp, MTK_DP_TOP_PWR_STATE, 0,
				   DP_PWR_STATE_MASK);
	mtk_dp_update_bits(mtk_dp, DP_PHY_DIG_PLL_CTL_1, 0,
				   RG_TPLL_SSC_EN);

	if (enable) {
		mtk_dp_update_bits(mtk_dp, DP_PHY_DIG_PLL_CTL_1, RG_TPLL_SSC_EN,
						RG_TPLL_SSC_EN);
	}
	mtk_dp_update_bits(mtk_dp, MTK_DP_TOP_PWR_STATE, DP_PWR_STATE_MASK,
				   DP_PWR_STATE_MASK);
}
			     ...
			     int target_lane_count)
{
	...
	dev_dbg(mtk_dp->dev, "Link train target_link_rate = 0x%x, target_lane_count = 0x%x\n",
...
static void mtk_dp_ssc_set_capabilities(struct mtk_dp *mtk_dp)
{
	if (mtk_dp->train_info.sink_ssc) {
		drm_dp_dpcd_writeb(&mtk_dp->aux, DP_DOWNSPREAD_CTRL,
					   DP_SPREAD_AMP_0_5);
	} else {
		drm_dp_dpcd_writeb(&mtk_dp->aux, DP_DOWNSPREAD_CTRL, 0);
	}
}
...
static void mtk_dp_dsc_capabilities(struct mtk_dp *mtk_dp)
{
	drm_dp_dpcd_read(&mtk_dp->aux, DP_DSC_SUPPORT, mtk_dp->dsc_cap,
			 DP_DSC_RECEIVER_CAP_SIZE);
	mtk_dp->has_dsc = !!(mtk_dp->dsc_cap[0] & DP_DSC_DECOMPRESSION_IS_SUPPORTED);
}
...
static bool mtk_dp_parse_capabilities(struct mtk_dp *mtk_dp)
{
	...
	if (mtk_dp->has_fec)
		mtk_dp_dsc_capabilities(mtk_dp);
	...
	mtk_dp_ssc_set_capabilities(mtk_dp);
						...
						struct mtk_dp_audio_cfg *cfg)
{
	...
	for (i = 0; i < sad_count; i++) {
		...
		sample_rate = 0;//drm_cea_sad_get_sample_rate(&sads[i]);
		word_length = 0;
...
static int mtk_dp_train_start(struct mtk_dp *mtk_dp)
{
	...
	if (mtk_dp->dsc_en)
		mtk_dp->train_info.lane_count =
			min(MTK_DP_MAX_LANES_DSC, lane_count);
	else
		mtk_dp->train_info.lane_count =
...
static void mtk_dp_dsc_dpcd_set(struct mtk_dp *mtk_dp, bool en)
{
	drm_dp_dpcd_writeb(&mtk_dp->aux, en, DP_DECOMPRESSION_EN);
}

static void mtk_dp_dsc_set_pps(struct mtk_dp *mtk_dp)
{
	struct mtk_dp_sdp_packet packet;
	u8 hb[4] = {0x0, 0x10, 0x7F, 0x0};

	memcpy(&packet.sdp.sdp_header, hb, sizeof(packet.sdp.sdp_header));
	packet.type = MTK_DP_SDP_PPS0;
	memcpy(&packet.sdp.db, &mtk_dp->pps[0], sizeof(packet.sdp.db));
	mtk_dp_setup_sdp(mtk_dp, &packet);
	packet.type = MTK_DP_SDP_PPS1;
	memcpy(&packet.sdp.db, &mtk_dp->pps[32], sizeof(packet.sdp.db));
	mtk_dp_setup_sdp(mtk_dp, &packet);
	packet.type = MTK_DP_SDP_PPS2;
	memcpy(&packet.sdp.db, &mtk_dp->pps[64], sizeof(packet.sdp.db));
	mtk_dp_setup_sdp(mtk_dp, &packet);
	packet.type = MTK_DP_SDP_PPS3;
	memcpy(&packet.sdp.db, &mtk_dp->pps[96], sizeof(packet.sdp.db));
	mtk_dp_setup_sdp(mtk_dp, &packet);
}

static void mtk_dp_set_chunk_size(struct mtk_dp *mtk_dp, u8 slice_num,
				u16 chunk_num, u8 remainder, u8 lane_count,
				u8 hde_last_num, u8 hde_num_even)
{

	mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_336C,
			   slice_num << DSC_SLICE_NUM_DP_ENC1_P0_SHIFT,
			   DSC_SLICE_NUM_DP_ENC1_P0_MASK);
	mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_336C,
			   remainder << DSC_CHUNK_REMAINDER_DP_ENC1_P0_SHIFT,
			   DSC_CHUNK_REMAINDER_DP_ENC1_P0_MASK);
	mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_3370,
			   chunk_num,
			   DSC_CHUNK_NUM_DP_ENC1_P0_MASK);

	mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_31FC,
		hde_last_num << VID_DE_LAST_NUM0_SW_DP_ENC0_P0_SHIFT,
		VID_DE_LAST_NUM0_SW_DP_ENC0_P0_MASK);
	mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_31FC,
		hde_num_even << HDE_NUM_EVEN_EN_SW_L0_DP_ENC0_P0_SHIFT,
		HDE_NUM_EVEN_EN_SW_L0_DP_ENC0_P0_MASK);
	if (lane_count == 2) {
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_31FC,
			hde_last_num << VID_DE_LAST_NUM1_SW_DP_ENC0_P0_SHIFT,
			VID_DE_LAST_NUM1_SW_DP_ENC0_P0_MASK);
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_31FC,
			hde_num_even << HDE_NUM_EVEN_EN_SW_L1_DP_ENC0_P0_SHIFT,
			HDE_NUM_EVEN_EN_SW_L1_DP_ENC0_P0_MASK);
	}
}

static void mtk_dp_dsc_set_param(struct mtk_dp *mtk_dp)
{
	u16 chunk_num, pic_width, slice_width;
	u8 r8, r16;
	u8 hde_last_num, hde_num_even, slice_num;
	u8 lane_count = mtk_dp->train_info.lane_count;
	u8 q16[16] = {0b110, 0b001, 0b001, 0b011,
		      0b011, 0b101, 0b101, 0b111,
		      0b111, 0b000, 0b000, 0b010,
		      0b010, 0b100, 0b100, 0b110};
	u8 q8[8] = {0b110, 0b001, 0b011, 0b101,
		    0b111, 0b000, 0b010, 0b100};

	chunk_num = (mtk_dp->pps[14] << 8) + mtk_dp->pps[15];
	pic_width = (mtk_dp->pps[8] << 8) + mtk_dp->pps[9];
	slice_width = (mtk_dp->pps[12] << 8) + mtk_dp->pps[13];
	slice_num = pic_width / slice_width;

	if (lane_count == MTK_DP_2LANE) {
	...
	mtk_dp_set_chunk_size(mtk_dp, slice_num-1, chunk_num, chunk_num%12,
			      lane_count, hde_last_num, hde_num_even);
}

static void mtk_dp_dsc_pps_send(struct mtk_dp *mtk_dp)
{
	mtk_dp_dsc_set_pps(mtk_dp);
	mtk_dp_dsc_set_param(mtk_dp);
}

static void mtk_dp_dsc_en_set(struct mtk_dp *mtk_dp, bool enable)
{
	if (enable) {
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC1_P0_336C,
				   DSC_EN_DP_ENC1_P0_MASK,
				   DSC_EN_DP_ENC1_P0_MASK);
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_300C,
				   COMPRESSEDSTREAM_FLAG_DP_ENC0_P0_MASK,
				   COMPRESSEDSTREAM_FLAG_DP_ENC0_P0_MASK);
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_303C,
				   VIDEO_COLOR_DEPTH_DP_ENC0_P0_DSC,
				   VIDEO_COLOR_DEPTH_DP_ENC0_P0_MASK);
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_303C,
				   PIXEL_ENCODE_FORMAT_DP_ENC0_P0_DSC,
				   PIXEL_ENCODE_FORMAT_DP_ENC0_P0_MASK);
		mtk_dp_update_bits(mtk_dp, MTK_DP_ENC0_P0_31FC,
...
{
	if ((pps_buf != NULL) && (size == 128)) {
		memcpy(g_mtk_dp->pps, pps_buf, sizeof(g_mtk_dp->pps));
		g_mtk_dp->dsc_en = dsc_en & g_mtk_dp->has_dsc;
	} else {
		g_mtk_dp->dsc_en = 0;
	}
}
...
{
	mtk_dp_mn_set(mtk_dp);

	if (mtk_dp->dsc_en)
		mtk_dp_dsc_dpcd_set(mtk_dp, true);
	...
	if (mtk_dp->dsc_en) {
		mtk_dp_dsc_pps_send(mtk_dp);
		mtk_dp_dsc_en_set(mtk_dp, true);
	}
}
...
static irqreturn_t mtk_dp_isr_event_thread(int hpd, void *dev)
{
	...
	if (mtk_dp->train_info.cable_state_change) {
		...
		if (event == connector_status_connected) {
			...
			notify_uevent_user(&dptx_notify_data,
					DPTX_STATE_NO_DEVICE);
			extcon_set_state_sync(dptx_extcon, EXTCON_DISP_HDMI, false);
		}

		if (!mtk_dp->train_info.cable_plugged_in || !mtk_dp_plug_state(mtk_dp)) {
			mtk_dp_video_mute(mtk_dp, true);
			mtk_dp_audio_mute(mtk_dp, true);

			mtk_dp_set_idle_pattern(mtk_dp, true);
			if (mtk_dp->dsc_en)
				mtk_dp_dsc_en_set(mtk_dp, false);
			if (mtk_dp->has_fec)
				mtk_dp_fec_enable(mtk_dp, false);
			mtk_dp_ssc_enable(mtk_dp, false);
			mtk_dp_sdp_stop_sending(mtk_dp);

			mtk_dp_edid_free(mtk_dp);
			mtk_dp_update_bits(mtk_dp, MTK_DP_TOP_PWR_STATE, DP_PWR_STATE_BANDGAP_TPLL,
					   DP_PWR_STATE_MASK);
			mtk_dp_initialize_priv_data(mtk_dp);
		} else {
			mtk_dp_update_bits(mtk_dp, MTK_DP_TOP_PWR_STATE,
...
static irqreturn_t mtk_dp_hpd_handler(struct mtk_dp *mtk_dp)
{
	bool connected;
	...
	if (mtk_dp_read(mtk_dp, MTK_DP_TRANS_P0_3414) & HPD_OVR_EN_DP_TRANS_P0_MASK)
		hwirq_status = 0;
	...
	} else if (train_info->irq_status & MTK_DP_HPD_DISCONNECT) {
		train_info->irq_status &= ~MTK_DP_HPD_DISCONNECT;
...
static irqreturn_t mtk_dp_isr_event(int hpd, void *dev)
{
	struct mtk_dp *mtk_dp = dev;
	u32 irq_status;
	...
	if (irq_status & RGS_IRQ_STATUS_TRANSMITTER)
		return mtk_dp_hpd_handler(mtk_dp);
	if (irq_status & RGS_IRQ_STATUS_AUXTOP)
		return IRQ_HANDLED;
				    ...
				    struct drm_connector *connector)
{
	...
	do {
		if (mtk_dp_parse_capabilities(mtk_dp))
			break;
		mtk_dp->train_info.check_cap_count++;
	} while ((mtk_dp->train_info.check_cap_count <
			    MTK_DP_CHECK_SINK_CAP_TIMEOUT_COUNT));
	mtk_dp->train_info.check_cap_count = 0;
				   ...
				   struct drm_dp_aux_msg *msg)
{
	...
	case DP_AUX_I2C_READ | DP_AUX_I2C_MOT:
		...
		break;
	default:
		dev_err(mtk_aux->dev, "invalid aux cmd = %d\n",
			msg->request);
		err = -EINVAL;
		goto err;
	}
...
static void mtk_dp_hpd_interrupt_set(struct mtk_dp *mtk_dp, u8 status)
{
	pr_info("[DPTX] status:%d [2:DISCONNECT, 4:CONNECT, 8:IRQ]\n", status);
	pr_info("[DPTX] hpd status:%d\n", mtk_dp->hpd_sts);

	if ((status == MTK_DP_HPD_CONNECT && !mtk_dp->hpd_sts)
		|| (status == MTK_DP_HPD_DISCONNECT && mtk_dp->hpd_sts)
		|| (status == MTK_DP_HPD_INTERRUPT && mtk_dp->hpd_sts)) {
		if (status == MTK_DP_HPD_CONNECT) {
			mtk_dp_init_port(mtk_dp);
			mtk_dp_usbc_hpd(mtk_dp, TRUE);
			mtk_dp->hpd_sts = TRUE;
		} else if (status == MTK_DP_HPD_DISCONNECT) {
...
{
	mutex_lock(&g_mtk_dp->dp_lock);
	mtk_dp_swirq_enable(g_mtk_dp, true);
	mtk_dp_hpd_interrupt_set(g_mtk_dp, status);
	mutex_unlock(&g_mtk_dp->dp_lock);
}
					...
					struct drm_bridge_state *old_state)
{
	...
	mtk_dp->enabled = true;
	mtk_dp_update_plugged_status(mtk_dp);
	extcon_set_state_sync(dptx_extcon, EXTCON_DISP_HDMI, true);
}
						     ...
						     unsigned int *num_input_fmts)
{
	...
	bpp = (display_info->color_formats & DRM_COLOR_FORMAT_YCRCB422) ? 16 : 24;
	rx_linkrate = (u32)mtk_dp->train_info.link_rate * 27000;
	*num_input_fmts = 0;
	input_fmts = kcalloc(ARRAY_SIZE(mt8195_input_fmts), sizeof(*input_fmts), GFP_KERNEL);
	if (!input_fmts)
		return NULL;
	...
	*num_input_fmts = ARRAY_SIZE(mt8195_input_fmts);
	...
	if (((rx_linkrate * mtk_dp->train_info.lane_count) < (mode->clock * 24 / 8)) &&
		((rx_linkrate * mtk_dp->train_info.lane_count) > (mode->clock * 16 / 8)) &&
		...
		struct drm_file *file_priv)
{
	...
	info->displayHeight = timings->vm.vactive;
	info->displayWidth = timings->vm.hactive;
	info->displayMode = DISPIF_MODE_VIDEO;
	info->displayType = DISPLAYPORT;
	info->isConnected = mtk_dp->enabled;
	info->isHwVsyncAvailable = true;
	info->vsyncFPS = timings->pix_rate_khz * 1000 / timings->htotal / timings->vtotal;
	dev_info(mtk_dp->dev, "%s, %d\n", __func__, __LINE__);
		...
		struct drm_file *file_priv)
{
	...
	dev_info(mtk_dp->dev, "audio_enable = %d\n", mtk_dp->audio_enable);
	...
	mtk_dp_audio_mute(mtk_dp, mtk_dp->audio_enable);
		...
		struct drm_file *file_priv)
{
	...
	int channels, sample_rate, word_length_bits;
	struct mtk_dp_audio_cfg *cfg = &mtk_dp->info.audio_caps;

	channels = (tmp >> DP_CAPABILITY_CHANNEL_SFT)
			& DP_CAPABILITY_CHANNEL_MASK;
	sample_rate =  (tmp >> DP_CAPABILITY_SAMPLERATE_SFT)
			  & DP_CAPABILITY_SAMPLERATE_MASK;
	word_length_bits = (tmp >> DP_CAPABILITY_BITWIDTH_SFT)
	...
	cfg->sample_rate = sample_rate;
	cfg->word_length_bits = word_length_bits;
		...
		struct drm_file *file_priv)
{
	...
	if (g_mtk_dp->enabled) {
		switch (cfg->sample_rate) {
		case 32000:
			sample_rate = DP_SAMPLERATE_32;
			break;
		case 44100:
			sample_rate = DP_SAMPLERATE_44;
			break;

		case 48000:
			sample_rate = DP_SAMPLERATE_48;
			break;

		case 96000:
			sample_rate = DP_SAMPLERATE_96;
			break;

		case 192000:
			sample_rate = DP_SAMPLERATE_192;
			break;
		default:
			sample_rate = DP_SAMPLERATE_48;
			break;
		...
		switch (cfg->word_length_bits) {
		case 16:
			word_length_bits = DP_BITWIDTH_16;
			break;
		case 20:
			word_length_bits = DP_BITWIDTH_20;
			break;
		case 24:
			word_length_bits = DP_BITWIDTH_24;
			break;
		default:
			word_length_bits = DP_BITWIDTH_24;
			break;
		...
		switch (cfg->channels) {
		case 2:
			channels = DP_CHANNEL_2;
			break;
		case 8:
			channels = DP_CHANNEL_8;
			break;
		default:
			channels = DP_CHANNEL_2;
			break;
		...
		*dp_cap = channels | (sample_rate << DP_CAPABILITY_SAMPLERATE_SFT)
			| (word_length_bits << DP_CAPABILITY_BITWIDTH_SFT);
	}
...
{
	if (!switch_dp_class) {
		switch_dp_class = class_create(THIS_MODULE, "dptxswitch");
		if (IS_ERR(switch_dp_class))
			return PTR_ERR(switch_dp_class);
...
int dptx_uevent_dev_register(struct notify_dev *sdev)
{
	...
	if (!switch_dp_class) {
		...
		if (ret == 0)
			...
			pr_err("create_switch_class fail\n");
			return ret;
		}
	}

	sdev->index = atomic_inc_return(&device_count);
	sdev->dev = device_create(switch_dp_class, NULL,
	...
	if (sdev->dev) {
		...
		pr_err("device create fail,index:0x%x\n", sdev->index);
		ret = -1;
	}

	ret = device_create_file(sdev->dev, &dev_attr_state);
	if (ret < 0) {
		device_destroy(switch_dp_class, MKDEV(0, sdev->index));
...
static int mtk_dp_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_threaded_irq(dev, irq_num, mtk_dp_isr_event,
					mtk_dp_isr_event_thread,
					IRQ_TYPE_LEVEL_HIGH,
					dev_name(dev), mtk_dp);
	if (ret) {
	...
	if (ret) {
		dev_err(dev, "failed to register dptx extcon: %d\n", ret);
		return ret;
	}
...
int mtk_drm_ioctl_enable_dp(struct drm_device *dev, void *data, struct drm_file *file_priv)
{
	struct mtk_dp_enable *dp_enable = data;
	struct mtk_dp *mtk_dp = g_mtk_dp;
	int ret = 0;

	if ((dp_enable == NULL) || (mtk_dp == NULL)) {
		dev_err(mtk_dp->dev, "IOCTL: ERROR!\n");
		return -EFAULT;
	}

	dev_info(mtk_dp->dev, "[DPTX] test %s, enable %s\n", __func__,
		dp_enable->enable ? "TRUE" : "FALSE");

	if (dp_enable->enable == true) {
		if (mtk_dp->dp_ui_enable == true) {
			dev_info(mtk_dp->dev, "[DPTX] DP has already been enabled, return\n");
			return 0;
		}
		mtk_dp->dp_ui_enable = true;
		if (mtk_dp->train_info.cable_plugged_in == true) {
			ret = mtk_dp_train_handler(mtk_dp);
			if (ret) {
				drm_err(mtk_dp->drm_dev, "[DPTX] Train handler failed %d\n", ret);
				return ret;
			}
			ret = mtk_dp_state_handler(mtk_dp);
			if (ret) {
				drm_err(mtk_dp->drm_dev, "[DPTX] State handler failed %d\n", ret);
				return ret;
			}
		}
	} else {
		if (mtk_dp->dp_ui_enable == false) {
			dev_info(mtk_dp->dev, "[DPTX] DP has already been disabled, return\n");
			return 0;
		}		mtk_dp->dp_ui_enable = false;
		notify_uevent_user(&dptx_notify_data, DPTX_STATE_NO_DEVICE);
	}
...
struct platform_driver mtk_dp_driver = {
	...
	.driver = {
		...
		.of_match_table = of_match_ptr(mtk_dp_of_match),
		.pm = &mtk_dp_pm_ops,
	},
};

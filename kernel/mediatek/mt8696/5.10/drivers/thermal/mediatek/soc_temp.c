...
static int soc_thermal_of_set_trips(void *data, int low, int high ) {
...
static const struct thermal_zone_of_device_ops soc_thermal_ops = {
	...
	.set_trips = soc_thermal_of_set_trips,
};

#ifdef CONFIG_THERMAL_VIRTUAL_SENSOR
static int mtk_soc_get_temp(struct device *dev, int index, int *temp)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct soc_thermal *data = platform_get_drvdata(pdev);
	struct sensor_data *sen_data = data->conf->sen_data;

	soc_thermal_read_temp(sen_data, temp);
	return 0;
}

static struct thermal_dev_ops mtk_soc_dev_ops = {
	.get_temp = mtk_soc_get_temp,
};
#endif
...
#ifdef CONFIG_THERMAL_VIRTUAL_SENSOR
	ret = virtual_sensor_dev_register(&pdev->dev, &mtk_soc_dev_ops, 0);
	if (ret) {

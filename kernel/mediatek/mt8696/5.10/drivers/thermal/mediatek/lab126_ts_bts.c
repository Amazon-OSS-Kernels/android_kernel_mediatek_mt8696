/*
 * drivers/thermal/mediatek/lab126_ts_bts.c
 *
 * ADC NTC BTS driver
 *
...
#define PRIOR_HVT_PULLUP_R 10000
...
#define Proto_KARA_BOARD_REV (0x00)
#define HVT_KARA_BOARD_REV (0x10)
#define EVT_KARA_BOARD_REV (0x20)
#define DVT_KARA_BOARD_REV (0x30)
#define PVT_KARA_BOARD_REV (0x40)
#endif /* CONFIG_IDME */
...
int mtkts_bts_get_hw_temp(int index, int *temp)
{
	...
	if (t_ret > 60000){ /* abnormal high temp */
		pr_info("[Power/BTS_Thermal][%d] T_AP=%d\n", index, t_ret);
	}
...
static int ntc_bts_probe(struct platform_device *pdev)
{
	...
	if (board_rev < 0 || board_rev > PVT_KARA_BOARD_REV) {
		pr_err("%s: board_rev:0x%x out of range\n", __func__, board_rev);
	}
	if (board_rev >= 0 && board_rev < HVT_KARA_BOARD_REV ) {
		bts_channel_param[serial].g_RAP_pull_up_R = PRIOR_HVT_PULLUP_R;
	}
	pr_info("bts device adjusted pullup R: %d\n", bts_channel_param[serial].g_RAP_pull_up_R);
...
static int gadc_thermal_get_temp(void *data, int index, int *temp)
{
	...
	if (ret < 0) {
		dev_err(gti->dev, "IIO channel read channel %d failed %d, use last temp %d\n", index, ret, gti->last_value[index]);
		*temp = gti->last_value[index];
		goto iio_read_fail;
	}
	/* pr_debug("%s: gti->channel[%d] from dt, maps to channel:%d from iio adc device\n",
			__func__, index, channel->channel->channel); */
	// This adjustment is done by MTK auxadc driver now in mt6577_auxadc.c
	//val = val * 1500 / 4096;
	*temp = mtkts_bts_volt_to_temp(index, val);
	gti->last_value[index] = *temp;
...
static int gadc_thermal_probe(struct platform_device *pdev)
{
	...
	gti->dev = &pdev->dev;
	gti->last_value = devm_kzalloc(&pdev->dev, sizeof(*(gti->last_value))*AUX_CHANNEL_NUM, GFP_KERNEL);
	if (!gti->last_value)
		return -ENOMEM;
	...
	for (i = 0; i < AUX_CHANNEL_NUM; i++) {
		ret = iio_read_channel_processed(&gti->channel[i], &val);
		if (ret < 0) {
			dev_err(gti->dev, "IIO channel %d read failed %d, initialize to 0\n", i, ret);
			gti->last_value[i] = 0;
			return ret;
		}
		gti->last_value[i] = val;
	}

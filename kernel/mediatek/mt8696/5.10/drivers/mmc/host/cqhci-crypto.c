				    ...
				    int slot)
{
	...
	if (cq_host->ops->ungate_clock)
		cq_host->ops->ungate_clock(cq_host->mmc);

	if (!(cqhci_readl(cq_host, CQHCI_CFG) & CQHCI_CRYPTO_GENERAL_ENABLE)) {
		arm_smccc_smc(MTK_SIP_KERNEL_HW_MSDC_CTL,
				1 << 0, 0, 0, 0, 0, 0, 0, &smccc_res);
		if (!(cqhci_readl(cq_host, CQHCI_CFG) &
			CQHCI_CRYPTO_GENERAL_ENABLE)) {
			WARN_ON(1);
		}
	}
	...
	if (cq_host->ops->gate_clock)
		cq_host->ops->gate_clock(cq_host->mmc);

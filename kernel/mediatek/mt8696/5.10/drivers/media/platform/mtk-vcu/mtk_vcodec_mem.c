	...
	if (vcu_queue->num_buffers != 0) {
		for (buffer = 0; buffer < vcu_queue->num_buffers; buffer++) {
			...
			if (vcu_buffer->dbuf == NULL)
				...
				dma_buf_put(vcu_buffer->dbuf);
			vcu_queue->bufs[buffer].mem_priv = NULL;
			vcu_queue->bufs[buffer].dbuf = NULL;
	...
	struct vb2_buffer *dst_vb)
{
	...
	if (mem_buff_data->len > CODEC_ALLOCATE_MAX_BUFFER_SIZE ||
		...
		pr_info("Set buffer fail: buffer len = %llu num_buffers = %d !!\n",
	...
	if (dbuf == NULL) {
		mutex_unlock(&vcu_queue->mmap_lock);
		pr_debug("Set buffer not found: buffer len = %llx iova = %llx !!\n",
	...
	pr_debug("[%s, %d] Num_buffers = %x iova = %llx dbuf = %p size = %x mem_priv = %lx, ref_cnt:%x\n",
						 ...
						 struct mem_obj *mem_buff_data)
{
	...
	if (mem_buff_data->len > CODEC_ALLOCATE_MAX_BUFFER_SIZE ||
		...
		pr_info("Get buffer fail: buffer len = %llu num_buffers = %d !!\n",
						...
						struct mem_obj *mem_buff_data)
{
	...
	if (ret != 0)
		pr_info("Can not free memory va %llx pa %llx len %llx!\n",
...
int vcu_buffer_flush_all(struct device *dev, struct mtk_vcu_queue *vcu_queue)
{
	...
	if (num_buffers != 0U) {
		for (buffer = 0; buffer < num_buffers; buffer++) {
			vcu_buffer = &vcu_queue->bufs[buffer];
			pr_debug("Cache flush %s buffer=%d, iova=%lx, size=%d, Q_num = %d\n",
	...
	dma_addr_t dma_addr, size_t size, int op)
{
	...
	for (buffer = 0; buffer < num_buffers; buffer++) {
		...
		if ((dma_addr + size) <=
			...
			pr_debug("Cache %s %s buffer iova=%llx range=%d (%lx %d)\n",
				(op == DMA_TO_DEVICE) ?
				"clean" : "invalidate",
				(vcu_buffer->dbuf == NULL) ?
				"working" : "io",
				(unsigned long)dma_addr, (unsigned int)size,
				(unsigned long long)vcu_buffer->iova,
				(unsigned int)vcu_buffer->size);

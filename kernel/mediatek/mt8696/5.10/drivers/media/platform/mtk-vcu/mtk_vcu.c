...
#define IPI_TIMEOUT_MS          (16000U * 1000)
#else
#define IPI_TIMEOUT_MS          (6000U * 1000)
#endif
#else
#ifdef CONFIG_MTK_ENG_BUILD
...
#define VCU_SHMEM_SIZE 0x100000
...
/* VCU MAP SHMEM should be exactly the same as User Driver MAP SHMEM depends on HW */
...
#define IOMMU_PADDING 0x100000000UL
	...
	u64 addr, u64 data, u32 mask, u32 gpr, u32 dma_offset, u32 dma_size)
{
	...
	if (vcu_check_reg_base(vcu, addr, 4) == 0)
		...
		pr_debug("[VCU] CMD_SEC_WRITE %s addr: 0x%llx, data: 0x%llx, offset: 0x%x, size: 0x%x\n",
...
static void vcu_gce_flush_callback(struct cmdq_cb_data data)
{
	...
	if (atomic_dec_and_test(&vcu->gce_job_cnt[i][core_id]) &&
		...
		if (i == VCU_VENC) {
			if (buff->cmdq_buff.event_type != VENC_SLC_EOF &&
	...
	pr_debug("[VCU] %s: buff %p type %d order %d handle %llx core_id :%d\n",
	...
	struct mtk_vcu_queue *q, unsigned long arg)
{
	...
	ret = down_interruptible(&vcu_ptr->gce_info[gce_idx].buff_sem[i]);
	if (ret < 0) {
		mutex_unlock(&vcu->gce_cmds_mutex[codec_type]);
		return -ERESTARTSYS;
	}
	memcpy(&vcu_ptr->gce_info[gce_idx].buff[i], &buff, sizeof(buff));
	pkt_ptr->err_cb.cb = vcu_gce_timeout_callback;
	pkt_ptr->err_cb.data = (void *)&vcu_ptr->gce_info[gce_idx].buff[i];
	pr_debug("[VCU][%d] %s: buff %p type %d cnt %d order %d hndl %llx %d %d\n",
		core_id, __func__, &vcu_ptr->gce_info[gce_idx].buff[i],
		buff.cmdq_buff.codec_type,
	...
	cmdq_pkt_flush_threaded(pkt_ptr,
		vcu_gce_flush_callback, (void *)&vcu_ptr->gce_info[gce_idx].buff[i]);
		 ...
		 struct vb2_buffer *dst_vb, unsigned long type)
{
	...
	dev_dbg(&pdev->dev, "[VCU] %s %p type %lu src_vb %p dst_vb %p\n",
		 ...
		 void *codec_ctx, unsigned long type)
{
	...
	dev_dbg(&pdev->dev, "[VCU] %s %p type %lu\n", __func__, codec_ctx, type);
			  ...
			  uintptr_t dtcm_dmem_addr)
{
	...
	d_va_start = (uintptr_t)VCU_DMEM0_VIRT(vcu);//probe alloc
...
static int mtk_vcu_open(struct inode *inode, struct file *file)
{
	...
	vcu_queue = mtk_vcu_mem_init(vcu_mtkdev[vcuid]->dev, vcu_mtkdev[vcuid]->clt_venc[0]);
	if (vcu_queue == NULL) {
	...
	pr_info("[VCU] %s name: %s pid %d tgid %d open_cnt %d current %p group_leader %p size %#x\n",
...
static void mtk_vcu_page_vm_open(struct vm_area_struct *vma)
{
	...
	pr_debug("[VCU] %s vma->start 0x%lx, end 0x%lx, pgoff 0x%lx, ref_cnt %d\n",
...
static void mtk_vcu_buf_vm_close(struct vm_area_struct *vma)
{
	...
	pr_debug("[VCU] %s vma->start 0x%lx, end 0x%lx, pgoff 0x%lx\n",
		 __func__, vma->vm_start, vma->vm_end, vma->vm_pgoff);
}
...
static int mtk_vcu_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	case VAL_MEM_TYPE_FOR_HW_CACHEABLE:
		...
		pr_debug("[VCU] line:%d pa_start 0x%llx, vm_pgoff 0x%lx\n",
	...
	pr_debug("[VCU] %s vma->start 0x%lx, end 0x%lx, pgoff 0x%lx, map_buf_pa 0x%llx\n",
		 __func__, vma->vm_start, vma->vm_end, vma->vm_pgoff, vcu_queue->map_buf_pa);
	...
	if (pa_start_base >= MAP_SHMEM_MM_BASE &&
		...
		(pa_start | IOMMU_PADDING) : pa_start;
				   ...
				   unsigned long arg)
{
	...
	case VCU_SET_OBJECT:
		...
		ret = (long)copy_from_user(&share_buff_data, user_data_addr,
			(unsigned long)sizeof(struct share_obj));
		if (ret != 0L || share_buff_data.id >= IPI_MAX ||
		    share_buff_data.id < IPI_VCU_INIT) {
...
int mtk_vcu_probe(struct platform_device *pdev)
{
	struct mtk_vcu *vcu;
	struct device *dev;
	struct resource *res;

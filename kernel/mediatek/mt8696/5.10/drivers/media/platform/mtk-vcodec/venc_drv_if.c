...
void venc_encode_prepare(void *ctx_prepare, unsigned int core_id, unsigned long *flags)
{
	...
	if (ctx->enc_params.use_irq == 1) {
		if (ctx->enc_params.wpp_mode == 1) {
			venc_lock(ctx, MTK_VENC_CORE_0, ctx->enc_params.svp_mode);
			venc_lock(ctx, MTK_VENC_CORE_1, ctx->enc_params.svp_mode);
		} else
	...
	unsigned int core_id, unsigned long *flags)
{
	...
	if (ctx->enc_params.use_irq == 0 && ctx->dev->enc_sem[core_id].count != 0) {
	...
	if (ctx->enc_params.use_irq == 1) {
		if (ctx->enc_params.wpp_mode == 1) {
			venc_unlock(ctx, MTK_VENC_CORE_1);
			venc_unlock(ctx, MTK_VENC_CORE_0);
		} else
...
void venc_encode_pmqos_gce_begin(void *ctx_begin, unsigned int core_id, int job_cnt)
{
	mtk_venc_pmqos_gce_flush(ctx_begin, core_id, job_cnt);
}
EXPORT_SYMBOL_GPL(venc_encode_pmqos_gce_begin);

void venc_encode_pmqos_gce_end(void *ctx_end, unsigned int core_id, int job_cnt)
{

...
static int fops_vcodec_open(struct file *file)
{
	...
	mutex_lock(&dev->dec_dvfs_inst_mutex);
	dev->dec_cnt++;
	mutex_unlock(&dev->dec_dvfs_inst_mutex);

	mutex_unlock(&dev->dev_mutex);
	mtk_v4l2_debug(2, "%s decoder [%d] open cnt %d", dev_name(&dev->plat_dev->dev),
				   ctx->id,
				   dev->dec_cnt);
#if ENABLE_FENCE
...
static int mtk_vcodec_dec_probe(struct platform_device *pdev)
{
	...
	mutex_init(&dev->dec_dvfs_inst_mutex);

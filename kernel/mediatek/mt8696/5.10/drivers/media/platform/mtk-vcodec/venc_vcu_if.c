...
static int handle_enc_get_bs_buf(struct venc_vcu_inst *vcu, void *data)
{
	...
	msg->bs_fd = get_mapped_fd(pbs_buf->dmabuf);
...
int vcu_enc_query_cap(struct venc_vcu_inst *vcu, unsigned int id, void *out)
{
	...
	if (vcu->dev == NULL) {
		mtk_vcodec_err(vcu, "vcu plat device is invalid");
		return -EINVAL;
	}
				   ...
				   unsigned int *bs_size)
{
	...
	if (frm_buf) {
		...
		for (i = 0; i < frm_buf->num_planes; i++) {
			...
			frm_buf->frm_fd[i] = out.input_fd[i];
		}
		if (frm_buf->has_meta) {
	...
	if (bs_buf && (bs_mode < VENC_BS_MODE_FRAME))

	...
	struct vdec_fb *fb, unsigned int *src_chg)
{
	...
	mtk_vcodec_debug(inst, "+ [%d] FB y_dma=%llx c_dma=%llx FB va=%p num_planes %d",
	...
	if (fb != NULL) {
		...
		mtk_vcodec_debug(inst, "%d", __LINE__);
		for (i = 0; i < num_planes; i++) {
			mtk_vcodec_debug(inst, "%d", __LINE__);
	...
	mtk_vcodec_debug(inst, "+ x FB y_fd=%llx c_fd=%llx BS fd=%llx format=%c%c%c%c,sizeof(vdec_vsi) = %#lx",
		inst->vsi->dec.fb_fd[0], inst->vsi->dec.fb_fd[1],
		inst->vsi->dec.bs_fd, fm_fourcc & 0xFF,
		(fm_fourcc >> 8) & 0xFF, (fm_fourcc >> 16) & 0xFF,
		(fm_fourcc >> 24) & 0xFF, sizeof(*inst->vsi));
	...
	bool disp_list, struct vdec_fb **out_fb)
{
	...
	*out_fb = fb;
	mtk_vcodec_debug(inst, "[FB] get %s fb st=%d poc=%d ts=%llu %llx gbuf fd %d dma %p, meta fd %d dma %p",
		disp_list ? "disp" : "free",
		fb->status, list->fb_list[list->read_idx].poc,
		list->fb_list[list->read_idx].timestamp,
		list->fb_list[list->read_idx].vdec_fb_va,
		fb->general_buf_fd, fb->dma_general_buf,
		fb->meta_buf_fd, fb->dma_meta_buf);
	...
	enum vdec_get_param_type type, void *out)
{
	...
	case GET_PARAM_DISP_FRAME_BUFFER:
	{
		...
		if (pfb != NULL) {
			...
			if (pfb->meta_buf_fd >= 0) {
				mtk_vcodec_debug(inst, "free pfb->meta_buf_fd:%d pfb->dma_meta_buf %p\n",
					pfb->meta_buf_fd,
					pfb->dma_meta_buf);
				close_mapped_fd((unsigned int)
					pfb->meta_buf_fd);
	...
	enum vdec_set_param_type type, void *in)
{
	...
	case SET_TYPE_SET_DOBLY_VISION:

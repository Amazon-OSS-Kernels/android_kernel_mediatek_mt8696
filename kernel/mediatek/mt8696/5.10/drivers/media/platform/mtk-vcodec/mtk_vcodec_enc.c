...
void mtk_enc_put_buf(struct mtk_vcodec_ctx *ctx)
{
	...
	do {
		...
		if (rResult.bs_va != 0 && virt_addr_valid(rResult.bs_va)) {
			...
			if (pbs) {
				close_mapped_fd(pbs->buf_fd);
				mtk_v4l2_debug(1, " output (fd:%lld)", pbs->buf_fd);
			}
		}
		...
		if (rResult.frm_va != 0 && virt_addr_valid(rResult.frm_va)) {
			...
			if (rResult.is_last_slc == 1 && pfrm) {
				for (i = 0; i < pfrm->num_planes; i++) {
					mtk_v4l2_debug(1, "input dmabuf:%lx fd:%d",
					(unsigned long)pfrm->fb_addr[i].dmabuf, pfrm->frm_fd[i]);
					if (pfrm->fb_addr[i].dmabuf != NULL)
						close_mapped_fd(pfrm->frm_fd[i]);
				}
			}
		}
			    ...
			    struct v4l2_buffer *buf)
{
	...
	if (buf->flags & V4L2_BUF_FLAG_QP_META &&
		...
		mtkbuf->frm_buf.qpmap_dma = dma_buf_get(buf->reserved2);
		...
		mtk_v4l2_debug(1, "[%d] Have Qpmap fd, buf->index:%d, qpmap_dma:%p, addr = %llx, fd:%u",
			ctx->id, buf->index, mtkbuf->frm_buf.qpmap_dma,
			mtkbuf->frm_buf.qpmap_dma_addr, buf->reserved2);
	}
...
static void vb2ops_venc_buf_finish(struct vb2_buffer *vb)
{
	...
	if (mtkbuf->frm_buf.meta_dma != 0) {
		mtk_v4l2_debug(1,
			"dma_buf_put dma_buf=%p, DMA=%llx",
			mtkbuf->frm_buf.meta_dma,
			(unsigned long long)mtkbuf->frm_buf.meta_addr);
	...
	if (mtkbuf->frm_buf.qpmap_dma != 0) {
		mtk_v4l2_debug(1,
			"dma_buf_put qpmap_dma=%p, DMA=%llx",
			mtkbuf->frm_buf.qpmap_dma,
			(unsigned long long)mtkbuf->frm_buf.qpmap_dma_addr);
...
static int mtk_venc_param_change(struct mtk_vcodec_ctx *ctx)
{
	...
	if (!ret &&
	mtk_buf->param_change & MTK_ENCODE_PARAM_QP_CONTROL_MODE) {
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	v4l2_ctrl_new_std(handler, ops, V4L2_CID_MPEG_VIDEO_MULTI_SLICE_MAX_MB,
			  0, 65535, 1, 0);
	v4l2_ctrl_new_std(handler, ops, V4L2_CID_MPEG_MTK_ENCODE_MAX_LTR_FRAMES,
			  0, 3, 1, 0);
	...
	v4l2_ctrl_new_std_menu(handler, ops,
		...
		V4L2_MPEG_VIDEO_H265_LEVEL_HIGH_TIER_LEVEL_5_1,
		0, V4L2_MPEG_VIDEO_H265_LEVEL_MAIN_TIER_LEVEL_1);
	...
	cfg.ops = ops;
	ctrl = v4l2_ctrl_new_custom(handler,
		&cfg, NULL);
	...
	/*
	 * g_ctrl case must init after v4l2_ctrl_handler_setup
	 * to avoid s_ctrl fail in v4l2_ctrl_handler_setup
	 */
...
void mtk_venc_unlock(struct mtk_vcodec_ctx *ctx, u32 hw_id)
{
	if (hw_id >= MTK_VDEC_HW_NUM) {
		mtk_v4l2_debug(0, "unlock hw_id %u invalid", hw_id);
		return;
	}
...
int mtk_venc_lock(struct mtk_vcodec_ctx *ctx, u32 hw_id, bool sec)
{
	...
	if (hw_id >= MTK_VENC_HW_NUM) {
		mtk_v4l2_debug(0, "lock hw_id %u invalid", hw_id);
		return -1;
	}
...
{
	struct vb2_v4l2_buffer *src_buf_v4l2 = NULL, *dst_buf_v4l2 = NULL;
	...
	while ((src_buf_v4l2 = v4l2_m2m_src_buf_remove(ctx->m2m_ctx)))
		v4l2_m2m_buf_done(src_buf_v4l2,
			VB2_BUF_STATE_ERROR);

	while ((dst_buf_v4l2 = v4l2_m2m_dst_buf_remove(ctx->m2m_ctx))) {
		dst_buf_v4l2->vb2_buf.planes[0].bytesused = 0;
		v4l2_m2m_buf_done(dst_buf_v4l2,
			VB2_BUF_STATE_ERROR);
	}

...
int vcu_dec_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	if (msg->msg_id == VCU_IPIMSG_DEC_WAITISR) {
		...
		case VCU_IPIMSG_DEC_GET_FRAME_BUFFER:
			...
			vdec_fb_va = (uint64_t)(uintptr_t)pfb;
			...
			if (pfb != NULL) {
				...
				if (pfb->dma_general_buf != 0) {
					...
					mtk_vcodec_debug(vcu, "get_mapped_fd fb->dma_general_buf = %p, mapped fd = %d, size = %zu",
						pfb->dma_general_buf,
						vsi->general_buf_fd,
						pfb->dma_general_buf->size);
				} else {
				...
				if (pfb->dma_meta_buf != 0) {
					...
					vsi->meta_buf_fd =
						pfb->meta_buf_fd;
					vsi->meta_buf_size =
						pfb->dma_meta_buf->size;
					mtk_vcodec_debug(vcu, "get_mapped_fd fb->dma_meta_buf = %p, mapped fd = %d, size = %zu",
						pfb->dma_meta_buf,
						vsi->meta_buf_fd,
						pfb->dma_meta_buf->size);
				} else {
					pfb->meta_buf_fd = -1;
					vsi->meta_buf_fd = -1;
					mtk_vcodec_debug(vcu, "no meta buf dmabuf");
				}
			} else {
				vsi->dec.index = 0xFF;
...
int vcu_dec_reset(struct vdec_vcu_inst *vcu)
{
	...
	mutex_lock(vcu->ctx_ipi_lock);
	err = vcodec_send_ap_ipi(vcu, AP_IPIMSG_DEC_RESET);
	mutex_unlock(vcu->ctx_ipi_lock);

	...
	bool got_early_eos)
{
	...
	if (dstbuf->used) {
		...
		//if (ctx->input_driven)
		dstbuf->vb.vb2_buf.timestamp = disp_frame_buffer->timestamp;

		if (got_early_eos &&
			dstbuf->vb.vb2_buf.timestamp == ctx->input_max_ts) {
			dstbuf->vb.flags |= V4L2_BUF_FLAG_LAST;
			mtk_v4l2_debug(3, "step in got_early_eos and dstbuf->vb.vb2_buf.timestamp == ctx->input_max_ts");
		}
...
#else
	framerate = 1000;
	do_div(delta, (1000*1000));
	div_s64(framerate, delta);
#endif
	if (framerate <= 35)
		framerate = 30;
							...
							struct v4l2_buffer *buf)
{
	...
	if (buf->length <= 0) {
		mtk_v4l2_err("[%d] buffer length is invalid",
			ctx->id);
		return -EINVAL;
	}
	...
	if (buf->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		if (buf->m.planes[0].bytesused == 0) {
			...
			mtk_v4l2_debug(1, "[%d] id=%d getdata BS(%d,%d) vb=%p pts=%llu %llu, %d",
				...
				buf->length, vb,
				timeval_to_ns(&buf->timestamp),
				ctx->input_max_ts, ctx->frame_rate);
		}
...
static void vb2ops_vdec_buf_queue(struct vb2_buffer *vb)
{
	...
	if (ret || !res_chg || mtk_vcodec_unsupport
		...
		vb2_v4l2 = v4l2_m2m_src_buf_remove(ctx->m2m_ctx);
...
static void vb2ops_vdec_stop_streaming(struct vb2_queue *q)
{
	...
	if (q->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		while ((src_vb2_v4l2 = v4l2_m2m_src_buf_remove(ctx->m2m_ctx))) {
			...
			if (src_vb2_v4l2->vb2_buf.state == VB2_BUF_STATE_DONE) {
...
#elif defined(CONFIG_VB2_MEDIATEK_DMA)

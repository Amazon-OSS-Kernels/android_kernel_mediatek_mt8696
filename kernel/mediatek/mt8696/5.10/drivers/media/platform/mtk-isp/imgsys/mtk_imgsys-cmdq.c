...
static void __iomem *g_temp_trawRegBA;
#define debug_w 0x190
#define debug_r 0x194
#define debug_dma_w 0x4070
#define debug_dma_r 0x4074
#define debug_dip_cq 0x200
...
void imgsys_cmdq_streamon(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	g_temp_trawRegBA = of_iomap(imgsys_dev->dev->of_node, REG_MAP_E_TRAW);
...
void imgsys_cmdq_streamoff(struct mtk_imgsys_dev *imgsys_dev)
{
	...
	if (g_temp_trawRegBA) {
		iounmap(g_temp_trawRegBA);
		g_temp_trawRegBA = 0L;
	}

}

static void traw_dump(void)
{
	void __iomem *trawRegBA = 0L;
	unsigned int TDR[3];
	unsigned int CQ_en[12];
	unsigned int CQ_dis[10];
	unsigned int CQ_dma[22];

	trawRegBA = g_temp_trawRegBA;
	writel(0x00010004, (trawRegBA + debug_w));
	TDR[0] = readl(trawRegBA + debug_r);
	writel(0x00040004, (trawRegBA + debug_w));
	TDR[1] = readl(trawRegBA + debug_r);
	writel(0x00080004, (trawRegBA + debug_w));
	TDR[2] = readl(trawRegBA + debug_r);
	pr_info("%s TDR 1/4/8 = 0x%08x, 0x%08x, 0x%08x\n",
		__func__, TDR[0], TDR[1], TDR[2]);

	writel(0x10000000, (trawRegBA + debug_dip_cq));
	writel(0x00000005, (trawRegBA + debug_w));
	CQ_en[0] = readl(trawRegBA + debug_r);
	writel(0x00010005, (trawRegBA + debug_w));
	CQ_en[1] = readl(trawRegBA + debug_r);
	writel(0x00020005, (trawRegBA + debug_w));
	CQ_en[2] = readl(trawRegBA + debug_r);
	writel(0x00040005, (trawRegBA + debug_w));
	CQ_en[3] = readl(trawRegBA + debug_r);
	writel(0x00050005, (trawRegBA + debug_w));
	CQ_en[4] = readl(trawRegBA + debug_r);
	writel(0x00060005, (trawRegBA + debug_w));
	CQ_en[5] = readl(trawRegBA + debug_r);
	writel(0x00080005, (trawRegBA + debug_w));
	CQ_en[6] = readl(trawRegBA + debug_r);
	writel(0x00090005, (trawRegBA + debug_w));
	CQ_en[7] = readl(trawRegBA + debug_r);
	writel(0x000A0005, (trawRegBA + debug_w));
	CQ_en[8] = readl(trawRegBA + debug_r);
	writel(0x000C0005, (trawRegBA + debug_w));
	CQ_en[9] = readl(trawRegBA + debug_r);
	writel(0x000D0005, (trawRegBA + debug_w));
	CQ_en[10] = readl(trawRegBA + debug_r);
	writel(0x000E0005, (trawRegBA + debug_w));
	CQ_en[11] = readl(trawRegBA + debug_r);
	pr_info("%s CQ en 1 0/1/2/4 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_en[0], CQ_en[1], CQ_en[2], CQ_en[3]);
	pr_info("%s CQ en 1 5/6/8/9 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_en[4], CQ_en[5], CQ_en[6], CQ_en[7]);
	pr_info("%s CQ en 1 A/C/D/E =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_en[8], CQ_en[9], CQ_en[10], CQ_en[11]);

	writel(0x00000000, (trawRegBA + debug_dip_cq));
	writel(0x00000005, (trawRegBA + debug_w));
	CQ_dis[0] = readl(trawRegBA + debug_r);
	writel(0x00010005, (trawRegBA + debug_w));
	CQ_dis[1] = readl(trawRegBA + debug_r);
	writel(0x00020005, (trawRegBA + debug_w));
	CQ_dis[2] = readl(trawRegBA + debug_r);
	writel(0x00030005, (trawRegBA + debug_w));
	CQ_dis[3] = readl(trawRegBA + debug_r);
	writel(0x00040005, (trawRegBA + debug_w));
	CQ_dis[4] = readl(trawRegBA + debug_r);
	writel(0x00050005, (trawRegBA + debug_w));
	CQ_dis[5] = readl(trawRegBA + debug_r);
	writel(0x00060005, (trawRegBA + debug_w));
	CQ_dis[6] = readl(trawRegBA + debug_r);
	writel(0x00070005, (trawRegBA + debug_w));
	CQ_dis[7] = readl(trawRegBA + debug_r);
	writel(0x00080005, (trawRegBA + debug_w));
	CQ_dis[8] = readl(trawRegBA + debug_r);
	writel(0x00090005, (trawRegBA + debug_w));
	CQ_dis[9] = readl(trawRegBA + debug_r);
	pr_info("%s CQ en 0 0/1/2/3 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dis[0], CQ_dis[1], CQ_dis[2], CQ_dis[3]);
	pr_info("%s CQ en 0 4/5/6/7 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dis[4], CQ_dis[5], CQ_dis[6], CQ_dis[7]);
	pr_info("%s CQ en 0 [19:16]=> 8/9 =  0x%08x, 0x%08x\n",
		__func__, CQ_dis[8], CQ_dis[9]);

	writel(0x00000003, (trawRegBA + debug_dma_w));
	CQ_dma[0] = readl(trawRegBA + debug_dma_r);
	writel(0x00000103, (trawRegBA + debug_dma_w));
	CQ_dma[1] = readl(trawRegBA + debug_dma_r);
	writel(0x00000203, (trawRegBA + debug_dma_w));
	CQ_dma[2] = readl(trawRegBA + debug_dma_r);
	writel(0x00000303, (trawRegBA + debug_dma_w));
	CQ_dma[3] = readl(trawRegBA + debug_dma_r);
	writel(0x00000503, (trawRegBA + debug_dma_w));
	CQ_dma[4] = readl(trawRegBA + debug_dma_r);
	writel(0x00010603, (trawRegBA + debug_dma_w));
	CQ_dma[5] = readl(trawRegBA + debug_dma_r);
	writel(0x00030603, (trawRegBA + debug_dma_w));
	CQ_dma[6] = readl(trawRegBA + debug_dma_r);
	pr_info("%s IMGIBI DMA 0/1/2/3 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dma[0], CQ_dma[1], CQ_dma[2], CQ_dma[3]);
	pr_info("%s IMGIBI DMA 5/106/306 =  0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dma[4], CQ_dma[5], CQ_dma[6]);

	writel(0x00000004, (trawRegBA + debug_dma_w));
	CQ_dma[7] = readl(trawRegBA + debug_dma_r);
	writel(0x00000104, (trawRegBA + debug_dma_w));
	CQ_dma[8] = readl(trawRegBA + debug_dma_r);
	writel(0x00000204, (trawRegBA + debug_dma_w));
	CQ_dma[9] = readl(trawRegBA + debug_dma_r);
	writel(0x00000304, (trawRegBA + debug_dma_w));
	CQ_dma[10] = readl(trawRegBA + debug_dma_r);
	pr_info("%s IMGIBI UFD 0/1/2/3 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dma[7], CQ_dma[8], CQ_dma[9], CQ_dma[10]);

	writel(0x00000005, (trawRegBA + debug_dma_w));
	CQ_dma[11] = readl(trawRegBA + debug_dma_r);
	writel(0x00000105, (trawRegBA + debug_dma_w));
	CQ_dma[12] = readl(trawRegBA + debug_dma_r);
	writel(0x00000205, (trawRegBA + debug_dma_w));
	CQ_dma[13] = readl(trawRegBA + debug_dma_r);
	writel(0x00000305, (trawRegBA + debug_dma_w));
	CQ_dma[14] = readl(trawRegBA + debug_dma_r);
	writel(0x00000505, (trawRegBA + debug_dma_w));
	CQ_dma[15] = readl(trawRegBA + debug_dma_r);
	writel(0x00010605, (trawRegBA + debug_dma_w));
	CQ_dma[16] = readl(trawRegBA + debug_dma_r);
	writel(0x00030605, (trawRegBA + debug_dma_w));
	CQ_dma[17] = readl(trawRegBA + debug_dma_r);
	pr_info("%s IMGICI DMA 0/1/2/3 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dma[11], CQ_dma[12], CQ_dma[13], CQ_dma[14]);
	pr_info("%s IMGICI DMA 5/106/306 =  0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dma[15], CQ_dma[16], CQ_dma[17]);

	writel(0x00000006, (trawRegBA + debug_dma_w));
	CQ_dma[18] = readl(trawRegBA + debug_dma_r);
	writel(0x00000106, (trawRegBA + debug_dma_w));
	CQ_dma[19] = readl(trawRegBA + debug_dma_r);
	writel(0x00000206, (trawRegBA + debug_dma_w));
	CQ_dma[20] = readl(trawRegBA + debug_dma_r);
	writel(0x00000306, (trawRegBA + debug_dma_w));
	CQ_dma[21] = readl(trawRegBA + debug_dma_r);
	pr_info("%s IMGICI UFD 0/1/2/3 =  0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
		__func__, CQ_dma[18], CQ_dma[19], CQ_dma[20], CQ_dma[21]);


	writel(0x00000000, (trawRegBA + debug_w));
	writel(0x00000000, (trawRegBA + debug_dma_w));
}

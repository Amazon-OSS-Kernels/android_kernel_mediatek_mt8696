				   ...
				   int enable)
{
	...
	if (enable) {
		pipe->fence_ctx = mtk_imgsys_fence_ctx_alloc();
		if (IS_ERR(pipe->fence_ctx))
			dev_info(pipe->imgsys_dev->dev,
			"%s:%s: pipe(%d) streamon fence ctx init failed\n",
		...
		if (ret)
			dev_info(pipe->imgsys_dev->dev,
				...
				__func__, pipe->desc->name, pipe->desc->id);
		mtk_imgsys_fence_ctx_free(pipe->fence_ctx);
	}
				       ...
				       enum vb2_buffer_state state)
{
	...
	list_for_each_entry_safe(b, b0, &node->buf_list, list) {
		...
		if (b->buf_fence)
			mtk_imgsys_fence_signal(b->buf_fence);
	}
	spin_unlock(&node->buf_list_lock);
}
				  ...
				  struct v4l2_buffer *buf)
{
	...
	if (buf->flags & V4L2_BUF_FLAG_OUT_FENCE) {
		dev_buf->buf_fence = mtk_imgsys_fence_alloc(pipe->fence_ctx);
		if (IS_ERR(dev_buf->buf_fence)) {
			pr_info("[%s] %s:%s: vb2_ioctl_qbuf create fence fail\n", __func__,
					pipe->desc->name, node->desc->name);
			return -EINVAL;
		}

		vb->fence_fd = dev_buf->buf_fence->fence_fd;
		dev_dbg(pipe->imgsys_dev->dev, "[%s]%s:%s fence_fd:%d\n", __func__,
				pipe->desc->name, node->desc->name,
				dev_buf->buf_fence->fence_fd);
	} else {
		dev_buf->buf_fence = NULL;
	}

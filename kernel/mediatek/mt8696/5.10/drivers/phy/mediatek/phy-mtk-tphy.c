...
/**
 * phy_get_device_properties - return a u32 value of property of a device
 ...
 */
static int phy_get_device_properties(struct device *dev,
					const char *propname, u32 *val)
{
	...
	h       = params[1] + params[2] - 1;
	l       = params[1];

	value   = get_devinfo_with_index(params[0]);
	value  &= GENMASK(h, l);
	value >>= params[1];

	dev_info(dev, "%s: efuse %s: i.%u s.%u w.%u: %u\n",
		 __func__, propname,
				...
				struct mtk_phy_instance *instance)
{
	...
	instance->efuse_sw_en = 1;

	ret = device_property_read_u32(dev, "auto-load-dis", &bit);
	if (ret < 0)
		return ret;

	instance->efuse_autoload_dis = BIT(bit);
	if ((instance->efuse_autoload_dis != MR1_EFUSE_AUTO_LOAD_DIS_P0) &&
	    (instance->efuse_autoload_dis != MR1_EFUSE_AUTO_LOAD_DIS_P1) &&
	    (instance->efuse_autoload_dis != MR1_EFUSE_AUTO_LOAD_DIS_P2) &&
	    (instance->efuse_autoload_dis != P3D_RG_EFUSE_AUTO_LOAD_DIS))
		return -EINVAL;

	dev_info(dev, "auto load disable efuse with bit: %u\n", bit);

	switch (instance->type) {
	case PHY_TYPE_USB2:
		alv = of_property_read_bool(dev->of_node, "auto-load-valid");
		if (alv) {
			instance->efuse_alv_en = alv;
			ret = phy_get_device_properties(dev,
					"auto-load-valid",
			...
			dev_info(dev,
				"u2 auto load valid efuse with value: %u\n",
				instance->efuse_autoloadvalid);
		}
		...
		if (!instance->efuse_intr) {
			dev_info(dev, "no u2 intr efuse, but dts enable it\n");
	...
	case PHY_TYPE_PCIE:
		...
		if (alv) {
			...
			dev_info(dev,
				"u3 auto load valid efuse with value: %u\n",
				instance->efuse_autoloadvalid);
		}
		...
		if (!instance->efuse_intr &&
		    !instance->efuse_rx_imp &&
		    !instance->efuse_tx_imp) {
			dev_info(dev, "no u3 intr efuse, but dts enable it\n");
			instance->efuse_sw_en = 0;
			break;
		}

		dev_info(dev, "u3 efuse - intr %x, rx-imp %x, tx-imp %x\n",
			 instance->efuse_intr, instance->efuse_rx_imp,
			 instance->efuse_tx_imp);

		break;
	default:
		dev_info(dev, "no sw efuse for type %d\n", instance->type);
		ret = -EINVAL;
	}
...
static void phy_efuse_set(struct mtk_phy_instance *instance)
{
	...
	case PHY_TYPE_USB2:
		...
		tmp |= instance->efuse_autoload_dis;
	...
	case PHY_TYPE_PCIE:
		...
		tmp |= P3D_RG_SSUSB_TX_IMPSEL_VAL(instance->efuse_tx_imp);
		tmp |= P3D_RG_SSUSB_FORCE_TX_IMPEL;
		writel(tmp, u3_banks->phyd + U3P_U3_PHYD_IMPCAL0);

		tmp = readl(u3_banks->phyd + U3P_U3_PHYD_IMPCAL1);
		tmp &= ~P3D_RG_SSUSB_RX_IMPSEL;
		tmp |= P3D_RG_SSUSB_RX_IMPSEL_VAL(instance->efuse_rx_imp);
		tmp |= P3D_RG_SSUSB_FORCE_RX_IMPEL;
		writel(tmp, u3_banks->phyd + U3P_U3_PHYD_IMPCAL1);

		tmp = readl(u3_banks->phya + U3P_U3_PHYA_REG0);
		tmp &= ~P3A_RG_SSUSB_IEXT_INTR_CTRL;
		tmp |= P3A_RG_SSUSB_IEXT_INTR_CTRL_VAL(instance->efuse_intr);
		writel(tmp, u3_banks->phya + U3P_U3_PHYA_REG0);

		dev_info(dev, "%s set efuse, tx_imp %x, rx_imp %x intr %x\n",
...
#define u2_phy_efuse_set(...)
#define u3_phy_efuse_set(...)
#else
	...
	struct mtk_phy_instance *instance)
{
	...
	if (tphy->run_once & RUN_ONCE_PHY_TYPE_USB3) {
		...
		tmp |= (P3C_REG_IP_SW_RST | P3C_FORCE_IP_SW_RST);
		tmp |= P3C_MCU_BUS_CK_GATE_EN;
		...
		writel(tmp, u3_banks->chip + U3P_U3_CHIP_GPIO_CTLD);
		tmp = tphy->run_once;
		tphy->run_once &= ~RUN_ONCE_PHY_TYPE_USB3;
		dev_info(tphy->dev, "%s run_once: 0x%08X -> 0x%08X\n",
			 __func__, tmp, tphy->run_once);
	}
				...
				struct mtk_phy_instance *instance)
{
	...
	instance->eye_src_en =
		device_property_read_bool(dev, "mediatek,eye-src");
	if (instance->eye_src_en)
		device_property_read_u32(dev, "mediatek,eye-src",
				 &instance->eye_src);

	instance->eye_vrt_en =
		device_property_read_bool(dev, "mediatek,eye-vrt");
	if (instance->eye_vrt_en)
		device_property_read_u32(dev, "mediatek,eye-vrt",
				 &instance->eye_vrt);

	instance->eye_term_en =
		device_property_read_bool(dev, "mediatek,eye-term");
	if (instance->eye_term_en)
		device_property_read_u32(dev, "mediatek,eye-term",
				 &instance->eye_term);

	instance->intr_en =
		device_property_read_bool(dev, "mediatek,intr");
	if (instance->intr_en)
		device_property_read_u32(dev, "mediatek,intr",
				 &instance->intr);

	instance->discth_en =
		device_property_read_bool(dev, "mediatek,discth");
	if (instance->discth_en)
		device_property_read_u32(dev, "mediatek,discth",
				 &instance->discth);
	if (instance->rev6_en)
		device_property_read_u32(dev, "mediatek,rev6",
				 &instance->rev6);
	dev_dbg(dev, "bc12:%d, src:%d, vrt:%d, term:%d, intr:%d, disc:%d, rev6:%d\n",
		instance->bc12_en, instance->eye_src,
		instance->eye_vrt, instance->eye_term,
		instance->intr, instance->discth, instance->rev6);
}
...
void mtk_phy_patch_pp(struct phy *phy, int port)
{
	...
	if (phy->init_count && phy->power_count) {
		...
		value = readl(u2_banks->com + U3P_U2PHYDTM1);
...
int mtk_phy_regular_ctrl(struct phy *phy, bool supply)
{
	...
	if (instance->support_vcc && !IS_ERR(instance->vcc)) {
		if (supply)
			ret = regulator_enable(instance->vcc);
		else
			ret = regulator_disable(instance->vcc);

		if (ret)
			dev_info(tphy->dev,
				 "Failed to %sable power since %i\n",
				    supply ? "en" : "dis", ret);
...
EXPORT_SYMBOL_GPL(mtk_phy_regular_ctrl);

int mtk_phy_mode_ctrl(struct phy *phy, enum phy_mode mode)
{
	int ret = 0;
	struct mtk_phy_instance *instance = NULL;
	struct mtk_tphy *tphy = NULL;
	...
	if (instance->type == PHY_TYPE_USB2) {
		u2_banks = &instance->u2_banks;
		com = u2_banks->com;
		writel(0x2000000, com + U3P_U2PHYDTM0);

		switch (mode) {
		case PHY_MODE_USB_HOST:
			value = 0x0003E2C;
			writel(0x0003E10, com + U3P_U2PHYDTM1);
			break;
		case PHY_MODE_USB_DEVICE:
			value = 0x0003E2E;
			writel(0x0003E10, com + U3P_U2PHYDTM1);
		...
		writel(value, com + U3P_U2PHYDTM1);

		dev_info(tphy->dev,
			 "%s: force on %s Role.\n", __func__,
			 (value == 0x0003E2C) ? "Host"   :
			 (value == 0x0003E2E) ? "Device" :
			 (value == 0x0003C2C) ? "None"   : "Unknown");
	    dev_info(tphy->dev, "[0x%p] = 0x%0X but 0x%0X\n",
				com + (U3P_U2PHYDTM1),
				value, readl(com + U3P_U2PHYDTM1));
	}

	return ret;
}
EXPORT_SYMBOL_GPL(mtk_phy_mode_ctrl);
...
static int mtk_tphy_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, child_np) {
		...
		instance->support_vcc =
			 of_property_read_bool(child_np, "vcc-supply");
		instance->vcc = devm_regulator_get(dev, "vcc");
		if (IS_ERR(instance->vcc)) {
			dev_dbg(dev, "Error getting vcc regulator: %ld\n",
						PTR_ERR(instance->vcc));
			if (instance->support_vcc)
				return -EPROBE_DEFER;
		}
...
#ifdef CONFIG_DEBUG_FS
	dev_info(dev,
		 "====%s nphys %i in u3phy 0x%p ===\n",
		 __func__,
		 tphy->nphys, tphy);
	tphy_phy_init_debugfs((void *)tphy);
#endif
	dev_info(tphy->dev, "%s run_once: 0x%08X\n", __func__, tphy->run_once);

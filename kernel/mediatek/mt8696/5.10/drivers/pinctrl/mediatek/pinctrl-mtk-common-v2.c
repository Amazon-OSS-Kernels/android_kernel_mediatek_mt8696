...
int mtk_pinconf_bias_set_pd(struct mtk_pinctrl *hw,
			    const struct mtk_pin_desc *desc,
			    u32 arg)
{
	int err, pd;
...
int mtk_pinconf_bias_set_pd_rsel(struct mtk_pinctrl *hw,
			 const struct mtk_pin_desc *desc,
			 u32 pullup, u32 arg)
{
	int err;
	...
	err = mtk_pinconf_bias_set_pd(hw, desc, MTK_ENABLE);

out:
	return err;
}
EXPORT_SYMBOL_GPL(mtk_pinconf_bias_set_pd_rsel);
				...
				u32 pullup, u32 arg)
{
	...
	if (try_all_type ||
	    (hw->soc->pull_type[desc->number] & MTK_PULL_OPEN_DRAIN_RSEL_TYPE)) {
		err = mtk_pinconf_bias_set_pd_rsel(hw, desc, pullup, arg);
		if (!err)
			return err;
	}

	if (try_all_type ||
	    (hw->soc->pull_type[desc->number] & MTK_PULL_PD_TYPE)) {
		err = mtk_pinconf_bias_set_pd(hw, desc, arg);
		if (!err)
			return err;
	}

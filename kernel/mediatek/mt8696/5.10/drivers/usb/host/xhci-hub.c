	...
	__must_hold(&xhci->lock)
{
	...
	if (on) {
		...
		if (hcd->speed >= HCD_USB3) {
			temp &= ~PORT_POWER;
			if (mtk->u3port_power[index])
				temp |= PORT_POWER;

			writel(temp, port->addr);
			temp = readl(port->addr);
			xhci_dbg(xhci,
				"[U3 PP %u]set port power, actual port %d status = 0x%x\n",
				mtk->u3port_power[index], index, temp);
			if (!mtk->u3port_power[index]) {
				if (mtk->num_phys && (index < mtk->num_phys)) {
					op_phy = &mtk->op_phys[index];
					mtk_phy_patch_pp(op_phy->phy, index);
				}
			}
		} else {
			temp &= ~PORT_POWER;
			if (mtk->u2port_power[index])
				temp |= PORT_POWER;

			writel(temp, port->addr);
			temp = readl(port->addr);
			xhci_dbg(xhci,
				"[U2 PP %u]set port power, actual port %d status = 0x%x\n",
				mtk->u2port_power[index], index, temp);
		...
		xhci_dbg(xhci, "clr port power, actual port %d status  = 0x%x\n",
						index, temp);
#if IS_ENABLED(CONFIG_USB_XHCI_MTK)
		if (mtk->num_phys && (index < mtk->num_phys)) {
			op_phy = &mtk->op_phys[index];
	...
	case ClearPortFeature:
		...
		case USB_PORT_FEAT_POWER:
			...
			if (mtk->num_phys && (wIndex < mtk->num_phys)) {
				op_phy = &mtk->op_phys[wIndex];
				mtk_phy_patch_pp(op_phy->phy, wIndex);
			}
#endif
			break;
		case USB_PORT_FEAT_TEST:

...
static int clk_mt8696_ether_probe(struct platform_device *pdev)
{
	...
	r = mtk_clk_register_gates(node, ether_clks, ARRAY_SIZE(ether_clks),
							clk_data);
	if (r)
		goto free_ether_data;
...
static int __init clk_mt8696_ether_init(void)
{
	return platform_driver_register(&clk_mt8696_ether_drv);
}

arch_initcall(clk_mt8696_ether_init);

...
#define REG_MMU_AXI_PORT0_OSTD			0x060
#define F_MMU_OUT_P0_TBLWLK_OSTD_MSK		(0xff << 16)
#define MMU_OSTD_CHECK_CYCLE			(100)
...
#define POLLING_OSTD_STS_FOR_TLB_INV	BIT(19)

#define HAS_APB				BIT(20)
...
#define MSG_DEV(d, msg, arg...)		\
	pr_debug("[iommu] <%s: %s> "msg, __func__, dev_name(d), ##arg)
...
static const struct mtk_iommu_iova_region mt8188_multi_dom_apu[] = {
	{ .iova_base = SZ_512M,		.size = SZ_512M, },	/* APU SECURE */
...
#if IS_ENABLED(CONFIG_ARCH_DMA_ADDR_T_64BIT)
	{ .iova_base = 0x370000000ULL,	.size = 0x12600000, },	/* APU CODE */
	{ .iova_base = 0x320000000ULL,	.size = 0xe0000000, },	/* APU DATA */
	{ .iova_base = 0x308000000ULL,	.size = 0x8000000, },	/* APU VLM */
#else /* todo */
	{ .iova_base = 0,		.size = SZ_4G - SZ_8M, },
#endif
};
					   ...
					   struct mtk_iommu_bank_data *bank)
{
	...
	struct mtk_iommu_data *data;
	unsigned long flags;
	void __iomem *base, *base_0;
	int ret, ostd_check_cnt;
	u32 tmp;
	...
	for_each_m4u(data, head) {
		...
		base = curbank->base;
		ostd_check_cnt = MMU_OSTD_CHECK_CYCLE;
		base_0 = data->bank[BASE_BANK_ID].base;
		...
		if (MTK_IOMMU_HAS_FLAG(data->plat_data, POLLING_OSTD_STS_FOR_TLB_INV)) {
			do {
				if (ostd_check_cnt-- <= 0) {
					dev_info(data->dev,
						 "polling tbl walk timeout @ flush TLB\n");
					break;
				}
			} while (readl(base_0 + REG_MMU_AXI_PORT0_OSTD) &
				       F_MMU_OUT_P0_TBLWLK_OSTD_MSK);
		}
				   ...
				   struct device *dev)
{
	...
	if (!dom->bank) {
		/* Record the information of pagetable for each iommu_group'
		 * domain and the mapping->domain in ARM32 path.
		 */
	...
	mtk_mapping = frstdata->mapping[region_id];
	if (!mtk_mapping) {
		region = frstdata->plat_data->iova_region + region_id;
		...
		if (IS_ERR_OR_NULL(mtk_mapping)) {
			dev_err(dev,
				...
				region_id, PTR_ERR(mtk_mapping));
			return -EFAULT;
		}
		frstdata->mapping[region_id] = mtk_mapping;
	}
	...
	if (!bank->m4u_dom) { /* Initialize the M4U HW for each a BANK */
		/*
		 * The power/clk is default on in lk. No need power on here.
		 * It will turn off the m4u clk/pm if pm_get/put here.
		 */
			 ...
			 phys_addr_t paddr, size_t size, int prot, gfp_t gfp)
{
	...
	if (WARN_ON(paddr < PHYS_OFFSET)) { /* The frst 1G is register's section */
		pr_err("%s, %d, err: iova 0x%lx pa %pa pgsize 0x%zx.\n",
...
static struct iommu_device *mtk_iommu_probe_device(struct device *dev)
{
	...
	if (MTK_IOMMU_IS_TYPE(data->plat_data, MTK_IOMMU_TYPE_MM)) {
		...
		/*
		 * The device link is to help the power control flow
		 * for the consume devices.
		 * But if the device use multiple larbs in dtsi,
		 * this is not helpful, the user should control the
		 * larb's power itself.
		 * Skip the dev-link in this case.
		 */
			...
			unsigned int resv_cnt, bool for_resv_region)
{
	...
	for (i = 0; i < resv_cnt; i++, resv++) {
		...
		if (resv->get_resv_data) {
			resv->get_resv_data(&resv_base, &resz_size, data);
			resv_end = resv_base + resz_size;
		} else {
		...
		pr_info("add %s-iova(0x%llx~0x%llx) for cur-dom(0x%llx~0x%llx)%s\n",
				       ...
				       struct list_head *head)
{
	...
	if (regionid < 0 || regionid >= MTK_IOMMU_GROUP_MAX)
		return;
	...
	DBG_DEV(dev, "get_resv_regions for %s region %d\n",
...
static void mtk_iommu_apply_resv_region(struct device *dev,
					...
					struct iommu_resv_region *region)
{
	struct dma_iommu_mapping *mtk_mapping = to_dma_iommu_mapping(dev);
	int ret = arm_iommu_reserve(mtk_mapping, (dma_addr_t)(region->start),
...
static int mtk_iommu_probe(struct platform_device *pdev)
{
	...
	if (MTK_IOMMU_HAS_FLAG(data->plat_data, HAS_APB)) {
		data->apb = devm_clk_get(dev, "apb");
		if (IS_ERR(data->apb))
			return PTR_ERR(data->apb);
	}
...
static int mtk_iommu_remove(struct platform_device *pdev)
{
	...
	struct mtk_iommu_data *frstdata =
				mtk_iommu_get_frst_data(data->hw_list);
	struct dma_iommu_mapping *mtk_mapping;
#endif
...
static int __maybe_unused mtk_iommu_runtime_suspend(struct device *dev)
{
	...
	clk_disable_unprepare(data->apb);
	DBG_DEV(dev, "suspend\n");
...
static int __maybe_unused mtk_iommu_runtime_resume(struct device *dev)
{
	...
	ret = clk_prepare_enable(data->apb);
	if (ret) {
		dev_err(data->dev, "Failed to enable apb clk(%d) in resume\n", ret);
		return ret;
	}
	...
	if (ret) {
		clk_disable_unprepare(data->apb);
		dev_err(data->dev, "Failed to enable bclk clk(%d) in resume\n", ret);
		return ret;
	}
...
static void mtk_get_disp_dm_region(dma_addr_t *base, size_t *size,
				   struct mtk_iommu_data *data)
{
/*
 * Display show fastlogo in lk. it is contiougous buffer. when entering kernel,
 * the iommu HW will be enabled. To make sure the display show fastlogo smoothly,
 ...
 */
#ifdef CONFIG_MTK_FB
		*base = mtkfb_get_fb_base();
		*size = mtkfb_get_fb_size();
#else
		*base = 0;
		*size = 0;
#endif
...
#if IS_BUILTIN(CONFIG_MTK_IOMMU_MISC_SECURE)
...
static const struct mtk_iommu_iova_region mt8168_iommu_rsv_list[] = {
	{	/* for VPU */
		.resv_type	= IOMMU_RESV_RESERVED,
		.iova_base	= 0x7D100000,
		.size		= 0x05500000,		/* s 9M + ns 76M */
	},
	{	/* for SVP */
		.resv_type	= IOMMU_RESV_RESERVED,
		.iova_base	= 0,
		.size		= MTK_IOMMU_RESERVE_SIZE,
	},
};

static const struct mtk_iommu_iova_region mt8696_iommu_rsv_list[] = {
#if IS_ENABLED(CONFIG_MTK_IOMMU_MISC_SECURE)
		{	.resv_type = IOMMU_RESV_RESERVED,
			.iova_base = 0,
			.size = 0,
...
static const struct mtk_iommu_plat_data mt8696_data = {
	.m4u_plat     = M4U_MT8696,
	.flags	      = MTK_IOMMU_TYPE_MM | HAS_BCLK | HAS_APB |
			    HAS_VLD_PA_RNG | WR_THROT_EN | MTK_IOMMU_DEV_ID(0),
	.hw_list      = &m4ulist,
	.resv_region  = &mt8696_iommu_rsv_list[0],
	.resv_cnt     = ARRAY_SIZE(mt8696_iommu_rsv_list),
	.inv_sel_reg  = REG_MMU_INV_SEL_GEN2,
	.bank_info    = BANK_NR(2) | USED_BANK_BITMAP(BANK_MSK(0) | BANK_MSK(1)) |
			NOR_BANK_BITMAP(BANK_MSK(0)),
	.larbid_remap = {{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}},
	.iova_region  = single_domain,
	.iova_region_nr = ARRAY_SIZE(single_domain),
...
static const struct of_device_id mtk_iommu_of_ids[] = {
	...
	{ .compatible = "mediatek,mt8696-iommu", .data = &mt8696_data},
	{}
};

...
void mdw_rsc_update_avl_bmp(int type)
{
	...
	bitmap_copy((unsigned long *)&cb, rsc_mgr.cmd_avl_bmp, APUSYS_DEVICE_MAX);
	bitmap_copy((unsigned long *)&db, rsc_mgr.dev_avl_bmp, APUSYS_DEVICE_MAX);
...
static int mdw_rsc_check_dev_state(struct mdw_dev_info *in)
{
	...
	if (in == NULL || in->idx >= MDW_RSC_TAB_DEV_MAX || in->idx < 0)
		return -EINVAL;

	...
	struct mdw_dev_info *d, struct apusys_cmd_hnd *h, int ret, int done)
{
	...
	/* trace cmd end */
	cmd.done = done;
	cmd.pid = sc->parent->pid;
	cmd.tgid = sc->parent->tgid;
	cmd.uid = sc->parent->hdr->uid;
	cmd.cmd_id = sc->parent->kid;
	cmd.sc_idx = sc->idx;
	cmd.num_sc = sc->parent->hdr->num_sc;
	cmd.type = d->type;
	snprintf(cmd.dev_name, MDW_DEV_NAME_SIZE,
		 "%s", d->name);
	cmd.dev_idx = d->idx;
	cmd.pack_id = sc->hdr->pack_id;
	cmd.multicore_idx = h->multicore_idx;
	cmd.exec_core_num = sc->multi_total;
	cmd.exec_core_bitmap = sc->multi_bmp;
	cmd.priority = sc->parent->hdr->priority;
	cmd.soft_limit = sc->parent->hdr->soft_limit;
	cmd.hard_limit = sc->parent->hdr->hard_limit;
	cmd.exec_time = sc->hdr->ip_time;
	cmd.suggest_time = sc->hdr->suggest_time;
	cmd.power_save = 0; //sc->par_cmd->power_save,
	cmd.ctx_id = sc->ctx;
	cmd.tcm_force = sc->hdr->tcm_force;
	cmd.tcm_usage = sc->hdr->tcm_usage;
	cmd.tcm_real_usage = sc->real_tcm_usage;
	cmd.boost = h->boost_val;
	cmd.ip_time = h->ip_time;
	cmd.ret = ret;
	trace_mdw_cmd(&cmd);
}
...
int mdw_sched_dev_routine(void *arg)
{
	...
	while (!kthread_should_stop() && d->stop == false) {
		...
		mdw_trace_begin("dev(%s-%d) exec|sc(0x%llx-%d) boost(%u)",

...
static bool is_scp_reset_qos_request;
struct pm_qos_request *scp_reset_qos_request;
...
static void scp_A_notify_ws(struct work_struct *ws)
{
	...
	if (is_scp_reset_qos_request) {
		cpu_latency_qos_remove_request(scp_reset_qos_request);
		kfree(scp_reset_qos_request);
		is_scp_reset_qos_request = false;
	}
#endif
	/* register scp dvfs*/
	msleep(2000);
...
#if SCP_DVFS_INIT_ENABLE
	/* prevent system from entering spm idle for keeping pll alive */
	if (!is_scp_reset_qos_request) {
		scp_reset_qos_request = kzalloc(sizeof(struct pm_qos_request), GFP_KERNEL);
		if (!scp_reset_qos_request)
			return;

		cpu_latency_qos_add_request(scp_reset_qos_request, PM_QOS_DEFAULT_VALUE);
		is_scp_reset_qos_request = true;
	}
#endif

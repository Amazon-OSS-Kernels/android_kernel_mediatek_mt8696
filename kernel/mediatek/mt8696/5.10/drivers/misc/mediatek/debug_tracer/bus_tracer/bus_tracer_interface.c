...
static DRIVER_ATTR_RW(bus_tracer_dump_to_buf);
...
static const struct proc_ops bus_tracer_dump_db_proc_fops = {
	.proc_open		= bus_tracer_dump_db_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= bus_tracer_dump_db_release,
};
...
static DRIVER_ATTR_RW(bus_tracer_enable);
...
static DRIVER_ATTR_RW(bus_tracer_recording);
...
static DRIVER_ATTR_RW(bus_dump_setting);
...
static int __init bus_tracer_init(void)
{
	...
	ret = driver_create_file(&bus_tracer_drv.plt_drv.driver,
			&driver_attr_bus_tracer_dump_to_buf);
	if (ret)
		pr_notice("%s: driver_create_file failed.\n", __func__);

	ret = driver_create_file(&bus_tracer_drv.plt_drv.driver,
			&driver_attr_bus_tracer_enable);
	if (ret)
		pr_notice("%s: driver_create_file failed.\n", __func__);

	ret = driver_create_file(&bus_tracer_drv.plt_drv.driver,
			&driver_attr_bus_tracer_recording);
	if (ret)
		pr_notice("%s: driver_create_file failed.\n", __func__);
	ret = driver_create_file(&bus_tracer_drv.plt_drv.driver,
			&driver_attr_bus_dump_setting);
	if (ret)
		pr_notice("%s: driver_create_file failed.\n", __func__);

...
static ssize_t mt8696_store_pwr_ctrl(int id,	const char *buf, size_t count)
{
	...
	for (i = 0 ; i < PW_MAX_COUNT; i++) {
		if (!strcmp(cmd, mt8696_pwr_ctrl_str[i])) {
...
static ssize_t
mt8696_generic_spm_read(char *ToUserBuf, size_t sz, void *priv);

static ssize_t
mt8696_generic_spm_write(char *FromUserBuf, size_t sz, void *priv);

struct mt8696_SPM_ENTERY {
	const char *name;
	int mode;
	struct mtk_lp_sysfs_handle handle;
};

struct mt8696_SPM_NODE {
	const char *name;
	int mode;
	struct mtk_lp_sysfs_handle handle;
...
struct mt8696_SPM_ENTERY mt8696_spm_root = {
	.name = "power",
	.mode = 0644,
};

struct mt8696_SPM_NODE mt8696_spm_idle = {
	.name = "idle_ctrl",
	.mode = 0644,
	.op = {
		.fs_read = mt8696_generic_spm_read,
		.fs_write = mt8696_generic_spm_write,
		.priv = (void *)&mt8696_spm_idle,
	},
};

struct mt8696_SPM_NODE mt8696_spm_suspend = {
	.name = "suspend_ctrl",
	.mode = 0644,
	.op = {
		.fs_read = mt8696_generic_spm_read,
		.fs_write = mt8696_generic_spm_write,
		.priv = (void *)&mt8696_spm_suspend,
	},
};
...
mt8696_generic_spm_read(char *ToUserBuf, size_t sz, void *priv)
{
	...
	if (priv == &mt8696_spm_idle)
		id = MT_SPM_DBG_SMC_UID_IDLE_PWR_CTRL;
	return mt8696_show_pwr_ctrl(id, ToUserBuf, sz);
}
...
mt8696_generic_spm_write(char *FromUserBuf, size_t sz, void *priv)
{
	...
	return mt8696_store_pwr_ctrl(id, FromUserBuf, sz);
}
...
int lpm_spm_fs_init(void)
{
	...
	r = mtk_lp_sysfs_entry_func_create(mt8696_spm_root.name,
					   mt8696_spm_root.mode, NULL,
					   &mt8696_spm_root.handle);
	if (!r) {
		mtk_lp_sysfs_entry_func_node_add(mt8696_spm_suspend.name,
						mt8696_spm_suspend.mode,
						&mt8696_spm_suspend.op,
						&mt8696_spm_root.handle,
						&mt8696_spm_suspend.handle);

		mtk_lp_sysfs_entry_func_node_add(mt8696_spm_idle.name,
						 mt8696_spm_idle.mode,
						 &mt8696_spm_idle.op,
						 &mt8696_spm_root.handle,
						 &mt8696_spm_idle.handle);
	}

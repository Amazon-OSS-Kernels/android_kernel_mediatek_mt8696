...
static const struct of_device_id mtk_ir_of_match[] = {
	...
	{.compatible = "mediatek,mt8195-irrx",},
	{},
};
#endif
...
static ssize_t switch_dev_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	if (strcmp(pattr->name, "switch_dev") == 0)
		SPRINTF_DEV_ATTR("no support!! used to switch ir device\n");
...
static ssize_t switch_dev_store(struct device *dev,
...
static ssize_t cuscode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct attribute *pattr = &(attr->attr);
	struct mtk_ir_core_platform_data *pdata = get_mtk_ir_ctl_data();
...
static ssize_t cuscode_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	int var;
...
static ssize_t timer_log_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct attribute *pattr = &(attr->attr);
	struct mtk_ir_core_platform_data *pdata = get_mtk_ir_ctl_data();
...
static ssize_t timer_log_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	int var;
...
static ssize_t log_to_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct attribute *pattr = &(attr->attr);
	struct mtk_ir_core_platform_data *pdata = get_mtk_ir_ctl_data();
...
static ssize_t log_to_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	int var;
...
static ssize_t clock_store(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	int val;
...
static ssize_t swirq_store(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	int val;
...
static ssize_t hwirq_store(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	int val;
...
static ssize_t press_timeout_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	int var;
...
static ssize_t alldev_info_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct mtk_ir_context *cxt;
...
static ssize_t mouse_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
...
#if IS_ENABLED(CONFIG_MTK_IRRX_MOUSE_DRAG)
	SPRINTF_DEV_ATTR("g_drag_device_mode = %d\n",
		mtk_ir_drag_mouse_get_device_mode());
#endif
	SPRINTF_DEV_ATTR("-------- devname(%s), protocol(%d)--------\n",
	...
	SPRINTF_DEV_ATTR("scanwheelup = 0x%x\n", p_mousecode->scanwheelup);
	SPRINTF_DEV_ATTR("scanwheeldown = 0x%x\n", p_mousecode->scanwheeldown);
	SPRINTF_DEV_ATTR("scanenter = 0x%x\n", p_mousecode->scanenter);
	SPRINTF_DEV_ATTR("scanswitch = 0x%x\n", p_mousecode->scanswitch);
#if IS_ENABLED(CONFIG_MTK_IRRX_MOUSE_DRAG)
	SPRINTF_DEV_ATTR("dragswitch = 0x%x\n", p_mousecode->dragswitch);
#endif
...
static ssize_t mouse_x_y_small_store(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	int varx = -1;
...
static ssize_t mouse_x_y_large_store(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	int varx = -1;
...
static DEVICE_ATTR_RO(alldev_info);
static DEVICE_ATTR_RW(switch_dev);
static DEVICE_ATTR_RW(debug_log);
static DEVICE_ATTR_RW(register);
static DEVICE_ATTR_WO(clock);
static DEVICE_ATTR_WO(swirq);
static DEVICE_ATTR_WO(hwirq);
static DEVICE_ATTR_WO(press_timeout);
static DEVICE_ATTR_RW(cuscode);
static DEVICE_ATTR_RW(timer_log);
static DEVICE_ATTR_RW(log_to);
static DEVICE_ATTR_RO(mouse);
static DEVICE_ATTR_WO(mouse_x_y_small);
static DEVICE_ATTR_WO(mouse_x_y_large);
...
void mtk_ir_mouse_switch(enum MTK_IR_DEVICE_MODE devmode)
{
	MTK_IR_FUN();

	if (devmode == MTK_IR_AS_IRRX)
...
#if IS_ENABLED(CONFIG_MTK_IRRX_MOUSE_DRAG)
void mtk_ir_drag_mouse_switch(struct input_dev *p_devmouse,
	enum MTK_IR_MOUSE_MODE mouse_mode)
{
	MTK_IR_FUN();

	if (mouse_mode == MTK_IR_AS_NORMAL_MOUSE) {
		mtk_ir_drag_mouse_set_device_mode(MTK_IR_AS_DRAG_MOUSE);
		input_event(p_devmouse, EV_MSC, MSC_SCAN, BTN_LEFT);
		...
		input_sync(p_devmouse);
		mtk_ir_drag_mouse_set_device_mode(MTK_IR_AS_NORMAL_MOUSE);
	}
}
#endif
...
void mtk_ir_core_send_scancode(u32 scancode)
{
	...
	rcdev = cxt->rcdev;
#if IS_ENABLED(CONFIG_MTK_IRRX_MOUSE_DRAG)
	p_devmouse = cxt->p_devmouse;
#endif

	MTK_IR_LOG(" get scancode 0x%08x !!\n", scancode);
	dev_mode = mtk_ir_mouse_get_device_mode();
#if IS_ENABLED(CONFIG_MTK_IRRX_MOUSE_DRAG)
	mouse_mode = mtk_ir_drag_mouse_get_device_mode();
#endif
	if (pdata->mouse_support && (scancode == p_mousecode->scanswitch)) {
		MTK_IR_LOG(" switch mode code 0x%08x\n", scancode);
		if (!(rcdev->keypressed))
			mtk_ir_mouse_switch(dev_mode);
	}
#if IS_ENABLED(CONFIG_MTK_IRRX_MOUSE_DRAG)
	else if ((dev_mode == MTK_IR_AS_MOUSE)
		&& (scancode == p_mousecode->dragswitch)) {
		MTK_IR_LOG(" Drag mouse switch mode code 0x%08x\n", scancode);
		if (!(rcdev->keypressed))
			mtk_ir_drag_mouse_switch(p_devmouse, mouse_mode);
	} else if ((dev_mode == MTK_IR_AS_MOUSE)
			&& (mouse_mode == MTK_IR_AS_DRAG_MOUSE)) {
		if (mtk_ir_drag_mouse_proc_key(scancode, cxt))
			MTK_IR_LOG(" process drag mouse key\n");
		else {	/* Not drag mouse key ,so as ir key process */
...
int __init mtk_ir_core_init(void)
{
	int ret = 0;
...
EXPORT_SYMBOL_GPL(mtk_ir_core_init);

void __exit mtk_ir_core_exit(void)
{
...
EXPORT_SYMBOL_GPL(mtk_ir_core_exit);

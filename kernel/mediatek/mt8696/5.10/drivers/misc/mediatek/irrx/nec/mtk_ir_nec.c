...
#define IR_NEC_LOG(fmt, args...) \
	do { \
		if (ir_log_debug_on) \
			pr_info(IR_NEC_TAG fmt, ##args); \
		else if (IR_NEC_DEBUG) \
			pr_debug(IR_NEC_TAG fmt, ##args); \
	} while (0)

#define IR_NEC_ERR(fmt, args...) \
	pr_info(MTK_IR_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int mtk_ir_nec_init_hw(void)
{
	u32 ir_mode = mtk_ir_core_getmode();
	...
	if (ir_mode == MTK_IR_FACTORY) {
		...
		mtk_ir_pdata_nec.mouse_code.scanwheelup =
			MTK_IR_MOUSE_NEC_WHEEL_UP;
		mtk_ir_pdata_nec.mouse_code.scanwheeldown =
			MTK_IR_MOUSE_NEC_WHEEL_DOWN;
		...
		mtk_ir_pdata_nec.mouse_code.dragswitch =
			MTK_IR_NEC_DRAG_MOUSE_SWITCH_CODE;
#endif
		memset((void *)mtk_ir_pdata_nec.mousename, '\0', 32);
		strncpy(mtk_ir_pdata_nec.mousename,
			mtk_ir_pdata_nec.device_name,
			sizeof(mtk_ir_pdata_nec.mousename) - 1);
		strncat(mtk_ir_pdata_nec.mousename, "_Mouse",
			sizeof(mtk_ir_pdata_nec.mousename) -
			strlen(mtk_ir_pdata_nec.device_name));
...
EXPORT_SYMBOL_GPL(mtk_ir_nec_decode);
...
EXPORT_SYMBOL_GPL(ir_nec_init);

void __exit ir_nec_exit(void)
{
	IR_NEC_FUN();
}
EXPORT_SYMBOL_GPL(ir_nec_exit);

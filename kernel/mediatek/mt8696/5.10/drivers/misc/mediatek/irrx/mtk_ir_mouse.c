...
static enum MTK_IR_MOUSE_MODE g_drag_device_mode = MTK_IR_AS_NORMAL_MOUSE;
#endif
...
#if IS_ENABLED(CONFIG_MTK_IRRX_MOUSE_DRAG)
enum MTK_IR_MOUSE_MODE mtk_ir_drag_mouse_get_device_mode(void)
{
	return g_drag_device_mode;
}

void mtk_ir_drag_mouse_set_device_mode(enum MTK_IR_MOUSE_MODE devmode)
{
	g_drag_device_mode = devmode;
	MTK_IR_LOG("g_drag_device_mode = %d\n", g_drag_device_mode);
}

int mtk_ir_drag_mouse_proc_key(u32 scancode,
	struct mtk_ir_context *p_mtk_rc_core)
{
	...
	if (scancode == p_mousecode->scanleft) {
		if (!(rcdev->keypressed)) {
			MTK_IR_LOG("Drag mouse X_LEFT PRESS\n");
			input_report_rel(p_devmouse, REL_X,
				-(x_small_step * 4));
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y, 0);
			input_sync(p_devmouse);
		} else {
			MTK_IR_LOG("Drag mouse X_LEFT REPEAT\n");
			input_report_rel(p_devmouse, REL_X,
				-(x_large_step * 4));
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y, 0);
		...
		if (!(rcdev->keypressed)) {
			MTK_IR_LOG("Drag mouse X_RIGHT PRESS\n");
			input_report_rel(p_devmouse, REL_X, (x_small_step * 4));
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y, 0);
			input_sync(p_devmouse);
		} else {
			MTK_IR_LOG("Drag mouse X_RIGHT REPEAT\n");
			input_report_rel(p_devmouse, REL_X, (x_large_step * 4));
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y, 0);
			input_sync(p_devmouse);
		}
		...
		if (!(rcdev->keypressed)) {
			MTK_IR_LOG("Drag mouse Y_UP PRESS\n");
			input_report_rel(p_devmouse, REL_X, 0);
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y,
				-(y_small_step * 4));
			input_sync(p_devmouse);
		} else {
			MTK_IR_LOG("youtpbe Y_UP REPEAT\n");
			input_report_rel(p_devmouse, REL_X, 0);
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y,
				-(y_large_step * 4));
		...
		if (!(rcdev->keypressed)) {
			MTK_IR_LOG("Drag mouse Y_DOWN PRESS\n");
			input_report_rel(p_devmouse, REL_X, 0);
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y, (y_small_step * 4));
			input_sync(p_devmouse);
		} else {
			MTK_IR_LOG("Drag mouse Y_DOWN REPEAT\n");
			input_report_rel(p_devmouse, REL_X, 0);
			input_sync(p_devmouse);
			input_report_rel(p_devmouse, REL_Y, (y_large_step * 4));
			input_sync(p_devmouse);
		}
	} else
		return 0;
...
int mtk_ir_mouse_proc_key(u32 scancode, struct mtk_ir_context *p_mtk_rc_core)
{
	...
	} else if (scancode == p_mousecode->scanwheelup) { /*wheel up*/
		MTK_IR_LOG("MOUSE WHEEL_UP PRESS\n");
		input_report_rel(p_devmouse, REL_WHEEL, 1);
	} else if (scancode == p_mousecode->scanwheeldown) { /*wheel down*/
		MTK_IR_LOG("MOUSE WHEEL_DOWN PRESS\n");
		input_report_rel(p_devmouse, REL_WHEEL, -1);

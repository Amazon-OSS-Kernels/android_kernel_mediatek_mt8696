...
#define vNrWriteReg(dAddr, dVal)  (writel(dVal, (void *)dAddr))
#define vNrReadReg(dAddr)  (readl((void *)dAddr))
#define vNRWriteRegMsk(dAddr, dVal, dMsk)  \
		vNrWriteReg((dAddr),  \
		(vNrReadReg(dAddr) & (~(dMsk))) | ((dVal) & (dMsk)))

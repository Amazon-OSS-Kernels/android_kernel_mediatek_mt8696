...
#define MMSYSRAM_SEC_ADDR0	(0x044)
#define MMSYSRAM_SEC_ADDR1	(0x048)
#define MMSYSRAM_SEC_ADDR2	(0x04C)
#define MMSYSRAM_SEC_ADDR3	(0x050)
#define MMSYSRAM_SEC_ADDR4	(0x054)
#define MMSYSRAM_SEC_ADDR5	(0x058)
#define MMSYSRAM_SEC_ADDR6	(0x05C)
...
#define DEC_MPU_VIO_ADDR(x)	((x) & 0x3FFFFF)
...
static void init_mmsram_reg(void)
{
	...
	writel((0x1 << 24) | mmsram->sram_size,
...
void mmsram_get_info(struct mmsram_data *data)
{
	...
	pr_notice("%s: pa:0x%p va:0x%p size:%zu\n",
...
int set_mmsram_data_if(struct mmsram_data *d)
{
	if (mmsram->sram_size) {
		...
		if (debug_enable)
			pr_notice("#@# %s(%d) 0x%p 0x%p %zu\n",
				__func__, __LINE__, d->paddr, d->vaddr, d->size);
		init_mmsram_reg();
	}
	return 0;
}

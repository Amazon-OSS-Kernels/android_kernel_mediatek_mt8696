...
#define IT5205_REG_GPIO_DIR        0x20
#define IT5205_REG_GPIO_IN_VAL        0x21
#define IT5205_REG_GPIO_OUT_VAL        0x22
#define IT5205_REG_GPIO_PULL        0x29
#define IT5205_REG_GPIO_PULL_EN        0x2a
#define PIN_GP5 (1 << 5)
	...
	if (event == TCP_NOTIFY_AMA_DP_STATE) {
		...
		if (state) {
			...
			if (irq) {
...
#ifdef IT5205_USE_HW_HPD
					it5205_write(g_it5205, IT5205_REG_GPIO_DIR, PIN_GP5);
					it5205_write(g_it5205, IT5205_REG_GPIO_PULL, PIN_GP5);
					it5205_write(g_it5205, IT5205_REG_GPIO_OUT_VAL, PIN_GP5);
#else
					mtk_dp_sw_interrupt_set(0x4);
#endif
				...
				it5205_write(g_it5205, IT5205_REG_GPIO_DIR, PIN_GP5);
				it5205_write(g_it5205, IT5205_REG_GPIO_PULL, PIN_GP5);
				it5205_write(g_it5205, IT5205_REG_GPIO_OUT_VAL, 0);
				udelay(600);
				it5205_write(g_it5205, IT5205_REG_GPIO_OUT_VAL, PIN_GP5);
#else
			    ...
			    const struct i2c_device_id *id)
{
	...
	sw_desc.name = "it5205fn";

	#if IS_ENABLED(CONFIG_MTK_USB_TYPEC_MUX)
	it5205->sw = mtk_typec_switch_register(dev, &sw_desc);
	#else
	...
	mux_desc.name = "it5205fn";

	#if IS_ENABLED(CONFIG_MTK_USB_TYPEC_MUX)
	it5205->mux = mtk_typec_mux_register(dev, &mux_desc);
	#else
	it5205->mux = typec_mux_register(dev, &mux_desc);
	#endif
	if (IS_ERR(it5205->mux)) {
		#if IS_ENABLED(CONFIG_MTK_USB_TYPEC_MUX)
		mtk_typec_switch_unregister(it5205->sw);
		#else
	...
	if (ret < 0) {
		#if IS_ENABLED(CONFIG_MTK_USB_TYPEC_MUX)
		mtk_typec_switch_unregister(it5205->sw);
		mtk_typec_mux_unregister(it5205->mux);
		#else
		typec_switch_unregister(it5205->sw);

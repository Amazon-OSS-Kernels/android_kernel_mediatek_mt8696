...
static int mt6375_tcpc_init_irq(struct mt6375_tcpc_data *ddata)
{
	...
	if (IS_ERR(ddata->irq_worker_task)) {
		dev_notice(ddata->dev, "%s create tcpc task fail\n", __func__);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to get irq pd_evt\n");
		return ret;
	}
	ddata->irq = ret;
	...
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to request irq %d\n", ddata->irq);
		return ret;
	}
	return 0;
}
...
static int mt6375_register_tcpcdev(struct mt6375_tcpc_data *ddata)
{
	...
	if (!boot_np) {
		dev_notice(ddata->dev, "failed to get bootmode phandle\n");
		return -ENODEV;
	}
	tag = of_get_property(boot_np, "atag,boot", NULL);
	if (!tag) {
		dev_notice(ddata->dev, "failed to get atag,boot\n");
		return -EINVAL;
	}
...
static int mt6375_check_revision(struct mt6375_tcpc_data *ddata)
{
	...
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to read vid(%d)\n", ret);
		return ret;
	}
	if (id != MT6375_VID) {
		dev_notice(ddata->dev, "incorrect vid(0x%04X)\n", id);
		return -ENODEV;
	}

	ret = mt6375_read16(ddata, TCPC_V10_REG_PID, &id);
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to read pid(%d)\n", ret);
		return ret;
	}
	if (id != MT6375_PID) {
		dev_notice(ddata->dev, "incorrect pid(0x%04X)\n", id);
		return -ENODEV;
	}

	ret = mt6375_read16(ddata, TCPC_V10_REG_DID, &id);
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to read did(%d)\n", ret);
		return ret;
	}
...
static int mt6375_tcpc_probe(struct platform_device *pdev)
{
	...
	if (!ddata->rmap) {
		dev_notice(ddata->dev, "failed to get regmap\n");
		return -ENODEV;
	}
	...
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to parse dt(%d)\n", ret);
		return ret;
	}
	...
	if (IS_ERR(ddata->adc_iio)) {
		...
		dev_notice(ddata->dev, "failed to get adc iio(%d)\n", ret);
		return ret;
	}

	ret = mt6375_register_tcpcdev(ddata);
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to register tcpcdev(%d)\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to init fod ctd(%d)\n", ret);
		goto err;
	}

	ret = mt6375_sw_reset(ddata);
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to reset sw(%d)\n", ret);
		goto err;
	}
	...
	if (!ddata->desc->en_fod) {
		...
		if (ret < 0) {
			dev_notice(ddata->dev, "failed to disable fod\n");
			goto err;
		}
	}
	...
	if (ret < 0) {
		dev_notice(ddata->dev, "failed to init irq\n");
		goto err;
	}

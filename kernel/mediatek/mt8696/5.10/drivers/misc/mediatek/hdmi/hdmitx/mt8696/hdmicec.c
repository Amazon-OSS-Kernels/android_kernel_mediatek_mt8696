...
struct APK_CEC_ACK_INFO cec_send_result;
...
inline bool mtkcec_hwrx_header_arrived(struct mtk_cec *cec)
{
	...
	is_rx_fsm_irq = cec->cec_readbit(cec,
		CEC2_INT_STA, CEC2_RX_FSM_CHG_INT_STA);
	is_rx_fsm_header = ((cec->cec_read(cec,
...
{
	return (cec->cec_read(cec,
		CEC2_RX_BUF_HEADER) & CEC2_RX_HEADER_SRC) >> 4;
}

inline unsigned int mtkcec_hwrx_get_data_4bytes(
	struct mtk_cec *cec, unsigned short reg)
{
...
{
	cec->cec_mask(cec, CEC2_RX_BUF_HEADER,
...
{
	return cec->cec_readbit(cec,
		CEC2_INT_STA, CEC2_RX_BUF_FULL_INT_STA);
}

inline void mtkcec_hwrx_buffer_full_clear(struct mtk_cec *cec)
{
	cec->cec_mask(cec, CEC2_INT_CLR,
		CEC2_RX_BUF_FULL_INT_CLR, CEC2_RX_BUF_FULL_INT_CLR);
...
inline void mtkcec_hwrx_reset(struct mtk_cec *cec)
{
	...
	cec->cec_write(cec, CEC2_TR_CONFIG,
		val | (CEC2_TX_RESET_WRITE | CEC2_RX_RESET_WRITE));
}

inline bool mtkcec_hwtx_fail(struct mtk_cec *cec)
{
	return (cec->cec_read(cec,
		CEC2_INT_STA) & CEC2_TX_INT_FAIL) ? true : false;
}

inline bool mtkcec_hwtx_fail_fsm(struct mtk_cec *cec)
{
	return ((cec->cec_read(cec,
		CEC2_TX_STATUS) & CEC2_TX_FSM) >> 16) == 0x10 ? true : false;
}

inline void mtkcec_hwtx_int_fail_clear(struct mtk_cec *cec)
{
	cec->cec_mask(cec, CEC2_INT_CLR,
		CEC2_TX_INT_FAIL_CLR, CEC2_TX_INT_FAIL_CLR);
...
{
	return cec->cec_readbit(cec,
		CEC2_INT_STA, CEC2_TX_FAIL_HEADER_ACK_INT_STA);
}
...
{
	return cec->cec_readbit(cec,
		CEC2_INT_STA, CEC2_TX_FAIL_DATA_ACK_INT_STA);
}
...
inline void mtkcec_hwtx_fail_src_intclr(struct mtk_cec *cec)
{
	...
	cec->cec_mask(cec, CEC2_INT_CLR,
		0, CEC2_TX_FAIL_SRC_INT_CLR);
}

inline bool mtkcec_hwtx_fail_longlow(struct mtk_cec *cec)
{
	return cec->cec_readbit(cec,
		CEC2_INT_STA, CEC2_LINE_lOW_LONG_INT_STA);
}
...
inline void mtkcec_hwtx_fail_longlow_intclr(struct mtk_cec *cec)
{
	...
	cec->cec_mask(cec, CEC2_INT_CLR,
		0, CEC2_LINE_lOW_LONG_INT_CLR);
}

inline bool mtkcec_hwtx_fsm_idle(struct mtk_cec *cec)
{
	return ((cec->cec_read(cec,
		CEC2_TX_STATUS) & CEC2_TX_FSM) >> 16) == 0x01 ? true : false;
}

inline bool mtkcec_hwtx_finish(struct mtk_cec *cec)
{
	return cec->cec_readbit(cec,
		CEC2_INT_STA, CEC2_TX_DATA_FINISH_INT_STA);
}
...
static void mtkcec_hwtx_setdatalen(
	struct mtk_cec *cec,
	unsigned int len)
{
...
static void mtkcec_hwtx_setdata(
	struct mtk_cec *cec,
	...
	unsigned char data_len)
{
	...
	msg[0] = blocks->opcode;
	if ((data_len - 1) > 15) {
		TX_DEF_LOG("%s: invalid data length: %d, opcode:0x%x\n",
			__func__, data_len, msg[0]);
		return;
	}
	...
	for (i = 0; i < data_len; i++) {
		...
		byte_shift = (i % 4 * 8);
		cec->cec_mask(cec, addr,
			msg[i] << byte_shift, 0xff << byte_shift);
	}
}
...
inline void mtkcec_hwtx_fail_header_int_en(
	struct mtk_cec *cec, unsigned int en)
{
	cec->cec_mask(cec,
		CEC2_INT_EN, en << 10, CEC2_TX_FAIL_HEADER_INT_EN);
}

inline void mtkcec_hwtx_fail_header_ack_int_en(
	struct mtk_cec *cec, unsigned int en)
{
	cec->cec_mask(cec,
		CEC2_INT_EN, en << 13, CEC2_TX_FAIL_HEADER_ACK_INT_EN);
}

inline void mtkcec_hwtx_fail_retransmit_int_en(
	struct mtk_cec *cec, unsigned int en)
{
	cec->cec_mask(cec,
		CEC2_INT_EN, en << 12, CEC2_TX_FAIL_RETRANSMIT_INT_EN);
}

inline void mtkcec_hwtx_finish_int_en(
	struct mtk_cec *cec, unsigned int en)
{
	cec->cec_mask(cec, CEC2_INT_EN, en << 8, CEC2_TX_DATA_FINISH_INT_EN);
}

inline void mtkcec_hwtx_fail_longlow_int_en(
	struct mtk_cec *cec, unsigned int en)
{
...
{
	return (cec->cec_read(cec,
		CEC2_INT_EN) & CEC2_TX_INT_FAIL_EN) ? true : false;
}

inline bool mtkcec_hwtx_fail_retransmit_intsta(struct mtk_cec *cec)
{
	return cec->cec_readbit(cec,
		CEC2_INT_STA, CEC2_TX_FAIL_RETRANSMIT_INT_STA);
}
...
inline void mtkcec_hwtx_fail_retransmit_intclr(struct mtk_cec *cec)
{
	cec->cec_mask(cec, CEC2_INT_CLR,
		...
		CEC2_TX_FAIL_RETRANSMIT_INT_CLR);
	cec->cec_mask(cec, CEC2_INT_CLR,
		0, CEC2_TX_FAIL_RETRANSMIT_INT_CLR);
}

inline bool mtkcec_hwtx_fail_retransmit_times(struct mtk_cec *cec)
{
	return (cec->cec_read(cec,
		CEC2_TX_STATUS) & CEC2_TX_NUM_RETRANSMIT) >> 26;
}
...
{
	cec->cec_mask(cec, CEC2_TR_CONFIG,
		la << CEC2_LA1_SHIFT, CEC2_DEVICE_ADDR1);
}

inline void mtkcec_hw_set_la2(struct mtk_cec *cec, unsigned char la)
{
	cec->cec_mask(cec, CEC2_TR_CONFIG,
		la << CEC2_LA2_SHIFT, CEC2_DEVICE_ADDR2);
}

inline void mtkcec_hw_set_la3(struct mtk_cec *cec, unsigned char la)
{
	cec->cec_mask(cec, CEC2_TR_CONFIG,
		la << CEC2_LA3_SHIFT, CEC2_DEVICE_ADDR3);
}
...
static void mtkcec_hwclk_div(struct mtk_cec *cec, unsigned int clk_src)
{
	if (clk_src == cec_clk_26m) {
		...
		cec->cec_mask(cec,
		CEC2_CKGEN, CEC2_CLK_27M_EN, CEC2_CLK_27M_EN);
		...
		cec->cec_mask(cec,
			CEC2_CKGEN, CEC2_CLK_32K_EN, CEC2_CLK_32K_EN);
		cec->cec_mask(cec,
			CEC2_CKGEN, CEC2_CLK_SEL_DIV, CEC2_CLK_SEL_DIV);
		return;
	}
}
...
static void mtkcec_hwrx_check_addr(struct mtk_cec *cec, bool enable)
{
	if (enable) {
		cec->cec_mask(cec,
			CEC2_TR_CONFIG, CEC2_RX_CHK_DST, CEC2_RX_CHK_DST);
...
static void mtkcec_hwrx_timing_config(
	struct mtk_cec *cec, unsigned int clk_src)
{
	if (clk_src == cec_clk_26m) {
	...
	cec->cec_mask(cec, CEC2_TR_CONFIG,
		CEC2_RX_ACK_ERROR_BYPASS, CEC2_RX_ACK_ERROR_BYPASS);
	mtkcec_hwrx_reset(cec);
}

static void mtkcec_hwtx_timing_config(struct mtk_cec *cec,
	unsigned int clk_src)
{
	if (clk_src == cec_clk_26m) {
		...
		cec->cec_mask(cec, CEC2_TX_TIMER_ACK, 0x00aa0028,
			CEC2_TX_TIMER_ACK_MAX | CEC2_TX_TIMER_ACK_MIN);
		cec->cec_mask(cec, CEC2_CKGEN, CEC2_CLK_27M_EN,
			CEC2_CLK_27M_EN);
	} else {
		...
		cec->cec_mask(cec, CEC2_TX_TIMER_ACK, 0x0036000d,
			CEC2_TX_TIMER_ACK_MAX | CEC2_TX_TIMER_ACK_MIN);
		return;
	}
	cec->cec_mask(cec, CEC2_TX_ARB, 0x01 << 24,
		CEC2_TX_MAX_RETRANSMIT_NUM_ARB);
	cec->cec_mask(cec, CEC2_TX_ARB, 0x01 << 20,
		CEC2_TX_MAX_RETRANSMIT_NUM_COL);
	cec->cec_mask(cec, CEC2_TX_ARB, 0x01 << 16,
		CEC2_TX_MAX_RETRANSMIT_NUM_NAK);
	cec->cec_mask(cec, CEC2_TX_ARB, 0x03 << 8,
...
inline void mtkcec_hwtxrx_interrupts_config(struct mtk_cec *cec)
{
	...
	cec->cec_mask(cec, CEC2_INT_EN,
	CEC2_RX_INT_ALL_EN, CEC2_RX_INT_ALL_EN);
	...
	CEC2_TX_FAIL_RETRANSMIT_INT_EN);
	cec->cec_mask(cec, CEC2_INT_EN,
		CEC2_TX_DATA_FINISH_INT_EN, CEC2_TX_DATA_FINISH_INT_EN);
}
...
void mtkcec_irq2gic_clear(struct mtk_cec *cec)
{
	...
	cec->cec_mask(cec,
	CEC2_INT_CLR, CEC2_RX_INT_ALL_CLR,
	CEC2_RX_INT_ALL_CLR);
...
void hdmi_cec_power_on(struct mtk_cec *cec, bool pwr)
{
	...
	if (pwr == false) {
		cec->cec_mask(cec,
			CEC2_CKGEN, CEC2_CLK_PDN, CEC2_CLK_PDN);
...
{
	static struct CEC_FRAME_DESCRIPTION_IO *frame =
		&ActiveRXFrame;
	unsigned int data[4];
	...
	for (i = 0; i < rxlen; i++) {
		...
		if (shift == 0)
			data[data_index] =
			mtkcec_hwrx_get_data_4bytes(cec,
			CEC2_RX_DATA0 + 4 * data_index);
		oper_index = i > 1 ? (i - 1) : 0;
		frame->blocks.operand[oper_index] =
			(data[data_index] >> shift) & 0xff;
	}
	...
	if (true != CEC_rx_enqueue(frame)) {
		...
		TX_DEF_LOG("[CEC] Tx fsm=0x%lx\n",
			(cec->cec_read(cec,
			CEC2_TX_STATUS) & CEC2_TX_FSM) >> 16);
		TX_DEF_LOG("[CEC] Rx fsm=0x%lx\n",
			(cec->cec_read(cec,
			CEC2_RX_STATUS) & CEC2_RX_FSM) >> 16);
		/* clear Rx queue*/
		memset(CEC_rx_msg_queue, 0,
			sizeof(struct CEC_FRAME_DESCRIPTION_IO) * RX_Q_SIZE);
		CEC_rxQ_write_idx = 0;
		CEC_rxQ_read_idx = 0;
		...
		ClrCECStatus(STATE_WAIT_TX_DATA_TAKEN |
			STATE_TX_NOACK |
			STATE_TXING_FRAME |
			STATE_TX_FRAME_SUCCESS |
			STATE_HW_RETX |
			STATE_WAIT_RX_FRAME_COMPLETE |
			STATE_RX_COMPLETE_NEW_FRAME |
			STATE_HW_RX_OVERFLOW |
			STATE_RX_GET_NEW_HEADER |
			STATE_WAIT_TX_CHECK_RESULT |
			STATE_TXFAIL_DNAK |
			STATE_TXFAIL_HNAK |
			STATE_TXFAIL_RETR|
			STATE_TXFAIL_DATA |
			...
			STATE_TXFAIL_SRC |
			STATE_TXFAIL_LOW);
		ClrCECErrorFlag(ERR_TX_BUFFER_LOW |
			ERR_TX_UNDERRUN |
			ERR_TX_MISALARM |
			ERR_RX_LOST_EOM |
			ERR_RXQ_OVERFLOW |
...
static struct CEC_FRAME_DESCRIPTION_IO *
...
static void _CEC_tx_msg_notify(
	...
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	if (result == 0x00) {
		...
		HDMI_CEC_COMMAND_LOG("%s-->success: 0x%x(%x|%x)\n",
			"sending opcode",
			frame->blocks.opcode,
		...
		if ((frame->size == 0x2) &&
			(frame->blocks.header.initiator == 4)
		    && (frame->blocks.header.destination == 4) &&
		    (frame->blocks.opcode == 0))
			HDMI_CEC_COMMAND_LOG("get LA of player\n");
		else
			TX_DEF_LOG(
			"[CEC] tx result fail, op=0x%x\n",
			frame->blocks.opcode);
...
void hdmi_cec_api_get_txsts(struct APK_CEC_ACK_INFO *pt)
{
	memcpy(pt, &cec_send_result, sizeof(struct APK_CEC_ACK_INFO));
}

static void vApiNotifyCECDataArrival(
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
...
static unsigned char vGetcecmsg_poweron(
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (frame->blocks.opcode == OPCODE_USER_CONTROL_PRESSED) {
		if ((frame->blocks.operand[0] == 0x40) ||
		   ...
		   (frame->blocks.operand[0] <= 0x08)) {
			TX_DEF_LOG(
"[CECWAKE]waking up on power command(0x%x:0x%x) from tv\n",
	...
	if ((frame->blocks.opcode == OPCODE_SET_STREAM_PATH) ||
		(frame->blocks.opcode == OPCODE_ROUTING_INFORMATION))
		msg_phy_addr = ((
		frame->blocks.operand[0] & 0xff) << 8) |
		(frame->blocks.operand[1] & 0xff);

	if (frame->blocks.opcode == OPCODE_ROUTING_CHANGE)
		msg_phy_addr = ((
		frame->blocks.operand[2] & 0xff) << 8) |
		(frame->blocks.operand[3] & 0xff);

	if (msg_phy_addr != 0xFFFF) {
		TX_DEF_LOG(
"[CECWAKE]received power command(0x%x), msg_phy_addr=0x%x\n",
			frame->blocks.opcode, msg_phy_addr);
		TX_DEF_LOG(
"[CECWAKE]_rCECPhysicAddr.ui2_pa=0x%x, cec_wakeup_addr=0x%x\n",
			_rCECPhysicAddr.ui2_pa, cec_wakeup_addr);

		if (msg_phy_addr == _rCECPhysicAddr.ui2_pa) {
			TX_DEF_LOG(
"[CECWAKE]waking up on power command(0x%x) from tv\n",
frame->blocks.opcode);
			return 1;
		}
		...
		TX_DEF_LOG(
"[CECWAKE]updated _rCECPhysicAddr.ui2_pa=0x%x, cec_wakeup_addr=0x%x\n",
			_rCECPhysicAddr.ui2_pa, cec_wakeup_addr);
...
static void PrintFrameDescription(
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	unsigned char i;
...
static unsigned char _CEC_SendFrame(
	struct mtk_cec *cec,
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	struct CEC_FRAME_BLOCK_IO *blocks;
	...
	ClrCECStatus(STATE_TX_FRAME_SUCCESS |
		STATE_TX_NOACK | STATE_WAIT_TX_CHECK_RESULT);
	ClrCECStatus(STATE_TXFAIL_DNAK |
		STATE_TXFAIL_HNAK | STATE_TXFAIL_RETR);
	ClrCECStatus(STATE_TXFAIL_DATA |
		STATE_TXFAIL_HEAD | STATE_TXFAIL_SRC | STATE_TXFAIL_LOW);
...
{
	struct CEC_FRAME_DESCRIPTION_IO *frame = NULL;
	...
	if (IsCECStatus(STATE_TXFAIL_RETR)) {
		if ((frame != NULL) && (frame->blocks.header.destination ==
	...
	if (IsCECStatus(STATE_TX_FRAME_SUCCESS)) {
		...
		ClrCECStatus(STATE_TX_FRAME_SUCCESS);
		if (frame) {
			if (frame->blocks.header.destination ==
				CEC_LOG_ADDR_UNREGED_BRDCST)
	...
	if (IsCECStatus(STATE_WAIT_TX_CHECK_RESULT)) {
		...
		if (frame != NULL) {
			...
			if (!_CEC_SendFrame(cec, frame))
				HDMI_CEC_COMMAND_LOG("%s: 0x%x(%x|%x)\n",
					"sending opcode",
					frame->blocks.opcode,
					frame->blocks.header.initiator,
					frame->blocks.header.destination);
...
{
	struct CEC_FRAME_DESCRIPTION_IO *txmsg;
	...
	while (!(tx_read_idx == tx_write_idx)) {
		...
		if (txmsg->size > 0)
			len += sprintf(str,
			"txmsgq[%02d]=0x%02x", (i - 1),
			(txmsg->blocks.header.initiator << 4) |
			txmsg->blocks.header.destination);
		if (txmsg->size > 1)
			len += sprintf(str, "0x%02x",
			txmsg->blocks.opcode);
		for (j = 2; j < txmsg->size; j++)
			len += sprintf(str, "0x%02x",
			txmsg->blocks.operand[j - 2]);
...
{
	struct CEC_FRAME_DESCRIPTION_IO *rxmsg;
	...
	while (!(rx_read_idx == rx_write_idx)) {
		...
		if (rxmsg->size > 0)
			len += sprintf(str,
			"rxmsgq[%02d]=0x%02x", (i - 1),
			(rxmsg->blocks.header.initiator << 4) |
			rxmsg->blocks.header.destination);
		if (rxmsg->size > 1)
			len += sprintf(str, "0x%02x",
			rxmsg->blocks.opcode);
		for (j = 2; j < rxmsg->size; j++)
			len += sprintf(str, "0x%02x",
			rxmsg->blocks.operand[j - 2]);
...
static int hdmi_cec_args_dump(char *str)
{
	...
	memcpy(&name, &cec->kh.saved_arg.osd_name,
		cec->kh.saved_arg.osd_namelen);
	...
	if (len <= 0)
		...
		__LINE__, __func__, len);
	len += sprintf(str,
		"vendor_id[0-2]=0x%02x,0x%02x,0x%02x\n",
		cec->kh.saved_arg.vendor_id[0],
		cec->kh.saved_arg.vendor_id[1],
		cec->kh.saved_arg.vendor_id[2]);
	if (len <= 0)
		TX_DEF_LOG("%d %s sprintf len err, %d\n",
		__LINE__, __func__, len);
	len += sprintf(str, "osd_name=%s\n", name);
	if (len <= 0)
		TX_DEF_LOG("%d %s sprintf len err, %d\n",
		__LINE__, __func__, len);
...
unsigned char _CEC_TX_Enqueue(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (IS_TX_Q_FULL()) {
		...
		/* clear Tx queue*/
		memset(CEC_tx_msg_queue, 0,
			sizeof(struct CEC_FRAME_DESCRIPTION_IO) * TX_Q_SIZE);
		CEC_txQ_write_idx = 0;
		CEC_txQ_read_idx = 0;
...
void CTS_RXProcess(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	...
	if (frame->blocks.opcode == OPCODE_ABORT) {
		...
		_CEC_TX_Enqueue(&CTSTestFrame);
		HDMI_CEC_LOG("C_T_S Send: OPCODE_FEATURE_ABORT\n");
		...
		_CEC_TX_Enqueue(&CTSTestFrame);
		HDMI_CEC_LOG("C_T_S Send: OPCODE_REPORT_PHYSICAL_ADDRESS\n");
	}
}

void FAC_RXProcess(struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	cec_receive_msg = frame;
	if (frame->blocks.opcode == OPCODE_FEATURE_ABORT)
		TX_DEF_LOG("CEC %s pass:0x%x\n", __func__,
		frame->blocks.opcode);
	else
		TX_DEF_LOG("CEC %s err:0x%x\n", __func__,
		frame->blocks.opcode);
}

static void CEC_KHandle_RXNotify(
	struct CEC_FRAME_DESCRIPTION_IO *frame)
{
	struct mtk_cec *cec = global_cec;
...
static int CEC_KHandle_TxWaitResult(struct mtk_cec *cec)
{
	...
	ret = wait_event_interruptible_timeout(cec->kh.waitq_tx,
		(cec->kh.tx_result.pv_tag == cec->kh.txing_frame.txtag),
...
static void CEC_KHandle_BuildReplyMsg(struct mtk_cec *cec)
{
	...
	cec->kh.txing_frame.blocks.header.initiator =
		CEC_KHandle_GetBestLa(cec);
	cec->kh.txing_frame.blocks.header.destination =
		rx_frame->blocks.header.initiator;
...
unsigned char hdmi_cec_isrprocess(
...
void CECMWSetLA(struct CEC_LA_ADDRESS *prLA)
{
	...
	HDMI_CEC_LOG("LA num = 0x%x , LA = 0x%x 0x%x 0x%x\n",
		_rCECLaAddr.ui1_num,
		     _rCECLaAddr.aui1_la[0],
		     _rCECLaAddr.aui1_la[1],
		     _rCECLaAddr.aui1_la[2]);
	HDMI_CEC_LOG("PA = %04x\n", _rCECLaAddr.ui2_pa);
}

void hdmi_CECMWSetLA(struct CEC_DRV_ADDR_CFG *prAddr)
{
...
bool hdmi_CECSendWaitResult(struct mtk_cec *cec)
{
	...
	ret = wait_event_interruptible_timeout(cec->waitq_tx,
		(cec->txmsg_usr.result.pv_tag ==
		cec->txmsg_usr.pv_tag),
		msecs_to_jiffies(2000));

	if ((ret > 0) && (cec->txmsg_usr.result.e_ack_cond ==
		APK_CEC_ACK_COND_OK)) {
...
void hdmi_CECMWSend(struct CEC_SEND_MSG *msg)
{
	unsigned int i4Ret;
	unsigned char i;
	struct mtk_cec *cec = global_cec;
	...
	cec->txmsg_ker.blocks.header.initiator =
		cec->txmsg_usr.t_frame_info.ui1_init_addr;
	cec->txmsg_ker.blocks.header.destination =
		cec->txmsg_usr.t_frame_info.ui1_dest_addr;
	cec->txmsg_ker.sendidx = 0;
	cec->txmsg_ker.reTXcnt = 0;
	if ((cec->txmsg_usr.t_frame_info.ui2_opcode == 0xffff)
		|| (cec->txmsg_usr.t_frame_info.ui2_opcode == 0x0 &&
		   cec->txmsg_usr.t_frame_info.z_operand_size == 0))
		cec->txmsg_ker.size = 1;
	else {
		cec->txmsg_ker.blocks.opcode =
			cec->txmsg_usr.t_frame_info.ui2_opcode;
		cec->txmsg_ker.size =
			cec->txmsg_usr.t_frame_info.z_operand_size + 2;
	}
	for (i = 0; i < cec->txmsg_ker.size - 2; i++)
		cec->txmsg_ker.blocks.operand[i] =
		cec->txmsg_usr.t_frame_info.aui1_operand[i];
...
void hdmi_CECMWSetEnableCEC(unsigned char u1EnCec)
{
	...
	if ((u1EnCec == 1) && (hdmi_cec_on == 0)) {
		...
		HDMI_CEC_COMMAND_LOG(
		"[hdmi_cec]on/off fail,en=%d,hdmi_cec_on=%d\n",
			u1EnCec, hdmi_cec_on);
	}
}

void hdmi_CECMWSetEnableARC(unsigned char u1EnArc)
{
	cec_enable_arc(u1EnArc);
	ARCIN_Enable(u1EnArc);
}
...
void hdmi_SetPhysicCECAddress(unsigned short u2pa, unsigned char u1la)
{
	...
	HDMI_CEC_LOG(
		"cec_wakeup_addr=0x%x, u2pa=0x%x,u1la=0x%x\n",
...
bool hdmi_cec_factory_test(void)
{
	...
	struct CEC_SEND_MSG txdata;
	unsigned int result;
	struct CEC_FRAME_DESCRIPTION_IO rxdata;
	unsigned char backup_rxcecmode = hdmi_rxcecmode;
	int wait_times, retry = 2;

	memset(&txdata, 0, sizeof(txdata));
	memset(&rxdata, 0, sizeof(rxdata));
	memset(&cecsetAddr, 0, sizeof(cecsetAddr));
...
inline bool impl_cec_readbit(struct mtk_cec *cec,
	unsigned short reg, unsigned int offset)
{
	return (readl(cec->cec_base + reg) & offset) ? true : false;
}
...
inline void impl_cec_write(struct mtk_cec *cec,
			 ...
			 unsigned int val, unsigned int mask)
{
	...
	tmp = readl(cec->cec_base + reg) & ~mask;
	tmp |= (val & mask);
	writel(tmp, cec->cec_base + reg);
}
...
inline void mtkcec_kernel_handle_probe(struct mtk_cec *cec)
{
	static const char name[15] = "Amazon FireTV";
	...
	memcpy(&cec->kh.saved_arg.osd_name,
		name, cec->kh.saved_arg.osd_namelen);
...
static irqreturn_t cec_irq_handler(int irq, void *dev_id)
{
	...
	if (hdmi_cec_on == 1)
		hdmi_cec_isrprocess(cec, hdmi_rxcecmode);
...
int hdmi_cec_probe(struct platform_device *pdev)
{
	...
	cec->cec_base = of_iomap(of_node, REG_HDMI_CEC);
	if (cec->cec_base == NULL) {
	...
	mtkcec_kernel_handle_probe(cec);
	cec->irq = irq_of_parse_and_map(of_node, 1);
	cec_irq_type = irq_get_trigger_type(cec->irq);
	if (request_irq(cec->irq, cec_irq_handler,
		cec_irq_type, "cecirq", cec) < 0)

...
enum HDMI_INTERNAL_SUSPEND_RETURN {
	HDMI_INTERNAL_SUSPEND_IN_NORMAL_OK = 0,
	HDMI_INTERNAL_SUSPEND_IN_RECOVERY_OK = 1,
};

enum HDMI_INTERNAL_RESUME_RETURN {
	HDMI_INTERNAL_RESUME_IN_NORMAL_OK = 0,
	HDMI_INTERNAL_RESUME_IN_RECOVERY_OK = 1,
};
...
struct HDMI_DRIVER {
	...
	void (*setcecla)(struct CEC_DRV_ADDR_CFG *prAddr);
	...
	void (*setceccmd)(struct CEC_SEND_MSG *msg);
	void (*getcectxstatus)(struct APK_CEC_ACK_INFO *pt);
	void (*cecenable)(unsigned char u1EnCec);
	void (*arcenable)(unsigned char u1EnArc);

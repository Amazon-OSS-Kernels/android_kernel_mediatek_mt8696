...
/* HDMI Timer */
/*-----------------------------------------------------*/
static ktime_t hdmi_ktime;
static ktime_t cec_ktime;
static struct hrtimer r_hdmi_timer;
...
#define HDMI_TIMER_INTERVAL_NS 10000000 //10ms
#define HDMI_TIMER_INTERVAL_S  0 //0s
#define CEC_TIMER_INTERVAL_NS 20000000 //20ms
#define CEC_TIMER_INTERVAL_S  0 //0s
...
static unsigned int _HdmiTmrValue[MAX_HDMI_TMR_NUMBER];
...
unsigned char hdmi_boot_powerenable = 1;
static unsigned char hdmi_port_status_in_boot = 0xff;
...
struct clk *vdout_hd_sel;
struct clk *vdout_sd_sel;
struct clk *hdmitx_pxl_clk;
struct clk *hdmitx_pxl_clk_d6;

struct workqueue_struct *hdmi_wq;
struct delayed_work dolby_work;
struct delayed_work hfvsif_delay_work;
struct delayed_work hdr10p_delay_work;

unsigned int port_edge_cnt[PORT_EDGE_RECORD_CNT];
unsigned int port_irq_value[PORT_IRQ_CNT];
unsigned int port_status_value[PORT_IRQ_CNT];
unsigned char port_status;

static enum hrtimer_restart hdmi_poll_isr(struct hrtimer *timer);
...
static void vPlugDetectService(
	enum HDMI_CTRL_STATE_T e_state);
...
unsigned int hdmi_reg_pa_base[HDMI_REG_NUM] = {
	...
	0x10014000,		/*REG_HDMI_CEC */
	0x1a000000,		/*REG_HDMI_DIG */
	0x11f30000,		/*REG_HDMI_ANA */
	0x14001000,		/*REG_HDMI_VDOUT */
};
...
void hdmi_parse_videolfb(struct device *dev)
{
	...
	if (videolfb_tag) {
		...
		current_hdr_mode = hdmi_boot_forcedolby & 0xF;
		if (current_hdr_mode == HDMI_FORCE_SDR)
			hdmi_force_sdr = TRUE;
		else
			hdmi_force_sdr = FALSE;
	}
	TX_DEF_LOG("[DT]hdmi_boot_res=%d,%d,%d,0x%x,%d,%d\n", hdmi_boot_res,
		hdmi_boot_colorspace, hdmi_boot_colordepth,
		hdmi_boot_forcedolby,
		hdmi_boot_forcehdr, hdmi_force_sdr);
}

void mtk_hdmi_audio_extcon_state(bool state)
{
	extcon_set_state_sync(hdmi_extcon, EXTCON_DISP_HDMI, state);
}

const char *hdmi_use_clock_name_spy(
	enum HDMI_REF_CLOCK_ENUM module)
{
	switch (module) {
	...
	case MMSYS_HDMI_PXD3:
		return "hdmi_pxd3";
	case MMSYS_HDMI_SPDDF:
		return "hdmi_sdppf";
	case MMSYS_HD20_DACR_SEL:
		return "hd20_dacrsel";
	case MMSYS_HD20_HDCP_SEL:
		return "hd20_hdcpsel";
	case MMSYS_HDMI_APB_SEL:
		return "hdmi_apbsel";
	case MMSYS_HDMI_SEL:
		return "hdmi_sel";
	case MMSYS_HDMI_P2I:
		return "hdmi_p2i";
	case TOP_HDMI_OSDPLL:
		return "hdmi_osdpll";
...
const char *hdmi_use_module_name_spy(
	enum HDMI_REF_MODULE_ENUM module)
{
	switch (module) {
	case AP_CCIF0:
	...
	case P2I_REG:
		return "mediatek,mt8696-p2i";
	case VDOUT10_REG:
		return "mediatek,mt8696-fmt";
	case VDOUT15_REG:
		return "mediatek,mt8696-vdout";
	case VDOUT1C_REG:
		return "mediatek,mt8696-vdout";
...
{
	if (hdmi_boot_res < HDMI_VIDEO_RESOLUTION_NUM)
	...
	if ((hdmi_boot_colorspace == HDMI_YCBCR_444) ||
		(hdmi_boot_colorspace == HDMI_YCBCR_444_FULL)
	    || (hdmi_boot_colorspace == HDMI_YCBCR_420) ||
	    (hdmi_boot_colorspace == HDMI_YCBCR_420_FULL)
	    || (hdmi_boot_colorspace == HDMI_XV_YCC)) {
		vWriteByteHdmiGRL(TOP_VMUTE_CFG1, 0x8000);
		vWriteByteHdmiGRL(TOP_VMUTE_CFG2, 0x80001000);
	} else if ((hdmi_boot_colorspace == HDMI_YCBCR_422) ||
	(hdmi_boot_colorspace == HDMI_YCBCR_422_FULL)) {
		vWriteByteHdmiGRL(TOP_VMUTE_CFG1, 0x8000);
...
void hdmi_timing_monitor(void)
{
	...
	if ((dReadIoHdmiAna(HDMI_1_CFG_0) &
	...
	if (pll_read_state == 0) {
		...
		if (monitor_count > 300) {
			if (p_stable_fail) {
				p_stable_fail = FALSE;
				TX_DEF_LOG(
			"p_stable fail, pix:%d/%dkhz, tmds:%d/%dkhz\n",
					p_stable_fail_pixel_clk,
					((p_stable_fail_pixel_clk * 52000) /
					1024),
					p_stable_fail_tmds_clk,
					((p_stable_fail_tmds_clk * 52000) /
					1024));
			}

			if (pll_read_value_err) {
				TX_DEF_LOG(
		"pixel clk change, base:%d/%dkhz, err:%d/%dkhz, temp:%d/%dkhz\n",
					pll_read_value,
					((pll_read_value * 52000) / 1024),
					pll_read_value_bak,
...
/* hdmi_suspend_callback is for deep sleep */
static int hdmi_suspend_callback(struct notifier_block *nb,
		unsigned long action, void *data)
{
	...
	if (r_hdmi_timer.function) {
		ret = hrtimer_cancel(&r_hdmi_timer);
		TX_DEF_LOG("[hdmi]cancel r_hdmi_timer ret = %d.\n", ret);
	}
	...
	hdmi_CECMWSetEnableCEC(false);

	/* close hdmi power domain */
	mdelay(500);
	if (hdmi_hdmi_on == 1) {
...
{
	/* we do suspend work in func hdmi_suspend_callback */
	TX_DEF_LOG("[hdmi]%s.\n", __func__);
...
/*------------------------------------------------------*/
static int hdmi_internal_suspend(void)
{
	int ret = HDMI_INTERNAL_SUSPEND_IN_NORMAL_OK;
	HDMI_DRV_FUNC();
	TX_DEF_LOG("hdmi_early_suspend %d\n", hdmi_suspend_en);
	display_off = 1;
	hdmi_drv_write(0x200, _stAvdAVInfo.e_resolution << 24);
	hdmi_drv_write(0x208, (_stAvdAVInfo.e_deep_color_bit << 24) |
		(_stAvdAVInfo.e_video_color_space << 28));
	hdmi_drv_write(0x20c, (HDMI_AKSV[0] << 24) |
		(HDMI_AKSV[1] << 16));
	hdmi_drv_write(0x210, (HDMI_AKSV[2] << 24) |
		(HDMI_AKSV[3] << 16) |
		(HDMI_AKSV[4] << 8));
	...
	if (hdmi_suspend_en && (HdmiEarlySuspendMode == HDMI_EARLY_SUSPEND_MODE_POWER_OFF)) {
		if (hdmi_boot_powerenable == 1) {
			ret = HDMI_INTERNAL_SUSPEND_IN_RECOVERY_OK;
...
static int hdmi_internal_resume(void)
{
	int ret = HDMI_INTERNAL_RESUME_IN_NORMAL_OK;
	HDMI_DRV_FUNC();
	TX_DEF_LOG("hdmi_late_resume %d\n", hdmi_boot_powerenable);
	display_off = 0;
	if (!hdmi_boot_powerenable)
		hdmi_internal_power_on();

	if (hdmi_boot_powerenable == 1) {
		ret = HDMI_INTERNAL_RESUME_IN_RECOVERY_OK;
		TX_DEF_LOG("hdmi resume in recovery.\n");
	}
	hdmi_drv_write(0x200, 0x0);
	if (_fgDolbyHdrEnable == false)
		vHDMIAVUnMute();
...
{
	vWriteByteHdmiGRL(TOP_INT_MASK00, 0x02000010);
	vWriteByteHdmiGRL(TOP_INT_MASK01, 0x00000000);
}
...
int force_video_config(enum HDMI_VIDEO_RESOLUTION vformat)
{
	...
	if (hpd_toggle_videoconfig_flag == HPDTOGGLE_VIDEOCONFIG) {
		...
		HDMI_HDCP_LOG("set video_config from switch resolution\n");
	}
	hdmistate_debug = 0;
...
int pmx_parameter(enum HDMI_VIDEO_RESOLUTION vformat, int temp)
{
...
void hdmitx_confighdmisetting(unsigned int resolutionmode)
{
	...
	vWriteIoHdmiAnaMsk(HDMI_ANA_CTL, reg_ana_hdmi20_fifo_en,
	...
	enum HDMI_VIDEO_RESOLUTION vformat)
{
	...
	if ((vformat == HDMI_VIDEO_720x480i_60Hz) ||
		(vformat == HDMI_VIDEO_720x576i_50Hz) ||
		(vformat == HDMI_VIDEO_1920x1080i_60Hz) ||
		(vformat == HDMI_VIDEO_1920x1080i_50Hz)) {
		if (hdmi_p2icksel == 0) {
			ret = clk_prepare(hdmi_ref_clock[MMSYS_HDMI_P2I]);
			if (ret != 0) {
				HDMI_DRV_LOG("[hdmi]clk_prepare fail\n");
				return;
			}
			ret = clk_enable(hdmi_ref_clock[MMSYS_HDMI_P2I]);
			if (ret) {
				HDMI_DRV_LOG("[hdmi]clk_enable fail\n");
				return;
			}
			hdmi_p2icksel = 1;
		}
		...
		if (hdmi_p2icksel == 1) {
			clk_disable(hdmi_ref_clock[MMSYS_HDMI_P2I]);
			clk_unprepare(hdmi_ref_clock[MMSYS_HDMI_P2I]);
			hdmi_p2icksel = 0;
		}
	}
}

static void hdmi_420_setting(void)
{
	if (_stAvdAVInfo.e_video_color_space == HDMI_YCBCR_420) {
		...
		vWriteHdmiGRLMsk(VID_DOWNSAMPLE_CONFIG,
		...
		vWriteHdmiGRLMsk(VID_OUT_FORMAT,
			0, OUTPUT_FORMAT_DEMUX_420_ENABLE);
	}
}

int hdmi_internal_video_config(
	enum HDMI_VIDEO_RESOLUTION vformat)
{
	unsigned int i;
	...
	TX_DEF_LOG("%s, format=0x%x, cs=%d\n",
		__func__, vformat, _stAvdAVInfo.e_video_color_space);

	_fgVoutInVsync = FALSE;

#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
	switch (hdcp_max_level) {
	case HDCP_V2_2:
	...
	case HDCP_V1_4:
		...
		break;
	case NO_HDCP:
		vCaHDMIWriteHdcpCtrl(0x88880000, 0xaaaa0003);
		break;
	default:
		vCaHDMIWriteHdcpCtrl(0x88880000, 0xaaaa0003);
		break;
	...
	/* need vdout to setting 480P/576P NG */
	hdmi_config_ref_clock(vformat);
...
{
	static struct HDMITX_AUDIO_PARA last_audio_para;
	...
	if ((_stAvdAVInfo.e_hdmi_aud_in == last_audio_para.e_hdmi_aud_in) &&
		...
		last_audio_para.u1Aud_Input_Chan_Cnt) &&
		...
		last_audio_para.bhdmi_LCh_status[0]) &&
		(_stAvdAVInfo.bhdmiLChstatus[1] ==
		last_audio_para.bhdmi_LCh_status[1]) &&
		(_stAvdAVInfo.bhdmiLChstatus[2] ==
		last_audio_para.bhdmi_LCh_status[2]) &&
		(_stAvdAVInfo.bhdmiLChstatus[3] ==
		last_audio_para.bhdmi_LCh_status[3]) &&
		(_stAvdAVInfo.bhdmiLChstatus[4] ==
		last_audio_para.bhdmi_LCh_status[4]) &&
		(_stAvdAVInfo.bhdmiRChstatus[0] ==
		last_audio_para.bhdmi_RCh_status[0]) &&
		(_stAvdAVInfo.bhdmiRChstatus[1] ==
		last_audio_para.bhdmi_RCh_status[1]) &&
		(_stAvdAVInfo.bhdmiRChstatus[2] ==
		last_audio_para.bhdmi_RCh_status[2]) &&
		(_stAvdAVInfo.bhdmiRChstatus[3] ==
		last_audio_para.bhdmi_RCh_status[3]) &&
		(_stAvdAVInfo.bhdmiRChstatus[4] ==
		last_audio_para.bhdmi_RCh_status[4])) {
		skip_audio_setting = 1;
	} else {
		skip_audio_setting = 0;
		/* SV_I2S; */
		last_audio_para.e_hdmi_aud_in = audio_para->e_hdmi_aud_in;
		/* IEC_48K; */
		last_audio_para.e_iec_frame = audio_para->e_iec_frame;
		/* HDMI_FS_48K; */
		last_audio_para.e_hdmi_fs = audio_para->e_hdmi_fs;
		/* AVD_LPCM; */
		last_audio_para.e_aud_code = audio_para->e_aud_code;
		/* AUD_INPUT_2_0; */
		last_audio_para.u1Aud_Input_Chan_Cnt =
		audio_para->u1Aud_Input_Chan_Cnt;
		/* HDMI_I2S_24BIT; */
		last_audio_para.e_I2sFmt = audio_para->e_I2sFmt;
		/* MCLK_128FS; */
		last_audio_para.u1HdmiI2sMclk = audio_para->u1HdmiI2sMclk;
		last_audio_para.bhdmi_LCh_status[0] =
			audio_para->bhdmi_LCh_status[0];
		last_audio_para.bhdmi_LCh_status[1] =
			audio_para->bhdmi_LCh_status[1];
		last_audio_para.bhdmi_LCh_status[2] =
			audio_para->bhdmi_LCh_status[2];
		last_audio_para.bhdmi_LCh_status[3] =
			audio_para->bhdmi_LCh_status[3];
		last_audio_para.bhdmi_LCh_status[4] =
			audio_para->bhdmi_LCh_status[4];
		last_audio_para.bhdmi_RCh_status[0] =
			audio_para->bhdmi_RCh_status[0];
		last_audio_para.bhdmi_RCh_status[1] =
			audio_para->bhdmi_RCh_status[1];
		last_audio_para.bhdmi_RCh_status[2] =
			audio_para->bhdmi_RCh_status[2];
		last_audio_para.bhdmi_RCh_status[3] =
			audio_para->bhdmi_RCh_status[3];
		last_audio_para.bhdmi_RCh_status[4] =
			audio_para->bhdmi_RCh_status[4];
	}
	...
	av_hdmiset(HDMI_SET_AUDIO_CHG_SETTING, &_stAvdAVInfo, 1);
		TX_DEF_LOG(
		"e_hdmi_aud_in=%d,e_iec_frame=%d,e_hdmi_fs=%d\n",
			_stAvdAVInfo.e_hdmi_aud_in,
		     _stAvdAVInfo.e_iec_frame, _stAvdAVInfo.e_hdmi_fs);
		TX_DEF_LOG(
		"e_aud_code=%d,u1Aud_Input_Chan_Cnt=%d,e_I2sFmt=%d\n",
...
int hdmi_internal_audio_config(
...
int hdmi_internal_power_on(void)
{
	...
	hdmi_is_boot_time = 0;
	hdmi_boot_powerenable = 0;
	HDMI_PLUG_LOG("[hdmi]hdmi_powerenable = %d\n", hdmi_powerenable);
	...
	memset((void *)&r_hdmi_timer, 0, sizeof(r_hdmi_timer));
	hrtimer_init(&r_hdmi_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	r_hdmi_timer.function = &hdmi_poll_isr;
	hdmi_ktime = ktime_set(HDMI_TIMER_INTERVAL_S, HDMI_TIMER_INTERVAL_NS);
	hrtimer_start(&r_hdmi_timer, hdmi_ktime, HRTIMER_MODE_REL);
...
void vReadHdcpVersion(void)
{
	...
	if (fgDDCDataRead(RX_ID, RX_REG_HDCP2VERSION, 1,
		&bTemp)) {
		if (bTemp & 0x4) {
	...
	if (fgDDCDataRead(RX_ID, RX_REG_BCAPS, 1,
		&bTemp)) {
		if (bTemp != 0xff) {
			hdcp2_version_flag = FALSE;
	...
	vCaHDMIWriteHdcpCtrl(0x88880000, 0xaaaa0003);
#endif
	HDMI_PLUG_LOG("sink don't support HDCP\n");
}
...
void hdmi_internal_power_off(void)
{
	...
	if (r_hdmi_timer.function) {
		TX_DEF_LOG("cancel hdmi timer\n");
		hrtimer_cancel(&r_hdmi_timer);
	}
...
enum HDMI_STATE hdmi_get_state(void)
{
	...
	if (bCheckPortStatus(PORD_MODE | HOTPLUG_MODE) == TRUE)
		return HDMI_STATE_ACTIVE;
	else if (bCheckPortStatus(HOTPLUG_MODE) == TRUE)
...
void hdmi_colordeep(unsigned char u1colorspace,
	unsigned char u1deepcolor)
{
	...
	TX_DEF_LOG("%s:u1colorspace = %d; u1deepcolor = %d\n",
		__func__,
	...
	if ((u1colorspace == hdmi_boot_colorspace) &&
		(u1deepcolor == hdmi_boot_colordepth)) {
		TX_DEF_LOG(
		"LK & Kernel same colorspcace and deepcolor: %d;  %d\n",
			hdmi_boot_colorspace, hdmi_boot_colordepth);
		return;
	}
}
...
void hdmi_read(unsigned int u2Reg, unsigned int *p4Data)
{
	...
	case 0x1a000000:
		internal_hdmi_read(hdmi_reg[REG_HDMI_DIG] +
			u2Reg - 0x1a000000, p4Data);
		break;

	case 0x10014000:
		internal_hdmi_read(hdmi_reg[REG_HDMI_CEC] +
			u2Reg - 0x10014000, p4Data);
		break;
	...
	case 0x14001800:
		internal_hdmi_read(hdmi_reg[REG_HDMI_RGB2HDMI] +
			u2Reg - 0x14001800, p4Data);
		break;

	case 0x11f30000:
		internal_hdmi_read(hdmi_reg[REG_HDMI_ANA] +
			u2Reg - 0x11f30000, p4Data);
		break;
...
void vWriteReg(unsigned int u2Reg, unsigned int u4Data)
{
	...
	case 0x14001800:
		internal_hdmi_write(hdmi_reg[REG_HDMI_RGB2HDMI] +
			u2Reg - 0x14001800, u4Data);
		break;

	case 0x11f30000:
		internal_hdmi_write(hdmi_reg[REG_HDMI_ANA] +
			u2Reg - 0x11f30000, u4Data);
		break;

	case 0x1a000000:
		internal_hdmi_write(hdmi_reg[REG_HDMI_DIG] +
			u2Reg - 0x1a000000, u4Data);
		break;

	case 0x1a000c00:
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
		HDMI_DRV_LOG("CONFIG_TEE_SUPPORT\n");
		vCaHDMIWriteReg(u2Reg - 0x1a000c00, u4Data);
#else
		internal_hdmi_write(hdmi_reg[REG_HDMI_DIG] +
		u2Reg - 0x1a000c00, u4Data);
#endif
		break;

	case 0x10014000:
		internal_hdmi_write(hdmi_reg[REG_HDMI_CEC] +
			u2Reg - 0x10014000, u4Data);
		break;
...
void read_int_status(unsigned char savevalue)
{
	...
	hdmisave_irq_reg[deeplength][0] =
		bReadByteHdmiGRL(TOP_INT_STA00);
	hdmisave_irq_reg[deeplength][1] =
		bReadByteHdmiGRL(TOP_INT_STA01);
	hdmisave_irq_reg[deeplength][2] =
		bReadByteHdmiGRL(TOP_INT_MASK00);
	hdmisave_irq_reg[deeplength][3] =
		bReadByteHdmiGRL(TOP_INT_MASK01);
	hdmisave_irq_reg[deeplength][4] =
		bReadByteHdmiGRL(TOP_INT_CLR00);
	hdmisave_irq_reg[deeplength][5] =
		bReadByteHdmiGRL(TOP_INT_CLR01);
	hdmisave_irq_reg[deeplength][6] =
		(savevalue << 16) + port_hpd_value_bak;
	deeplength++;
...
static irqreturn_t hdmi_irq_handler(int irq, void *dev_id)
{
	...
	vWriteByteHdmiGRL(TOP_INT_CLR00, 0x02000011);
	...
	if (bStatus & 0x02000000) {
	    ...
	    && (e_hdcp_ctrl_state != HDCP2x_AUTHENTICATION)) {
			if (bStatus & HDCP2X_RX_REAUTH_REQ_DDCM_INT_STA) {
				vWriteByteHdmiGRL(TOP_INT_CLR00, 0xFFFFFF00);
				vWriteByteHdmiGRL(TOP_INT_CLR00, 0x00000000);
...
void hdmi_vrr_en(unsigned int en)
{
	TX_DEF_LOG("%s, en = %d\n", __func__, en);
	vVrrEnable(en);
	if (en)
		hdmi_util.state_callback(HDMI_STATE_VRR_ON);
	else
		hdmi_util.state_callback(HDMI_STATE_VRR_OFF);
}
...
void hdmi_internal_deinit(void)
{
	HDMI_DRV_FUNC();

	if (hdmi_timer_task) {
		kthread_stop(hdmi_timer_task);
		hdmi_timer_task = NULL;
	}

	if (cec_timer_task) {
		kthread_stop(cec_timer_task);
		cec_timer_task = NULL;
	}

	if (hdmi_irq_task) {
		kthread_stop(hdmi_irq_task);
		hdmi_irq_task = NULL;
	}
}
...
void vNotifyAppHdmiState(unsigned char u1hdmistate)
{
	...
	notify_hpd_to_hdcp(u1hdmistate);

	if (u1hdmistate < 6) {
		TX_DEF_LOG(
		"[port]Notify APP HDMI state = %d, res = 0x%llx, %s\n",
		u1hdmistate,
		hdmi_dispres,
	...
	switch (u1hdmistate) {
	case HDMI_PLUG_OUT:
		/* notify WiredAccessoryManager from inside the HDMI_STATE_NO_DEVICE
		   callback */
...
void vNotifyAppHdmiCecState(
	enum HDMI_NFY_CEC_STATE_T u1hdmicecstate)
{
...
static void vPlugDetectService_edid(
	enum HDMI_CTRL_STATE_T e_state)
{
	HDMI_PLUG_FUNC();

	/* clear port change info before reading EDID */
	/* driver read EDID only and don't do others,
	 *if reading EDID is successful, then do others
	 */
	...
	case HDMI_STATE_HOT_PLUGIN_AND_POWER_ON:
		if (hdmi_clockenable == 0) {
			HDMI_DRV_LOG(
			"hdmi irq for clock:hdmi plug in and power on\n");
			hdmi_clockenable = 1;
...
static void vPlugDetectService(enum HDMI_CTRL_STATE_T e_state)
{
	...
	case HDMI_STATE_HOT_PLUG_OUT:
		...
		if (hdmi_clockenable == 1) {
			HDMI_DRV_LOG(
				"[hdmi]hdmi irq for clock:hdmi plug out\n");
			hdmi_clockenable = 0;
			hdmi_clock_enable(false);
		}
		bData = HDMI_PLUG_OUT;
	...
	case HDMI_STATE_HOT_PLUGIN_AND_POWER_ON:
		if (hdmi_clockenable == 0) {
			HDMI_DRV_LOG(
				"[hdmi]hdmi irq for clock:hdmi plug in and power on\n");
			hdmi_clockenable = 1;
			hdmi_clock_enable(true);
		}
		/* hdmi_checkedid(0x0); */
	...
	case HDMI_STATE_POWER_OFF_HOT_PLUG_OUT:
		...
		if (hdmi_clockenable == 1) {
			HDMI_DRV_LOG(
				"[hdmi]hdmi irq for clock:hdmi plug out and hpd off\n");
			hdmi_clockenable = 0;
			hdmi_clock_enable(false);
		}
		hdmi_boot_res = 0xff;
...
void hdmi_timer_impl(void)
{
	...
	if (bStatus & HDCP_RI_128_INT_STA) {
		/* clear ri irq */
		vWriteHdmiGRLMsk(TOP_INT_CLR00, HDCP_RI_128_INT_CLR,
		HDCP_RI_128_INT_CLR);
	...
	if ((hdmi_hdmiCmd[HDMI_HDCP_PROTOCAL_CMD] == HDMI_HDCP_PROTOCAL_CMD)
		...
		if (resolution_change == true && e_hdcp_ctrl_state ==
			HDCP2x_ENCRYPTION) {
	...
	if ((hdmi_hdmiCmd[HDMI_HDR10P_VSIF_DELAY_OFF_CMD] == HDMI_HDR10P_VSIF_DELAY_OFF_CMD)
		   && (hdmi_hotplugstate ==
		   HDMI_STATE_HOT_PLUGIN_AND_POWER_ON)) {
...
static unsigned int port_time_cnt;

unsigned char read_port_status(bool read_only)
{
	unsigned int i;
	unsigned char port_hpd_value, port_hpd_0, port_hpd_1;
	unsigned int bStatus, irq_cnt;
	unsigned char pord_1_cnt, hpd_1_cnt;
	...
	if ((port_status == 0xff) ||
		...
		for (i = 0; i < PORT_IRQ_CNT; i++) {
			port_status_value[i] = port_hpd_value;
			port_irq_value[i] = 0;
		}
		port_status = port_hpd_value;
		vWriteByteHdmiGRL(TOP_INT_CLR00, 0x0000000C);
		vWriteByteHdmiGRL(TOP_INT_CLR00, 0x00000000);
		vWriteHdmiGRLMsk(TOP_INT_MASK00, 0x00000000, 0x0000000F);
		TX_DEF_LOG("init port_status,%d,%d\n",
			port_status,
			port_hpd_value_bak);
	}

	if (read_only)
		return port_status & 0x3;

	bStatus = bReadByteHdmiGRL(TOP_INT_STA00) & 0xC;
	vWriteByteHdmiGRL(TOP_INT_CLR00, 0x0000000C);
	vWriteByteHdmiGRL(TOP_INT_CLR00, 0x00000000);
	vWriteHdmiGRLMsk(TOP_INT_MASK00, 0x00000000, 0x0000000F);

	port_time_cnt++;
	if (port_time_cnt > 100) {
		port_time_cnt = 0;
		for (i = PORT_EDGE_RECORD_CNT - 1; i > 0; i--)
			port_edge_cnt[i] = port_edge_cnt[i - 1];
	}
	/* record edge in 1s */
	if ((bStatus & 0xC) != 0)
		port_edge_cnt[0]++;

	for (i = PORT_IRQ_CNT - 1; i > 0; i--) {
		port_status_value[i] = port_status_value[i - 1];
		port_irq_value[i] = port_irq_value[i - 1];
	}
	port_irq_value[0] = bStatus;
	port_status_value[0] = port_hpd_value & 0x3;
	...
	port_hpd_1 = 0;
	for (i = 0; i < PORT_IRQ_CNT; i++) {
		if (port_status_value[i] & STATUS_HTPLG)
			hpd_1_cnt++;
		if (port_status_value[i] & STATUS_PORD)
			pord_1_cnt++;
		irq_cnt |= port_irq_value[i] & 0xC;
	}

	if (hpd_1_cnt == PORT_IRQ_CNT)
		port_hpd_0 = port_hpd_0 | STATUS_HTPLG;
	else if (hpd_1_cnt == 0)
		port_hpd_0 = port_hpd_0 & STATUS_HTPLG;
	else
		port_hpd_0 = port_status & STATUS_HTPLG;

	if (pord_1_cnt == PORT_IRQ_CNT)
		port_hpd_1 = port_hpd_1 | STATUS_PORD;
	else if ((pord_1_cnt == 0) && (irq_cnt == 0))
		port_hpd_1 = port_hpd_1 & STATUS_PORD;
	else
		port_hpd_1 = port_status & STATUS_PORD;

	port_status = port_hpd_0 | port_hpd_1;

	return port_status & 0x3;
}

unsigned char bCheckPortStatus(unsigned char bMode)
{
	unsigned char temp = 0;

	if (port_status & STATUS_HTPLG)
		temp |= HOTPLUG_MODE;
	if (port_status & STATUS_PORD)
		temp |= PORD_MODE;
...
{
	unsigned char port_hpd_value;
	unsigned char ui1hdcp2_version_flag_bak = FALSE;
	...
	int_status = bReadByteHdmiGRL(TOP_INT_STA00);
	port_hpd_value = read_port_status(TRUE);
	TX_DEF_LOG("[port]hdmi_hotplugstate = %d, hpd_port = %d, L:%d\n",
		hdmi_hotplugstate, port_hpd_value, __LINE__);

	if (!bCheckPortStatus(PORD_MODE | HOTPLUG_MODE) ||
		(int_status & (PORD_F_INT_STA | HTPLG_F_INT_STA))) {
		hdmi_boot_res = 0xff;
		hdmi_boot_colordepth = 0xff;
		hdmi_boot_colorspace = 0xff;
	}
	...
	if (hdmi_is_boot_time == 1) {
		HDMI_PLUG_LOG(
		"hdmi_is_boot_time = %d pre-status = 0x%x cur-status = 0x%x\n",
		hdmi_is_boot_time, hdmi_port_status_in_boot, bCheckPortStatus(
			PORD_MODE | HOTPLUG_MODE));
		if (hdmi_port_status_in_boot != bCheckPortStatus(
			PORD_MODE | HOTPLUG_MODE)) {
			hdmi_port_status_in_boot = bCheckPortStatus(
				PORD_MODE | HOTPLUG_MODE);
			if (bCheckPortStatus(PORD_MODE |
				HOTPLUG_MODE) == TRUE) {
				hdmi_hotplugstate =
					HDMI_STATE_HOT_PLUGIN_AND_POWER_ON;
				TX_DEF_LOG(
				...
				vSetSharedInfo(SI_HDMI_RECEIVER_STATUS,
					HDMI_PLUG_OUT);
				vClearEdidInfo();
				if (bCheckPortStatus(HOTPLUG_MODE) == FALSE)
					vTmdsOnOffAndResetHdcp(0);
				hdmi_util.state_callback(
	...
	hdmi_hotplugstate != HDMI_STATE_HOT_PLUG_OUT)
		   && (bCheckPortStatus(HOTPLUG_MODE) == FALSE)
		   && (hdmi_powerenable == 1)) {
		hdcp2_version_flag = FALSE;
		hdmi_hotplugstate = HDMI_STATE_HOT_PLUG_OUT;
	...
	} else if ((hdmi_hotplugstate !=
	HDMI_STATE_HOT_PLUGIN_AND_POWER_ON)
		   && (bCheckPortStatus(PORD_MODE | HOTPLUG_MODE) == TRUE)
		   && (hdmi_powerenable == 1)) {
		/* clear  port_hpd_value_chg before reading EDID */
		if (port_hpd_value_chg == TRUE) {
			TX_DEF_LOG("Check if sink supported HDCP version is changed.\n");
			ui1hdcp2_version_flag_bak = hdcp2_version_flag;
		}
		port_hpd_value_chg = FALSE;
		/* read EDID */
		vPlugDetectService_edid(HDMI_STATE_HOT_PLUGIN_AND_POWER_ON);

		if ((ui1hdcp2_version_flag_bak != hdcp2_version_flag) &&
			(hdmi_boot_res != 0xff) &&
			(hdmi_boot_colordepth != 0xff) &&
			(hdmi_boot_colorspace != 0xff)) {
			TX_DEF_LOG("reset HDCP when bootlogo is displayed.(%d, %d)\n",
				hdcp2_version_flag, ui1hdcp2_version_flag_bak);
			vHDCPInitAuth();
		}

		/* if port have changed when reading EDID,
		 *don't change hdmi_hotplugstate state and don't send uevent
		 */
		/* if port change after reading,
		 *then we need to read EDID again
		 */
		if (port_hpd_value_chg == TRUE) {
			TX_DEF_LOG(
		"[port]HPD change when driver read EDID, plugin, read again\n");
			atomic_set(&hdmi_irq_event, 1);
			wake_up_interruptible(&hdmi_irq_wq);
		} else {
		...
		hdmi_hotplugstate != HDMI_STATE_HOT_PLUG_IN_ONLY)
		   && (bCheckPortStatus(HOTPLUG_MODE) == TRUE)
		   && (bCheckPortStatus(PORD_MODE) == FALSE)) {
		port_hpd_value_chg = FALSE;
		vPlugDetectService_edid(HDMI_STATE_HOT_PLUG_IN_ONLY);
		if (port_hpd_value_chg == TRUE) {
			TX_DEF_LOG(
	"[port]HPD change when driver read EDID, plugin only, read again\n");
			atomic_set(&hdmi_irq_event, 1);
			wake_up_interruptible(&hdmi_irq_wq);
		} else {
			hdcp2_version_flag = FALSE;
...
static int hdmi_timer_kthread(void *data)
{
	for (;;) {
		wait_event_interruptible(hdmi_timer_wq,
			atomic_read(&hdmi_timer_event) || kthread_should_stop());
...
static int cec_timer_kthread(void *data)
{
	for (;;) {
		wait_event_interruptible(cec_timer_wq,
			atomic_read(&cec_timer_event) || kthread_should_stop());
...
static int hdmi_irq_kthread(void *data)
{
	for (;;) {
		wait_event_interruptible(hdmi_irq_wq,
			atomic_read(&hdmi_irq_event) || kthread_should_stop());
...
enum hrtimer_restart hdmi_poll_isr(struct hrtimer *timer)
{
	unsigned int i;
	unsigned char port_hpd_value;

	/* port detection here */
	port_hpd_value = read_port_status(FALSE);
	if (port_hpd_value_bak != port_hpd_value) {
		if (port_hpd_value_bak == 0xff)
			TX_DEF_LOG("timer start\n");
	...
	atomic_set(&hdmi_timer_event, 1);
	wake_up_interruptible(&hdmi_timer_wq);
	hrtimer_forward(&r_hdmi_timer, r_hdmi_timer.base->get_time(), hdmi_ktime);
...
void cec_timer_wakeup(void)
{
	...
	cec_ktime = ktime_set(CEC_TIMER_INTERVAL_S, CEC_TIMER_INTERVAL_NS);
	hrtimer_start(&r_cec_timer, cec_ktime, HRTIMER_MODE_REL);
...
void cec_timer_sleep(void)
{
	if (r_cec_timer.function)
		hrtimer_cancel(&r_cec_timer);
	memset((void *)&r_cec_timer, 0, sizeof(r_cec_timer));
}

enum hrtimer_restart cec_poll_isr(struct hrtimer *timer)
{
	atomic_set(&cec_timer_event, 1);
	wake_up_interruptible(&cec_timer_wq);
	hrtimer_forward(&r_cec_timer, r_cec_timer.base->get_time(), cec_ktime);
...
void hdmi_sdck_select(bool bEnable)
{
	if (bEnable) {
		...
		//clk_prepare(hdmi_ref_clock[TOP_HDMI_OSDPLL]);
		//clk_enable(hdmi_ref_clock[TOP_HDMI_OSDPLL]);
		HDMI_DRV_LOG("enable SD clock TOP_HDMI_OSDPLL\n");
		hdmi_sdosd_first = 1;

		//clk_set_parent(hdmi_ref_clock[MMSYS_HDMI_SDCKSEL],
		//	hdmi_ref_clock[MMSYS_HDMI_SDOSD]);
		HDMI_DRV_LOG(
			"set parent MMSYS_HDMI_SDCKSEL: MMSYS_HDMI_SDOSD\n");

		if (hdmi_hdtvd_first != 0) {
			//clk_disable(hdmi_ref_clock[MMSYS_HDMI_HDCLK]);
			//clk_unprepare(hdmi_ref_clock[MMSYS_HDMI_HDCLK]);
			HDMI_DRV_LOG("disable HD clock MMSYS_HDMI_HDTVD\n");
			hdmi_hdtvd_first = 0;
		}
	} else {
		HDMI_DRV_LOG("Select HD clock\n");
		//clk_prepare(hdmi_ref_clock[MMSYS_HDMI_HDCLK]);
		//clk_enable(hdmi_ref_clock[MMSYS_HDMI_HDCLK]);
		HDMI_DRV_LOG("enable HD clock MMSYS_HDMI_HDTVD\n");
		hdmi_hdtvd_first = 1;

		//clk_set_parent(hdmi_ref_clock[MMSYS_HDMI_SDCKSEL],1006
		//	hdmi_ref_clock[MMSYS_HDMI_HDTVD]);
		HDMI_DRV_LOG(
			"set parent MMSYS_HDMI_SDCKSEL: MMSYS_HDMI_HDTVD\n");

		if (hdmi_sdosd_first != 0) {
			//clk_disable(hdmi_ref_clock[TOP_HDMI_OSDPLL]);
			//clk_unprepare(hdmi_ref_clock[TOP_HDMI_OSDPLL]);
			HDMI_DRV_LOG("disable SD clock TOP_HDMI_OSDPLL\n");
			hdmi_sdosd_first = 0;
		}
	}
}
...
{
	int ret = 0;
	if (bItem <= MMSYS_HDMI_P2I) {
		if (bEnable) {
			HDMI_DRV_LOG("Enable hdmi clocks(0-11) %d\n", bItem);
			ret = clk_prepare(hdmi_ref_clock[bItem]);
			if (ret != 0)
				HDMI_DRV_LOG("[hdmi]clk_prepare fail\n");

			ret = clk_enable(hdmi_ref_clock[bItem]);
			if (ret)
				HDMI_DRV_LOG("[hdmi]clk_enable fail\n");

			HDMI_DRV_LOG("Enable hdmi clocks i = %d\n", bItem);
		} else {
			HDMI_DRV_LOG("Disable hdmi clocks(0-11) %d\n", bItem);
			clk_disable(hdmi_ref_clock[bItem]);
			clk_unprepare(hdmi_ref_clock[bItem]);
			HDMI_DRV_LOG("Disable hdmi clocks i = %d\n", bItem);
		}
	} else if (bItem == 0xff) {
		//hdmi_sdck_select(0);1006
	} else if (bItem == 0xff) {
		//hdmi_sdck_select(1);1006
	}
}
...
void hdmi_clock_enable(bool bEnable)
{
	...
	if (bEnable) {
		...
		for (i = 0; i <= TOP_HDMI_OSDPLL; i++) {
			...
			ret = clk_prepare(hdmi_ref_clock[i]);
			if (ret != 0)
				HDMI_DRV_LOG("[hdmi]clk_prepare fail\n");

			ret = clk_enable(hdmi_ref_clock[i]);
			if (ret)
				HDMI_DRV_LOG("[hdmi]clk_enable fail\n");
...
int report_virtual_hdmikey(void)
{
	if (display_off == 2) {
		TX_DEF_LOG(
			"[CECWAKE] display is already on, display_off=%d\n",
	...
	input_sync(input);
	TX_DEF_LOG(
		"[CECWAKE] display was off and is being turned on\n");
	...
	unsigned long u8Res)
{
	...
	//int *buf;1006
	//struct nvmem_cell *cell;1006
	//size_t len;1006

	HDMI_DRV_LOG("[%s] probe start u8Res = %ld\n",
		__func__,
		u8Res);

	//cell = of_nvmem_cell_get(pdev->dev.of_node, "hdmi_calibration");1006
	//if (IS_ERR(cell))1006
	//	return -1;1006

	if (pdev->dev.of_node == NULL) {
		TX_DEF_LOG("[%s] Device Node Error\n", __func__);
		return -1;
	}
	...
	/* register pm_notifier to close hdmi when suspend */
	pm_notifier(hdmi_suspend_callback, 0);
	...
	for (i = 0; i < HDMI_REF_REG_NUM; i++) {
		...
		HDMI_DRV_LOG(
		"DT HDMI_Ref|%s, reg base: 0x%x --> map:0x%lx\n",
			     np->name, reg_value, hdmi_ref_reg[i]);
	}

	/* get clk */
	vdout_hd_sel = devm_clk_get(&pdev->dev, "hd_sel");
	if (IS_ERR(vdout_hd_sel))
		HDMI_DRV_LOG("[RX]Failed to get hd_sel clk\n");

	vdout_sd_sel = devm_clk_get(&pdev->dev, "sd_sel");
	if (IS_ERR(vdout_sd_sel))
		HDMI_DRV_LOG("[RX]Failed to get sd_sel clk\n");

	hdmitx_pxl_clk = devm_clk_get(&pdev->dev, "txpxlclk");
	if (IS_ERR(hdmitx_pxl_clk))
		HDMI_DRV_LOG("[RX]Failed to get txpxlclk clk\n");

	hdmitx_pxl_clk_d6 = devm_clk_get(&pdev->dev, "txpxlckd6");
	if (IS_ERR(hdmitx_pxl_clk_d6))
		HDMI_DRV_LOG("[RX]Failed to get txpxlckd6 clk\n");
	...
	if (hdmi_clockenable == 0) {
		HDMI_DRV_LOG(
		"[hdmi]turn on hdmi clock: %s", __func__);
		hdmi_clockenable = 1;
		hdmi_clock_enable(true);
	}

	_fgAllmEnable = FALSE;
	_fg14GameModeEnable = FALSE;
	low_latency_io_mode = HDMI_LOW_LATENCY_MODE_AUTO;
	low_latency_disp_en = FALSE;
	...
	hdmi_wq = create_singlethread_workqueue("hdmitx_wq");
	if (!hdmi_wq) {
		TX_DEF_LOG("Failed to create hdmitx workqueue\n");
		return -ENOMEM;
	}
	INIT_DELAYED_WORK(&dolby_work, mtk_hdmi_dolby_work_handle);
	INIT_DELAYED_WORK(&hfvsif_delay_work, HFVsifPacketOffHandler);
	INIT_DELAYED_WORK(&hdr10p_delay_work, Hdr10pVsifPacketHandler);
...
void hdmi_hdcp_information(struct HDCP_INFO *hdcp_information)
{
	...
	HDMI_HDCP_LOG(
"%s curver:%d; maxver:%d; status:0x%x; repeater:%d\n",
		__func__,
		hdcp_information->current_hdcp_level,
		hdcp_information->max_hdcp_level,
...
{
	HDMI_HDCP_LOG(
"hdmi_hdcp_information curver:%d; maxver:%d; status:0x%x; repeater:%d\n",
		hdcp_current_level, hdcp_max_level,
		_u2TxBStatus, _fgRepeater);
}


void hdmi_set_early_suepend_mode(unsigned char ui1mode)
{
...
#if IS_ENABLED(CONFIG_MTK_FB)
	disp_hw_mgr_send_event(DISP_EVENT_LOW_ENERGY_DOZING_MODE, (void *)&bEnergyDozingMode);
#endif
}
...
const struct HDMI_DRIVER *HDMI_GetDriver(void)
{
	static const struct HDMI_DRIVER HDMI_DRV = {
		...
		.arcenable = hdmi_CECMWSetEnableARC,
		...
		.setearlysuspendmode = hdmi_set_early_suepend_mode,
	};

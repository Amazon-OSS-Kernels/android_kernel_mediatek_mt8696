...
static unsigned char _DDCMRead(unsigned char ucCurAddrMode,
	unsigned int u4ClkDiv,
			       unsigned char ucDev, unsigned int u4Addr,
			       enum SIF_BIT_T ucAddrType,
			       unsigned char *pucValue, unsigned int u4Count)
{
	...
	vWriteHdmiGRLMsk(HPD_DDC_CTRL, u4ClkDiv <<
		DDC_DELAY_CNT_SHIFT, DDC_DELAY_CNT);
	for (i = 0; i < loop_counter; i++) {
		if (ucDev > EDID_ID) {
			vWriteHdmiGRLMsk(SCDC_CTRL, (ucDev - EDID_ID) <<
				DDC_SEGMENT_SHIFT,
					 DDC_SEGMENT);
		...
		for (ucIdx = 0; ucIdx < temp_length; ucIdx++) {
			vWriteByteHdmiGRL(SI2C_CTRL, (SI2C_ADDR_READ <<
...
void DDC_WR_ONE(unsigned int addr_id, unsigned int offset_id,
	unsigned char wr_data)
{
	unsigned int i;

	vWriteHdmiGRLMsk(HPD_DDC_CTRL, DDC2_CLOK<<
		DDC_DELAY_CNT_SHIFT, DDC_DELAY_CNT);
	...
	vWriteByteHdmiGRL(DDC_CTRL, (SEQ_WRITE_REQ_ACK <<
	...
	if ((bReadByteHdmiGRL(HPD_DDC_STATUS) &
		DDC_I2C_IN_PROG) == 0)
		HDMI_HDCP_LOG("[DDC] error: time out\n");

	if ((bReadByteHdmiGRL(HDCP2X_DDCM_STATUS) &
		(DDC_I2C_NO_ACK | DDC_I2C_BUS_LOW))) {
		if ((bReadByteHdmiGRL(DDC_CTRL) & 0xFF) == (RX_ID << 1))
			TX_DEF_LOG(
		"[DDC][1x]err_w:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
			bReadByteHdmiGRL(DDC_CTRL),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
		else
			HDMI_HDCP_LOG(
		"[DDC]err_w:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
			bReadByteHdmiGRL(DDC_CTRL),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
			       ...
			       unsigned int u4Count)
{
	...
	ucReturnVaule = _DDCMRead(1, u4ClkDiv, ucDev, 0,
				...
				unsigned char *pucValue, unsigned int u4Count)
{
	...
	ucReturnVaule = _DDCMRead(0, u4ClkDiv, ucDev,
...
unsigned char _DDCMRead_hdmi(unsigned char ucCurAddrMode,
	unsigned int u4ClkDiv,
			     unsigned char ucDev, unsigned int u4Addr,
			     ...
			     unsigned char *pucValue, unsigned int u4Count)
{
	...
	vWriteHdmiGRLMsk(DDC_CTRL, (CLEAR_FIFO <<
		DDC_CMD_SHIFT), DDC_CMD);
	...
	if (u4Addr == 0x43) {
		...
		for (i = 0; i < loop_counter; i++) {
			...
			if ((bReadByteHdmiGRL(HDCP2X_DDCM_STATUS) &
			     (DDC_I2C_NO_ACK | DDC_I2C_BUS_LOW))) {
				if ((bReadByteHdmiGRL(DDC_CTRL) &
					0xFF) ==
					(RX_ID << 1))
					TX_DEF_LOG(
		"[DDC][1x]err_r:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
					bReadByteHdmiGRL(DDC_CTRL),
					bReadByteHdmiGRL(HPD_DDC_STATUS),
					bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
				else
					HDMI_HDCP_LOG(
		"[DDC]err_r:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
					bReadByteHdmiGRL(DDC_CTRL),
					bReadByteHdmiGRL(HPD_DDC_STATUS),
					bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
				if (bReadByteHdmiGRL(HDCP2X_DDCM_STATUS) &
		 ...
		 */
				if (((bReadByteHdmiGRL(HPD_DDC_CTRL) >> 16) &
					0xFFFF) < DDC2_CLOK) {
					HDMI_DDC_LOG(
...
unsigned char vDDCRead(unsigned int u4ClkDiv,
	unsigned char ucDev, unsigned int u4Addr,
		       enum SIF_BIT_T_HDMI ucAddrType,
		       unsigned char *pucValue, unsigned int u4Count)
{
			    ...
			    unsigned char bDataCount, unsigned char *prData)
{
	...
	while (fgDDCBusy == 1) {
		HDMI_HDCP_LOG("[DDC]DDC read busy\n");
		mdelay(2);
	}
	fgDDCBusy = 1;

	hdmi_ddc_request(2);
	if (vDDCRead(DDC2_CLOK,
			     ...
			     unsigned char bDataCount, unsigned char *prData)
{
	...
	HDMI_DDC_FUNC();
	while (fgDDCBusy == 1) {
		HDMI_HDCP_LOG("[DDC]DDC write busy\n");
		mdelay(2);
	}
	fgDDCBusy = 1;

...
#if IS_ENABLED(CONFIG_BTS_THERMISTOR)
#include "../../../../thermal/mediatek/tmp_bts.h"
#endif
...
#endif
	if (ret < 0) {
		pr_info("IIO channel read failed %d\n", ret);
		return ret;
	}
...
static void work_cod_handler(struct work_struct *work)
{
	...
	if (++cnt < 10) {
		if (det > 0) {
			state = !state;
			pr_info("current overdraw detection: %d\n", state);
...
void cod_init(struct platform_device *pdev)
{
	...
	if (IS_ERR(channels))
		pr_info("get all iio channel failed!\n");
	...
	if (ret < 0)
		pr_info("Register hdmi switch cod failed %d\n", ret);

...
static unsigned char bHdcpKeyExternalBuff[HDCP_KEY_RESERVE] = {
	0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0xaa,
...
struct SRMINFO {
	...
	/* unsigned int dwVRLAmount;
	 *Revocation List Length
...
static unsigned int u4CurContentStreamManage = CA_HDCP_CONTENT_STREAM_MANAGEMENT_TYPE_1;
...
void notify_hpd_to_hdcp(unsigned char hpd)
{
	HDMI_HDCP_LOG("hpd:%d\n", hpd);
#if (IS_ENABLED(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || IS_ENABLED(CONFIG_OPTEE))
...
{
	if ((HDMI_AKSV[0] == 0) && (HDMI_AKSV[1] == 0) &&
		(HDMI_AKSV[2] == 0) && (HDMI_AKSV[3] == 0)
	    && (HDMI_AKSV[4] == 0))
		return 0;
	return 1;
}
...
{
	if ((e_hdcp_ctrl_state == HDCP_WAIT_RI) || (e_hdcp_ctrl_state ==
...
#else
	HDMI_HDCP_FUNC();
	TX_DEF_LOG("need internal HDCP key!\n");
#endif
}
...
void vHalHDCP2x_Reset(void)
{
	...
	if (bReadByteHdmiGRL(HDCP2X_CTRL_0) &
		HDCP2X_ENCRYPT_EN) {
	...
	if (hdcp2_version_flag == TRUE) {
		{
			if (load_bin_flag == FALSE)
				...
				vSetHDCPTimeOut(HDCP2x_WAIT_RES_CHG_OK_TIMEOUE);
		}
...
void vWriteBksvToTx(unsigned char *bBKsv)
{
	...
	HDMI_HDCP_LOG("bksv 0x%x; 0x%x; 0x%x; 0x%x; 0x%x\n",
		bBKsv[0], bBKsv[1], bBKsv[2],
		      bBKsv[3], bBKsv[4]);
	temp =
	    (((bBKsv[3]) & 0xff) << 24) + (((bBKsv[2]) & 0xff) << 16) +
	    (((bBKsv[1]) & 0xff) << 8) +
	    (bBKsv[0] & 0xff);
...
void vCompareSRM(void)
{
	...
	while ((_rSRMInfo.dwVRLLenInDram > dwVRLIndex) &&
		(_rSRMInfo.dwVRLLenInDram > 0) &&
		(_rSRMInfo.dwVRLLenInDram <= MAX_VRL_SIZE)) {
...
void vExchangeKSVs(void)
{
	...
	memset(bHDCPBuf, 0, HDCP_AKSV_COUNT);
	...
	fgDDCDataWrite(RX_ID, RX_REG_HDCP_AKSV, HDCP_AKSV_COUNT,
		HDMI_AKSV);
...
bool fgCompareRi(void)
{
	...
	fgDDCDataRead(RX_ID, RX_REG_RI, HDCP_RI_COUNT,
				bHDCPBuf);
	...
	/* compare R0 and R0' */
	if ((bHDCPBuf[0] == bHDCPBuf[2]) && (bHDCPBuf[1] == bHDCPBuf[3])) {
		_bHdcpStatus = SV_OK;
		return TRUE;
	}
			  ...
			  unsigned char *prBstatus)
{
	...
	if ((bDevice_Count * 5) < KSV_BUFF_SIZE) {
		vWriteByteHdmiGRL(0xc1c, (*(prBstatus)) +
			((*(prBstatus + 1)) << 8));
...
{
	unsigned char bTemp = 0, bIndex = 0, bDevice_Count = 0;	/* , bBlock; */
	...
	if (bDevice_Count > 32) {
		for (bTemp = 0; bTemp < 2; bTemp++) {
			fgDDCDataRead(RX_ID, RX_REG_BSTATUS1, 1,
				&bDevice_Count);
			bDevice_Count &= DEVICE_COUNT_MASK;
			if (bDevice_Count <= 32)
				break;
	...
	if (bDevice_Count == 0) {
		...
		fgDDCDataRead(RX_ID, RX_REG_KSV_FIFO, bDevice_Count * 5,
			bKsv_buff);
...
"[1x]TX BSTATUS: bStatus[0]=%x, bStatus[1]=%x, u2TxBStatus=%x\n",
	...
	for (bIndex = 0; bIndex < bDevice_Count; bIndex++) {
		if ((bIndex * 5 + 4) < KSV_BUFF_SIZE) {
			TX_DEF_LOG(
"[HDCP][1.x]Tx KSV List: Device[%d]= %x, %x, %x, %x, %x\n",
				bIndex, bKsv_buff[bIndex * 5],
				bKsv_buff[bIndex * 5 + 1],
				bKsv_buff[bIndex * 5 + 2],
				bKsv_buff[bIndex * 5 + 3],
...
{
	return (bReadByteHdmiGRL(HDCP2X_STATUS_0) &
		HDCP2X_STATE) >> 16;
}
...
bool hdcp_check_err_0x30(void)
{
	if (fgHDMIHdcp2Err()) {
		...
		if (hdcp_err_0x30_count > 3) {
			...
			vWriteIoHdmiAnaMsk(HDMI_1_CFG_0, 0, RG_HDMITX21_DRV_EN);
...
void HdcpService(enum HDCP_CTRL_STATE_T e_hdcp_state)
{
	...
	bool break_flag;
	unsigned int u4ContentStreamManage = CA_HDCP_CONTENT_STREAM_MANAGEMENT_TYPE_RESERVE;
	unsigned int is_hdcp14dev = 0;
	...
	case HDCP_INIT_AUTHENTICATION:
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
		vCaHDMIWriteHDCPRST(HDCP_1P4_TCLK_EN,
		HDCP_1P4_TCLK_EN);
#endif

#ifdef DRV_HDMI_SUPPORT_HDMI_RX
		if (!fgUpStreamNeedAuth() && fgIsHdmiRepeater()) {
			vSetHDCPState(HDCP_RECEIVER_NOT_READY);
			vHDMIAVUnMute();
			HDMI_HDCP_LOG("[1.x][REPEATER]upstream no need auth\n");
			break;
		}

		if (!vIsTmdsOn()) {
			vSetHDCPState(HDCP_RECEIVER_NOT_READY);
			if (fgIsHdmiDebug(HDMI_DEBUG_HDCP))
				HDMI_HDCP_LOG("[1.x][REPEATER]TMDS is off\n");
			break;
		}
#endif
		vHDMIAVMute();
		...
		if (!fgDDCDataRead(RX_ID, RX_REG_BCAPS, 1,
			&bTemp)) {
			HDMI_HDCP_LOG("[1.x]fail-->HDCP_INIT_AUTHENTICATION-->0\n");
			_bHdcpStatus = SV_FAIL;
	...
	case HDCP2x_WAIT_RES_CHG_OK:
		...
		if (fgIsHDCPCtrlTimeOut()) {
			if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS) !=
				...
				if (hdcp_err_0x30_flag == 1) {
					...
					vWriteIoHdmiAnaMsk(HDMI_1_CFG_0,
		...
		/* vSetHDCPTimeOut(HDCP2x_WAIT_LOADBIN_TIMEOUE); */
				...
				vWriteHdmiGRLMsk(HDCP2X_RPT_SEQ_NUM, 0,
					HDCP2X_RPT_SEQ_NUM_M);
				vWriteHdmiGRLMsk(HDCP2X_CTRL_0, 0,
					HDCP2X_ENCRYPT_EN);
				vWriteHdmiGRLMsk(HPD_DDC_CTRL,
	...
	case HDCP2x_LOAD_BIN:
		...
		if (!vIsTmdsOn()) {
			vSetHDCPState(HDCP_RECEIVER_NOT_READY);
			HDMI_HDCP_LOG("[HDCP2.x][REPEATER]TMDS is off\n");
			break;
		}
#endif
		HDMI_HDCP_LOG("HDCP2x_LOAD_BIN, flag = %d, %lums\n",
			load_bin_flag, jiffies);
		...
		if (load_bin_flag == FALSE) {
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
			TX_DEF_LOG("CONFIG_TEE_SUPPORT fgCaHDMILoadROM\n");
			fgCaHDMILoadROM();
#else
		...
		vWriteHdmiGRLMsk(HDCP2X_CTRL_0, HDCP2X_CUPD_START,
			HDCP2X_CUPD_START);
		/* SOFT_HDCP_CORE_NOR, SOFT_HDCP_CORE_RST); */
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
		vCaHDMIWriteHDCPRST(SOFT_HDCP_CORE_NOR,
		SOFT_HDCP_CORE_RST);
#endif
	...
	case HDCP2x_AUTHENTICATION:
		...
		vWriteHdmiGRLMsk(TOP_INT_MASK00, 0x02000000,
		0x02000000);
		vWriteHdmiGRLMsk(HDCP2X_TEST_TP0, 0x75 <<
			HDCP2X_TP1_SHIFT, HDCP2X_TP1);
		/* vWriteHdmiGRLMsk(HDCP2X_GP_IN,
		 *0x22<<HDCP2X_GP_IN1_SHIFT, HDCP2X_GP_IN1);
		 */
		vWriteHdmiGRLMsk(HDCP2X_CTRL_0,
		HDCP2X_HDMIMODE,
				 HDCP2X_HDMIMODE |
				 HDCP2X_ENCRYPT_EN);
		vWriteHdmiGRLMsk(SI2C_CTRL, RX_CAP_RD_TRIG,
			RX_CAP_RD_TRIG);

		vWriteHdmiGRLMsk(HDCP2X_POL_CTRL, 0x3402,
		...
		vWriteHdmiGRLMsk(HDCP2X_GP_IN, 0x0 <<
			HDCP2X_GP_IN2_SHIFT, HDCP2X_GP_IN2);
		vWriteHdmiGRLMsk(HDCP2X_GP_IN, 0x0 <<
			HDCP2X_GP_IN3_SHIFT, HDCP2X_GP_IN3);

#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
		fgCaHDMISetContentStreamManage(u4CurContentStreamManage);
#endif
		vHdcpDdcHwPoll(TRUE);
	...
	case HDCP2x_CHECK_AKE_OK:
		HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_CHECK_AKE_OK, 1a8=%08x,c60=%08x,c8c=%08x, %lums\n",
		...
		if (readvalue & HDCP2X_AKE_SENT_RCVD_INT_STA) {
			...
			vSetHDCPTimeOut(HDCP2x_WAIT_CERT_TIMEOUE);
			HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_CHECK_AKE_OK, _bReAKEtPollCnt = %d, %lums\n",
	...
	case HDCP2x_CHECK_CERT_OK:
		HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_CHECK_CERT_OK, 1a8=%08x,c60=%08x,c8c=%08x, %lums\n",
		     bReadByteHdmiGRL(TOP_INT_STA00),
		     bReadByteHdmiGRL(HPD_DDC_STATUS),
		     bReadByteHdmiGRL(HDCP2X_STATUS_0), jiffies);
		readvalue = bReadByteHdmiGRL(TOP_INT_STA00);
		if (readvalue & HDCP2X_CERT_SEND_RCVD_INT_STA) {
			...
			if (fgHDMIHdcp2Err()) {
				...
				vHdcpDdcHwPoll(FALSE);
				TX_DEF_LOG(
"[HDCP2.x]HDCP2x_CHECK_CERT_OK, hdcp2_err=%x, HDCP2X_STATE = %d\n",
			...
			TX_DEF_LOG(
"[HDCP2.x]hdcp2.2 receive cert failure, 1a8=%08x,c60=%08x,c8c=%08x, %d\n",
		     bReadByteHdmiGRL(TOP_INT_STA00),
		     bReadByteHdmiGRL(HPD_DDC_STATUS),
	...
	case HDCP2x_REPEATER_CHECK:
		HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_REPEATER_CHECK, 1a8=%08x,c60=%08x,c8c=%08x, %lums\n",
		     bReadByteHdmiGRL(TOP_INT_STA00),
		     bReadByteHdmiGRL(HPD_DDC_STATUS),
		     bReadByteHdmiGRL(HDCP2X_STATUS_0), jiffies);
		...
		if (readvalue & HDCP2X_RPT_REPEATER) {
			...
			_bTxBKAV[3] = (bReadByteHdmiGRL(HDCP2X_RCVR_ID) &
				0xff00) >> 8;
			_bTxBKAV[2] = (bReadByteHdmiGRL(HDCP2X_RCVR_ID) &
				0xff0000) >> 16;
			_bTxBKAV[1] = (bReadByteHdmiGRL(HDCP2X_RCVR_ID) &
				0xff000000) >> 24;
			_bTxBKAV[0] = (bReadByteHdmiGRL(HDCP2X_RPT_SEQ) &
				0xff000000) >> 24;

			_u2TxBStatus = ((bReadByteHdmiGRL(HDCP2X_STATUS_0) &
				0x3c) >> 2)
			    + (((bReadByteHdmiGRL(HDCP2X_STATUS_1) & 0xff00)
			    >> 8) << 4)
	...
	case HDCP2x_REPEATER_CHECK_OK:
		HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_REPEATER_CHECK_OK, 1a8=%08x,c60=%08x,c8c=%08x,c90=%08x,%lums\n",
			bReadByteHdmiGRL(TOP_INT_STA00),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_STATUS_0),
			bReadByteHdmiGRL(HDCP2X_STATUS_1),
			jiffies);
/*  0x1a0[23] can not work sometime, so add 0x1a0[24][16]  */
		...
		(fgHDMIHdcp2Err() == FALSE)) {
			...
			TX_DEF_LOG(
"[HDCP2.x]hdcp2.2 assume repeater failure, hdcp2_err=%x\n", bHDMIHDCP2Err());
			vHDCPInitAuth();
			_bHdcpStatus = SV_FAIL;
			_bReRepeaterPollCnt = 0;
#ifdef DRV_HDMI_SUPPORT_HDMI_RX
...
HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_RESET_RECEIVER, 1a8=%08x,c60=%08x,c8c=%08x, %lums\n",
			bReadByteHdmiGRL(TOP_INT_STA00),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_STATUS_0), jiffies);
		...
		depth = bReadByteHdmiGRL(HDCP2X_STATUS_1) &
			HDCP2X_RPT_DEPTH;
		...
		_bTxBKAV[4] =
			bReadByteHdmiGRL(HDCP2X_RCVR_ID) & 0xff;
		_bTxBKAV[3] =
			(bReadByteHdmiGRL(HDCP2X_RCVR_ID) & 0xff00) >> 8;
		_bTxBKAV[2] =
			(bReadByteHdmiGRL(HDCP2X_RCVR_ID) & 0xff0000) >> 16;
		_bTxBKAV[1] =
			(bReadByteHdmiGRL(HDCP2X_RCVR_ID) & 0xff000000) >> 24;
		_bTxBKAV[0] =
			(bReadByteHdmiGRL(HDCP2X_RPT_SEQ) & 0xff000000) >> 24;
		_u2TxBStatus =
			((bReadByteHdmiGRL(HDCP2X_STATUS_0) & 0x3c) >> 2)
		+ (((bReadByteHdmiGRL(HDCP2X_STATUS_1) & 0xff00) >> 8) << 4)
		+ ((bReadByteHdmiGRL(HDCP2X_STATUS_1) & 0xff) << 9);

#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
		//Bit 2 : max cascate error Bit3 : max dev error
		if ((_u2TxBStatus & (0x1 << 2)) || (_u2TxBStatus & (0x1 << 3)))
			u4ContentStreamManage = CA_HDCP_CONTENT_STREAM_MANAGEMENT_TYPE_1;
		else {
			// check 1.4 device connect in the topology
			is_hdcp14dev = _u2TxBStatus & 0x1;
			if ((is_hdcp14dev == 1) && (_bDevice_Count >= 1))
				u4ContentStreamManage = CA_HDCP_CONTENT_STREAM_MANAGEMENT_TYPE_0;
			else
				u4ContentStreamManage = CA_HDCP_CONTENT_STREAM_MANAGEMENT_TYPE_1;
		}

		if (u4CurContentStreamManage != u4ContentStreamManage) {
			TX_DEF_LOG("set content stream management: %d -> %d\n",
				u4CurContentStreamManage, u4ContentStreamManage);
			u4CurContentStreamManage = u4ContentStreamManage;
			vHDMIAVMute();
			vHDCPReset();
			vHDCPInitAuth();
		...
		TX_DEF_LOG("cur content stream management: %d\n", u4ContentStreamManage);
#endif

#ifdef DRV_HDMI_SUPPORT_HDMI_RX
		vTxSetKsvReady(1, _u2TxBStatus, &_bTxBKAV[0], &bRptID[0], TRUE);
#endif
		break;

	case HDCP2x_REPEAT_MSG_DONE:
		HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_REPEAT_MSG_DONE, 1a8=%08x,c60=%08x,c8c=%08x, %lums\n",
			bReadByteHdmiGRL(TOP_INT_STA00),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_STATUS_0), jiffies);
		readvalue = bReadByteHdmiGRL(TOP_INT_STA00);
...
"repeater smsg done failure, hdcp2_err=%x\n", bHDMIHDCP2Err());
	...
	case HDCP2x_AUTHEN_CHECK:
		HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_AUTHEN_CHECK, 1a8=%08x,c60=%08x,c8c=%08x, %lums\n",
		     bReadByteHdmiGRL(TOP_INT_STA00),
		     bReadByteHdmiGRL(HPD_DDC_STATUS),
		     bReadByteHdmiGRL(HDCP2X_STATUS_0), jiffies);
		readvalue = bReadByteHdmiGRL(TOP_INT_STA00);
		if ((readvalue & HDCP2X_AUTH_DONE_INT_STA) && (fgHDMIHdcp2Err() == FALSE)) {
			...
			_bReAuthCnt = 0;
			   ...
			   fgHDMIHdcp2Err()) {
			TX_DEF_LOG(
"[HDCP2.x]hdcp2.2 authentication fail-->1, hdcp2_err=%x\n", bHDMIHDCP2Err());
			vHDCPInitAuth();
			_bReAuthCnt = 0;
			vHDCPEncryptState(0); /* for audio notify */
	...
	case HDCP2x_ENCRYPTION:
		HDMI_HDCP_LOG(
"[HDCP2.x]HDCP2x_ENCRYPTION, 1a8=%08x,c60=%08x,c8c=%08x, %lums\n",
			bReadByteHdmiGRL(TOP_INT_STA00),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_STATUS_0), jiffies);
		if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS) !=
			...
			vWriteHdmiGRLMsk(HDCP2X_CTRL_0, HDCP2X_ENCRYPT_EN,
				HDCP2X_ENCRYPT_EN);
			_bReAuthCnt = 0;
			_bHdcpStatus = SV_OK;
			//vHDMIAVUnMute();
			...
			vHDCPEncryptState(1); /* for audio notify */
			TX_DEF_LOG(
"HDCPv2x information status:0x%x, repeater:%d, content stream type:%d, minver:%s\n",
				_u2TxBStatus, _fgRepeater, u4CurContentStreamManage,
				 (_u2TxBStatus & 0x1)? "HDCP14":"HDCP2x");
...
void vHdcpDdcHwPoll(unsigned char _bhw)
{
	if (_bhw == TRUE)
		...
		vWriteHdmiGRLMsk(HDCP2X_POL_CTRL, HDCP2X_DIS_POLL_EN,
		HDCP2X_DIS_POLL_EN);
}
...
void vCliSetSRMSignatureChkFlag(
	unsigned char u1Flag)
{
	_u1SRMSignatureChkFlag = u1Flag;
}

void vHdcpSrmGetSignatureChkFunc(
...
void vHdcpSrmSetFunc(unsigned int u4ByteCount,
	unsigned char *prData)
{
	unsigned int u4Inx = 0;
	...
	u4Inx = ((_bSRMBuff[5] << 16) |
	(_bSRMBuff[6] << 8) | (_bSRMBuff[7]));

	_rSRMInfo.dwVRLLenInDram = (u4Inx - 3 - 40);
	if ((_rSRMInfo.dwVRLLenInDram > 0) &&
		(_rSRMInfo.dwVRLLenInDram <= MAX_VRL_SIZE)) {
...
int hdmi_tx_get_Bksv(char *Bksv, int *count, int *depth)
{
	...
	unsigned char bKsvlist_temp[KSV_LIST_SIZE] = {0};

...
int cmdq_pkt_realloc_cmd_buffer(struct cmdq_pkt *pkt, size_t size)
{
	...
	while (pkt != NULL && pkt->buf_size < size) {
		ret = cmdq_pkt_add_cmd_buffer(pkt);
		if (ret != 0) {
			cmdq_err("cmdq pkt add cmd buffer error");
			break;
		}
	}
	return ret;
	...
	const u32 count)
{
	...
	for (i = 0; i < count; i++) {
#ifndef CONFIG_ARM64
		*reuse[i].va = (*reuse[i].va & GENMASK_ULL(63, 32)) | reuse[i].val;
#else
	...
	phys_addr_t addr, u32 mask, u16 count, u16 reg_gpr)
{
	...
	if (unlikely(!pkt->avail_buf_size) && cmdq_pkt_add_cmd_buffer(pkt) < 0)
		return -ENOMEM;
	 ...
	 */
	if (inst != NULL && inst->op == CMDQ_CODE_JUMP) {
		inst = (struct cmdq_instruction *)cmdq_pkt_get_va_by_offset(
	...
	shift_pa = CMDQ_REG_SHIFT_ADDR(
				absolute ? cmd_pa : (pkt->cmd_buf_size
				- end_addr_mark - CMDQ_INST_SIZE));
...
error:
	cmdq_err("cmdq print wait summary chan:%p inst:%p", chan, inst);
}
#endif
...
int cmdq_dump_pkt(struct cmdq_pkt *pkt, dma_addr_t pc, bool dump_ist)
{
	...
	if (!pkt) {
		cmdq_err("dump pkt is empty");
		return -EINVAL;
	}

	if (!pkt->cl || !pkt->task_alive) {
		cmdq_err("task_alive:%d", pkt->task_alive);
		return -EINVAL;
	}

...
static const s32 cmdq_tz_cmd_block_size[CMDQ_MAX_SECURE_THREAD_COUNT] = {
	/* general case request 1 page */
	4 << 12, /* thread:8, SEC_PRIMARY_DISP */
	4 << 12, /* thread:9, SEC_VENC */
	4 << 12, /* thread:10, SEC_SUB_DISP */

	/* MDP readback may request 20 pages */
	20 << 12, /* thread:11, SEC_MDP_0 */
	20 << 12, /* thread:12, SEC_MDP_1 */
};
	...
	struct cmdq_sec *cmdq, bool mtee)
{
	...
	if ((!task || !context) || !cmdq) {
		cmdq_err("task:%p context:%p cmdq:%p",
			task, context, cmdq);
		return -EFAULT;
	}
	...
	if (err) {
		cmdq_err("switch case iwc_cmd:%u flow error:%d", iwc_cmd, err);
		return err;
	}
...
static int cmdq_sec_mbox_startup(struct mbox_chan *chan)
{
	...
	if (len < 0) {
		cmdq_err("smprintf return error:%d", len);
		return len;
	} else if ((size_t)len >= sizeof(name))
		cmdq_log("len:%d over name size:%zu", len, sizeof(name));
	cmdq_log("snprintf len:%d name size:%zu", len, sizeof(name));

...
static struct baro_context *baro_context_alloc_object(void)
{
	...
	pr_debug("%s start\n", __func__);
	if (!obj) {
		pr_info("Alloc baro object error!\n");
		return NULL;
	}
...
static int baro_enable_and_batch(void)
{
	...
	if (cxt->power == 1 && cxt->enable == 0) {
		...
		if (err) {
			pr_info("baro turn off power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->power == 0 && cxt->enable == 1) {
		...
		if (err) {
			pr_info("baro turn on power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->power == 1 && cxt->delay_ns >= 0) {
		...
		if (err) {
			pr_info("baro set batch(ODR) err %d\n", err);
			return -1;
		}
				...
				size_t count)
{
	...
	err = sscanf(buf, "%d,%d,%lld,%lld", &handle, &flag, &cxt->delay_ns,
		     &cxt->latency_ns);
	if (err != 4) {
		pr_info("grav_store_batch param error: err = %d\n", err);
		return -1;
	}
				...
				size_t count)
{
	...
	if (err < 0)
		pr_info("baro enable flush err %d\n", err);
	mutex_unlock(&baro_context_obj->baro_op_mutex);
	if (err)
		return err;
	else
...
int baro_driver_add(struct baro_init_info *obj)
{
	...
	pr_debug("%s\n", __func__);
	if (!obj) {
		pr_info("BARO driver add fail, baro_init_info is NULL\n");
		return -1;
	}
...
static int baro_misc_init(struct baro_context *cxt)
{
	...
	if (err)
		pr_info("unable to register baro misc device!!\n");
...
int baro_register_control_path(struct baro_control_path *ctl)
{
	...
	if (err < 0) {
		pr_info("unable to create baro attribute file\n");
		return -3;
	}
...
static int baro_probe(void)
{
	...
	if (err) {
		pr_info("baro real driver init fail\n");

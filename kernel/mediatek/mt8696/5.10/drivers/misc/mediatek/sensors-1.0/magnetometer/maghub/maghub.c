...
static ssize_t trace_show(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL) {
		pr_info("maghub_ipi_data is null!!\n");
		return 0;
	}
...
static void scp_init_work_done(struct work_struct *work)
{
	...
	if (atomic_xchg(&obj->first_ready_after_boot, 1) == 0) {
		...
		if (err < 0) {
			pr_info("set_cmd_to_hub fail, (ID: %d),(action: %d)\n",
...
static int maghub_enable(int en)
{
	...
	if (res)
		pr_info("maghub_m_setPowerMode is failed!!\n");
	return res;
}
...
static int maghub_set_delay(u64 ns)
{
	...
	if (err < 0) {
		pr_info("maghub_m_set_delay fail!\n");
		return err;
	}
...
static int maghub_get_data(int *x, int *y, int *z, int *status)
{
	...
	if (ret < 0) {
		pr_info("maghub_GetMData fail, ret:%d\n", ret);
		return ret;
	}

	if (sscanf(buff, "%x %x %x %x", x, y, z, status) != 4)
		pr_info("maghub_m_get_data sscanf fail!!\n");
	return 0;
}
...
static int maghub_probe(struct platform_device *pdev)
{
	...
	if (err) {
		pr_info("mag_factory_device_register register failed\n");
		goto exit_kfree;
	}
	...
	if (err) {
		pr_info("register mag control path err\n");
	...
	if (err) {
		pr_info("register data control path err\n");
		goto create_attr_failed;
	}
	pr_debug("%s: OK\n", __func__);
	maghub_init_flag = 1;
...
static int maghub_remove(struct platform_device *pdev)
{
	...
	if (err)
		pr_info("maghub_delete_attr fail: %d\n", err);

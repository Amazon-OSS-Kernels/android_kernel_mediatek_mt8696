...
static int handle_to_index(int handle)
{
	...
	default:
		index = -1;
		pr_info("%s invalid handle:%d, index:%d\n", __func__,
			handle, index);
		return index;
	}
...
static struct biometric_context *biometric_context_alloc_object(void)
{
	...
	pr_debug("%s++++\n", __func__);
	if (!obj) {
		pr_info("Alloc biometric object error!\n");
		return NULL;
	}
...
static int biometric_enable_and_batch(int index)
{
	...
	if (cxt->ctl_context[index].power == 1 &&
		...
		if (err) {
			pr_info("biometric turn off power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->ctl_context[index].power == 0 &&
		...
		if (err) {
			pr_info("biometric turn on power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->ctl_context[index].power == 1 &&
		...
		if (cxt->ctl_context[index].biometric_ctl.is_support_batch)
			...
			err = cxt->ctl_context[index].biometric_ctl.batch(0,
				cxt->ctl_context[index].delay_ns, 0);
		if (err) {
			pr_info("biometric set batch(ODR) err %d\n", err);
			return -1;
		}
...
static ssize_t bioactive_store(struct device *dev,
...
/*----------------------------------------------------------------------------*/
static ssize_t bioactive_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	struct biometric_context *cxt = NULL;
...
static ssize_t biobatch_store(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t count)
{
	struct biometric_context *cxt = biometric_context_obj;
...
static ssize_t biobatch_show(struct device *dev,
				     struct device_attribute *attr,
				     char *buf)
{
	int len = 0;
...
static ssize_t bioflush_store(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t count)
{
	struct biometric_context *cxt = NULL;
	...
	if (cxt->ctl_context[index].biometric_ctl.flush != NULL)
		...
		pr_info("BIOMETRIC DRIVER OLD ARCHITECTURE DON'T SUPPORT BIOMETRIC COMMON VERSION FLUSH\n");
	}
	if (err < 0)
		pr_info("biometric enable flush err %d\n", err);
	mutex_unlock(&biometric_context_obj->biometric_op_mutex);
	return err;
}

static ssize_t bioflush_show(struct device *dev,
				     struct device_attribute *attr,
				     char *buf)
{
	int len = 0;
...
static ssize_t biodevnum_show(struct device *dev,
...
int biometric_driver_add(struct biometric_init_info *obj, int handle)
{
	...
	if (!obj) {
		pr_info("[%s] fail, biometric_init_info is NULL\n", __func__);
		return -1;
	}
	...
	if (platform_driver_register(&biometric_driver))
		pr_info("failed to register biometric driver already exist\n");
...
static int biometric_misc_init(struct biometric_context *cxt)
{
	...
	if (err)
		pr_info("unable to register biometric misc device!!\n");

	return err;
}

DEVICE_ATTR_RW(bioactive);
DEVICE_ATTR_RW(biobatch);
DEVICE_ATTR_RW(bioflush);
DEVICE_ATTR_RO(biodevnum);
...
static int bio_probe(void)
{
	...
	if (err) {
		pr_info("biometric real driver init fail\n");
	...
	if (err < 0) {
		pr_info("unable to create biometric attribute file\n");
		goto real_driver_init_fail;
	}
...
{
	if (bio_probe()) {
		pr_info("failed to register biometric driver\n");
		return -ENODEV;
	}

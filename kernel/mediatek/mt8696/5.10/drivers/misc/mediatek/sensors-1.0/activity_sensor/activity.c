...
static void act_work_func(struct work_struct *work)
{
	...
	if (cxt->act_data.get_data == NULL)
		pr_info("act driver not register data path\n");
...
static struct act_context *act_context_alloc_object(void)
{
	...
	pr_debug("%s start\n", __func__);
	if (!obj) {
		pr_info("Alloc act object error!\n");
		return NULL;
	}
...
static int act_enable_and_batch(void)
{
	...
	if (cxt->power == 1 && cxt->enable == 0) {
		...
		if (err) {
			pr_info("act turn off power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->power == 0 && cxt->enable == 1) {
		...
		if (err) {
			pr_info("act turn on power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->power == 1 && cxt->delay_ns >= 0) {
		...
		if (err) {
			pr_info("act set batch(ODR) err %d\n", err);
			return -1;
		}
...
#endif
static ssize_t actactive_store(struct device *dev,
...
/*----------------------------------------------------------------------------*/
static ssize_t actactive_show(struct device *dev,
...
static ssize_t actbatch_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	struct act_context *cxt = act_context_obj;
...
static ssize_t actbatch_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 0);
}

static ssize_t actflush_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	struct act_context *cxt = NULL;
	...
	if (err < 0)
		pr_info("act enable flush err %d\n", err);
	mutex_unlock(&act_context_obj->act_op_mutex);
	return err;
}

static ssize_t actflush_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 0);
}

static ssize_t actdevnum_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", 0);
}
...
static int act_misc_init(struct act_context *cxt)
{
	...
	if (err)
		pr_info("unable to register act misc device!!\n");

	return err;
}

DEVICE_ATTR_RW(actactive);
DEVICE_ATTR_RW(actbatch);
DEVICE_ATTR_RW(actflush);
DEVICE_ATTR_RO(actdevnum);
...
int act_register_control_path(struct act_control_path *ctl)
{
	...
	err = sysfs_create_group(&act_context_obj->mdev.this_device->kobj,
				 &act_attribute_group);
	if (err < 0) {
		pr_info("unable to create act attribute file\n");
		return -3;
	}
...
static int act_probe(void)
{
	...
	if (err) {
		pr_info("act real driver init fail\n");
		goto real_driver_init_fail;
	}
	/* err = act_factory_device_init(); */
	/* if(err) */
	/* { */
	/* pr_info("act_factory_device_init fail\n"); */
	/* } */
...
static int __init act_init(void)
{
	...
	if (act_probe()) {
		pr_info("failed to register act driver\n");
		return -ENODEV;
	}

...
static int accelhub_WriteCalibration(int dat[ACCELHUB_AXES_NUM])
{
	...
	if (err < 0) {
		pr_info("accelhub_WriteCalibration_scp fail\n");
		return err;
	}
...
static int accelhub_ReadAllReg(char *buf, int bufsize)
{
	...
	if (err) {
		pr_info("Power on accelhub error %d!\n", err);
		return err;
	}
...
static int gsensor_factory_clear_cali(void)
{
	...
	if (err) {
		pr_info("gsensor_ResetCalibration failed!\n");
		return -1;
	}
#endif
	return 0;
...
static int gsensor_factory_set_cali(int32_t data[3])
{
	...
	if (err) {
		pr_info("gsensor_WriteCalibration failed!\n");
		return -1;
	}
#endif
	return 0;
...
static int gsensor_factory_get_cali(int32_t data[3])
{
	...
	if (err) {
		pr_info("gsensor_ReadCalibration failed!\n");
		return -1;
	}
#else
...
static int gsensor_enable_nodata(int en)
{
	...
	if (atomic_read(&obj->suspend) == 0) {
		...
		if (err < 0) {
			pr_info("scp_gsensor_enable_nodata fail!\n");
			return -1;
		}
	}
...
static int gsensor_get_data(int *x, int *y, int *z, int *status)
{
	...
	if (err < 0) {
		pr_info("accelhub_ReadSensorData fail!!\n");
		return -1;
	}
...
static int accelhub_probe(struct platform_device *pdev)
{
	...
	if (err) {
		pr_info("gsensor_factory_device register failed\n");
		goto exit_kfree;
	}
	...
	if (err) {
		pr_info("register acc control path err\n");
		goto exit_create_attr_failed;
	}
	...
	if (err) {
		pr_info("register acc data path err\n");
		goto exit_create_attr_failed;
	}
	gsensor_init_flag = 0;
	pr_debug("%s: OK\n", __func__);
...
static int accelhub_remove(struct platform_device *pdev)
{
	...
	err = accelhub_delete_attr(
		&accelhub_init_info.platform_diver_addr->driver);
	if (err)
		pr_info("accelhub_delete_attr fail: %d\n", err);
...
static int __init accelhub_init(void)
{
	...
	if (platform_device_register(&accelhub_device)) {
		pr_info("accel platform device error\n");
		return -1;
	}

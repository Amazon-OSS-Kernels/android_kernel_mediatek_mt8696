...
static void step_c_work_func(struct work_struct *work)
{
	...
	if (err) {
		pr_info("get floor_c data fails!!\n");
		goto step_c_loop;
	} else {
		{
...
static struct step_c_context *step_c_context_alloc_object(void)
{
	...
	pr_debug("%s start\n", __func__);
	if (!obj) {
		pr_info("Alloc step_c object error!\n");
		return NULL;
	}
...
static int significant_real_enable(int enable)
{
	...
	if (enable == 1) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (err == 0)
				...
				pr_info("significant E(%d)err 3 = %d\n",
					enable, err);
			}
		}
	...
	if (enable == 0) {
		...
		if (err)
			pr_info("enable_significantenable(%d) err = %d\n",
				enable, err);
...
static int floor_c_real_enable(int enable)
{
	...
	if (enable == 1) {
		for (i = 0; i < 3; i++) {
			...
			if (err == 0)
				...
				pr_info("floor_c enable(%d) err 3 = %d\n",
					enable, err);
		}
	...
	if (enable == 0) {
		...
		if (err)
			pr_info("floor_c enable(%d) err = %d\n",
				enable, err);
...
static int step_c_enable_data(int enable)
{
	...
	if (cxt->step_c_ctl.open_report_data == NULL) {
		pr_info("no step_c control path\n");
		return -1;
	}
...
int step_c_enable_nodata(int enable)
{
	...
	if (cxt->step_c_ctl.enable_nodata == NULL) {
		pr_info("%s:step_c ctl path is NULL\n", __func__);
		return -1;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "1", 1))
		...
		pr_info(" step_c_store enable nodata cmd error !!\n");
	mutex_unlock(&step_c_context_obj->step_c_op_mutex);
	return err;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case ID_STEP_DETECTOR:
		if (en == 1)
			...
			pr_info(" step_d_real_enable error !!\n");
		break;
	case ID_SIGNIFICANT_MOTION:
		if (en == 1)
			...
			pr_info(" significant_real_enable error !!\n");
		break;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (handle == ID_STEP_COUNTER) {
		...
		if (cxt->step_c_ctl.step_c_batch != NULL)
			...
			pr_info("SUPPORT STEP COUNTER COM BATCH\n");
		if (res < 0)
			pr_info("step counter enable batch err %d\n",
				res);
		...
		if (cxt->step_c_ctl.step_d_batch != NULL)
			...
			pr_info("NOT SUPPORT STEP DETECTOR COM BATCH\n");
		if (res < 0)
			pr_info("step detector enable batch err %d\n",
				res);
		...
		if (cxt->step_c_ctl.smd_batch != NULL)
			...
			pr_info("STEP SMD OLD NOT SUPPORT COM BATCH\n");
		if (res < 0)
			pr_info("step smd enable batch err %d\n", res);
		...
		if (cxt->step_c_ctl.floor_c_batch != NULL)
			...
			pr_info("NOT SUPPORT FLOOR COUNT COM BATCH\n");
		if (res < 0)
			pr_info("floor count enable batch err %d\n", res);
	}
	mutex_unlock(&step_c_context_obj->step_c_op_mutex);
				  ...
				  const char *buf, size_t count)
{
	...
	if (handle == ID_STEP_COUNTER) {
		if (cxt->step_c_ctl.step_c_flush != NULL)
			...
			pr_info("NOT SUPPORT STEP COUNTER COM FLUSH\n");
		if (err < 0)
			pr_info("step counter enable flush err %d\n",
				err);
	} else if (handle == ID_STEP_DETECTOR) {
		if (cxt->step_c_ctl.step_d_flush != NULL)
			err = cxt->step_c_ctl.step_d_flush();
		else
			pr_info("NOT SUPPORT STEP DETECTOR COM FLUSH\n");
		if (err < 0)
			pr_info("step detector enable flush err %d\n",
				err);
	} else if (handle == ID_SIGNIFICANT_MOTION) {
		if (cxt->step_c_ctl.smd_flush != NULL)
			err = cxt->step_c_ctl.smd_flush();
		else
			pr_info("NOT SUPPORT SMD COMMON VERSION FLUSH\n");
		if (err < 0)
			pr_info("smd enable flush err %d\n", err);
	} else if (handle == ID_FLOOR_COUNTER) {
		if (cxt->step_c_ctl.floor_c_flush != NULL)
			err = cxt->step_c_ctl.floor_c_flush();
		else
			pr_info("NOT SUPPORT FLOOR COUNTER COM FLUSH\n");
		if (err < 0)
			pr_info("floor counter enable flush err %d\n",
				err);
...
int step_c_driver_add(struct step_c_init_info *obj)
{
	...
	if (platform_driver_register(&step_counter_driver))
		pr_info("fail to register gensor driver already exist\n");
	for (i = 0; i < MAX_CHOOSE_STEP_C_NUM; i++) {
...
static int step_c_misc_init(struct step_c_context *cxt)
{
	...
	if (err)
		pr_info("unable to register step_c misc device!!\n");
	return err;
}
...
int step_c_register_control_path(struct step_c_control_path *ctl)
{
	...
	if (err < 0) {
		pr_info("unable to create step_c attribute file\n");
		return -3;
	}
...
static int step_c_probe(void)
{
	...
	if (err) {
		pr_info("step_c real driver init fail\n");

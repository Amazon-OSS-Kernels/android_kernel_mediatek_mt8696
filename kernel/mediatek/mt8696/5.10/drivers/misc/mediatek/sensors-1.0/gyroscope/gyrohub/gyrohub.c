...
static int gyrohub_ResetCalibration(void)
{
	...
	if (err < 0)
		pr_info("sensor_set_cmd_to_hub fail,(ID:%d),(action:%d)\n",
...
static int gyrohub_WriteCalibration(int dat[GYROHUB_AXES_NUM])
{
	...
	if (err < 0) {
		pr_info("gyrohub_WriteCalibration_scp fail\n");
		return -1;
	}
...
static int gyrohub_SetPowerMode(bool enable)
{
	...
	if (err < 0)
		pr_info("sensor_enable_to_hub fail!\n");
...
static int gyrohub_ReadGyroData(char *buf, int bufsize)
{
	...
	if (err < 0) {
		pr_info("sensor_get_data_from_hub fail!\n");
		return err;
	}
...
static int gyrohub_ReadAllReg(char *buf, int bufsize)
{
	...
	if (err)
		pr_info("Power on mpu6050 error %d!\n", err);
	msleep(50);
...
static void scp_init_work_done(struct work_struct *work)
{
	...
	if (atomic_read(&obj->scp_init_done) == 0) {
		pr_info("scp is not ready to send cmd\n");
		return;
	}
	...
	if (err < 0)
		pr_info("sensor_cfg_to_hub fail\n");
#endif
}
	...
	int64_t sample_periods_ms)
{
	...
	if (enabledisable == true) {
		...
		if (err) {
			pr_info("sensor_set_delay_to_hub failed!\n");
			return -1;
		}
	}
	err = sensor_enable_to_hub(ID_GYROSCOPE, enabledisable);
	if (err) {
		pr_info("sensor_enable_to_hub failed!\n");
		return -1;
	}
	return 0;
}
...
static int gyrohub_factory_clear_cali(void)
{
	...
	if (err) {
		pr_info("gyrohub_ResetCalibration failed!\n");
		return -1;
	}
#endif
	return 0;
...
static int gyrohub_factory_set_cali(int32_t data[3])
{
	...
	if (err) {
		pr_info("gyrohub_WriteCalibration failed!\n");
		return -1;
	}
#endif
	return 0;
...
static int gyrohub_factory_get_cali(int32_t data[3])
{
	...
	if (err) {
		pr_info("gyrohub_ReadCalibration failed!\n");
		return -1;
	}
#else
...
static int gyrohub_enable_nodata(int en)
{
	...
	if (res < 0) {
		pr_info("GYROHUB_SetPowerMode fail\n");
		return res;
	}
	pr_debug("%s OK!\n", __func__);
	return 0;
...
static int gyrohub_set_delay(u64 ns)
{
	...
	if (err < 0) {
		pr_info("sensor_set_delay_to_hub fail!\n");
		return err;
	}
...
static int gpio_config(void)
{
	...
	if (gyroPltFmDev == NULL) {
		pr_info("Cannot find gyro device!\n");
		return 0;
	}
	...
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
		pr_info("Cannot find gyro pinctrl!\n");
		return ret;
	}
	pins_default = pinctrl_lookup_state(pinctrl, "pin_default");
	if (IS_ERR(pins_default)) {
		ret = PTR_ERR(pins_default);
		pr_info("Cannot find gyro pinctrl default!\n");
	}

	pins_cfg = pinctrl_lookup_state(pinctrl, "pin_cfg");
	if (IS_ERR(pins_cfg)) {
		ret = PTR_ERR(pins_cfg);
		pr_info("Cannot find gyro pinctrl pin_cfg!\n");
		return ret;
	}
...
static int gyrohub_get_data(int *x, int *y, int *z, int *status)
{
	...
	if (err < 0) {
		pr_info("gyrohub_ReadGyroData fail!!\n");
		return -1;
	}
...
static int gyrohub_probe(struct platform_device *pdev)
{
	...
	if (err) {
		pr_info("gyrohub create attribute err = %d\n", err);
	...
	if (err) {
		pr_info("register gyro control path err\n");
		goto exit_create_attr_failed;
	}
	...
	if (err) {
		pr_info("gyro_register_data_path fail = %d\n", err);
		goto exit_create_attr_failed;
	}
	gyrohub_init_flag = 0;
...
static int gyrohub_remove(struct platform_device *pdev)
{
	...
	if (err)
		pr_info("gyrohub_delete_attr fail: %d\n", err);

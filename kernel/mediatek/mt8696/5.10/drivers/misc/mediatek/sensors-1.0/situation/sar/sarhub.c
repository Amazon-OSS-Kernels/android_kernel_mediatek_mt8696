...
static int sar_factory_get_data(int32_t sensor_data[3])
{
	...
	if (err < 0) {
		pr_info_ratelimited("sensor_get_data_from_hub fail!!\n");
		return -1;
	}
...
static int sar_factory_get_cali(int32_t data[3])
{
	...
	if (!err) {
		pr_info("sar factory get cali fail!\n");
		return -1;
	}
...
static int sarhub_local_init(void)
{
	...
	if (err) {
		pr_info("register sar control path err\n");
		goto exit;
	}
	...
	if (err) {
		pr_info("register sar data path err\n");
		goto exit;
	}

	err = sar_factory_device_register(&sarhub_factory_device);
	if (err) {
		pr_info("sar_factory_device register failed\n");
		goto exit;
	}
	...
	err = scp_sensorHub_data_registration(ID_SAR,
		sar_recv_data);
	if (err) {
		pr_info("SCP_sensorHub_data_registration fail!!\n");
		goto exit;
	}
	return 0;
exit:

...
long alspshub_read_ps(u8 *ps)
{
	...
	if (res < 0) {
		*ps = -1;
		pr_info("sensor_get_data_from_hub fail, (ID: %d)\n",
			ID_PROXIMITY);
		return -1;
	}
...
long alspshub_read_als(u16 *als)
{
	...
	if (res < 0) {
		*als = -1;
		pr_info_ratelimited("sensor_get_data_from_hub fail, (ID: %d)\n",
			ID_LIGHT);
		return -1;
	}
	*als = data_t.light;
...
static void alspshub_init_done_work(struct work_struct *work)
{
	...
	if (atomic_read(&obj->scp_init_done) == 0) {
		pr_info("wait for nvram to set calibration\n");
		return;
	}
	...
	if (err < 0)
		pr_info("sensor_cfg_to_hub ps fail\n");
	...
	err = sensor_cfg_to_hub(ID_LIGHT,
		(uint8_t *)cfg_data, sizeof(cfg_data));
	if (err < 0)
		pr_info("sensor_cfg_to_hub als fail\n");
#endif
}
...
static int pshub_factory_set_threshold(int32_t threshold[2])
{
	...
	if (threshold[0] < threshold[1] || threshold[0] <= 0 ||
		threshold[1] <= 0) {
		pr_info("PS set threshold fail! invalid value:[%d, %d]\n",
...
static int ps_enable_nodata(int en)
{
	...
	if (res < 0) {
		pr_info("als_enable_nodata is failed!!\n");
		return -1;
	}
...
static int alspshub_probe(struct platform_device *pdev)
{
	...
	if (err) {
		pr_info("alsps_factory_device_register register failed\n");
		goto exit_kfree;
	}
	...
	if (err) {
		pr_info("create attribute err = %d\n", err);
	...
	if (!obj->ps_wake_lock) {
		pr_info("wakeup source init fail\n");
		err = -ENOMEM;
		goto exit_create_attr_failed;
	}
...
static int alspshub_remove(struct platform_device *pdev)
{
	...
	if (err)
		pr_info("alspshub_delete_attr fail: %d\n", err);
...
static int __init alspshub_init(void)
{
	if (platform_device_register(&alspshub_device)) {
		pr_info("alsps platform device error\n");
		return -1;
	}

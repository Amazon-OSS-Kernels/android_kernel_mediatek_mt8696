					...
					unsigned long arg)
{
	...
	case SAR_IOCTL_READ_SENSORDATA:
		if (sar_factory.fops != NULL &&
			...
			pr_info("SAR_IOCTL_READ_SENSORDATA NULL\n");
			return -EINVAL;
		}
		return 0;
	case SAR_IOCTL_ENABLE_CALI:
		if (sar_factory.fops != NULL &&
			...
			pr_info("SAR_IOCTL_ENABLE_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
	case SAR_IOCTL_GET_CALI:
		if (sar_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("SAR_IOCTL_GET_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("SAR_IOCTL_GET_CALI NULL\n");
			return -EINVAL;
		}

...
static struct situation_context *situation_context_alloc_object(void)
{
	...
	pr_debug("%s start\n", __func__);
	if (!obj) {
		pr_info("Alloc situ object error!\n");
		return NULL;
	}
	...
	int64_t time_stamp)
{
	...
	if (index < 0) {
		pr_info_ratelimited("[%s] invalid index\n", __func__);
		return -1;
	}
...
static int situation_enable_and_batch(int index)
{
	...
	if (cxt->ctl_context[index].power == 1 &&
		...
		if (err) {
			pr_info("situation turn off power err = %d\n",
				err);
			return -1;
		}
	...
	if (cxt->ctl_context[index].power == 0 &&
		...
		if (err) {
			pr_info("situation turn on power err = %d\n",
				err);
			return -1;
		}
	...
	if (cxt->ctl_context[index].power == 1 &&
		...
		if (cxt->ctl_context[index].situation_ctl.is_support_batch)
			...
			err = cxt->ctl_context[index].situation_ctl.batch(0,
				cxt->ctl_context[index].delay_ns, 0);
		if (err) {
			pr_info("situation set batch(ODR) err %d\n",
				err);
			return -1;
		}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cxt->ctl_context[index].enable == 1) {
		if (cxt->ctl_context[index].situation_ctl.open_report_data
			== NULL) {
			pr_info("open_report_data() is NULL, %d\n", index);
			goto err_out;
		}
		err = cxt->ctl_context[index].situation_ctl.open_report_data(1);
		if (err) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cxt->ctl_context[index].situation_ctl.flush != NULL)
		...
		pr_info("SITUATION OLD ARCH NOT SUPPORT COMM FLUSH\n");
	if (err < 0)
		pr_info("situation enable flush err %d\n", err);
	mutex_unlock(&situation_context_obj->situation_op_mutex);
	if (err)
		return err;
	else
...
int situation_driver_add(struct situation_init_info *obj, int handle)
{
	...
	if (!obj) {
		pr_info("[%s] fail, situation_init_info is NULL\n",
			__func__);
		return -1;
	}
...
static int situation_misc_init(struct situation_context *cxt)
{
	...
	if (err)
		pr_info("unable to register situ misc device!!\n");
	...
	int handle)
{
	...
	if (!cxt->ws[index]) {
		pr_info("%s: wakeup source init fail\n", __func__);
		return -ENOMEM;
	}
...
static int situation_probe(void)
{
	...
	if (err) {
		pr_info("situ real driver init fail\n");
	...
	if (err < 0) {
		pr_info("unable to create situ attribute file\n");
		goto real_driver_init_fail;
	}

...
static ssize_t sensordata_show(struct device_driver *ddri, char *buf)
{
	...
	if (err < 0) {
		pr_info("barohub_set_powermode fail!!\n");
		return 0;
	}
...
static int barohub_factory_get_data(int32_t *data)
{
	...
	err = barohub_get_pressure(strbuf, BAROHUB_BUFSIZE);
	if (err < 0) {
		pr_info("barohub_get_pressure fail\n");
		return -1;
	}
...
static int barohub_get_data(int *value, int *status)
{
	...
	if (err) {
		pr_info("get compensated pressure value failed, err = %d\n",
			err);
		return -1;
	}
...
static int barohub_probe(struct platform_device *pdev)
{
	...
	if (err) {
		pr_info("baro_factory_device_register failed, err = %d\n",
			err);
		goto exit_kfree;
	}
	...
	if (err) {
		pr_info("create attribute failed, err = %d\n", err);
	...
	if (err) {
		pr_info("register baro control path err\n");
		goto exit_create_attr_failed;
	}
	...
	if (err) {
		pr_info("baro_register_data_path failed, err = %d\n", err);
		goto exit_create_attr_failed;
	}
...
static int barohub_remove(struct platform_device *pdev)
{
	...
	if (err)
		pr_info("barohub_delete_attr failed, err = %d\n", err);
...
static int __init barohub_init(void)
{
	...
	if (platform_device_register(&barohub_device)) {
		pr_info("baro platform device error\n");
		return -1;
	}

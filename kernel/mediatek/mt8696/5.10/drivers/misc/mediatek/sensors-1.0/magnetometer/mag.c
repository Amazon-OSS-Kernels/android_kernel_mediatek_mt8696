...
static struct mag_context *mag_context_alloc_object(void)
{
	...
	pr_debug("%s start\n", __func__);
	if (!obj) {
		pr_info("Alloc magel object error!\n");
		return NULL;
	}
...
static int mag_enable_and_batch(void)
{
	...
	if (cxt->power == 1 && cxt->enable == 0) {
		...
		if (err) {
			pr_info("mag turn off power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->power == 0 && cxt->enable == 1) {
		...
		if (err) {
			pr_info("mag turn on power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->power == 1 && cxt->delay_ns >= 0) {
		...
		if (err) {
			pr_info("mag set batch(ODR) err %d\n", err);
			return -1;
		}
			       ...
			       size_t count)
{
	...
	if (err < 0)
		pr_info("mag enable flush err %d\n", err);
	mutex_unlock(&mag_context_obj->mag_op_mutex);
	if (err)
		return err;
	else
...
int mag_driver_add(struct mag_init_info *obj)
{
	...
	pr_debug("%s\n", __func__);
	if (!obj) {
		pr_info("%s fail, mag_init_info is NULL\n", __func__);
		return -1;
	}
...
static int mag_misc_init(struct mag_context *cxt)
{
	...
	if (err)
		pr_info("unable to register mag misc device!!\n");
...
int mag_register_control_path(struct mag_control_path *ctl)
{
	...
	err = sysfs_create_group(&mag_context_obj->mdev.this_device->kobj,
				 &mag_attribute_group);
	if (err < 0) {
		pr_info("unable to create mag attribute file\n");
		return -3;
	}
...
static int mag_probe(void)
{
	...
	if (err) {
		pr_info("mag_real_driver_init fail\n");

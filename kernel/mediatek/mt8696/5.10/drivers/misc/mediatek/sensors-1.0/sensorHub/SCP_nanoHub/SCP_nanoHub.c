...
static void ipi_work(struct work_struct *work)
{
	...
	list_for_each_entry_safe(m, _m, &master.queue, queue) {
		...
		list_for_each_entry(t, &m->transfers, transfer_list) {
			if (!t->tx_buf && t->len) {
				status = -EINVAL;
				pr_info("transfer param wrong :%d\n",
					status);
				break;
			}
			if (t->len)
	...
	uint *len, unsigned int wait)
{
	...
	if (*len > SENSOR_IPI_SIZE) {
		pr_info("over sensor data size!!\n");
		return -1;
	}

	if (in_interrupt()) {
		pr_info("Can't do %s in interrupt context!!\n", __func__);
		return -1;
	}

	if (data->rsp.sensorType > ID_SENSOR_MAX_HANDLE) {
		pr_info("SCP_sensorHub_IPI_handler invalid sensor type %d\n",
			data->rsp.sensorType);
		return -1;
	}
	...
	SCP_sensorHub_handler handler)
{
	...
	if (sensor > ID_SENSOR_MAX_HANDLE) {
		pr_info("SCP_sensorHub_rsp_registration invalid sensor %d\n",
			sensor);
		return -1;
	}

	if (handler == NULL)
		pr_info("SCP_sensorHub_rsp_registration null handler\n");
...
static void SCP_sensorHub_write_wp_queue(union SCP_SENSOR_HUB_DATA *rsp)
{
	...
	if (unlikely(wp_queue->head == wp_queue->tail))
		pr_info("dropped currWp due to ringbuffer is full\n");
...
static int SCP_sensorHub_fetch_next_wp(uint32_t *currWp)
{
	...
	/* pr_info("head:%d, tail:%d, currWp:%d\n",
			...
			int rx_len)
{
	...
	if (!hw->context) {
		pr_info("after ipi timeout ack occur then dropped this\n");
		goto out;
	}
 ...
 *			else
 *				pr_info("type:%d don't support this flow?\n",
 *					handle);
	...
	void *data, unsigned int len)
{
	...
	if (len > SENSOR_IPI_SIZE) {
		pr_info("%s len=%d error\n", __func__, len);
		return;
	}
	/*pr_info("sensorType:%d, action=%d event:%d len:%d\n",
	...
	long long samplingPeriodNs, long long maxBatchReportLatencyNs)
{
	...
	if (mSensorState[sensor_type].sensorType) {
		...
		if (ret < 0) {
			pr_info("fail enbatch h:%d, r: %d,l: %lld, cmd:%d\n",
				handle, cmd.rate, cmd.latency, cmd.cmd);
			return -1;
		}
	} else {
		pr_info("unhandle handle=%d, is inited?\n", handle);
		return -1;
	}
	return 0;
}
...
static int SCP_sensorHub_report_alt_data(struct data_unit_t *data_t)
{
	...
	if (obj->dispatch_data_cb[alt_id] == NULL) {
		pr_info("alt:%d don't support this flow?\n", alt_id);
		return 0;
	}
...
static int sensor_send_timestamp_wake_locked(void)
{
	...
	if (err < 0)
		pr_info("scp_sensorHub_req_send fail!\n");
	return err;
}
...
static int sensor_send_timestamp_to_hub(void)
{
	...
	if (READ_ONCE(rtc_compensation_suspend)) {
		pr_info("rtc_compensation_suspend suspend,drop time sync\n");
		return 0;
	}
...
static void sensor_disable_report_flush(uint8_t handle)
{
	...
	while (atomic_read(p_flush_count) > 0) {
		...
		if (ret < 0)
			pr_info("%d flush complete err when disable\n",
				handle);
	}
	mutex_unlock(&flush_mtx);
}
...
int sensor_cfg_to_hub(uint8_t handle, uint8_t *data, uint8_t count)
{
	...
	if (handle > ID_SENSOR_MAX_HANDLE) {
		...
		if (ret < 0) {
			pr_info("failed cfg data handle:%d, cmd:%d\n",
				handle, cmd->cmd);
			ret =  -1;
		}
		vfree(cmd);
...
int sensor_calibration_to_hub(uint8_t handle)
{
	...
	if (mSensorState[sensor_type].sensorType) {
		...
		ret = nanohub_external_write((const uint8_t *)&cmd,
			sizeof(struct ConfigCmd));
		if (ret < 0) {
			pr_info("failed calibration handle:%d\n",
				handle);
			return -1;
		}
	} else {
...
int sensor_selftest_to_hub(uint8_t handle)
{
	...
	if (mSensorState[sensor_type].sensorType) {
		...
		ret = nanohub_external_write((const uint8_t *)&cmd,
			sizeof(struct ConfigCmd));
		if (ret < 0) {
			pr_info("failed selfttest handle:%d\n",
				handle);
			return -1;
		}
	} else {
	...
	struct data_unit_t *data)
{
	...
	if (atomic_read(&power_status) == SENSOR_POWER_DOWN) {
		pr_info("scp power down, we can not access scp\n");
		return -1;
	}
	...
	if (sensorType != req.get_data_rsp.sensorType ||
		...
		pr_info("req Type: %d, rsp Type:%d action:%d, errcode:%d\n",
		...
		case CUST_ACTION_GET_RAW_DATA:
			...
			if (err == 0) {
				if ((req.set_cust_rsp.action !=
					...
					pr_info("scp_sHub_req_send fail!\n");
					return -1;
				}
				...
				*((uint8_t *) data) =
				...
				pr_info("scp_sensorHub_req_send failed!\n");
			}
			return 0;
...
static int sensorHub_probe(struct platform_device *pdev)
{
	...
	if (!obj) {
		pr_info("Allocate SCP_sensorHub_data fail\n");
		err = -ENOMEM;
		goto exit;
	}
	 ...
	 *	pr_info("direct_push_workqueue fail\n");
	 *	return -1;
	 *}
	 */
	...
	task = kthread_run(SCP_sensorHub_direct_push_work,
		NULL, "chre_kthread");
	if (IS_ERR(task)) {
		pr_info("SCP_sensorHub_direct_push_work create fail!\n");
	...
	if (!obj->ws) {
		pr_info("SCP_sensorHub: wakeup source init fail\n");
		err = -ENOMEM;
		goto exit_wakeup;
	}
	...
	if (IS_ERR(task_power_reset)) {
		pr_info("sensorHub_power_up_work create fail!\n");
	...
	const char *buf, size_t count)
{
	...
	if (res < 0) {
		pr_info("cmd_to_hub (ID: %d),(action: %d)err: %d\n", handle,

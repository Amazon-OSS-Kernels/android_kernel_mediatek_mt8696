					...
					unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_INIT:
		...
		if (gyro_factory.fops != NULL &&
			...
			err = gyro_factory.fops->enable_sensor(flag,
							       5); /* 5ms */
			if (err < 0) {
				pr_info("GYROSCOPE_IOCTL_INIT fail!\n");
				return -EINVAL;
			}
			pr_debug(
			...
			pr_info("GYROSCOPE_IOCTL_INIT NULL\n");
			return -EINVAL;
		}
		return 0;
	...
	case GYROSCOPE_IOCTL_READ_SENSORDATA:
		if (gyro_factory.fops != NULL &&
			...
			pr_info("GYROSCOPE_IOCTL_READ_SENSORDATA NULL\n");
			return -EINVAL;
		}
		return 0;
	...
	case GYROSCOPE_IOCTL_SET_CALI:
		...
		if (gyro_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GYROSCOPE_IOCTL_SET_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("GYROSCOPE_IOCTL_SET_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
	case GYROSCOPE_IOCTL_CLR_CALI:
		if (gyro_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GYROSCOPE_IOCTL_CLR_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("GYROSCOPE_IOCTL_CLR_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
	case GYROSCOPE_IOCTL_GET_CALI:
		if (gyro_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GYROSCOPE_IOCTL_GET_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("GYROSCOPE_IOCTL_GET_CALI NULL\n");
			return -EINVAL;
		}
	...
	case GYROSCOPE_IOCTL_ENABLE_CALI:
		if (gyro_factory.fops != NULL &&
			...
			pr_info("GYROSCOPE_IOCTL_ENABLE_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
	case GYROSCOPE_IOCTL_SELF_TEST:
		if (gyro_factory.fops != NULL &&
			...
			pr_info("GYROSCOPE_IOCTL_SELF_TEST NULL\n");
			return -EINVAL;
		}
		return 0;
	default:
...
int gyro_factory_device_register(struct gyro_factory_public *dev)
{
	...
	if (err) {
		pr_info("gyro_factory_device register failed\n");
		err = -1;
	}
	return err;
}

 ...
 *	if (ret) {
 *		dev_dbg(dev, "nanohub_download_kernel: err=%d\n", ret);
 *		return -EIO;
 *	}
 ...
 *	if (ret) {
 *		dev_dbg(dev, "nanohub_download_app(%s): err=%d\n",
 *			buffer, ret);
 *		return -EIO;
 *	}
...
static int nanohub_create_devices(struct nanohub_data *data)
{
	...
	for (i = 0; i < ID_NANOHUB_MAX; ++i) {
		...
		if (IS_ERR(io->dev)) {
			ret = PTR_ERR(io->dev);
			pr_info("nanohub: device_create failed for %s; err=%d\n",
			       names[i], ret);
			goto fail_dev;
		}
	}
...
struct iio_dev *nanohub_probe(struct device *dev, struct iio_dev *iio_dev)
{
	...
	if (!data->ws) {
		pr_info("nanohub: wakeup source init fail\n");
		ret = -ENOMEM;
		goto fail_wakeup;
	}
	...
	if (ret) {
		pr_info("nanohub: iio_device_register failed\n");
		goto fail_irq;
	}
...
static int __init nanohub_init(void)
{
	...
	if (IS_ERR(sensor_class)) {
		...
		pr_info("nanohub: class_create failed; err=%d\n", ret);
	}
	if (!ret)
	...
	if (major < 0) {
		ret = major;
		major = 0;
		pr_info("nanohub: can't register; err=%d\n", ret);
	}

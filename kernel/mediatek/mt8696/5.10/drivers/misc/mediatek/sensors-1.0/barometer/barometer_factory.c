					...
					unsigned long arg)
{
	...
	case BAROMETER_IOCTL_INIT:
		...
		if (baro_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("BAROMETER_IOCTL_INIT fail!\n");
				return -EINVAL;
			}
			pr_debug(
			...
			pr_info("BAROMETER_IOCTL_INIT NULL\n");
			return -EINVAL;
		}
		return 0;
	case BAROMETER_GET_PRESS_DATA:
		if (baro_factory.fops != NULL &&
			...
			pr_info("BAROMETER_GET_PRESS_DATA NULL\n");
			return -EINVAL;
		}
		return 0;
	case BAROMETER_IOCTL_ENABLE_CALI:
		if (baro_factory.fops != NULL &&
			...
			pr_info("BAROMETER_IOCTL_ENABLE_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
...
int baro_factory_device_register(struct baro_factory_public *dev)
{
	...
	if (err) {
		pr_info("baro_factory_device register failed\n");
		err = -1;
	}
	return err;
}

...
static struct fusion_context *fusion_context_alloc_object(void)
{
	...
	pr_debug("%s start\n", __func__);
	if (!obj) {
		pr_info("Alloc fusion object error!\n");
		return NULL;
	}
...
static int fusion_enable_and_batch(int index)
{
	...
	if (cxt->fusion_context[index].power == 1 &&
		...
		if (err) {
			pr_info("fusion turn off power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->fusion_context[index].power == 0 &&
		...
		if (err) {
			pr_info("fusion turn on power err = %d\n", err);
			return -1;
		}
	...
	if (cxt->fusion_context[index].power == 1 &&
		...
		if (cxt->fusion_context[index].fusion_ctl.is_support_batch)
			...
			err = cxt->fusion_context[index].fusion_ctl.batch(0,
				cxt->fusion_context[index].delay_ns, 0);
		if (err) {
			pr_info("fusion set batch(ODR) err %d\n", err);
			return -1;
		}
	}
	return 0;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cxt->fusion_context[index].fusion_ctl.flush != NULL)
		...
		pr_info("FUSION OLD ARCH NOT SUPPORT COMMON VRS FLUSH\n");
	if (err < 0)
		pr_info("fusion enable flush err %d\n", err);
	mutex_unlock(&fusion_context_obj->fusion_op_mutex);
	if (err)
		return err;
	else
...
static int fusion_misc_init(struct fusion_context *cxt)
{
	...
	if (err)
		pr_info("unable to register fusion misc device!!\n");
	...
	int handle)
{
	...
	if (NULL == ctl || NULL == ctl->set_delay
		...
		pr_info("fusion handle:%d register control path fail\n",
			handle);
		return -1;
	}
...
static int fusion_probe(void)
{
	...
	if (err) {
		pr_info("fusion real driver init fail\n");
	...
	if (err < 0) {
		pr_info("unable to create fusion attribute file\n");
		goto real_driver_init_fail;
	}
...
int fusion_driver_add(struct fusion_init_info *obj, int handle)
{
	...
	pr_debug("handle:%d\n", handle);
	if (!obj) {
		pr_info("FUSION handle: %d, driver add fail\n", handle);
		return -1;
	}
	...
	if (fusion_init_list[index] == NULL)
		...
		pr_info("fusion_init_list handle:%d already exist\n",
			handle);
	return err;
}
static int __init fusion_init(void)
{
	...
	if (fusion_probe()) {
		pr_info("failed to register fusion driver\n");
		return -ENODEV;
	}

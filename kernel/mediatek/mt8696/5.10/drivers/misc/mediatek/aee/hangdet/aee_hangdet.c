...
enum TIMEOUT_TYPE {
	TIMEOUT_TYPE_CPU_HANG = 1,
	TIMEOUT_TYPE_CNT_LACK
};
...
static void kwdt_dump_func(void)
{
	...
	if (toprgu_base) {
		tmp = ioread32(toprgu_base + WDT_MODE);
		if (!(tmp & WDT_MODE_EN))
			iowrite32((tmp | WDT_MODE_KEY | WDT_MODE_EN), toprgu_base + WDT_MODE);
...
static void aee_dump_timer_func(struct timer_list *t)
{
	...
	if (sched_clock() - aee_dump_timer_t < CHG_TMO_DLY_SEC * 1000000000LL) {
		g_change_tmo = 0;
		aee_dump_timer_t = 0;
		g_hang_detected = 0;
		spin_unlock(&lock);
	...
	if ((sched_clock() > all_k_timer_t) && (sched_clock() - all_k_timer_t)
		< (CHG_TMO_DLY_SEC + 1) * 1000000000LL) {
		g_change_tmo = 0;
		aee_dump_timer_t = 0;
		g_hang_detected = 0;
		spin_unlock(&lock);
		return;
	} else if ((all_k_timer_t > sched_clock()) && (ULLONG_MAX - all_k_timer_t + sched_clock())
		< (CHG_TMO_DLY_SEC + 1) * 1000000000LL) {
		g_change_tmo = 0;
		aee_dump_timer_t = 0;
		g_hang_detected = 0;
				...
				unsigned int original_kicker)
{
	...
	if (aee_dump_timer_t && ((sched_clock() - aee_dump_timer_t)
		> (CHG_TMO_DLY_SEC + 5) * 1000000000LL)) {
		if (!aee_dump_timer_c) {
			aee_dump_timer_c = 1;
	...
	/* aee_rr_rec_wdk_kick_jiffies(jiffies); */
			...
			> curInterval * 1000)) {
		g_hang_detected = 1;
		dump_timeout = TIMEOUT_TYPE_CPU_HANG;
	}

	if ((g_hang_detected == 0)
		&& (r_counter < DEFAULT_INTERVAL - 10) && !g_change_tmo) {
		g_hang_detected = 1;
		dump_timeout = TIMEOUT_TYPE_CNT_LACK;
	}

	wk_tsk_kick_time[cpu] = sched_clock();

	do_div(lastsuspend_t, 1000000);
	do_div(lastsuspend_syst, 1000000);
	do_div(lastresume_t, 1000000);
	do_div(lastresume_syst, 1000000);

	snprintf(msg_buf, WK_MAX_MSG_SIZE,
	 "[wdk-c] cpu=%d o_k=%u lbit=0x%x cbit=0x%x,%x,%d,%d,%lld,%x,%lld,%lld,%lld,%lld,[%lld,%ld] %d\n",
	 cpu,
	 original_kicker,
	 local_bit,
	 get_check_bit(),
	...
	if (dump_timeout) {
		...
		if (dump_timeout == TIMEOUT_TYPE_CNT_LACK) {
			pm_system_wakeup();

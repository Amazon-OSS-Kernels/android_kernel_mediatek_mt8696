...
static int mboot_params_check_header(struct mboot_params_buffer *buffer)
{
	if (!buffer || (buffer->sz_buffer != mboot_params_buffer->sz_buffer)
		|| !buffer->off_pl || buffer->off_pl > buffer->sz_buffer
...
static unsigned int mboot_params_addr;
static unsigned int mboot_params_size;
static void mboot_params_set_addr_size(unsigned int addr, unsigned int size)
{
	mboot_params_addr = addr;
	mboot_params_size = size;
}
...
static int __init mboot_params_early_init(void)
{
	...
	if (dt_get_mboot_params(&sram)) {
		...
		start = mboot_params_addr;
		size = mboot_params_size;
		if (start < 0x40000000) {
			pr_info("mboot_params: using sram:0x%x\n", start);
			bufp = (struct mboot_params_buffer *)ioremap_wc(start, size);
		} else {
			pr_info("mboot_params: using dram:0x%x\n", start);
			bufp = (struct mboot_params_buffer *)remap_lowmem(start, size);
		}
		mboot_params_buffer_pa =
			(struct mboot_params_buffer *)(unsigned long)start;
		if (bufp) {
...
int mboot_params_reserve_memory(struct reserved_mem *rmem)
{
	pr_info("[memblock]%s: 0x%llx - 0x%llx (0x%llx)\n",
		"mediatek,mboot_params",
		 ...
		 (unsigned long long)rmem->size);
	mboot_params_set_addr_size(rmem->base, rmem->size);

	return 0;
}

RESERVEDMEM_OF_DECLARE(reserve_memory_mboot_params, "mediatek,mboot_params",
	mboot_params_reserve_memory);

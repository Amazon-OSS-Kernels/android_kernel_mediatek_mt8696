...
#define PSCI_1_1_FN_SYSTEM_RESET	0x84000009
...
static unsigned int mrdump_cb_addr;
static unsigned int mrdump_cb_size;
static void mrdump_cblock_set_addr_size(unsigned int addr, unsigned int size)
{
	mrdump_cb_addr = addr;
	mrdump_cb_size = size;
}
...
#endif
	if (mparams.cb_addr == 0 && mparams.cb_size == 0) {
		mparams.cb_addr = mrdump_cb_addr;
		mparams.cb_size = mrdump_cb_size;
	}
...
#endif
	pr_notice("ipanic: startup\n");
	return 0;
}

arch_initcall(mrdump_panic_init);

int mrdump_cblock_reserve_memory(struct reserved_mem *rmem)
{
	pr_info("[memblock]%s: 0x%llx - 0x%llx (0x%llx)\n",
		"mediatek,mrdump_cb",
		 ...
		 (unsigned long long)rmem->size,
		 (unsigned long long)rmem->size);
	mrdump_cblock_set_addr_size(rmem->base, rmem->size);

	return 0;
}

RESERVEDMEM_OF_DECLARE(reserve_memory_mrdump_cblock, "mediatek,mrdump_cb",
	mrdump_cblock_reserve_memory);

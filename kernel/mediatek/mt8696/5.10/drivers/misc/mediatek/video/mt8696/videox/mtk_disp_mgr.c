...
struct mutex set_input_buffer_comap_ioctl_lock;
static struct compat_mtk_disp_config *p_global_compat_config;
static struct mtk_disp_config *p_global_config;
...
static int mtk_disp_mgr_get_layer_info(uint32_t id,
				       struct mtk_disp_layer_info *info)
{
	int ret = 0;
...
static int mtk_disp_mgr_set_input_buffer(struct mtk_disp_config *config)
{
	...
	if (is_DAL_Enabled() && config->user == DISP_USER_HWC) {
		...
		if (ui_num <= 1)
			DAL_Get_Layer_Info(
				&(config->buffer_info[DISP_BUFFER_MAX - 1]));
		else
			DAL_Get_Layer_Info(
				&(config->buffer_info[top_osd_layer_id]));
	}
#endif
	set_config_cnt++;
...
static long mtk_disp_mgr_ioctl(struct file *file, unsigned int cmd,
			       unsigned long arg)
{
	...
	case MTK_DISP_IOCTL_SET_INPUT_BUFFER:
		...
		if (copy_from_user(pConfig, argp,
				   sizeof(struct mtk_disp_config))) {
			if (pConfig != NULL) {
				vfree(pConfig);
				pConfig = NULL;
			}
		...
		if (!ret &&
		    copy_to_user(argp, pConfig, sizeof(struct mtk_disp_config)))
			ret = -EFAULT;
	...
	case MTK_DISP_IOCTL_GET_LAYER_INFO:
		...
		ret = mtk_disp_mgr_get_layer_info(layer_info.display_id,
						  &layer_info);
...
struct compat_mtk_disp_film_grain_md_t {
	uint64_t pts;
	uint32_t len;
	bool svp;
	uint32_t sec_handle;
...
struct compat_mtk_disp_buffer {
	...
	struct compat_mtk_disp_film_grain_md_t film_grain_info;
...
#define COMPAT_MTK_DISP_IOCTL_SET_INPUT_BUFFER                                 \
	MTK_DISP_IOWR(0xd2, struct compat_mtk_disp_config)

void assignment(struct compat_mtk_disp_config *compat_config,
		struct mtk_disp_config *config)
{
	int i = 0;

	for (i = 0; i < config->buffer_num; i++) {
		compat_config->buffer_info[i].release_fence_fd =
			config->buffer_info[i].release_fence_fd;
		compat_config->buffer_info[i].present_fence_fd =
			config->buffer_info[i].present_fence_fd;
	}
}

void compat_assignment(struct mtk_disp_config *config,
		       struct compat_mtk_disp_config *compat_config)
{
	...
	for (i = 0; i < compat_config->buffer_num; i++) {
		...
		config->buffer_info[i].dovi_info.pts =
			compat_config->buffer_info[i].dovi_info.pts;
		config->buffer_info[i].dovi_info.len =
			compat_config->buffer_info[i].dovi_info.len;
		config->buffer_info[i].dovi_info.svp =
			compat_config->buffer_info[i].dovi_info.svp;
		config->buffer_info[i].dovi_info.sec_handle =
			compat_config->buffer_info[i].dovi_info.sec_handle;
		config->buffer_info[i].dovi_info.addr = (void *)((
			long)(compat_config->buffer_info[i].dovi_info.addr &
			      0xffffffff));
		config->buffer_info[i].dovi_info.fd =
			compat_config->buffer_info[i].dovi_info.fd;
		config->buffer_info[i].dovi_info.offset =
			compat_config->buffer_info[i].dovi_info.offset;

		config->buffer_info[i].dovi_info.keyfrm_offset =
			compat_config->buffer_info[i].dovi_info.keyfrm_offset;
		config->buffer_info[i].dovi_info.keyfrm_len =
			compat_config->buffer_info[i].dovi_info.keyfrm_len;
		config->buffer_info[i].film_grain_info.pts =
			compat_config->buffer_info[i].film_grain_info.pts;
		config->buffer_info[i].film_grain_info.len =
			compat_config->buffer_info[i].film_grain_info.len;
		config->buffer_info[i].film_grain_info.svp =
			compat_config->buffer_info[i].film_grain_info.svp;
		config->buffer_info[i].film_grain_info.sec_handle =
			compat_config->buffer_info[i].film_grain_info.sec_handle;

		config->buffer_info[i].film_grain_info.addr = (void *)((
			long)(compat_config->buffer_info[i].film_grain_info.addr &
			      0xffffffff));
		config->buffer_info[i].film_grain_info.fd =
			compat_config->buffer_info[i].film_grain_info.fd;
		config->buffer_info[i].film_grain_info.offset =
			compat_config->buffer_info[i].film_grain_info.offset;

		config->buffer_info[i].meta_data = (void *)((
			long)(compat_config->buffer_info[i].meta_data &
			      0xffffffff));
		config->buffer_info[i].src_base_addr = (void *)((
			long)(compat_config->buffer_info[i].src_base_addr &
			      0xffffffff));
		config->buffer_info[i].src_phy_addr = (void *)((
			long)(compat_config->buffer_info[i].src_phy_addr &
			      0xffffffff));
		config->buffer_info[i].res_mode =
			compat_config->buffer_info[i].res_mode;
		config->buffer_info[i].allm_en =
			compat_config->buffer_info[i].allm_en;
		config->buffer_info[i].qms_en =
			compat_config->buffer_info[i].qms_en;
		config->buffer_info[i].stream_fps =
			compat_config->buffer_info[i].stream_fps;

	}
}

int compat_mtk_disp_mgr_set_input_buffer(
	struct compat_mtk_disp_config *compat_config)
{
	int ret = 0;
	if (p_global_config == NULL) {
		pr_info("%s, p_global_config is null\n", __func__);
		return -EFAULT;
	}
	memset(p_global_config, 0, sizeof(struct mtk_disp_config));
	compat_assignment(p_global_config, compat_config);

	ret = mtk_disp_mgr_set_input_buffer(p_global_config);
	if (ret) {
		pr_info("%s, set input buffer failed\n", __func__);
		return -EFAULT;
	}
	assignment(compat_config, p_global_config);
				      ...
				      unsigned long arg)
{
	if (cmd == COMPAT_MTK_DISP_IOCTL_SET_INPUT_BUFFER) {
		...
		mutex_lock(&set_input_buffer_comap_ioctl_lock);
		if (p_global_compat_config == NULL) {
			pr_info("%s, p_global_compat_config is null\n", __func__);
			mutex_unlock(&set_input_buffer_comap_ioctl_lock);
			return -EFAULT;
		}
		memset(p_global_compat_config, 0, sizeof(struct compat_mtk_disp_config));
		if (copy_from_user(p_global_compat_config, argp,
				   sizeof(struct compat_mtk_disp_config))) {
			pr_info("%s, copy from user failed\n", __func__);
			mutex_unlock(&set_input_buffer_comap_ioctl_lock);
			return -EFAULT;
		}
		if (p_global_compat_config->buffer_num > DISP_BUFFER_MAX) {
			pr_info("%s, buffer num is invalid\n", __func__);
			mutex_unlock(&set_input_buffer_comap_ioctl_lock);
			return -EFAULT;
		}
		ret = compat_mtk_disp_mgr_set_input_buffer(p_global_compat_config);
		if (ret) {
			pr_info("%s failed\n", __func__);
			mutex_unlock(&set_input_buffer_comap_ioctl_lock);
			return -EFAULT;
		...
		ret = copy_to_user(argp, p_global_compat_config,
				   sizeof(struct compat_mtk_disp_config));
		mutex_unlock(&set_input_buffer_comap_ioctl_lock);
		return ret;
	}
...
static ssize_t mtk_disp_mgr_dump(struct device *dev,
...
static ssize_t mtk_disp_mgr_ut(struct device *dev,
...
static int mtk_disp_mgr_probe(struct platform_device *pdev)
{
	...
	ret = alloc_chrdev_region(&disp_devno, 0, 1, DISP_SESSION_DEVICE);
	if (ret < 0) {
		pr_info("%s, failed to dynamically assign device number\n", __func__);
		return ret;
	}
	...
	if (ret < 0) {
		pr_info("%s, failed to create device\n", __func__);
		goto err_create_file;
	}

	mutex_init(&set_input_buffer_ioctl_lock);
	mutex_init(&set_input_buffer_comap_ioctl_lock);

	p_global_compat_config = vmalloc(sizeof(struct compat_mtk_disp_config));
	if (p_global_compat_config == NULL)
		goto err_create_file;
	memset(p_global_compat_config, 0, sizeof(struct compat_mtk_disp_config));

	p_global_config = vmalloc(sizeof(struct mtk_disp_config));
	if (p_global_config == NULL)
		goto err_create_file;
	memset(p_global_config, 0, sizeof(struct mtk_disp_config));
...
static int mtk_disp_mgr_remove(struct platform_device *pdev)
{
	...
	class_destroy(disp_class);
	if (p_global_compat_config != NULL) {
		vfree(p_global_compat_config);
		p_global_compat_config = NULL;
	}
	if (p_global_config != NULL) {
		vfree(p_global_config);
		p_global_config = NULL;
	}
	return ret;
}
...
static const struct of_device_id mgr_of_ids[] = {
	{
		.compatible = "mediatek,mt8696-dispmgr",
	},
	{} };

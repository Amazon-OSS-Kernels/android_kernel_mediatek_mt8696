...
enum FMT_STATUS { FMT_STA_UNUSED, FMT_STA_USED };

enum DISP_FMT { DISP_FMT1 = 0, DISP_FMT2 = 1, DISP_FMT_CNT = 2 };
...
struct disp_fmt_t {
	...
	spinlock_t fmt_shadow_lock;
	unsigned long fmt_enable_start_time;
	unsigned long fmt_enable_end_time;
	unsigned long fmt_reset_start_time;
	unsigned long fmt_reset_end_time;
	unsigned long fmt_shadow_enable_start_time;
	unsigned long fmt_shadow_enable_end_time;
};
...
struct fmt_context {
	...
	uintptr_t disp_fmt_phy_base[DISP_FMT_CNT]; /*physical base for cmdq*/
#endif
...
#define FMT_LOG_E(format...) pr_info("[FMT] " format)

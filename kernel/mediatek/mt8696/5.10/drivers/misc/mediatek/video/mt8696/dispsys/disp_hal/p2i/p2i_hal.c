...
uint32_t p2i_480i_time[] = {
	/* 0x17 -> 0x17+ 858(H total)  */
	/*jitao 0x17  -> 857(H total -1) */
	0x00180359,
	0x00180359,
...
int p2i_hal_init(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8696-p2i");
	if (np == NULL) {
...
void p2i_hal_set_time(enum HDMI_VIDEO_RESOLUTION res)
{
	uint32_t index;

	switch (res) {
	case HDMI_VIDEO_1920x1080i_60Hz:
		for (index = 0; index < (sizeof(p2i_1080i60_time) / 4);
			index += 1)
			vWriteP2I(P2I_H_TIME0 + index * 4,
			p2i_1080i60_time[index]);
		vWriteP2IMsk(P2I_CTRL_2, 0x02, LINE_SHIFT_MSK);
		break;

	case HDMI_VIDEO_1920x1080i_50Hz:
		for (index = 0; index < (sizeof(p2i_1080i50_time) / 4);
			index += 1)
			vWriteP2I(P2I_H_TIME0 + index * 4,
			p2i_1080i50_time[index]);
		vWriteP2IMsk(P2I_CTRL_2, 0x02, LINE_SHIFT_MSK);
		break;

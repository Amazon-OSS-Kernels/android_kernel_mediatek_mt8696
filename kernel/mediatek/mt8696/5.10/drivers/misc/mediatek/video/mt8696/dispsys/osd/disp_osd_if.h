...
#define OSD_MAX_NUM_RGN_LIST 3
#endif
extern bool enable_osd_colorbar;
extern bool region_replace_bmp;
...
enum OSD_DUMP_INFO_LEVEL {
	LEVEL_BASIC_INFO = 1,
	LEVEL_ENABLE_COLORBAR,
	LEVEL_DISABLE_COLORBAR,
	LEVEL_ENABLE_REPLACE_BMP,
	LEVEL_DISABLE_REPLACE_BMP
};
...
#define UPDATE_PKT_COUNT 10

struct cmdq_update_pkt {
	struct cmdq_pkt *pkt;
	bool state;
...
struct disp_osd_layer_config {
	...
	unsigned int src_buffer_width;
	unsigned int src_buffer_height;
	...
	bool osd_swap;
	bool is_pvric;
	struct tee_mmu *osd_buf_mmu;
};

struct disp_osd_input_config {
	int fence_fd;
	struct disp_osd_layer_config config;
	/*
	 *the following info is for sdr2hdr,
	 *saving osd buffer info and tv info at osd config,
	 *and used them at osd config thread.
	 */
	struct mtk_disp_buffer osd_disp_buffer;
	struct disp_hw_common_info common_info;
};

struct Osd_buffer_list {
	enum  List_Buffer_STATE list_buf_state;
...
struct osd_resolution_change {
	...
	enum HDMI_VIDEO_RESOLUTION cur_res_mode;
...
struct osd_reg_pararm {
	...
	phys_addr_t osd_reg_base_pa[OSD_PLANE_MAX_NUM];
	phys_addr_t osd_fmt_reg_base_pa[OSD_FMT_MAX_NUM];
	phys_addr_t osd_pln_reg_base_pa[OSD_PLANE_MAX_NUM];
	phys_addr_t osd_scl_reg_base_pa[OSD_SCALER_MAX_NUM];
};
...
struct osd_context_t {
	...
	struct mutex cmdq_client_handle_lock;
	...
	struct Osd_buffer_list *osd_current_frame[OSD_PLANE_MAX_WINDOW];
	...
	bool aee_enable;
	struct workqueue_struct *reclaim_cmdq_pkt_workq;
	bool osd_swap;
};
...
extern BOOL fg_enable_faker_hdr;
int disp_osd_update_register(uint32_t plane, int index);
void disp_osd_trigger_hw(uint32_t plane);
extern void osd_dump(void);
extern int test_osd_buffer(struct disp_osd_input_config *buffer_info);
extern void osd_engine_clk_enable(
	enum HDMI_VIDEO_RESOLUTION res_mode,
	bool enable);
extern void osd_clk_enable(
	enum HDMI_VIDEO_RESOLUTION res_mode,
	bool enable,
	unsigned int layer_id);

...
#define OSD_TRACE_BUFFER_IDLE                                                  \
	(OSD_TRACE_BUFFER_SIZE -                                               \
...
bool enable_osd_colorbar;
bool region_replace_bmp;
...
static char *STR_HELP =
	...
	"       echo [ACTION]>/d/osd\n"
	"ACTION:\n"
	"		regr:addr                :regr:0x14003000\n"
	"		regw:addr,value          :regw:0x14003000,0x1\n"
	"		dump_reg:addr,length     :dump_reg:0x14003000,0x300\n";
...
void set_osd_log_level(unsigned int level, unsigned int on)
{
...
void set_osd_debug_irq_log_level(unsigned int level, unsigned int on)
{
	OSDMSG("set %s  %d  on %d\n", __func__, level, on);
	if (on) {
...
{
	return osd_dbg_level; //8696 review
}
...
int disp_osd_dump_plane(unsigned int plane)
{
	...
	union OSD_RGN_UNION_T region;
	struct file *filp;
	int i4WriteSize = 0;
	int ret = 0;
	struct BITMAPINFOHEADER t_bmpinfo;
	char *p_temp = NULL;

	_OSD_PLA_GetHeaderAddr(plane, &p_value);

	memcpy((void *)(&region), (void *)&p_value,
		sizeof(union OSD_RGN_UNION_T));
	...
	if (IS_ERR(filp)) {
		...
		i4WriteSize =
			filp->f_op->write(filp, p_temp, size, &filp->f_pos);
...

	{
		...
		if (IS_ERR(filp)) {
			...
			i4WriteSize = filp->f_op->write(
				filp, (UINT8 *)&t_bmpinfo, 2, &filp->f_pos);
			if (i4WriteSize != 2) {
			...
			OSDMSG("[dump]Write BMP header done!! offset:0x%x\n",
			       (unsigned int)filp->f_pos);
			/*p_temp = VIRTUAL(p_bitmap);*/
			for (i4Idx = height - 1; i4Idx >= 0; i4Idx--) {
				i4WriteSize = filp->f_op->write(
					filp, (p_temp + i4Idx * pitch), pitch,
					&filp->f_pos);
				if (i4WriteSize != pitch) {
					OSDMSG
					("[DUMP] Write data fail index=%d\n",
					i4Idx);
					filp_close(filp, NULL);
					ret = -1;
				}
...
static struct OSD_TRACE_INFO osd_trace_info;
...
static int disp_osd_debug_thread(void *data)
{
	...
	while (osd_trace_info.fg_trace_en) {
		...
		if ((lock == 0) &&
			...
			mm_segment_t old_fs;
			/* get old fs, avoid USER/KERNEL file
			 * system conflict
			 */
			old_fs = get_fs();
			...
			use_trace_size = (osd_trace_info.write_pc -
					  osd_trace_info.tracebuf);

			i4WriteSize = osd_trace_info.ptrfile->f_op->write(
				osd_trace_info.ptrfile, osd_trace_info.tracebuf,
				use_trace_size,
				&(osd_trace_info.ptrfile->f_pos));
			osd_trace_info.write_pc = osd_trace_info.tracebuf;
			set_fs(old_fs);
		} else {
			OSDMSG("buffer->locked,write file in next loop\n");
		}
		msleep(30);
	}
...
int disp_osd_dump_byframe_init(unsigned int plane)
{
	...
	if (!osd_debug_task) {
		osd_debug_task = kthread_create(disp_osd_debug_thread, NULL,
						"disp_osd_debug_thread");
...
int osd_trace_write(void)
{
	...
	if (IS_ERR(filp)) {
		...
		i4WriteSize = filp->f_op->write(filp, (posd_reg_base), size,
						&filp->f_pos);
		if (i4WriteSize != size) {
		...
		posd_reg_base =
			(unsigned char *)
				osd.osd_reg.osd_pln_reg_base[OSD_PLANE_1];
		i4WriteSize = filp->f_op->write(filp, (posd_reg_base), size,
						&filp->f_pos);
		if (i4WriteSize != size) {
		...
		posd_reg_base =
			(unsigned char *)
				osd.osd_reg.osd_scl_reg_base[OSD_PLANE_1];
		i4WriteSize = filp->f_op->write(filp, (posd_reg_base), 0x30,
						&filp->f_pos);
		if (i4WriteSize != size) {
			OSDMSG("Write header fail!!\n");
			filp_close(filp, NULL);
		...
		posd_reg_base =
			(unsigned char *)
				osd.osd_reg.osd_pln_reg_base[OSD_PLANE_2];
		i4WriteSize = filp->f_op->write(filp, (posd_reg_base), size,
						&filp->f_pos);
		if (i4WriteSize != size) {
			OSDMSG("Write header fail!!\n");
		...
		posd_reg_base =
			(unsigned char *)
				osd.osd_reg.osd_scl_reg_base[OSD_PLANE_2];
		i4WriteSize = filp->f_op->write(filp, (posd_reg_base), 0x30,
						&filp->f_pos);
		if (i4WriteSize != size) {
			OSDMSG("Write header fail!!\n");
...
static unsigned int osd_read_reg(uintptr_t addr)
{
...
static unsigned int osd_write_reg(uintptr_t addr, unsigned int value)
{
...
static UINT8 osd_range_reg(unsigned int addr,
			     unsigned int *len, uintptr_t *paddr)
{
	unsigned int offset = 0;

	if ((addr >= 0x14004000) && (addr < 0x14004000 + 0x300)) {
		offset = addr - 0x14004000;
		if (offset % 4) {
			OSDMSG("Address illegal access!\n");
			return false;
		}
		*paddr = (uintptr_t)osd.osd_reg.osd_fmt_reg_base[0] + offset;
		if (len && ((*len + addr) > (0x14004000 + 0x300)))
			*len = 0x300 - offset;
		return true;
	} else if ((addr >= 0x14003000) && (addr < 0x14003000 + 0x300)) {
		offset = addr - 0x14003000;
		if (offset % 4) {
			OSDMSG("Address illegal access!\n");
			return false;
		}
		*paddr = (uintptr_t)osd.osd_reg.osd_fmt_reg_base[1] + offset;
		if (len && ((*len + addr) > (0x14003000 + 0x300)))
			*len = 0x300 - offset;
		return true;
	}
...
static void disp_osd_process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "regr:", 5) == 0) {
		...
		if (osd_range_reg(addr, NULL, &v_addr)) {
			regVal = osd_read_reg(v_addr);
			OSDMSG("regr: 0x%x = 0x%08x\n", addr, regVal);
		} else {
			OSDMSG("out of address access range!\n");
		}
		...
		p = (char *)opt + 9;
		STR_CONVERT(&p, &addr, uint, goto Error);
		STR_CONVERT(&p, &length, uint, goto Error);
		if (osd_range_reg(addr, &length, &v_addr)) {
			for (i = 0; i < length; i += 4) {
				regVal = osd_read_reg(v_addr + i);
				OSDMSG("regr: 0x%x = 0x%08x\n",
					addr + i, regVal);
			}
		} else {
		...
		if (osd_range_reg(addr, NULL, &v_addr)) {
			osd_write_reg(v_addr, val);
			regVal = osd_read_reg(v_addr);
			OSDMSG("regw: 0x%x, 0x%08x = 0x%08x\n",
				addr, val, regVal);
		} else {
			OSDMSG("out of address access range!\n");
		}
		...
		p = (char *)opt + 10;
		ret = kstrtou32(p, 10, (unsigned int *)&value);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		p = (char *)opt + 8;
		ret = kstrtou32(p, 8, (unsigned int *)&value);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		p = (char *)opt + 7;
		ret = kstrtou32(p, 7, (unsigned int *)&value);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		p = (char *)opt + 4;
		ret = kstrtou32(p, 4, (unsigned int *)&plane);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		p = (char *)opt + 9;
		ret = kstrtou32(p, 9, (unsigned int *)&en);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		_OSD_BASE_GetOsdChkSum(OSD_PLANE_1, &OsdChkSum);
		_OSD_BASE_GetOsdScChkSum(OSD_PLANE_1, &OsdScChkSum);
		OSDMSG("OsdChkSum:%x,OsdScChkSum:%x\n", OsdChkSum, OsdScChkSum);

		_OSD_BASE_GetOsdChkSum(OSD_PLANE_2, &OsdChkSum);
		_OSD_BASE_GetOsdScChkSum(OSD_PLANE_2, &OsdScChkSum);
		OSDMSG("OsdChkSum:%x,OsdScChkSum:%x\n", OsdChkSum, OsdScChkSum);
	} else if (strncmp(opt, "dump:", 5) == 0) {
		unsigned int value = 0;

		p = (char *)opt + 5;
		ret = kstrtou32(p, 5, (unsigned int *)&value);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		p = (char *)opt + 11;
		ret = kstrtou32(p, 11, (unsigned int *)&plane);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		p = (char *)opt + 13;
		ret = kstrtou32(p, 13, (unsigned int *)&by_frame_en);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
		...
		p = (char *)opt + 4;
		ret = kstrtou32(p, 4, (unsigned int *)&idx);
		if (ret) {
			OSDMSG("%s: errno %d\n", __func__, ret);
			goto Error;
		}
...
static ssize_t disp_osd_debug_read(struct file *file, char __user *ubuf,
				   size_t count, loff_t *ppos)
{
	if (strlen(osd_dbg_buf))
		return simple_read_from_buffer(ubuf, count, ppos, osd_dbg_buf,
					       strlen(osd_dbg_buf));
	else
...
static ssize_t disp_osd_debug_write(struct file *file, const char __user *ubuf,
				    size_t count, loff_t *ppos)
{
...
void disp_osd_debug_init(void)
{
	if (!osd_debug_init) {
		osd_debug_init = 1;
		osd_debugfs =
			debugfs_create_file("osd", S_IFREG | 0444, NULL,
					    (void *)0, &osd_debug_fops);

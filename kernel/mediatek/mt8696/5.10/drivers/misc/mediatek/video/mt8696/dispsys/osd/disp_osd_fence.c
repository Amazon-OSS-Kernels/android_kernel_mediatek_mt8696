 ...
 *RECEIVED
 *     FROM MEDIATEK AND/OR ITS REPRESENTATIVES ARE PROVIDED TO BUYER ON AN
 *"AS-IS" BASIS
 *     ONLY. MEDIATEK EXPRESSLY DISCLAIMS ANY AND ALL WARRANTIES, EXPRESS OR
 *IMPLIED,
 *     INCLUDING BUT NOT LIMITED TO THE IMPLIED WARRANTIES OF MERCHANTABILITY,
 *FITNESS FOR
 *     A PARTICULAR PURPOSE OR NONINFRINGEMENT. NEITHER DOES MEDIATEK PROVIDE
 *ANY WARRANTY
 *     WHATSOEVER WITH RESPECT TO THE SOFTWARE OF ANY THIRD PARTY WHICH MAY BE
 *USED BY,
 *     INCORPORATED IN, OR SUPPLIED WITH THE MEDIATEK SOFTWARE, AND BUYER AGREES
 *TO LOOK
 *     ONLY TO SUCH THIRD PARTY FOR ANY WARRANTY CLAIM RELATING THERETO.
 *MEDIATEK SHALL ALSO
 ...
 *LIABILITY WITH
 *     RESPECT TO THE MEDIATEK SOFTWARE RELEASED HEREUNDER WILL BE, AT
 *MEDIATEK'S OPTION,
 *     TO REVISE OR REPLACE THE MEDIATEK SOFTWARE AT ISSUE, OR REFUND ANY
 *SOFTWARE LICENSE
 *     FEES OR SERVICE CHARGE PAID BY BUYER TO MEDIATEK FOR SUCH MEDIATEK
 *SOFTWARE AT ISSUE.
 *
 *     THE TRANSACTION CONTEMPLATED HEREUNDER SHALL BE CONSTRUED IN ACCORDANCE
 *WITH THE LAWS
 *     OF THE STATE OF CALIFORNIA, USA, EXCLUDING ITS CONFLICT OF LAWS
 *PRINCIPLES.
 ***********************************************************************/
...
#include "mtk_sync.h"
#include "sync_file.h"
//#include "sw_sync.h" 8696 review
//#include "sync.h" 8696 review
...
static atomic_t osd_pre_timeline_counter[MAX_OSD_INPUT_CONFIG] = {
	ATOMIC_INIT(0)};
static atomic_t osd_pre_fence_counter[MAX_OSD_INPUT_CONFIG] = {ATOMIC_INIT(0)};
static struct sync_timeline *osd_pre_timeline[MAX_OSD_INPUT_CONFIG];
...
static struct sync_timeline *osd_timeline[MAX_OSD_INPUT_CONFIG];
struct mutex osd_fence_mutex[MAX_OSD_INPUT_CONFIG];

static void osd_destroy_timeline(struct sync_timeline *obj)
{
	timeline_destroy(obj);
}

static unsigned int osd_get_fence_counter(unsigned int lay_id, int type)
{
	if (type == 0)
		return (unsigned int)atomic_add_return(
			FENCE_STEP_COUNTER, &osd_fence_counter[lay_id]);
	else
		return (unsigned int)atomic_add_return(
			FENCE_STEP_COUNTER, &osd_pre_fence_counter[lay_id]);
}
...
struct sync_timeline *osd_create_timeline(unsigned int lay_id)
{
	...
	OSD_PRINTF(OSD_FENCE_LOG, "%s come in!\n", __func__);
	ret = snprintf(name, sizeof(name),"%s", prefix);
	if (ret < 0) {
		OSD_LOG_E("snprintf return error:%d", ret);
		return NULL;
	} else if ((size_t)ret >= sizeof(name))
		OSD_LOG_E("size:%d over buf size:%zu",
			ret,
			sizeof(name));

	osd_timeline[lay_id] = timeline_create(name); // 8696 review
	...
	if (ret < 0) {
		...
		OSD_LOG_E("size:%d over buf size:%zu",
			ret,
			sizeof(name));
	osd_pre_timeline[lay_id] = timeline_create(name); //8696 review
...
#if 0
static int fence_create(struct sync_timeline *obj, struct fence_data *data)
{
 // 8696 review
	...
	//pt = sync_pt_create(obj, data->value); 8696 review
	if (pt == NULL) {
		err = -ENOMEM;
		goto err;
	}
	...
	//fence = sync_fence_create(data->name, pt); // 8696 review
	if (fence == NULL) {
		//sync_pt_free(pt); 8696 review
		err = -ENOMEM;
		goto err;
	}

	data->fence = fd;
	//sync_fence_install(fence, fd); 8696 review
	return 0;
		     ...
		     unsigned int *rvalue, unsigned int lay_id)
{
	...
	*rfence = MTK_OSD_NO_FENCE_FD;
	...
	ret = snprintf(data.name,
			sizeof(data.name),
			"%s-%d", prefix, data.value);
	if (ret < 0) {
		OSD_LOG_E("snprintf return error:%d", ret);
		return ret;
	} else if ((size_t)ret >= sizeof(data.name))
		OSD_LOG_E("size:%d over buf size:%zu",
			ret,
			sizeof(data.name));
...
void osd_release_all_fence(unsigned int lay_id, unsigned int fence_count,
			   unsigned int pre_fence_count)
{
	int inc = fence_count - atomic_read(&osd_timeline_counter[lay_id]);
	int pre_inc =
		fence_count - atomic_read(&osd_pre_timeline_counter[lay_id]);

	if (inc > 0) {
		OSD_PRINTF(
			OSD_FENCE_LOG,
			"rel all fence:lay_id=%d, inc=%d, fence1=%d, fence2=%d!\n",
			lay_id, inc, atomic_read(&osd_fence_counter[lay_id]),
			atomic_read(&osd_timeline_counter[lay_id]));

		if (osd_timeline[lay_id] != NULL) {
			/* timeline_inc(hdmi_timeline, inc); */
			timeline_inc(osd_timeline[lay_id], inc);
			  // 8696 review
		}
	...
	if (pre_inc > 0) {
		OSD_PRINTF(
			OSD_FENCE_LOG,
			"rel all pre-fence:lay_id=%d, inc=%d, fence1=%d, fence2=%d!\n",
			lay_id, pre_inc,
		...
		if (osd_pre_timeline[lay_id] != NULL) {
			/* timeline_inc(hdmi_timeline, inc); */
			timeline_inc(osd_pre_timeline[lay_id], pre_inc);
		}
...
{
	unsigned int fence_cnt = 0, timeline_cnt = 0;
	int inc = 0;
	...
	if (inc < 0 || inc > 3) {
		OSD_LOG_I(
			"fence error: inc=%d, fence_cnt=%d, timeline_cnt=%d!\n",
			inc, fence_cnt, timeline_cnt);
		inc = 0;
	}
	mutex_lock(&osd_fence_mutex[lay_id]);
 ...
 // 8696 review
	...
	if (inc > 3)
		OSD_PRINTF(OSD_FENCE_LOG,
			"signal fence warning: inc=%d, fence_cnt=%d, timeline_cnt=%d!\n",
			inc,
			fence_cnt,
			timeline_cnt);
	...
	if (osd_timeline[lay_id] != NULL) {
		...
		OSD_PRINTF(OSD_FENCE_LOG,
		"vsync_cnt=%ld, fence_inc=%d, fence_cnt=%d, timeline_cnt=%d!\n",
...
void osd_signal_pre_fence(unsigned int lay_id, unsigned int index)
{
	...
	if (inc < 0 || inc > 3)
		OSD_PRINTF(OSD_FENCE_LOG,
			"sign pre_fence warning: inc=%d, fence_cnt=%d, timeline_cnt=%d!\n",
			inc, fence_cnt, timeline_cnt);
	...
	if (osd_pre_timeline[lay_id] != NULL) {
		timeline_inc(osd_pre_timeline[lay_id], inc);
		/*OSDDBG("fence  : value=%d\n", osd_timeline[lay_id]->value);*/
		OSD_PRINTF(
			OSD_FENCE_LOG,
			"vsync_cnt=%ld, pre_fence_inc=%d, pre_fence_cnt=%d, pre_timeline_cnt=%d!\n",
			vsync_cnt, inc,
			atomic_read(&osd_pre_fence_counter[lay_id]),
			atomic_read(&osd_pre_timeline_counter[lay_id]));
	} else {

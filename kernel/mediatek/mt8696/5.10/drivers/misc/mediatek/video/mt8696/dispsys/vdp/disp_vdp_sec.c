...
static int _vdp_sec_create_mem_session(void)
{
	...
	do {
		if (vdp_mem_session == 0) {
			...
			pr_info("%s vdp_mem_session %u\n",
				__func__, vdp_mem_session);
		}
	} while (0);
		      ...
		      unsigned int normal_mva_size)
{
	...
	uint32_t size = sizeof(struct vdp_sec_init_info);
...
int disp_vdp_sec_deinit(unsigned int layer_id)
{
	...
	vdp_data = kmalloc(sizeof(struct vdp_hal_data_info), GFP_KERNEL);
	if (!vdp_data) {
		DISP_LOG_E("%s alloc vdp_data fail\n", __func__);
		return VDP_FAIL;
	}

	vdp_disp = kmalloc(sizeof(struct vdp_hal_disp_info), GFP_KERNEL);
	if (!vdp_disp) {
		DISP_LOG_E("%s alloc vdp_disp fail\n", __func__);
		kfree(vdp_data);
		return VDP_FAIL;
	}

	vdp_shadow = kmalloc(sizeof(struct vdo_sw_shadow), GFP_KERNEL);
	if (!vdp_shadow) {
		DISP_LOG_E("%s alloc vdp_shadow fail\n", __func__);
		kfree(vdp_data);
		kfree(vdp_disp);
		return VDP_FAIL;
	}

	vdp_shm_param1.buffer = vdp_data;
	...
	vdp_shm_param2.buffer = vdp_disp;
	...
	vdp_shm_param3.buffer = vdp_shadow;
			...
			uint32_t osd_enable)
{
	union MTEEC_PARAM vdp_param[4];
	int ret;
	struct tz_disp_hw_common_info *tz_disp_common_info;
	KREE_SHAREDMEM_HANDLE vdp_shm_handle1,
		vdp_shm_handle2, vdp_shm_handle3;
	struct KREE_SHAREDMEM_PARAM vdp_shm_param1,
		vdp_shm_param2, vdp_shm_param3;

	struct vdp_hal_config_info *config;
	struct dispfmt_setting *dispfmt;
	...
	config = kmalloc(sizeof(struct vdp_hal_config_info), GFP_KERNEL);
	if (!config) {
		DISP_LOG_E("%s alloc config fail\n", __func__);
		return VDP_FAIL;
	}
	memcpy(config, config_info, sizeof(struct vdp_hal_config_info));

	dispfmt = kmalloc(sizeof(struct dispfmt_setting), GFP_KERNEL);
	if (!dispfmt) {
		DISP_LOG_E("%s alloc init_info fail\n", __func__);
		kfree(config);
		return VDP_FAIL;
	}
	memcpy(dispfmt, dispfmt_info, sizeof(struct dispfmt_setting));

	tz_disp_common_info = kmalloc(sizeof(struct tz_disp_hw_common_info), GFP_KERNEL);
	if (!tz_disp_common_info) {
		DISP_LOG_E("%s alloc tz_disp_common_info fail\n", __func__);
		kfree(config);
		kfree(dispfmt);
		return VDP_FAIL;
	}
	...
	memcpy(&tz_disp_common_info->resolution, disp_common_info.resolution,
	       sizeof(struct disp_hw_resolution_vdp_use));
	memcpy(&tz_disp_common_info->tv, &disp_common_info.tv,
	       sizeof(struct disp_hw_tv_capbility_vdp_use));
	tz_disp_common_info->osd_enable = osd_enable;


	vdp_shm_param1.buffer = config;
	vdp_shm_param1.size = sizeof(struct vdp_hal_config_info);

	REC_VDP_LINE(config->vdp_id, VDP_SEC_IN_2);
	...
	REC_VDP_LINE(config->vdp_id, VDP_SEC_IN_3);
	...
	vdp_shm_param3.buffer = dispfmt;
	vdp_shm_param3.size = sizeof(struct dispfmt_setting);

	REC_VDP_LINE(config->vdp_id, VDP_SEC_IN_4);
	ret = KREE_RegisterSharedmem(vdp_mem_session,
		&vdp_shm_handle3, &vdp_shm_param3);
	if (ret != TZ_RESULT_SUCCESS) {
		DISP_LOG_E(
	...
	REC_VDP_LINE(config->vdp_id, VDP_SEC_IN_5);
	...
	REC_VDP_LINE(config->vdp_id, VDP_SEC_IN_6);
			 ...
			 uint32_t size)
{
	...
	ret = KREE_RegisterSharedmem(vdp_mem_session,
		&vdp_shm_handle1, &vdp_shm_param1);
	if (ret != TZ_RESULT_SUCCESS) {
		DISP_LOG_E("[VDP_CA] %s KREE_RegisterSharedmem1 fail\n", __func__);
		return VDP_FAIL;
	}

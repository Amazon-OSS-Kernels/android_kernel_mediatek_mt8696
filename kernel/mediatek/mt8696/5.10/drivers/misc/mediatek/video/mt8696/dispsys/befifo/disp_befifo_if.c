...
int disp_befifo_vrr_enable(struct vrr_mode *vrr_info)
{
	bool vrr_en;
	uint32_t vtotal_ext;

	vrr_en = vrr_info->vrr_en;
	/*befifo vtotal keep max if qms startline mode enable*/
	if (vrr_en)
	...
	disp_befifo_drv_set_vrr(vrr_en, vtotal_ext, 0);
...
#ifdef DISP_QMS_SUPPORT
	.vrr_enable = disp_befifo_vrr_enable,
#endif
	.get_info = NULL,

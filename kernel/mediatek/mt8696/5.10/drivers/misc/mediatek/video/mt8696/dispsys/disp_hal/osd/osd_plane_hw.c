...
/* software register */
union OSD_PLA_CORE_UNION_T _rOsdPlaneCoreReg[OSD_PLANE_MAX_NUM];
/* hardware register map */
static union OSD_PLA_CORE_UNION_T *_prHwOsdPlaneCoreReg[OSD_PLANE_MAX_NUM];
phys_addr_t _osd_plan_base_reg_pa[OSD_PLANE_MAX_NUM];
...
void _OSD_PLA_GET_REG_BASE(
	uintptr_t *osd_pln_base_reg,
	phys_addr_t *osd_pln_base_reg_pa)
{
	_prHwOsdPlaneCoreReg[OSD_PLANE_1] =
		(union OSD_PLA_CORE_UNION_T *)osd_pln_base_reg[0];
	_prHwOsdPlaneCoreReg[OSD_PLANE_2] =
		(union OSD_PLA_CORE_UNION_T *)osd_pln_base_reg[1];

	_osd_plan_base_reg_pa[OSD_PLANE_1] = osd_pln_base_reg_pa[0];
	_osd_plan_base_reg_pa[OSD_PLANE_2] = osd_pln_base_reg_pa[1];
...
INT32 _OSD_PLA_UpdateHwReg(UINT32 u4Plane)
{
	...
	for (u4Idx = 0; u4Idx < OSD_CORE_REG_NUM; u4Idx++) {
		_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[u4Idx] =
		    _rOsdPlaneCoreReg[u4Plane].au4Reg[u4Idx];
		if (u4Idx < 8)
			OSD_PRINTF(OSD_WRITE_HW_LOG, "%s u4Value =0x%x\n",
				   __func__,
				   _prHwOsdPlaneCoreReg[u4Plane]
				   ->au4Reg[u4Idx]);
	}
	_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[0x39]
		= _rOsdPlaneCoreReg[u4Plane].au4Reg[0x39];
#endif
	return (INT32)OSD_RET_OK;
}
...
INT32 osd_pla_non_shadow_update(UINT32 u4Plane)
{
	if ((fading_ratio[u4Plane].fg_fading_ratio_update ||
		...
		if (fading_ratio[u4Plane].header_addr ==
			...
			OSD_LOG_E(
				"fading ratio not match[%d] %x,%x(%x %x)\n",
				u4Plane,
				fading_ratio_shadow[u4Plane].fading_ratio_alpha,
				fading_ratio[u4Plane].fading_ratio_alpha,
				_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[0x1],
...
INT32 _OSD_PLA_Update(UINT32 u4Plane, struct cmdq_pkt *pkt)
{
	UINT32 u4Idx = 0;
	...
	for (u4Idx = 0; u4Idx < OSD_CORE_COMMON_REG_NUM; u4Idx++) {
		...
		cmdq_pkt_write_value_addr(pkt,
				_osd_plan_base_reg_pa[u4Plane]+u4Idx*4,
				_rOsdPlaneCoreReg[u4Plane].au4Reg[u4Idx],
				~0);
	#else
		_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[u4Idx] =
	...
	if (_prHwOsdPlaneCoreReg[u4Plane]->au4Reg[0x2] !=
		 ...
		 * rOsdPlaneCoreReg[u4Plane].au4Reg[0x2]);
		 */
		if (fading_ratio[u4Plane].fg_fading_ratio_update == true &&
		    fading_ratio_shadow[u4Plane].fg_fading_ratio_update ==
			    false) {
			fading_ratio_shadow[u4Plane].header_addr =
				_rOsdPlaneCoreReg[u4Plane].au4Reg[0x1];
			fading_ratio_shadow[u4Plane].fading_ratio_alpha =
				_rOsdPlaneCoreReg[u4Plane].au4Reg[0x2];
			fading_ratio_shadow[u4Plane].fg_fading_ratio_update =
				true;
		} else if (fading_ratio[u4Plane].fg_fading_ratio_update ==
			   false) {
			fading_ratio[u4Plane].header_addr =
				_rOsdPlaneCoreReg[u4Plane].au4Reg[0x1];
			fading_ratio[u4Plane].fading_ratio_alpha =
...
INT32 osd_plane_update_extend(UINT32 u4Plane, struct cmdq_pkt *pkt)
{
	UINT32 u4Idx = 0;
...
INT32 osd_plane_update_faker_header(UINT32 u4Plane, struct cmdq_pkt *pkt)
{
	UINT32 u4Idx = 0;
	...
	//for pvric set pvric header address and pitch
...
INT32 osd_plane_update_pvric(UINT32 u4Plane, struct cmdq_pkt *pkt)
{
	UINT32 u4Idx = 0;
	//for faker header
	for (u4Idx = 0x30; u4Idx < 0x32; u4Idx++) {
	#if OSD_SUPPORT_GCE
		cmdq_pkt_write_value_addr(pkt,
				_osd_plan_base_reg_pa[u4Plane]+u4Idx*4,
				_rOsdPlaneCoreReg[u4Plane].au4Reg[u4Idx],
	...
	UINT32 u4Idx)
{
	...
	#endif
	OSD_PRINTF(OSD_WRITE_HW_LOG, "PLA_WRITE(%d + 'h%x, 32'h%x)\n", u4Plane,
		   (u4Idx * 4), _rOsdPlaneCoreReg[u4Plane].au4Reg[u4Idx]);
	return (INT32)OSD_RET_OK;
}
...
inline INT32 _OSD_PLA_SetEnable(UINT32 u4Plane, UINT32 u4Value)
{
	...
	if (enable_osd_colorbar)
		_rOsdPlaneCoreReg[u4Plane].rField.fgPrngEn = u4Value;
	else
		_rOsdPlaneCoreReg[u4Plane].rField.fgPrngEn = false;
	return (INT32)OSD_RET_OK;
}
...
inline INT32 _OSD_PLA_SetPreMulti(UINT32 u4Plane, UINT32 u4Value)
{
	OSD_VERIFY_PLANE(u4Plane);
	_rOsdPlaneCoreReg[u4Plane].rField.fgPremulti = u4Value;
	return (INT32)OSD_RET_OK;
}

inline INT32 _OSD_PLA_GetPreMulti(UINT32 u4Plane, UINT32 *pu4Value)
{
	OSD_VERIFY_PLANE(u4Plane);
	if (pu4Value == NULL)
		return -(INT32)OSD_RET_INV_ARG;
	*pu4Value = _rOsdPlaneCoreReg[u4Plane].rField.fgPremulti;
	return (INT32)OSD_RET_OK;
}
...
{
	OSD_VERIFY_PLANE(u4Plane);
	_rOsdPlaneCoreReg[u4Plane].rField.u4HeaderAddr =
...
inline INT32 _OSD_PLA_GetHeaderAddr(UINT32 u4Plane, UINT32 *pu4Value)
{
	...
	if (pu4Value == NULL)
		return -(INT32)OSD_RET_INV_ARG;
	*pu4Value = (UINT32)VIRTUAL(
		_rOsdPlaneCoreReg[u4Plane].rField.u4HeaderAddr << 4);
	return (INT32)OSD_RET_OK;
}
...
inline INT32 _OSD_PLA_SetBurst8_Enable(UINT32 u4Plane, UINT32 u4Value)
{
	...
	_rOsdPlaneCoreReg[u4Plane].rField.mptr_lmt_select = u4Value;
	return (INT32)OSD_RET_OK;
}
...
int osd_pla_set_sync_threshold(UINT32 u4Plane, UINT32 u4Value)
{
	...
	_rOsdPlaneCoreReg[u4Plane].rField.u4SyncThres_H = 0;
	return (INT32)OSD_RET_OK;
}
...
int osd_pla_set_fbdc_header_start_addr(UINT32 u4Plane, UINT32 u4Value)
{
	_rOsdPlaneCoreReg[u4Plane].rField.u4FbdcHeaderStartAddr = u4Value;
	return (INT32)OSD_RET_OK;
}

int osd_pla_set_fbdc_header_src_pitch(UINT32 u4Plane, UINT32 u4Value)
{
	_rOsdPlaneCoreReg[u4Plane].rField.u4FbdcHeaderSrcPitch = u4Value;
	return (INT32)OSD_RET_OK;
}

int osd_pla_set_faker_header_register(UINT32 u4Plane,
	union OSD_RGN_UNION_T *prRgn)
{
	...
	for (i = 0; i < 4; i++)
		_rOsdPlaneCoreReg[u4Plane].au4Reg[52 + i] =
			prRgn->au4Reg[8 + i];
	return (INT32)OSD_RET_OK;
}

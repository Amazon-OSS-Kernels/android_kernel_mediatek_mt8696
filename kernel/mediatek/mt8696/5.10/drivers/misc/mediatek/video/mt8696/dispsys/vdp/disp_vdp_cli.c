...
static int _vdp_cli_write_normal(int argc, const char **argv)
{
	...
	registerVA = (uint32_t *)((char *)iomapVA +
		(registerPA - registerBase));
...
static int _vdp_cli_alloc_memory(int argc, const char **argv)
{
	...
	if (!allocated) {
		...
		pYlenAddr = dma_alloc_coherent(pDev, size/256, &ylenPA,
			GFP_KERNEL);
		pClenAddr = dma_alloc_coherent(pDev, size/256, &clenPA,
			GFP_KERNEL);
		allocated = true;
	}
...
static int _vdp_cli_debug_mva(int argc, const char **argv)
{
	int value = 0;
	...
	cli_setting.enable_mva_debug = (value != 0);
	pr_err_ratelimited("set mva enable:%d\n", cli_setting.enable_mva_debug);
	return 0;
}
...
static struct CLI_EXEC_T items[] = {
	{"timer",  NULL, _vdp_cli_set_timer, NULL,
		"VDP: test timer function", CLI_GUEST},
	{"read",  NULL, _vdp_cli_read_normal, NULL,
		"VDP: read normal register", CLI_GUEST},
	{"write",  NULL, _vdp_cli_write_normal, NULL,
		"VDP: write normal register", CLI_GUEST},
	{"use_vdo3",  NULL, _vdp_cli_use_vdo3, NULL,
		"VDP: use vdo3", CLI_GUEST},
	{"alloc",  NULL, _vdp_cli_alloc_memory, NULL,
		"VDP: alloc memory for debug", CLI_GUEST},
	{"force_mdp",  NULL, _vdp_cli_debug_force_mdp, NULL,
		"VDP: force use MDP", CLI_GUEST},
	{"mva_enable",  NULL, _vdp_cli_debug_mva, NULL,
		"VDP: map mva va", CLI_GUEST},
	{"show",  NULL, _vdp_cli_debug_set_disp_area, NULL,
		"VDP: set display area", CLI_GUEST},
	{"secure_loglevel",  NULL, _vdp_cli_set_secure_loglevel, NULL,
		"VDP: set secure loglevel", CLI_GUEST},
	{"debug",  NULL, _vdp_cli_debug, NULL,
		"VDP: debug", CLI_GUEST},

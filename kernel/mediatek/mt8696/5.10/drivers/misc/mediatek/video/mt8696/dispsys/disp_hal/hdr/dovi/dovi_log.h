...
#define dovi_reg(format, ...) do { \
	if (dovi_dbg_level & (DOVI_SETTING_LOG)) { \
		dovi_printf(""format, ##__VA_ARGS__); \
	} \
} while (0)

#define dovi_flow(format, ...) do { \
	if (dovi_dbg_level & (DOVI_FLOW)) { \
		dovi_printf(""format, ##__VA_ARGS__); \
	} \
} while (0)

...
{
	if (disp_log.disp_mmp_event[event].value == 0) {
		DISP_LOG_E("mmp event %d is not register\n", event);
		return;
	}

	DISP_LOG_MMP("%s %d %d\n", disp_log.disp_mmp_event[event].name,
		     (uint32_t)data1, (uint32_t)data2);

	switch (type) {
	case MMP_START:
		mmprofile_log_ex(disp_log.disp_mmp_event[event].value,
...
void *disp_mmp_map_mva_to_va(unsigned long mva, unsigned int size)
{
	...
	if (dev) {
		...
		if (domain == NULL) {
			DISP_LOG_D("domain is null, map mva to va failed\n");
			return va;
		}
		...
		for (i = 0; i < page_count; i++) {
			addr = iommu_iova_to_phys(
				domain, (dma_addr_t)(mva + i * PAGE_SIZE));
			pages[i] = pfn_to_page(addr >> PAGE_SHIFT);
		}

		va = vmap(pages, page_count, VM_MAP, PAGE_KERNEL);
		DISP_LOG_D(
			"ddp_map_mva_to_va: mva=0x%lx, pa=0x%lx, va=0x%p, size=%d\n",
		   ...
		   enum DISP_HW_COLOR_FORMAT fmt, uint32_t w, uint32_t h)
{
	...
	if (!raw) {
		...
		Bitmap.p_data =
			(void *)addr; /*disp_mmp_map_mva_to_va(addr, size);*/
		if (Bitmap.p_data) {
		...
		meta.size = size;
		if (meta.p_data) {
			mmprofile_log_meta(disp_log.disp_mmp_event[event].value,
...
{
	int i;
	struct mmp_disp_event *event = disp_log.disp_mmp_event;
	MMP_Event *parent = disp_log.parent;

	mmprofile_enable(1);

	for (i = 0; i < MMP_EVENTS_MAX; i++) {
		event = &(disp_log.disp_mmp_event[i]);
		if (event->level == MMP_LEVEL_0) {
			event->value = mmprofile_register_event(MMP_ROOT_EVENT,
								event->name);
			parent[event->level] = event->value;
			DISP_LOG_D("%s value=%d, name=%s\n",
				   __func__, event->value, event->name);
		} else {
			event->value = mmprofile_register_event(
				parent[event->level - 1], event->name);
			parent[event->level] = event->value;
			DISP_LOG_D("%s value=%d, name=%s\n",
				   __func__, event->value, event->name);
		}
	...
	mmprofile_enable_event_recursive(
		disp_log.disp_mmp_event[MMP_DISP_ROOT].value, 1);

...
static void *aee_va;
static dma_addr_t aee_mva;
static int _get_buffer(struct device *dev)
{
	int size = (1920 * 1080 * 2 + 4096);

	if (aee_mva == 0)
		aee_va = dma_alloc_coherent(dev, size, (dma_addr_t *)&aee_mva,
					    GFP_KERNEL);
...
enum DAL_STATUS DAL_Init(struct device *dev)
{
	...
	if (ret != MFC_STATUS_OK) {
		pr_debug(
			"DISP/DAL: Warning: call MFC_XXX function failed in %s(), line: %d, ret: %x\n",
...
enum DAL_STATUS DAL_Clean(void)
{
	...
	if (isAEEEnabled == 1) {
		...
		config.buffer_info[0].is_pvric = 0;
		config.buffer_info[0].buffer_source = DISP_BUFFER_MVA;
...
enum DAL_STATUS DAL_Printf(const char *fmt, ...)
{
	...
	if (isAEEEnabled == 0) {
		...
		config.buffer_info[0].src_phy_addr =
			(void *)((unsigned long)aee_mva);
		config.buffer_info[0].src_fmt = DISP_HW_COLOR_FORMAT_RGB565;
		config.buffer_info[0].type = DISP_LAYER_AEE;
		config.buffer_info[0].src.x = 0;
		config.buffer_info[0].src.y = 0;
...
enum DAL_STATUS DAL_Get_Layer_Info(struct mtk_disp_buffer *info)
{
	...
	info->layer_id = layer;
	info->layer_enable = 1;
	info->buffer_source = DISP_BUFFER_MVA;
	info->is_pvric = 0;

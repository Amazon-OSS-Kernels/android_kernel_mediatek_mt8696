...
static char VDP_STR_HELP[] =
	...
	"ACTION:\n"
	"       regr:addr                :regr:0x15001000\n"
	"       regw:addr,value          :regw:0x15001000,0x1\n"
	"       dump_reg:addr,length     :dump_reg:0x15001000,0x1000\n";
/* ---------------------------------------------------------------------------
 */
/* Command Processor */
/* ---------------------------------------------------------------------------
...
static unsigned int vdp_read_reg(uintptr_t addr)
{
...
static unsigned int vdp_write_reg(uintptr_t addr, unsigned int value)
{
	/*unsigned int reg_pa = addr;*/
	unsigned int reg_value = 0;
...
static UINT8 vdp_range_reg(unsigned int addr,
			     unsigned int *len, uintptr_t *paddr)
{
	unsigned int offset = 0;

	if ((addr >= 0x15001000) && (addr < 0x15001000 + 0x1000)) {
		offset = addr - 0x15001000;
		if (offset % 4) {
			DISP_LOG_E("Address illegal access!\n");
			return false;
		}
		*paddr = (uintptr_t)disp_vdo_reg_base[0] + offset;
		if (len && ((*len + addr) > (0x15001000 + 0x1000)))
			*len = 0x1000 - offset;
		return true;
	} else if ((addr >= 0x15008000) && (addr < 0x15008000 + 0x1000)) {
		offset = addr - 0x15008000;
		if (offset % 4) {
			DISP_LOG_E("Address illegal access!\n");
			return false;
		}
		*paddr = (uintptr_t)disp_vdo_reg_base[1] + offset;
		if (len && ((*len + addr) > (0x15008000 + 0x1000)))
			*len = 0x1000 - offset;
		return true;
	}
...
static void vdp_process_dbg_opt(const char *opt)
{
	...
	if (0) {
		...
		if (vdp_range_reg(addr, NULL, &v_addr)) {
			regVal = vdp_read_reg(v_addr);
			DISP_LOG_I("regr: 0x%x = 0x%08x\n", addr, regVal);
		} else {
			DISP_LOG_E("out of address access range!\n");
		}
		...
		if (vdp_range_reg(addr, &length, &v_addr)) {
			for (i = 0; i < length; i += 4) {
				regVal = vdp_read_reg(v_addr + i);
				DISP_LOG_I("regr: 0x%x = 0x%08x\n",
					addr + i, regVal);
			}
		} else {
	...
	else if (strncmp(opt, "qms:", 4) == 0) {
		char *p;
		uint32_t qms_en = 0;
		uint32_t mvrr_frac = 0;

		p = (char *)opt + 4;
		STR_CONVERT(&p, &set_src_fps, uint, goto Error);
		STR_CONVERT(&p, &qms_en, uint, goto Error);
		STR_CONVERT(&p, &mvrr_frac, uint, goto Error);

		set_qms_en = (bool)qms_en;
		vdp_en_mvrr_frac = (bool)mvrr_frac;
		DISP_LOG_I("vdp set qms fps %d qms %d %d\n",
		set_src_fps, set_qms_en, vdp_en_mvrr_frac);
	}
#endif
	else {
		DISP_LOG_E(

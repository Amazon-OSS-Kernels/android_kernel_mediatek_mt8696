...
void dovi_update_gfx_fe_setting(void)
{
	...
	for (regidx = 0; regidx < DOVI_BE_REG_NUM; regidx++) {
		if (dovi_out_format == DOVI_FORMAT_DOVI) {
			Dv_WriteREG(dovi_reg_base[DOVI_VDO_BE] +
				0x200 + (regidx*4),
				dovi_v26_1080p_sdr_ipt_be_reg[regidx]);
		} else if (dovi_out_format == DOVI_FORMAT_HDR10) {
			Dv_WriteREG(dovi_reg_base[DOVI_VDO_BE] +
				0x200 + (regidx*4),
				dovi_v26_1080p_sdr_hdr10_be_reg[regidx]);
		} else if (dovi_out_format == DOVI_FORMAT_HLG) {
			Dv_WriteREG(dovi_reg_base[DOVI_VDO_BE] +
				0x200 + (regidx*4),
				dovi_v26_1080p_sdr_hlg_be_reg[regidx]);
		} else if (dovi_out_format == DOVI_FORMAT_DOVI_LOW_LATENCY) {
			Dv_WriteREG(dovi_reg_base[DOVI_VDO_BE] +
				0x200 + (regidx*4),
				dovi_v26_1080p_sdr_ll_be_reg[regidx]);
		} else if (dovi_out_format == DOVI_FORMAT_SDR) {
			Dv_WriteREG(dovi_reg_base[DOVI_VDO_BE] +
				0x200 + (regidx*4),
				dovi_v26_1080p_sdr_sdr_be_reg[regidx]);
		} else if (dovi_out_format == DOVI_FORMAT_VSEM_DOVI) {
			Dv_WriteREG(dovi_reg_base[DOVI_VDO_BE] +
				0x200 + (regidx*4),
				dovi_v26_1080p_sdr_vsem_be_reg[regidx]);
		} else if (dovi_out_format == DOVI_FORMAT_VSEM_DOVI_LOW_LATENCY) {
			Dv_WriteREG(dovi_reg_base[DOVI_VDO_BE] +
				0x200 + (regidx*4),
...
int disp_dovi_gfx_set_defaut_setting(uint32_t id)
{
	...
	//change fe width by res
	/* close r2y */
	Dv_WriteREG(reg_base + SC5_REG_GFX_012C_GFX_DV_WP, 0x0);
	Dv_WriteREG(reg_base + SC5_REG_GFX_03D0_GFX_HDR_FE, height);
	Dv_WriteREG(reg_base + SC5_REG_GFX_03C4_GFX_HDR_FE, adl_mode);
...
int disp_dovi_force_gfx_vs10(void)
{
	...
	dovi_printf("%s %d %d\n", __func__, width, dovi_out_format);
...
void disp_dovi_set_osd_enable(bool enable)
{
	...
	if (dovi_sdk_test) {
		...
		if (fhd_scale_to_uhd) {
			Dv_WriteREGMsk(fhd_scl_reg_base, 0x94, 0xFF);
			Dv_WriteREGMsk(fhd_scl_reg_base + 0x4,
				0x07800438, 0xFFFFFFFF);
			Dv_WriteREGMsk(fhd_scl_reg_base + 0xc,
				0x780, 0xFFF);
			Dv_WriteREGMsk(fhd_scl_reg_base + 0x14,
				0x1FFD, 0xFFFF);
			Dv_WriteREGMsk(fhd_scl_reg_base + 0x1c,
				0x1FFC, 0xFFFF);
		} else {
			Dv_WriteREGMsk(fhd_scl_reg_base, 0x80, 0xFF);
			Dv_WriteREGMsk(fhd_scl_reg_base + 0x4,
				0x07800438, 0xFFFFFFFF);
			Dv_WriteREGMsk(fhd_scl_reg_base + 0xc,
				0x780, 0xFFF);

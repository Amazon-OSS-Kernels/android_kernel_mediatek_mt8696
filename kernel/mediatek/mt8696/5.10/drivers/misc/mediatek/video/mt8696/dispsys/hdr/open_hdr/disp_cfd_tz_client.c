...
static enum cfd_status cfd_sec_create_share_memory(void)
{
	...
	if (status != CFD_STATUS_OK)
		...
		cfd_info("create share memory ok size %lld 0x%p %d\n",
		sizeof(struct cfd_share_memory_info_t),
...
enum cfd_status cfd_parser_hdr10plus_vsif(uint32_t layer_id,
	struct mtk_vdp_dovi_md_t *dovi_md)
{
	...
	if (cfd_main_vid_info == NULL || dovi_md == NULL) {
		cfd_error("%s NULL\n", __func__);
		return CFD_STATUS_ERROR;
	}

	if (dovi_md->svp) {
		cfd_main_vid_info->svp = true;
		cfd_main_vid_info->sec_handle_type = 0;
		cfd_main_vid_info->sec_handle_in = dovi_md->sec_handle;
		cfd_main_vid_info->sec_handle_len = dovi_md->len;
		cfd_main_vid_info->sec_handle_offset =
			dovi_md->sec_handle_offset;
		cfd_output_info("cfd svp %d %d %d\n",
			cfd_main_vid_info->sec_handle_in,
			cfd_main_vid_info->sec_handle_len,
			cfd_main_vid_info->sec_handle_offset);
	} else {
		cfd_main_vid_info->svp = false;
		cfd_main_vid_info->sec_handle_type = 0;
		cfd_main_vid_info->hdr10p_len = dovi_md->len;
		if (cfd_main_vid_info->hdr10p_len > CFD_MD_MAX_LEN)
			cfd_main_vid_info->hdr10p_len = CFD_MD_MAX_LEN;
		memcpy((void *)(cfd_main_vid_info->hdr10p_md),
			(void *)(dovi_md->buff), cfd_main_vid_info->hdr10p_len);
		cfd_output_info("cfd local %d 0x%x 0x%x 0x%x 0x%x\n",
			cfd_main_vid_info->hdr10p_len,
			cfd_main_vid_info->hdr10p_md[0],
			cfd_main_vid_info->hdr10p_md[1],
			cfd_main_vid_info->hdr10p_md[2],
			cfd_main_vid_info->hdr10p_md[3]);
	}

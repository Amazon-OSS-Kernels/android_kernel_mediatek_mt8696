...
uint32_t disp_hdr_allm_ctl_path(void)
{
	...
	def_out_format = dovi_judge_out_format(tv_cap, disp_common_info.resolution);
...
void disp_hdr_handle_allm_change(void *data)
{
	...
	/* allm ui change only influence display output format on dolby tv
...
void disp_hdr_handle_allm_change_for_gfx(bool b_allm_gfx)
{
	...
	if (hdr_gfx_allm_en != old_hdr_gfx_allm_en) {
		...
		if (hdr_gfx_allm_en) {
			...
			if (b_allm_ctl_force_hdr || tv_cap->is_support_dovi_low_latency) {
				forcehdr = ui_force_hdr_type;
				b_allm_ctl_force_hdr = false;
			}
		}
	...
	struct mtk_vdp_dovi_md_t *dovi_md_info)
{
	 ...
	 */
	dovi_sec_find_rpu_buffer(layer_id, disp_dovi_info->sec_handle,
		disp_dovi_info->len, disp_dovi_info->offset);
...
bool disp_hdr_prejudge_dovi_output(bool is_dovi, bool is_hdr10p)
{
	...
	/* tv cap can support dovi signal,
	 * forcehdr or dovi src will output dovi signal
	 * if hdr10+ src on hdr10+tv, no dovi signal output
	 */

	hdr_info("prejudge qms %d %d %d %d %d %d %d\n", def_out_format,
	is_dovi, is_hdr10p, g_force_hdr, dovi_vs10_path_en, dovi_path_en,
	tv_cap->is_support_hdr10_plus);

	if (is_hdr10p && tv_cap->is_support_hdr10_plus)
		return false;

	if (((def_out_format == DOVI_FORMAT_DOVI) ||
		(def_out_format == DOVI_FORMAT_DOVI_LOW_LATENCY) ||
		(def_out_format == DOVI_FORMAT_VSEM_DOVI) ||
		(def_out_format == DOVI_FORMAT_VSEM_DOVI_LOW_LATENCY)) &&
		((((g_force_hdr == 2) && (dovi_vs10_path_en == 0) &&
		dovi_path_en) ||
		(dovi_vs10_path_en == 2)) || is_dovi))
		ret = true;

...
static UINT8 cfd_reg_area_0(unsigned int addr,
			     unsigned int *len, uintptr_t *paddr)
{
	unsigned int offset = 0;

	if ((addr >= 0x14009000) && (addr < 0x14009000 + 0x1000)) {
		offset = addr - 0x14009000;
		if (offset % 4) {
			cfd_error("Address illegal access!\n");
			return false;
		}
		*paddr = _disp_cfd_hw_base[2] + offset;
		if (len && ((*len + addr) > (0x14009000 + 0x1000)))
			*len = 0x1000 - offset;
		return true;
	} else if ((addr >= 0x1400A000) && (addr < 0x1400A000 + 0x1000)) {
		offset = addr - 0x1400A000;
		if (offset % 4) {
			cfd_error("Address illegal access!\n");
			return false;
		}
		*paddr = _disp_cfd_hw_base[3] + offset;
		if (len && ((*len + addr) > (0x1400A000 + 0x1000)))
			*len = 0x1000 - offset;
		return true;
	} else if ((addr >= 0x1400B000) && (addr < 0x1400B000 + 0x400)) {
		offset = addr - 0x1400B000;
		if (offset % 4) {
			cfd_error("Address illegal access!\n");
			return false;
		}
		*paddr = _disp_cfd_be_hw_base + offset;
		if (len && ((*len + addr) > (0x1400B000 + 0x400)))
			*len = 0x400 - offset;
		return true;
	}
...
static UINT8 cfd_reg_area_1(unsigned int addr,
			     unsigned int *len, uintptr_t *paddr)
{
	unsigned int offset = 0;

	if ((addr >= 0x15012000) && (addr < 0x15012000 + 0x1000)) {
		offset = addr - 0x15012000;
		if (offset % 4) {
			cfd_error("Address illegal access!\n");
			return false;
		}
		*paddr = _disp_cfd_hw_base[0] + offset;
		if (len && ((*len + addr) > (0x15012000 + 0x1000)))
			*len = 0x1000 - offset;
		return true;
	} else if ((addr >= 0x15015000) && (addr < 0x15015000 + 0x1000)) {
		offset = addr - 0x15015000;
		if (offset % 4) {
			cfd_error("Address illegal access!\n");
			return false;
		}
		*paddr = _disp_cfd_hw_base[1] + offset;
		if (len && ((*len + addr) > (0x15015000 + 0x1000)))
			*len = 0x1000 - offset;
		return true;
	} else if ((addr >= 0x15016000) && (addr < 0x15016000 + 0x1000)) {
		offset = addr - 0x15016000;
		if (offset % 4) {
			cfd_error("Address illegal access!\n");
			return false;
		}
		*paddr = _disp_cfd_xvycc_hw_base + offset;
		if (len && ((*len + addr) > (0x15016000 + 0x1000)))
			*len = 0x1000 - offset;
		return true;
	}
...
static UINT8 cfd_range_reg(unsigned int addr,
			     unsigned int *len, uintptr_t *paddr)
{
	if ((addr >= 0x14009000) && (addr < 0x1400B000 + 0x400))
		return cfd_reg_area_0(addr, len, paddr);
	else if ((addr >= 0x15012000) && (addr < 0x15016000 + 0x1000))
		return cfd_reg_area_1(addr, len, paddr);
...
static bool cfd_reg_dbg_opt(const char *opt)
{
	if (strncmp(opt, "regr:", 5) == 0) {
		...
		if (cfd_range_reg(addr, NULL, &v_addr)) {
			regVal = cfd_read_reg(v_addr);
			cfd_default("regr: 0x%x = 0x%08x\n", addr, regVal);
		} else {
			cfd_error("out of address access range!\n");
		}
		return true;
		...
		if (cfd_range_reg(addr, &length, &v_addr)) {
			for (i = 0; i < length; i += 4) {
				regVal = cfd_read_reg(v_addr + i);
				cfd_default("regr: 0x%x = 0x%08x\n",
					addr + i, regVal);
			}
		} else {
...
static void cfd_process_dbg_opt(const char *opt)
{
	...
	} else if (strncmp(opt, "cfd_sof:", 8) == 0) {
		char *p = (char *)opt + 8;

		STR_CVT_U32(&p, &cfd_sof_start, goto Error);
		STR_CVT_U32(&p, &cfd_sof_end, goto Error);
		cfd_default("set cfd_sof_start %x %x\n",
			cfd_sof_start, cfd_sof_end);

	} else {
		if (!cfd_reg_dbg_opt(opt)) {
			cfd_error(

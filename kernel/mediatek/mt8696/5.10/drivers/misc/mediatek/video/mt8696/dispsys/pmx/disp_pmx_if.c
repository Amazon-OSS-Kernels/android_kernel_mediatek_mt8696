/*
 * Copyright (C) 2017 MediaTek Inc.
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 */

#define LOG_TAG "PMX"

#include <linux/kthread.h>
#include <linux/semaphore.h>
#include <linux/of.h>
#include <linux/of_irq.h>
#include <linux/of_address.h>

#include "disp_hw_log.h"
#include "disp_hw_mgr.h"
#include "disp_pmx_if.h"
#include "disp_def.h"
#include "fmt_hal.h"
#include "vdout_sys_hal.h"
#include "disp_sys_hal.h"
#include "disp_irq.h"
#include "disp_mix_hal.h"
#include "disp_hw_debug.h"
/** Plane mixer configuration.
 */
struct pmx_context_t {
	int u4BgColor;
	int u4PlaneOrder;
	int tv_type;
	struct disp_hw_resolution hdmi_res;
	struct vdout_init_param vdout_reg;
	struct fmt_init_pararm fmt_reg;
	struct disp_sys_init_param disp_reg;
	struct disp_mix_init_param disp_mix_reg;
};


static struct pmx_context_t pmx = {
	0x048080, 0x76543210,
};

static int _pmx_parse_dev_node(void)
{
	struct device_node *np;
	unsigned int reg_value;
	unsigned int irq_value;
	struct disp_hw *pmx_drv = disp_pmx_get_drv();

	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8696-fmt");
	if (np == NULL) {
		PMX_ERR("dts error, no fmtter device node.\n");
		return PMX_DTS_FAIL;
	}

	of_property_read_u32_index(np, "reg", 1, &reg_value);
	of_property_read_u32_index(np, "interrupts", 1, &irq_value);

	/* 10000000 */
	pmx.fmt_reg.io_reg_base = (uintptr_t)of_iomap(np, 0);
	hw_reg_base.fmt_reg_base[0] = pmx.fmt_reg.io_reg_base;
	/* 14001000 */
	pmx.fmt_reg.vdout_fmt_reg_base = (uintptr_t)of_iomap(np, 1);
	hw_reg_base.fmt_reg_base[1] = pmx.fmt_reg.vdout_fmt_reg_base;
	/* 15000000 */
	pmx.disp_reg.dispsys_reg_base = (uintptr_t)of_iomap(np, 2);
	hw_reg_base.fmt_reg_base[2] = pmx.disp_reg.dispsys_reg_base;
	/* 15001000 */
	pmx.fmt_reg.disp_fmt_reg_base[0] = (uintptr_t)of_iomap(np, 3);
	hw_reg_base.fmt_reg_base[3] = pmx.fmt_reg.disp_fmt_reg_base[0];
	/* 15008000 */
	pmx.fmt_reg.disp_fmt_reg_base[1] = (uintptr_t)of_iomap(np, 4);
	hw_reg_base.fmt_reg_base[4] = pmx.fmt_reg.disp_fmt_reg_base[1];
#if 1
	pmx_drv->irq[0].value = irq_of_parse_and_map(np, 0);  /* vsync irq*/
	pmx_drv->irq[1].value = irq_of_parse_and_map(np, 1);  /* active start*/
	if (pmx_drv->irq[0].value == 0 ||
		pmx_drv->irq[1].value == 0) {
		PMX_ERR("dts error, get interrupt id fail.\n");
		return PMX_DTS_FAIL;
	}
	PMX_LOG("irq 1 value: %d, irq 2 value: %d\n",
		pmx_drv->irq[0].value, pmx_drv->irq[1].value);
	pmx_drv->irq[0].irq = DISP_IRQ_FMT_VSYNC;
	pmx_drv->irq[1].irq = DISP_IRQ_FMT_ACTIVE_START;
	pmx_drv->irq_num = 2;
#endif

	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8696-vdout");
	if (np == NULL) {
		PMX_ERR("dts error, no vdout device node.\n");
		return PMX_DTS_FAIL;
	}


	of_property_read_u32_index(np, "reg", 1, &reg_value);

	/* 14001000 */
	pmx.vdout_reg.sys_reg_base = (uintptr_t)of_iomap(np, 0);
	hw_reg_base.vdout_reg_base = pmx.vdout_reg.sys_reg_base;

	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8696-mmsys");
	if (np) {
		/*14000000 */
		of_property_read_u32_index(np, "reg", 1, &reg_value);
		pmx.vdout_reg.mmsys_reg_base = (uintptr_t)of_iomap(np, 0);
		pmx.disp_reg.mmsys_reg_base = (uintptr_t)of_iomap(np, 0);
		hw_reg_base.mmsys_reg_base = pmx.disp_reg.mmsys_reg_base;
	}

	/*parse disp mix register base*/
	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8696-dispmix");
	if (np == NULL) {
		PMX_ERR("dts error, no vdout device node.\n");
		return PMX_DTS_FAIL;
	}
	of_property_read_u32_index(np, "reg", 1, &reg_value);
	/* 14014000 */
	pmx.disp_mix_reg.disp_mix_reg_base = (uintptr_t)of_iomap(np, 0);
	hw_reg_base.disp_mix_reg_base = pmx.disp_mix_reg.disp_mix_reg_base;

	return PMX_OK;
}

static int _pmx_init(struct disp_hw_common_info *info)
{
	enum FMT_TV_TYPE tv_type;
	enum HDMI_VIDEO_RESOLUTION res = info->resolution->res_mode;
	int sof_start = 0, sof_end = 0;
	uint32_t i = 0;

	PMX_LOG("%s begin, res: %d\n\n", __func__, res);

	/* hdmi service is not ready , so use temporary solution*/
#if 0
	hdmi_internal_video_config(info->resolution->res_mode);

	hdmi_internal_audio_config(0x0);
#endif

	if (info->resolution->frequency == 0 ||
		res >= HDMI_VIDEO_RESOLUTION_NUM)
		return PMX_PARAM_ERR;

	if (info->resolution->frequency == 25 ||
		info->resolution->frequency == 50)
		tv_type = FMT_TV_TYPE_PAL;
	else
		tv_type = FMT_TV_TYPE_NTSC;

	/*It has no lk at early porting stage, so needs to set pll at early*/
	/*poritng stage and restore it at ldvt*/

	fmt_hal_set_pllgp_hdmidds(info->resolution->res_mode, true, false,
		info->resolution->is_fractional);

	/*clock on vdout sys at disp clock probe*/
	/*vdout_sys_hal_clock_on_off(true);*/ /* open the vdout sys clock */

	fmt_hal_clock_on_off(VDOUT_FMT, true);

	disp_mix_hal_clock_on_off(true);

#ifdef DISP_PATH_SHADOW_ENABLE
	fmt_hal_sof_grp0_shadow_enable(true);
	fmt_hal_sof_grp1_shadow_enable(true);
	fmt_hal_hw_shadow_enable(VDOUT_FMT);
	vdout_sys_hal_shadow_en(true);
#endif

	/*It has no lk at early porting stage, so needs to set mode at early*/
	/*poritng stage and restore it at ldvt*/
	fmt_hal_set_mode(VDOUT_FMT, info->resolution->res_mode, false);

	fmt_hal_set_tv_type(VDOUT_FMT, tv_type);

	vdout_sys_hal_set_hdmi(info->resolution->res_mode);   /* set hdmi pll */
	vdout_sys_hal_reorder_before_mmsys_mix_out
		(COLOR_FORMAT_YUV, false);
	if (!g_force_dovi)
		vdout_sys_hal_hdr_vdo_be_enable(false);

	disp_mix_hal_mix_enable(true);

	/*set disp mix background  will do in disp mix*/
	disp_mix_hal_set_background_color(pmx.u4BgColor);

	disp_mix_hal_set_dst_wigth_and_height(info->resolution->res_mode, true);

	disp_path_get_sof_info(DISP_SOF_20_DISP_MIX_STA,
		DISP_SOF_20_DISP_MIX_END,
		&sof_start, &sof_end);
	fmt_hal_set_sof(FMT_SOF_20_DISP_MIX_STA,
		FMT_SOF_20_DISP_MIX_END, sof_start, sof_end);

	pmx.hdmi_res.res_mode = info->resolution->res_mode;
	pmx.hdmi_res.frequency = info->resolution->frequency;
	pmx.hdmi_res.width = info->resolution->width;
	pmx.hdmi_res.height = info->resolution->height;
	pmx.hdmi_res.is_fractional = info->resolution->is_fractional;
	pmx.tv_type = tv_type;

	for (i = 0; i < DISP_SOF_NUM; i += 2) {
		disp_path_get_sof_info(i, i+1, &sof_start, &sof_end);
		fmt_hal_set_sof(i, i+1, sof_start, sof_end);
	}

#ifdef DISP_PATH_SHADOW_ENABLE
	fmt_hal_shadow_update();
#endif

	PMX_LOG("%s end.\n", __func__);

	return PMX_OK;
}

static int disp_pmx_suspend(void)
{
	PMX_FUNC();
	if (!disp_common_info.low_energy_dozing_mode_enable) {
		fmt_hal_enable(VDOUT_FMT, false);

		fmt_hal_clock_on_off(VDOUT_FMT, false);

		vdout_sys_hal_clock_on_off(false); /* open the vdout sys clock */

		disp_mix_hal_clock_on_off(false);

		fmt_hal_set_pllgp_hdmidds(pmx.hdmi_res.res_mode, false, false,
			pmx.hdmi_res.is_fractional);
	}
	//due to upgrade kernel4.14 to kernel 5.10,   display suspends is early than
	//hdmi suspends,  so hdmi mutes cannot perform well, it will see sreen flickers
	// when suspends video playing.
	//Now when suspends the platform, display outputs black pattern firstly to avoid
	//the screen occurs strange phenomenon.
	disp_mix_hal_set_black_pattern(true);
	PMX_INFO("%s - leave.\n", __func__);
	return PMX_OK;
}

static int disp_pmx_resume(void)
{
	if (!disp_common_info.low_energy_dozing_mode_enable) {
		int sof_start = 0, sof_end = 0;

		PMX_FUNC();

		fmt_hal_set_pllgp_hdmidds(pmx.hdmi_res.res_mode, true, true,
			pmx.hdmi_res.is_fractional);

		vdout_sys_hal_clock_on_off(true); /* open the vdout sys clock */

		fmt_hal_clock_on_off(VDOUT_FMT, true);

		disp_mix_hal_clock_on_off(true);

#ifdef DISP_PATH_SHADOW_ENABLE
		fmt_hal_sof_grp0_shadow_enable(true);
		fmt_hal_sof_grp1_shadow_enable(true);
		fmt_hal_hw_shadow_enable(VDOUT_FMT);
		vdout_sys_hal_shadow_en(true);
#endif

		fmt_hal_set_mode(VDOUT_FMT, pmx.hdmi_res.res_mode, true);
		/*fmt_hal_hw_shadow_enable(VDOUT_FMT);*/

		fmt_hal_set_tv_type(VDOUT_FMT, pmx.tv_type);

		disp_mix_hal_set_background_color(pmx.u4BgColor);

		fmt_hal_reset(VDOUT_FMT);

		vdout_sys_hal_set_hdmi(pmx.hdmi_res.res_mode);   /* set hdmi pll */
		vdout_sys_hal_reorder_before_mmsys_mix_out
			(COLOR_FORMAT_YUV, false);
		if (!dovi_path_en)
			vdout_sys_hal_hdr_vdo_be_enable(false);

		disp_mix_hal_set_dst_wigth_and_height(pmx.hdmi_res.res_mode, true);
		disp_mix_hal_mix_enable(true);
		disp_path_get_sof_info(DISP_SOF_20_DISP_MIX_STA,
			DISP_SOF_20_DISP_MIX_END,
			&sof_start, &sof_end);
		fmt_hal_set_sof(FMT_SOF_20_DISP_MIX_STA,
			FMT_SOF_20_DISP_MIX_END, sof_start, sof_end);

#ifdef DISP_PATH_SHADOW_ENABLE
		fmt_hal_shadow_update();
#endif
		PMX_INFO("%s - leave.\n", __func__);
	}

	disp_mix_hal_set_black_pattern(false);
	return PMX_OK;
}

#ifdef CONFIG_HDMI_BLACK
static int disp_pmx_deep_suspend(void)
{
	PMX_FUNC();

	fmt_hal_enable(VDOUT_FMT, false);

	fmt_hal_clock_on_off(VDOUT_FMT, false);

	vdout_sys_hal_clock_on_off(false); /* open the vdout sys clock */

	disp_mix_hal_clock_on_off(false);

	fmt_hal_set_pllgp_hdmidds(pmx.hdmi_res.res_mode, false, false,
		pmx.hdmi_res.is_fractional);

	PMX_INFO("%s - leave.\n", __func__);
	return PMX_OK;
}

static int disp_pmx_deep_resume(void)
{
	int sof_start = 0, sof_end = 0;

	PMX_FUNC();

	fmt_hal_set_pllgp_hdmidds(pmx.hdmi_res.res_mode, true, true,
		pmx.hdmi_res.is_fractional);

	vdout_sys_hal_clock_on_off(true); /* open the vdout sys clock */

	fmt_hal_clock_on_off(VDOUT_FMT, true);

	disp_mix_hal_clock_on_off(true);

#ifdef DISP_PATH_SHADOW_ENABLE
	fmt_hal_sof_grp0_shadow_enable(true);
	fmt_hal_sof_grp1_shadow_enable(true);
	fmt_hal_hw_shadow_enable(VDOUT_FMT);
	vdout_sys_hal_shadow_en(true);
#endif

	fmt_hal_set_mode(VDOUT_FMT, pmx.hdmi_res.res_mode, true);
	/*fmt_hal_hw_shadow_enable(VDOUT_FMT);*/

	fmt_hal_set_tv_type(VDOUT_FMT, pmx.tv_type);

	disp_mix_hal_set_background_color(pmx.u4BgColor);

	fmt_hal_reset(VDOUT_FMT);

	vdout_sys_hal_set_hdmi(pmx.hdmi_res.res_mode);   /* set hdmi pll */
	vdout_sys_hal_reorder_before_mmsys_mix_out
		(COLOR_FORMAT_YUV, false);
	if (!dovi_path_en)
		vdout_sys_hal_hdr_vdo_be_enable(false);

	disp_mix_hal_set_dst_wigth_and_height(pmx.hdmi_res.res_mode, true);
	disp_mix_hal_mix_enable(true);
	disp_path_get_sof_info(DISP_SOF_20_DISP_MIX_STA,
		DISP_SOF_20_DISP_MIX_END,
		&sof_start, &sof_end);
	fmt_hal_set_sof(FMT_SOF_20_DISP_MIX_STA,
		FMT_SOF_20_DISP_MIX_END, sof_start, sof_end);

#ifdef DISP_PATH_SHADOW_ENABLE
	fmt_hal_shadow_update();
#endif
	PMX_INFO("%s - leave.\n", __func__);

	return PMX_OK;
}
#endif

static int disp_pmx_get_info(struct disp_hw_common_info *info)
{
	return PMX_OK;
}

static int disp_pmx_change_resolution(const struct disp_hw_resolution *info)
{
	enum FMT_TV_TYPE tv_type;
	enum HDMI_VIDEO_RESOLUTION res = info->res_mode;

	PMX_FUNC();
	PMX_INFO("disp_fmt_change_resolution res_mode: %d\n", info->res_mode);

	if (pmx.hdmi_res.res_mode == info->res_mode &&
		pmx.hdmi_res.is_fractional == info->is_fractional) {
		PMX_INFO("res mode is not change, res_mode: %d\n",
			info->res_mode);
		return PMX_OK;
	}
	/* hdmi service is not ready , so use temporary solution*/
	/* hdmi_internal_video_config(info->res_mode); */
	if (info->frequency == 0 || res >= HDMI_VIDEO_RESOLUTION_NUM)
		return PMX_PARAM_ERR;

	if (info->frequency == 25 || info->frequency == 50)
		tv_type = FMT_TV_TYPE_PAL;
	else
		tv_type = FMT_TV_TYPE_NTSC;

	fmt_hal_set_pllgp_hdmidds(pmx.hdmi_res.res_mode, false, false,
		pmx.hdmi_res.is_fractional);
	fmt_hal_set_pllgp_hdmidds(info->res_mode,
		true, true, info->is_fractional);

#ifdef DISP_PATH_SHADOW_ENABLE
	fmt_hal_sof_grp0_shadow_enable(true);
	fmt_hal_sof_grp1_shadow_enable(true);
	fmt_hal_hw_shadow_enable(VDOUT_FMT);
	vdout_sys_hal_shadow_en(true);
#endif

	fmt_hal_set_mode(VDOUT_FMT, info->res_mode, true);

	fmt_hal_set_tv_type(VDOUT_FMT, tv_type);
	fmt_hal_reset(VDOUT_FMT);

	vdout_sys_hal_set_hdmi(info->res_mode);   /* set hdmi pll */

	disp_mix_hal_set_dst_wigth_and_height(info->res_mode, true);

	pmx.hdmi_res.res_mode = info->res_mode;
	pmx.hdmi_res.frequency = info->frequency;
	pmx.hdmi_res.width = info->width;
	pmx.hdmi_res.height = info->height;
	pmx.hdmi_res.is_fractional = info->is_fractional;
	pmx.tv_type = tv_type;

#ifdef DISP_PATH_SHADOW_ENABLE
	fmt_hal_shadow_update();
#endif
	PMX_DEBUG("disp_fmt_change_resolution done.\n");

	return PMX_OK;
}

#ifdef DISP_QMS_SUPPORT
int	disp_pmx_vrr_enable(struct vrr_mode *vrr_info)
{
	bool vrr_en;
	bool start_line_en;
	uint32_t vtotal_ext;
	uint32_t trigger_start_line;

	vrr_en = vrr_info->vrr_en;
	start_line_en = vrr_info->vrr_en;
	trigger_start_line = vrr_info->trigger_start_line;
	if (vrr_en) {
		if (trigger_start_line)
			vtotal_ext = vrr_info->start_line_mode_vtotal;
		else {
			vtotal_ext = vrr_info->current_vtotal;
			start_line_en = false;
		}
	} else
		vtotal_ext = vrr_info->brr_vtotal;

	DISP_LOG_VRR("%s, enable:%d, vtotal:%d, start_line:%d 0x%x\n",
		__func__, vrr_en, vtotal_ext, trigger_start_line,
		fmt_hal_vrr_startline_read());

	//_fmt_adjust_total_pixel_enable(vrr_en);

	_fmt_set_vtotal_ext(vtotal_ext, vrr_en);

	_fmt_set_vrr_trigger_start_line_and_vrr_mode(trigger_start_line, start_line_en);
	if (start_line_en)
		if (fmt_hal_vrr_startline_read()) {
			fmt_hal_vrr_timing_trigger();
			DISP_LOG_VRR("%s startline trigger %d\n",
				__func__, vrr_info->mvrr_delat_step);
		}
#ifdef DISP_PATH_SHADOW_ENABLE
	fmt_hal_shadow_update();
#endif
	return 0;
}
#endif

static int disp_pmx_config(struct mtk_disp_buffer *config,
	struct disp_hw_common_info *info)
{
	/*do nothing*/
	return PMX_OK;
}

static int disp_pmx_irq_handler(uint32_t irq)
{
	if (irq == DISP_IRQ_FMT_VSYNC) {
		disp_sys_hal_update_hw_register();
		fmt_hal_update_hw_register();
	}

	return PMX_OK;
}

static int disp_pmx_dump(uint32_t level)
{
	return PMX_OK;
}

/*hw manager need tell fmt init at which resolution*/
bool pmx_initiated;
static int disp_pmx_init(struct disp_hw_common_info *info)
{
	int ret = PMX_OK;

	PMX_FUNC();

	if (!pmx_initiated) {
		ret = _pmx_parse_dev_node();
		if (ret != PMX_OK) {
			PMX_ERR("parse device node fail.\n");
			return ret;
		}

		ret = vdout_sys_hal_init(&(pmx.vdout_reg));
		if (ret != PMX_OK) {
			PMX_ERR("set vdout sys register base fail.\n");
			return ret;
		}

		ret = disp_sys_hal_init(&(pmx.disp_reg));
		if (ret != PMX_OK) {
			PMX_ERR("set disp sys register base fail.\n");
			return ret;
		}

		ret = fmt_hal_init(&(pmx.fmt_reg));
		if (ret != PMX_OK) {
			PMX_ERR("set fmt register base fail.\n");
			return ret;
		}

		ret = disp_mix_hal_init(&(pmx.disp_mix_reg));
		if (ret != PMX_OK) {
			PMX_ERR("set disp mix register base fail.\n");
			return ret;
		}

		ret = _pmx_init(info);
		if (ret != PMX_OK) {
			PMX_ERR("init pmx fail.\n");
			return ret;
		}

		pmx_initiated = true;
	}

	return ret;

}

static int disp_pmx_deinit(void)
{
	/*release sw resource*/
	fmt_hal_set_pllgp_hdmidds(pmx.hdmi_res.res_mode, false, false,
		pmx.hdmi_res.is_fractional);
	fmt_hal_clock_on_off(VDOUT_FMT, false);
	vdout_sys_hal_clock_on_off(false);
	disp_mix_hal_clock_on_off(false);
	disp_sys_hal_deinit();
	pmx_initiated = false;
	return PMX_OK;
}

/***************** driver************/
struct disp_hw disp_pmx_driver = {
	.name = PMX_DRV_NAME,
	.init = disp_pmx_init,
	.deinit = disp_pmx_deinit,
	.start = NULL,
	.stop = NULL,
	.suspend = disp_pmx_suspend,
	.resume = disp_pmx_resume,
#ifdef CONFIG_HDMI_BLACK
	.deep_suspend = disp_pmx_deep_suspend,
	.deep_resume = disp_pmx_deep_resume,
#endif
#ifdef DISP_QMS_SUPPORT
	.vrr_enable = disp_pmx_vrr_enable,
#endif
	.get_info = disp_pmx_get_info,
	.change_resolution = disp_pmx_change_resolution,
	.config = disp_pmx_config,
	.irq_handler = disp_pmx_irq_handler,
	.set_listener = NULL,
	.wait_event = NULL,
	.dump = disp_pmx_dump,
};

struct disp_hw *disp_pmx_get_drv(void)
{
	return &disp_pmx_driver;
}

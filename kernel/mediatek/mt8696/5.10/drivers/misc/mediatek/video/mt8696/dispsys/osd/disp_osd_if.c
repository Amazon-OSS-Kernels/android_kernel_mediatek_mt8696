...
long int got_current_time_ns(void)
{
	struct timespec64 t;
...
static void cmdq_pkt_flush_cb_handler(struct cmdq_cb_data data)
{
	...
	OSD_PRINTF(OSD_FENCE_LOG, "%s plane=%d index=%d vsync_cnt=%ld\n",
		__func__,
		update_pkt->plane,
		update_pkt->index,
		vsync_cnt);
...
static void osd_status(unsigned int u4OsdNum)
{
	...
	UINT32 u4DataAddr  = 0;
	UINT32 u4DataAddrL = 0;
	UINT32 u4DataAddrMD = 0;
	UINT32 u4DataAddrHI = 0;

	if (u4OsdNum < OSD_PLANE_MAX_NUM) {
		prHwOsdFmtReg_dump =
		...
		u4DataAddrL = (*((UINT32 *)(osd.osd_reg.osd_pln_reg_base[u4OsdNum] +
				9 * 4)) & 0x00ffffff) << 4;
		u4DataAddrMD = (*((UINT32 *)(osd.osd_reg.osd_pln_reg_base[u4OsdNum] +
				15 * 4)) & 0x01800000) << 5;
		u4DataAddrHI = (*((UINT32 *)(osd.osd_reg.osd_pln_reg_base[u4OsdNum] +
				14 * 4)) & 0x0000000c) << 28;
		u4DataAddr = u4DataAddrL | u4DataAddrMD | u4DataAddrHI;

		OSD_PRINTF(OSD_VYSNC_LOG,
			"**** status:%d p_checksum=0x%x s_checksum=0x%x mva=0x%x\n",
				status,
				u4PlaneChecksum,
				u4SclChecksum,
				u4DataAddr);
	...
	if (i < MAX_OSD_INPUT_CONFIG) {
		...
		while (!list_empty(&(OSD_Configed_Head[i]))) {
			...
			if (pBuffList) {
				...
				if (pBuffList->buffer_info.config.osd_buf_mmu
						!= NULL) {
					OSD_PRINTF(OSD_CONFIG_SW_LOG,
						" %s layer_%d release dma buffer:%p\n",
						__func__,
						i,
						pBuffList->buffer_info.config.osd_buf_mmu);
					disp_hw_mgr_dma_buffer_release(
						pBuffList->buffer_info.config.osd_buf_mmu);
					vfree(pBuffList->buffer_info.config.osd_buf_mmu);
					pBuffList->buffer_info.config.osd_buf_mmu = NULL;
				}
				OSD_PRINTF(OSD_CONFIG_SW_LOG,
...
#if !CONFIG_DRV_FAST_LOGO
	osd_clk_enable(info->resolution->res_mode, false, OSD_PLANE_2);
	osd.osd_stop_ctl.osd_stop_status[OSD_PLANE_2] = OSD_LAYER_STOPPED;
#endif
...
void osd_release_buffer(unsigned int layer_id)
{
	...
	if (!list_empty(&(OSD_Configed_Head[i]))) {
			...
			if (pBuffList->list_buf_state == list_updated) {
				if ((pBuffList->buffer_info.config.index <
					...
					if (pBuffList->buffer_info.config.osd_buf_mmu
						!= NULL) {
						OSD_PRINTF(OSD_CONFIG_SW_LOG,
							" %s layer_%d release dma buffer add:0x%x\n",
							__func__,
							i,
							pBuffList->buffer_info.config.osd_buf_mmu
		...
		OSD_PRINTF(OSD_FENCE_LOG,
				"release buffer OSD_Configed_Head is empty\n");
	mutex_unlock(&(osd.osd_config_queue_lock[i]));
}
	...
	while (1) {
		...
		if (osd.res_chg[plane_id].fg_need_change_res) {
			...
			if (osd.res_chg[plane_id].fg_res_vsync1) {
				if (OSD_Is_PLA_Enabled(plane_id)) {
					if (plane_id == OSD_PLANE_2)
...
{
	int wait_ret = 0;
	unsigned short fence_ret = 0;
	unsigned short wait_times = 0;
#if 0
...
static int disp_mgr_post_osd_buffer(struct disp_osd_input_config *buffer_info)
{
	...
	if (buffer_info->config.ion_fd != -1) {
	...
	OSD_PRINTF(OSD_CONFIG_SW_LOG,
		"vsync=%ld ion_fd=%d fence_value=%d,%d ret=%d\n",
		vsync_cnt,
		buffer_info->config.ion_fd,
		value,
		value_present,
		ret);
					...
					struct disp_hw_common_info *info)
{
	...
	struct tee_mmu *osd_buf_mmu = NULL;
	unsigned int mva = 0;
	int ret = OSD_RET_OK;
	...
	if (config->type == DISP_LAYER_OSD || config->type == DISP_LAYER_AEE) {
		if (osd.plug_out[OSD_PLANE_1].fg_hdmi_plug_out ||
		    ...
		    osd.osd_in_suspend[OSD_PLANE_2] ||
		    (config->layer_id > MAX_OSD_INPUT_CONFIG)) {
		...
		if (config->layer_enable) {
			if (config->buffer_source != DISP_BUFFER_ALPHA) {
				...
				if (config->ion_fd != -1) {
					osd_buf_mmu = vmalloc(sizeof(struct tee_mmu));
					if (!osd_buf_mmu) {
						ret = -OSD_RET_INV_ARG;
						OSD_LOG_E("could not allocate osd_buf_mmu\n");
						goto error;
					}
					osd_plane_config.config.osd_buf_mmu = osd_buf_mmu;
					disp_hw_mgr_dma_fd_to_buffer(config->ion_fd, osd_buf_mmu);
					if (osd_buf_mmu->buffer_addr.dma_addr == 0) {
						OSD_LOG_I(
						"disp osd config get dma_addr err\n");
						ret = -OSD_RET_INV_ARG;
						vfree(osd_buf_mmu);
						osd_buf_mmu = NULL;
						goto error;
					}
					mva = osd_buf_mmu->buffer_addr.dma_addr;
				} else {
			...
			osd_plane_config.config.ion_fd = config->ion_fd;
			...
			/*layer disable*/
			osd_plane_config.config.ion_handle = NULL;
			osd_plane_config.config.ion_fd = -1;
		...
		OSD_PRINTF(OSD_CONFIG_SW_LOG,
			...
			osd_plane_config.config.src_phy_addr);
		OSD_PRINTF(OSD_FENCE_LOG,
			"acq_fd=%d layer_id=%d ion_fd=%d mva=0x%x\n",
			config->acquire_fence_fd,
			osd_plane_config.config.layer_id,
			config->ion_fd,
			osd_plane_config.config.src_phy_addr);

		ret = disp_mgr_post_osd_buffer(&osd_plane_config);
		if (ret != OSD_RET_OK && osd_buf_mmu != NULL)
			vfree(osd_buf_mmu);
...
int disp_osd_dump(uint32_t level)
{
	...
	switch (level) {
	case LEVEL_ENABLE_COLORBAR:
		OSD_LOG_I("LEVEL_ENABLE_COLORBAR\n");
		enable_osd_colorbar = true;
		break;
	case LEVEL_DISABLE_COLORBAR:
		OSD_LOG_I("LEVEL_DISABLE_COLORBAR\n");
		enable_osd_colorbar = false;
		break;
	case LEVEL_ENABLE_REPLACE_BMP:
		OSD_LOG_I("LEVEL_ENABLE_REPLACE_BMP\n");
		region_replace_bmp = true;
		break;
	case LEVEL_DISABLE_REPLACE_BMP:
		OSD_LOG_I("LEVEL_DISABLE_REPLACE_BMP\n");
		region_replace_bmp = false;
		break;
	default:
		break;
	}

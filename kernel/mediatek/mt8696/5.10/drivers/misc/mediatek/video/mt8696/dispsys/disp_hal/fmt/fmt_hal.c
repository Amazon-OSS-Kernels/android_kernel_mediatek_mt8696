...
#define H_DOWNSCALE_PIPELINE_DELAY_UHD		38
...
static uint32_t _fmt_get_vsync_width(enum HDMI_VIDEO_RESOLUTION resolution)
{
	uint32_t width = 0;
...
static uint32_t _fmt_get_dispfmt_vsync_width
...
static void _fmt_hal_reset(uint32_t fmt_id)
{
	...
	if (fmt_id >= VDOUT_FMT) {
		...
		if (vdout_fmt && vdout_fmt->status == FMT_STA_USED) {
			WriteREG32Msk((vdout_fmt->hw_fmt_base +
				       VDOUT_FMT_REG_FMT_CTRL),
				      (0 << 10), (1 << 10));
			WriteREG32Msk((vdout_fmt->hw_fmt_base +
				       VDOUT_FMT_REG_FMT_CTRL),
				      (1 << 10), (1 << 10));
		...
		if (!disp_fmt || disp_fmt->is_sec)
			return;

		if (disp_fmt && disp_fmt->status == FMT_STA_USED) {
			/* enable 0x10e4[2] = 1, make sure line cnt 0xbc = 0x1
			 * after reset 0xac[10] = 1 */
			WriteREG32Msk((disp_fmt->hw_fmt_base +
				       DISP_FMT_REG_RATIO),
				      (1 << 2), (1 << 2));

			disp_fmt->fmt_reset_start_time = sched_clock();

			mdelay(1);

			WriteREG32Msk((disp_fmt->hw_fmt_base +
				       DISP_FMT_REG_VDO_FMT_CTRL),
				      (1 << 10), (1 << 10));
			WriteREG32Msk((disp_fmt->hw_fmt_base +
				       DISP_FMT_REG_VDO_FMT_CTRL),
				      (0 << 10), (1 << 10));

			disp_fmt->fmt_reset_end_time = sched_clock();
		}
	}
}
...
static void _fmt_dispfmt_trigger(uint32_t fmt_id)
{
	...
	if ((disp_fmt->status == FMT_STA_USED) &&
		...
		FMT_LOG_D(
			"[DISP_FMT_REG %d] trigger disp fmt shadow register..\n",
			fmt_id);
...
static void _fmt_vdoutfmt_setting(uint32_t fmt_id)
{
	...
	if (fmt_info == NULL) {
		FMT_LOG_E("%s get %d fmt_info fail\n",
			__func__, fmt_id);
		return;
	}

	for (u4RegIdx = 0, regMask = 1; u4RegIdx < HAL_VDOUT_FMT_REG_NUM;
	     u4RegIdx++, regMask <<= 1) {
...
static void _fmt_vdoutfmt_trigger(bool is_force)
{
	...
	if (((sub_fmt_info->shadow_trigger == 1 ||
	      fmt_info->shadow_trigger == 1) &&
	     fmt_info->shadow_en == 1) ||
	    is_force) {
		FMT_LOG_D(
			"[VDOUT_FMT_REG] trigger vdout fmt shadow register..\n");
		WriteREG32Msk((fmt_info->hw_fmt_base + VDOUT_FMT_REG_FMT_CTRL),
			      (1 << 31), (1 << 31));
...
static int32_t _fmt_write_reg(uint32_t fmt_id, void *data, uint32_t reg,
			      bool hw_shadow)
{
	struct disp_fmt_t *disp_fmt = NULL;
	struct vdout_fmt_t *vdout_fmt = NULL;
	...
	if (!data) {
		FMT_LOG_I("%s data is null,fmt_id=%d, reg=0x%x.\n",
			  __func__, fmt_id, reg);
		return -1;
	}
	...
	if (fmt_id >= VDOUT_FMT) {
		...
		if (hw_shadow) {
			...
			FMT_LOG_D(
				"[DISP_FMT_REG %d] Write Reg= 0x%x, Val= 0x%x\n",
				fmt_id, reg,
				disp_fmt->sw_fmt_base->au4Reg[reg / 4]);
...
static void _fmt_set_vdoutfmt_mode(enum HDMI_VIDEO_RESOLUTION resolution,
				   bool config_hw)
{
	struct vdout_fmt_t *vdout_fmt = NULL;
	...
	case HDMI_VIDEO_1280x720p_60Hz:
		...
		vdout_fmt->sw_fmt_base->rField.u2V_TOTAL = 0x2EE;
		vdout_fmt->sw_fmt_base->rField.u2H_TOTAL = 0x672;
		break;
	...
	case HDMI_VIDEO_1280x720p_50Hz:
		...
		vdout_fmt->sw_fmt_base->rField.fgHD_TP = 1;
		vdout_fmt->sw_fmt_base->rField.u2V_TOTAL = 0x2EE;
		vdout_fmt->sw_fmt_base->rField.u2H_TOTAL = 0x7BC;
		break;
	...
	vdout_fmt->sw_fmt_base->rField.u1VSYNWIDTH =
		_fmt_get_vsync_width(resolution);
	...
	//set FMT 0x10D8 V_TOTAL_EXT same with 0x10D4 Vtotal for QMS_VRR
	vdout_fmt->sw_fmt_base->rField.V_TOTAL_EXT =
	vdout_fmt->sw_fmt_base->rField.u2V_TOTAL;

	_fmt_write_reg(VDOUT_FMT, (void *)vdout_fmt,
		VDOUT_FMT_REG_FMT_VTOTAL_EXT, true);

	if (config_hw == true) {
		/*write hw register */
		_fmt_write_reg(VDOUT_FMT, (void *)vdout_fmt,
			       VDOUT_FMT_REG_FMT_HV_TOTAL, true);
		_fmt_write_reg(VDOUT_FMT, (void *)vdout_fmt,
			       VDOUT_FMT_REG_FMT_MODE, true);
		_fmt_write_reg(VDOUT_FMT, (void *)vdout_fmt,
			       VDOUT_FMT_REG_FMT_CTRL, true);
...
static void _fmt_reset_fmt_info(uint32_t fmt_id)
{
	...
	if (fmt_id >= VDOUT_FMT) {
		...
		*(vdout_fmt->reg_mode) = 0;
		memset((void *)(vdout_fmt->sw_fmt_base), 0,
		       sizeof(union vdout_fmt_union_t));
	} else {
		disp_fmt = fmt_hal_get_dispfmt_reg(fmt_id);
		if (disp_fmt == NULL) {
		...
		*(disp_fmt->reg_mode) = 0;
		memset((void *)(disp_fmt->sw_fmt_base), 0,
		       sizeof(union disp_fmt_union_t));
	}
}
static void _fmt_set_dispfmt_mode(uint32_t fmt_id,
				  enum HDMI_VIDEO_RESOLUTION resolution)
{
	struct disp_fmt_t *disp_fmt = NULL;
	...
	case HDMI_VIDEO_1280x720p_60Hz:
		...
		disp_fmt->sw_fmt_base->rField.u2V_TOTAL = 0x2EE;
		disp_fmt->sw_fmt_base->rField.u2H_TOTAL = 0x672;
		disp_fmt->sw_fmt_base->rField.fgADJ_T_MIX = 1;
		disp_fmt->sw_fmt_base->rField.u2V_TOTAL_MIX = 0x2EE;
		disp_fmt->sw_fmt_base->rField.u2H_TOTAL_MIX = 0x672;
	...
	case HDMI_VIDEO_1280x720p_50Hz:
		disp_fmt->sw_fmt_base->rField.fgADJ_T = 1;
		disp_fmt->sw_fmt_base->rField.u2V_TOTAL = 0x2EE;
		disp_fmt->sw_fmt_base->rField.u2H_TOTAL = 0x7BC;
		disp_fmt->sw_fmt_base->rField.fgADJ_T_MIX = 1;
		disp_fmt->sw_fmt_base->rField.u2V_TOTAL_MIX = 0x2EE;
		disp_fmt->sw_fmt_base->rField.u2H_TOTAL_MIX = 0x7BC;
		disp_fmt->sw_fmt_base->rField.fgHD_ON = 1;
		disp_fmt->sw_fmt_base->rField.fgHD_TP = 1;
		disp_fmt->sw_fmt_base->rField.fgNOT_PST_D2 = 0;
		disp_fmt->sw_fmt_base->rField.fgEN_FIRST_PXL_LEAD = 0;
	...
	case HDMI_VIDEO_1920x1080p_50Hz:
		...
		disp_fmt->sw_fmt_base->rField.fgEN_FIRST_PXL_LEAD = 0;
		disp_fmt->sw_fmt_base->rField.u1FIRST_PXL_LEAD = 0;
		if (resolution == HDMI_VIDEO_1920x1080i_50Hz)
			disp_fmt->sw_fmt_base->rField.fgHD_C_POS = 1;
		else
			disp_fmt->sw_fmt_base->rField.fgHD_C_POS = 0;
		break;
	...
	case HDMI_VIDEO_3840x2160P_50HZ:
		...
		disp_fmt->sw_fmt_base->rField.fgADJ_T = 1;
		disp_fmt->sw_fmt_base->rField.u2V_TOTAL_MIX =
			disp_fmt->sw_fmt_base->rField.u2V_TOTAL;
		disp_fmt->sw_fmt_base->rField.u2H_TOTAL_MIX =
			disp_fmt->sw_fmt_base->rField.u2H_TOTAL;
		disp_fmt->sw_fmt_base->rField.fgADJ_T_MIX = 1;
	...
	/*write hw register */
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_PIXEL_LENGTH,
		       true);
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_HV_TOTAL_MIX,
		       true);
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_HV_TOTAL, true);
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_Y_LIMITATION,
		       true);
	...
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_VDO_FMT_CTRL,
		       true);
...
static int32_t _fmt_hd_scl_enable(uint32_t fmt_id, uint32_t factor, bool en,
				  bool fifo_en)
{
	struct disp_fmt_t *disp_fmt = NULL;
...
static int32_t _fmt_set_in_region(uint32_t fmt_id, uint32_t h_start,
				  uint32_t h_end, uint32_t odd_start,
				  uint32_t odd_end, uint32_t even_start,
				  uint32_t even_end)
{
	struct disp_fmt_t *disp_fmt = NULL;
	...
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_DOWN_RNG_VO,
		       true);
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_DOWN_RNG_VE,
		       true);

	return FMT_OK;
}

static int32_t _fmt_set_out_region(uint32_t fmt_id, uint32_t h_start,
				   uint32_t h_end)
{
	struct disp_fmt_t *disp_fmt = NULL;
	...
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_DOWN_RNG_OUTPUT,
		       true);
...
static int32_t _fmt_set_buildin_color(bool en)
{
	...
	_fmt_write_reg(VDOUT_FMT, (void *)vdout_fmt, VDOUT_FMT_REG_MIXER_MODE,
		       true);
...
void fmt_hal_enable(uint32_t fmt_id, bool enable)
{
	...
	if (fmt_id == VDOUT_FMT) {
		...
		if (enable) {
			...
			if (!disp_fmt->is_sec) {
				disp_fmt->fmt_enable_start_time = sched_clock();
				WriteREG32Msk((disp_fmt->hw_fmt_base +
					       DISP_FMT_REG_VDO_FMT_CTRL),
					      (1 << 0), (1 << 0));
				disp_fmt->fmt_enable_end_time = sched_clock();
			}
			else
				REG_SET(*disp_fmt->reg_mode,
					REG_MASK(DISP_FMT_REG_VDO_FMT_CTRL /
						 4));
		} else {
...
#define FMT_MAX_REC	4
struct fmt_rec_info {
	unsigned int rec_reg;
	unsigned int reg_0x94;
	unsigned int reg_0xa0;
	unsigned int reg_0xac;
	unsigned int reg_0xbc;
	unsigned long time;
};

#define REC_FMT_INFO(rec_info, idx, hw_base, _rec_reg) { \
	rec_info[idx].time = sched_clock(); \
	rec_info[idx].rec_reg = _rec_reg; \
	rec_info[idx].reg_0x94 = ReadREG32(hw_base + DISP_FMT_REG_MODE_CTRL); \
	rec_info[idx].reg_0xa0 = ReadREG32(hw_base + DISP_FMT_REG_ACTIVE_H); \
	rec_info[idx].reg_0xac = ReadREG32(hw_base + DISP_FMT_REG_VDO_FMT_CTRL); \
	rec_info[idx].reg_0xbc = ReadREG32(hw_base + DISP_FMT_REG_LINE_CNT_DEBUG); \
	idx++; \
}
...
void fmt_hal_hw_shadow_enable(uint32_t fmt_id)
{
	...
	if (fmt_id == VDOUT_FMT) {
		vdout_fmt = fmt_hal_get_vdoutfmt_reg(fmt_id);
		if (vdout_fmt->status == FMT_STA_UNUSED) {
			FMT_LOG_D("%s vdout fmt state error\n", __func__);
			_fmt_mutex_unlock();
			return;
		}
		...
		WriteREG32Msk((vdout_fmt->hw_fmt_base + VDOUT_FMT_REG_FMT_CTRL),
			      (1 << 28), (1 << 28));
	} else {
		disp_fmt = fmt_hal_get_dispfmt_reg(fmt_id);
		if (disp_fmt->status == FMT_STA_UNUSED) {
		...
		if (!disp_fmt->is_sec) {
			unsigned long flags;
			unsigned long udelay_start_time, udelay_end_time;
			unsigned int idx = 0;
			...
			spin_lock_irqsave(&disp_fmt->fmt_shadow_lock, flags);
			disp_fmt->fmt_enable_start_time = sched_clock();
			/* 0x10ac[0] */
			WriteREG32Msk((disp_fmt->hw_fmt_base +
				       DISP_FMT_REG_VDO_FMT_CTRL),
				      (1 << 0), (1 << 0));
			REC_FMT_INFO(fmt_rec, idx, disp_fmt->hw_fmt_base, DISP_FMT_REG_VDO_FMT_CTRL);

			value = ReadREG32(
				disp_fmt->hw_fmt_base + DISP_FMT_REG_MODE_CTRL);
			value &= 0x1F00;
			/*set vsync width = 0*/
			/* 0x1094[12:8] */
			WriteREG32Msk(
			(disp_fmt->hw_fmt_base + DISP_FMT_REG_MODE_CTRL),
				0, 0x1F << 8);
			REC_FMT_INFO(fmt_rec, idx, disp_fmt->hw_fmt_base, DISP_FMT_REG_MODE_CTRL);

			/*trigger with active zone clean*/
			/* 0x10a0[12:8] */
			WriteREG32(
				(disp_fmt->hw_fmt_base + DISP_FMT_REG_ACTIVE_H),
				0x80000000);
			REC_FMT_INFO(fmt_rec, idx, disp_fmt->hw_fmt_base, DISP_FMT_REG_ACTIVE_H);

			/*set vsync to right value*/
			/* 0x1094[12:8] */
			WriteREG32Msk(
			(disp_fmt->hw_fmt_base + DISP_FMT_REG_MODE_CTRL),
				value, 0x1F << 8);
			...
			for (idx = 0; idx < FMT_MAX_REC; idx++)
				FMT_LOG_D("rec 0x%X %lu 0x94|0x%X 0xa0|0x%X 0xac|0x%X 0xbc|0x%X\n",
					  ...
					  fmt_rec[idx].reg_0xa0,
					  fmt_rec[idx].reg_0xac,
					  fmt_rec[idx].reg_0xbc);

			/* reset dispfmt */
			_fmt_hal_reset(fmt_id);

			udelay_start_time = sched_clock();
			udelay(1);
			...
			FMT_LOG_D("fmt en %lu %lu %lu reset %lu %lu %lu udelay %lu %lu %lu\n",
				  ...
				  disp_fmt->fmt_reset_start_time,
				  disp_fmt->fmt_reset_end_time,
				  disp_fmt->fmt_reset_end_time - disp_fmt->fmt_reset_start_time,
				  udelay_start_time,
				  udelay_end_time,
				  udelay_end_time - udelay_start_time);
			...
			REG_SET(*disp_fmt->reg_mode,
				REG_MASK(DISP_FMT_REG_ACTIVE_H / 4));
	}
			       ...
			       bool fractional)
{
	...
	fgHd = fgIsHDRes(eRes);
	if (en) {
		/* remove tvp_pll, change src clk to tx_pll*/
		/*tx_pll clk on and off at hdmi module*/
		/*set hd_sel and sd_sel on tx_pll also do in hdmi module*/
		if (set_pll) {
			if (fractional)
...
void fmt_hal_set_tv_type(uint32_t fmt_id, uint32_t tv_type)
{
	...
	if (fmt_id == VDOUT_FMT) {
		...
		_fmt_write_reg(fmt_id, (void *)vdout_fmt,
			       VDOUT_FMT_REG_FMT_MODE, true);
	} else {
		disp_fmt = fmt_hal_get_dispfmt_reg(fmt_id);
		if (disp_fmt->status == FMT_STA_UNUSED) {
		...
		_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_MODE_CTRL,
			       true);
	}
				...
				struct fmt_active_info *active_info)
{
	...
	FMT_LOG_D("h_b: 0x%x, odd_b: 0x%x, even_b: 0x%x\n",
		  active_info->h_begine, active_info->v_even_begine,
		  active_info->v_odd_begine);
	...
	if (fmt_id >= VDOUT_FMT) {
		...
		vdout_fmt->sw_fmt_base->rField.u2HACTBGN =
			active_info->h_begine;
		vdout_fmt->sw_fmt_base->rField.u2HACTEND = active_info->h_end;

		vdout_fmt->sw_fmt_base->rField.u2VOACTBGN =
			active_info->v_even_begine;
		vdout_fmt->sw_fmt_base->rField.u2VOACTEND =
			active_info->v_even_end;

		vdout_fmt->sw_fmt_base->rField.u2VEACTBGN =
			active_info->v_odd_begine;
		vdout_fmt->sw_fmt_base->rField.u2VEACTEND =
			active_info->v_odd_end;

		_fmt_write_reg(fmt_id, (void *)vdout_fmt,
			       VDOUT_FMT_REG_ACTIVE_H, true);
		_fmt_write_reg(fmt_id, (void *)vdout_fmt,
			       VDOUT_FMT_REG_ODD_ACTIVE_V, true);
		_fmt_write_reg(fmt_id, (void *)vdout_fmt,
			       VDOUT_FMT_REG_EVEN_ACTIVE_V, true);
	} else {
		disp_fmt = fmt_hal_get_dispfmt_reg(fmt_id);
		if (disp_fmt->status == FMT_STA_UNUSED) {
			FMT_LOG_E("%s state error\n", __func__);
		...
		disp_fmt->sw_fmt_base->rField.u2VOACTBGN =
			active_info->v_even_begine;
		disp_fmt->sw_fmt_base->rField.u2VOACTEND =
			active_info->v_even_end;

		disp_fmt->sw_fmt_base->rField.u2VEACTBGN =
			active_info->v_odd_begine;
		disp_fmt->sw_fmt_base->rField.u2VEACTEND =
			active_info->v_odd_end;

		_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_ACTIVE_H,
			       true);
		_fmt_write_reg(fmt_id, (void *)disp_fmt,
			       DISP_FMT_REG_ODD_ACTIVE_V, true);
		_fmt_write_reg(fmt_id, (void *)disp_fmt,
			       DISP_FMT_REG_EVEN_ACTIVE_V, true);
	}
				 ...
				 enum HDMI_VIDEO_RESOLUTION resolution)
{
	...
	return fmt_hal_set_delay(fmt_id, delay->adj_f, delay->h_delay,
				 delay->v_delay);
}

#ifdef DISP_QMS_SUPPORT
uint32_t fmt_hal_read_vline(void)
{
	uint32_t read_counter = 0;
	struct vdout_fmt_t *vdout_fmt = NULL;

	vdout_fmt = fmt_hal_get_vdoutfmt_reg(VDOUT_FMT);
	read_counter = ReadREG32(vdout_fmt->hw_fmt_base + 0x28) & 0xFFF;
	return read_counter;
}

void fmt_hal_dispfmt_vtotal(uint32_t vtotal_ext)
{
	struct disp_fmt_t *disp_fmt = NULL;
	uint32_t vtotal_lsb, vtotal_msb;

	disp_fmt = fmt_hal_get_dispfmt_reg(DISP_FMT_MAIN);

	if (disp_fmt == NULL) {
		FMT_LOG_E("%s cannot get valid vout reg base\n", __func__);
		return;
	}

	vtotal_msb = (vtotal_ext & 0xF0000) >> 16;
	vtotal_lsb = vtotal_ext & 0xFFFF;

	//FMT_LOG_I("%s vtotal_ext 0x%x msb 0x%x lsb 0x%x\n",
	//	  __func__, vtotal_ext, vtotal_msb, vtotal_lsb);

	WriteREG32Msk((disp_fmt->hw_fmt_base +
				DISP_FMT_REG_HV_TOTAL_MIX),
				vtotal_lsb, 0xFFFF);

	WriteREG32Msk((disp_fmt->hw_fmt_base +
				DISP_FMT_REG_HV_TOTAL),
				vtotal_lsb, 0xFFFF);

	WriteREG32((disp_fmt->hw_fmt_base +
			DISP_FMT_REG_VTOTAL_HIGH),
			(vtotal_msb << 16) | vtotal_msb);

	_fmt_dispfmt_trigger(DISP_FMT_MAIN);
}

uint32_t fmt_hal_vrr_startline_read(void)
{
	struct vdout_fmt_t *vdout_fmt = NULL;
	uint32_t vrr_mode = 0;
	...
	vrr_mode = ReadREG32(vdout_fmt->hw_fmt_base +
		VDOUT_FMT_REG_FMT_VRR_TRIG_START_LINE);
	/*check bit 31*/
	vrr_mode = (vrr_mode >> 31) & 0x1;

	return vrr_mode;
}

void fmt_hal_vrr_timing_trigger(void)
{
	struct vdout_fmt_t *vdout_fmt = NULL;
	...
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			VDOUT_FMT_REG_VRR_TRIGGER_ACTIVE),
			(0x1 << 10), (0x1 << 10));
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			VDOUT_FMT_REG_VRR_TRIGGER_ACTIVE),
			(0x0 << 10), (0x1 << 10));
...
void _fmt_set_vrr_trigger_start_line_and_vrr_mode(uint32_t start_line,
	bool vrr_mode_enable)
{
	struct vdout_fmt_t *vdout_fmt = NULL;

	//FMT_LOG_I("%s, start_line:0x%x, enable:%d\n",
	//	__func__, start_line, vrr_mode_enable);

	vdout_fmt = fmt_hal_get_vdoutfmt_reg(VDOUT_FMT);

	vdout_fmt->sw_fmt_base->rField.VRR_TRIGGER_STATRT_LINE =
		start_line;

	vdout_fmt->sw_fmt_base->rField.VRR_MODE = vrr_mode_enable;

	_fmt_write_reg(VDOUT_FMT, (void *)vdout_fmt,
	       VDOUT_FMT_REG_FMT_VRR_TRIG_START_LINE, true);
}

void _fmt_set_vtotal_ext(uint32_t vtotal_ext, bool use_new_ext)
{
	struct vdout_fmt_t *vdout_fmt = NULL;

	//FMT_LOG_I("%s, ext:0x%x, use_new %d\n", __func__,
	//	vtotal_ext, use_new_ext);

	vdout_fmt = fmt_hal_get_vdoutfmt_reg(VDOUT_FMT);

	vdout_fmt->sw_fmt_base->rField.V_TOTAL_EXT =
		vtotal_ext;
	vdout_fmt->sw_fmt_base->rField.V_TOTAL_EXT_USE_NEW =
		use_new_ext;
...
void _fmt_adjust_total_pixel_enable(bool enable)
{
	struct vdout_fmt_t *vdout_fmt = NULL;
	unsigned int value = 0;
	bool fgADJ_T = false;
	...
	if (enable) {
		if (((ReadREG32(vdout_fmt->hw_fmt_base + 0xD4)) & (0x80000000)))
			fgADJ_T = true;
		else
			fgADJ_T = false;
		value = ReadREG32(vdout_fmt->hw_fmt_base + 0xD4);
		if (value == 0) {
			WriteREG32Msk((vdout_fmt->hw_fmt_base +
				VDOUT_FMT_REG_FMT_HV_TOTAL),
				(0x08980465 << 0), (0x0fff0fff << 0));
			WriteREG32Msk(
			(vdout_fmt->hw_fmt_base + VDOUT_FMT_REG_FMT_CTRL),
				(1 << 31), (1 << 31));
		}
		WriteREG32Msk((vdout_fmt->hw_fmt_base +
			VDOUT_FMT_REG_FMT_HV_TOTAL),
		...
		WriteREG32Msk((vdout_fmt->hw_fmt_base +
			VDOUT_FMT_REG_FMT_HV_TOTAL),
			(fgADJ_T << 31), (0x1 << 31));
	FMT_LOG_I("%s, enable:%d, fgADJ_T:%d, value:0x%x\n",
		__func__, enable, fgADJ_T, value);
}

void _fmt_set_vrr_trigger_start_line(uint32_t start_line)
{
	struct vdout_fmt_t *vdout_fmt = NULL;

	FMT_LOG_I("%s, start_line:0x%x\n",
		__func__, start_line);
...
int fmt_hal_sof_grp0_shadow_enable(bool enable)
{
	...
	if ((vdout_fmt == NULL) || (vdout_fmt->status != FMT_STA_USED))
		FMT_LOG_D("grp0 enable canot get valid vout reg base\n");

	FMT_LOG_D("%s, enable=%d\n", __func__, enable);
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP0_SHADOW_ENABLE),
			(0x1 << 1), (0x1 << 1));
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP0_SHADOW_ENABLE),
			(enable << 0), (enable << 0));
	...
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP0_SHADOW_UPDATE),
			(0x1 << 0), (0x1 << 0));

	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP0_BASE_E0),
			(0xffffffff << 0), (0xffffffff << 0));

	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP0_BASE_E4),
			(0xffffffff << 0), (0xffffffff << 0));

	return 0;
}

void fmt_hal_sof_grp0_shadow_update(void)
{
	struct vdout_fmt_t *vdout_fmt = NULL;
	...
	if ((vdout_fmt == NULL) || (vdout_fmt->status != FMT_STA_USED))
		FMT_LOG_D("grp0 update canot get valid vout reg base\n");
...
int fmt_hal_sof_grp1_shadow_enable(bool enable)
{
	...
	if (vdout_fmt == NULL)
		FMT_LOG_D("grep1 canot get valid vout reg base\n");

	FMT_LOG_D("%s, enable=%d\n", __func__, enable);
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP1_SHADOW_ENABLE),
			(0x1 << 1), (0x1 << 1));
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP1_SHADOW_ENABLE),
			(enable << 0), (enable << 0));
	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP1_SHADOW_UPDATE),
			(0x1 << 0), (0x1 << 0));

	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP1_BASE_E0),
			(0xffffffff << 0), (0xffffffff << 0));

	WriteREG32Msk((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_GRP1_BASE_E4),
			(0xffffffff << 0), (0xffffffff << 0));
...
void fmt_hal_sof_grp1_shadow_update(void)
{
	...
	if (vdout_fmt == NULL)
		FMT_LOG_E("grp1 shadow update canot get valid vout reg base\n");
...
int fmt_hal_set_sof(enum FMT_SOF_TYPE type_start,
	enum FMT_SOF_TYPE type_end, int sof_start, int sof_end)
{
	struct vdout_fmt_t *vdout_fmt = NULL;
	...
	if (vdout_fmt == NULL)
		FMT_LOG_I("canot get valid vout reg base\n");

	FMT_LOG_D("fmt sof type=%d, type_end=%d\n",
		type_start, type_end);
	FMT_LOG_D("sof_start=0x%x, sof_end=0x%x\n",
		sof_start, sof_end);
	switch (type_start) {
	case FMT_SOF_0_M_VDO_MAIN_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_0_M_VDO_MAIN_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_0_M_VDO_MAIN_END), sof_end);
		break;
	case FMT_SOF_1_M_VDO_AUX_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_1_M_VDO_AUX_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_1_M_VDO_AUX_END), sof_end);
		break;
	case FMT_SOF_2_M_FE_FIFO_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_2_M_FE_FIFO_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_2_M_FE_FIFO_END), sof_end);
		break;
	case FMT_SOF_3_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_3_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_3_END), sof_end);
		break;
	case FMT_SOF_4_M_FILM_GRAIN_1_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_4_M_FILM_GRAIN_1_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_4_M_FILM_GRAIN_1_END), sof_end);
		break;
	case FMT_SOF_5_M_FILM_GRAIN_2_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_5_M_FILM_GRAIN_2_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
	...
	case FMT_SOF_7_S_VDO_MAIN_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_7_S_VDO_MAIN_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_7_S_VDO_MAIN_END), sof_end);
		break;
	case FMT_SOF_8_S_VDO_AUX_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_8_S_VDO_AUX_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_8_S_VDO_AUX_END), sof_end);
		break;
	case FMT_SOF_9_S_FE_FIFO_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_9_S_FE_FIFO_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_9_S_FE_FIFO_END), sof_end);
		break;
	case FMT_SOF_10_S_FILM_GRAIN_0_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_10_S_FILM_GRAIN_0_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_10_S_FILM_GRAIN_0_END), sof_end);
		break;
	case FMT_SOF_11_S_FILM_GRAIN_1_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_11_S_FILM_GRAIN_1_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_11_S_FILM_GRAIN_1_END), sof_end);
		break;
	case FMT_SOF_12_S_FILM_GRAIN_2_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_12_S_FILM_GRAIN_2_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
	...
	case FMT_SOF_14_UHD_OSD_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_14_UHD_OSD_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_14_UHD_OSD_END), sof_end);
		break;
	case FMT_SOF_15_UHD_FE_FIFO_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_15_UHD_FE_FIFO_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
	...
	case FMT_SOF_17_FHD_OSD_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_17_FHD_OSD_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_17_FHD_OSD_END), sof_end);
		break;
	case FMT_SOF_18_FHD_FE_FIFO_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_18_FHD_FE_FIFO_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
	...
	case FMT_SOF_20_DISP_MIX_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_20_DISP_MIX_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
	...
	case FMT_SOF_22_BE_FIFO_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_22_BE_FIFO_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_22_BE_FIFO_END), sof_end);
		break;
	case FMT_SOF_23_DISP_MUNULOAD_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_23_DISP_MUNULOAD_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_23_DISP_MUNULOAD_END), sof_end);
		break;
	case FMT_SOF_24_MMSYS_MUNULOAD_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_24_MMSYS_MUNULOAD_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_24_MMSYS_MUNULOAD_END), sof_end);
		break;
	case FMT_SOF_25_VIDEO_MARK_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_25_VIDEO_MARK_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_25_VIDEO_MARK_END), sof_end);
		break;
	case FMT_SOF_26_R2R_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_26_R2R_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_26_R2R_END), sof_end);
		break;
	case FMT_SOF_27_M_FILM_GRAIN_3_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_27_M_FILM_GRAIN_3_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_27_M_FILM_GRAIN_3_END), sof_end);
		break;
	case FMT_SOF_28_M_FILM_GRAIN_4_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_28_M_FILM_GRAIN_4_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_28_M_FILM_GRAIN_4_END), sof_end);
		break;
	case FMT_SOF_29_S_FILM_GRAIN_3_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_29_S_FILM_GRAIN_3_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_29_S_FILM_GRAIN_3_END), sof_end);
		break;
	case FMT_SOF_30_S_FILM_GRAIN_4_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_30_S_FILM_GRAIN_4_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_30_S_FILM_GRAIN_4_END), sof_end);
		break;
	case FMT_SOF_31_STA:
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_FMT_SOF_31_STA), sof_start);
		WriteREG32((vdout_fmt->hw_fmt_base +
			DISP_FMT_REG_FMT_SOF_31_END), sof_end);
		break;
	default:
		FMT_LOG_I("error type=%d %d\n", type_start, type_start);
		break;
	}
...
int32_t fmt_hal_set_pixel_len(uint32_t fmt_id, uint32_t pixel_len)
{
	...
	FMT_FUNC();
	FMT_LOG_D("fmt_id: %d, pixel_len: %d\n", fmt_id, pixel_len);
				 ...
				 uint32_t hsfactor, uint32_t enable)
{
	...
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_PIXEL_LENGTH,
		       true);
	_fmt_write_reg(fmt_id, (void *)disp_fmt,
		       DISP_FMT_REG_HORIZONTAL_SCALING, true);
...
int32_t fmt_hal_dsd_enable(uint32_t fmt_id, struct fmt_dsd_scl_info *info)
{
	...
	FMT_FUNC();
	FMT_LOG_I(
		"fmt_id: %d, src xy(%d,%d) wh(%d,%d), dst xy(%d,%d) wh(%d,%d)\n",
		fmt_id, info->src_x, info->src_y, info->src_w, info->src_h,
	...
	/*0x6c H scale factor  Hdownscale need change scale factor by srcw and*/
	/* outw*/
	disp_fmt->sw_fmt_base->rField.fgHDWN_EN = 1;
	disp_fmt->sw_fmt_base->rField.fgFULL_FIFO = 1;
	disp_fmt->sw_fmt_base->rField.fgHDWN_444 = 1;
	disp_fmt->sw_fmt_base->rField.u2HDWN_FAC =
		(info->src_w > info->out_w)
			? (0x1000 * info->src_w / info->out_w)
			: 0x1000;
	...
	case DSD_4K_TO_720P:
		...
		disp_fmt->sw_fmt_base->rField.u2HDWN_HBEG_14_13 =
			((start * ratio - pixel_advance1 * ratio) & 0x6FFF) >>
			13;
		/*7c_end = c0_start+active w * 8*/
		disp_fmt->sw_fmt_base->rField.u2HDWN_HO_END =
			(start * ratio - pixel_advance1 * ratio +
			 1280 * ratio) &
			0x1FFF;
		...
		info->out_h_start =
			disp_fmt->sw_fmt_base->rField.u2DowndScaleStart;
		info->out_h_end =
			disp_fmt->sw_fmt_base->rField.u2DowndScaleEnd - 5;
	...
	case DSD_4K_TO_480P:
		...
		disp_fmt->sw_fmt_base->rField.u2HDWN_HO_END =
			(start * ratio - pixel_advance1 * ratio + 720 * ratio) &
			0x1FFF;
		disp_fmt->sw_fmt_base->rField.u2HDWN_HEND_14_13 =
			((start * ratio - pixel_advance1 * ratio +
			  720 * ratio) &
	...
	/*0x74*/
	disp_fmt->sw_fmt_base->rField.u2HDWN_VOBGN =
		disp_fmt->sw_fmt_base->rField.u2VOACTBGN;
	disp_fmt->sw_fmt_base->rField.u2HDWN_VOEND =
		disp_fmt->sw_fmt_base->rField.u2VEACTEND;
	/*0x78*/
	disp_fmt->sw_fmt_base->rField.u2HDWN_VEBGN =
		disp_fmt->sw_fmt_base->rField.u2VEACTBGN;
	disp_fmt->sw_fmt_base->rField.u2HDWN_VEEND =
		disp_fmt->sw_fmt_base->rField.u2VEACTEND;
	/*0xb0, reset factor*/
	...
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_DOWN_RNG_OUTPUT,
		       true);
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_DOWN_CONFIG,
		       true);
	...
	enum HDMI_VIDEO_RESOLUTION resolution)
{
	...
	disp_fmt->sw_fmt_base->rField.u2DSDHSYN_DELAY =
		disp_fmt->sw_fmt_base->rField.u2HSYN_DELAY;
	disp_fmt->sw_fmt_base->rField.u2DSDVSYN_DELAY =
		disp_fmt->sw_fmt_base->rField.u2VSYN_DELAY;
	disp_fmt->sw_fmt_base->rField.fgDSDADJ_FWD =
		disp_fmt->sw_fmt_base->rField.fgADJ_FWD;
	...
	struct fmt_hd_scl_info *info)
{
	...
	disp_fmt->sw_fmt_base->rField.u2HDWN_VOBGN = info->out_y_even_pos;
	disp_fmt->sw_fmt_base->rField.u2HDWN_VOEND = info->out_y_even_pos_e;
	disp_fmt->sw_fmt_base->rField.u2HDWN_VEBGN = info->out_y_odd_pos;
	disp_fmt->sw_fmt_base->rField.u2HDWN_VEEND = info->out_y_odd_pos_e;
	disp_fmt->sw_fmt_base->rField.u2HDWN_HO_BGN = info->dispfmt_out_h_start;
	disp_fmt->sw_fmt_base->rField.u2HDWN_HO_END = info->dispfmt_out_h_end;
...
int32_t fmt_hal_h_down_enable(uint32_t fmt_id, struct fmt_hd_scl_info *info)
{
	...
	if (!info)
		return FMT_PARAR_ERR;
	out_v_start = info->out_y_odd_pos;
	out_v_end = info->out_y_odd_pos_e;

	FMT_LOG_D(
		"[DOWNSCL]fmt_id: %d, src_w: %d, out_w: %d, x_pos: %d, out_x_oft: %d\n",
		fmt_id, info->src_w, info->out_w, info->in_x_pos,
		info->in_x_pos);
	FMT_LOG_D(
		"[DOWNSCL]odd_s: %d, odd_e: %d, even_s: %d, even_e: %d, res: %d\n",
		info->out_y_odd_pos, info->out_y_odd_pos_e,
		info->out_y_even_pos, info->out_y_even_pos_e, info->res);
	...
	case HDMI_VIDEO_720x576p_50Hz:
		...
		out_h_end = info->in_x_pos + info->out_x_offset +
			    (info->out_w - 1) + 3;
	...
	case HDMI_VIDEO_3840x2160P_50HZ:
		in_h_start = info->in_x_pos + 5;
		in_h_end = info->in_x_pos + (info->src_w - 1) + 6;
		/* this delay value is got from hardware simulation */
		out_h_end = info->in_x_pos + (3840 - 1) + H_DOWNSCALE_PIPELINE_DELAY_UHD;
		out_h_start = out_h_end - (info->out_w - 1);
	...
	FMT_LOG_D(
		"[DOWNSCL]in_h_start: %d, in_h_end: %d, out_h_start: %d, out_h_end: %d\n",
		in_h_start, in_h_end, out_h_start, out_h_end);

	if (out_h_start >= out_h_end ||
	    info->out_y_even_pos >= info->out_y_even_pos_e ||
	    info->out_y_odd_pos >= info->out_y_odd_pos_e ||
	    info->out_w >= info->src_w || info->out_w == 0 ||
	    info->src_w == 0 || factor > 0xFFFF /*|| info->out_w & 1*/ ||
	    info->src_w > in_h_limit) {
		FMT_LOG_I("parameter error\n");
		_fmt_mutex_unlock();
		return FMT_PARAR_ERR;
	}

	info->out_y_odd_pos = out_v_start;
	info->out_y_odd_pos_e = out_v_end;
	info->out_y_even_pos = out_v_start;
	info->out_y_odd_pos_e = out_v_end;

	_fmt_set_in_region(fmt_id, in_h_start, in_h_end, info->out_y_odd_pos,
			   info->out_y_odd_pos_e, info->out_y_even_pos,
			   info->out_y_odd_pos_e);
	...
	info->vdout_out_h_end =
		info->in_x_pos + info->out_x_offset + (info->out_w - 1);
	...
	/* Ref DE's comment :
	 * for main video :
	 * if video <= 1080, not full fifo = 0, use full fifo
	 * if video > 1080, not full fifo = 1, not use full fifo
	 * for sub video :
	 * if video < 1080, not full fifo = 0, use full fifo
	 * if video >= 1080, not full fifo = 1, not use full fifo
	 */
			   ...
			   uint32_t out_h)
{
	...
	if ((out_w > src_w) || (out_w == 0) || (src_w == 0) ||
	    (factor > 0xFFFF) || (out_w & 1) || (src_w > in_h_limit) ||
...
int32_t fmt_hal_set_output_444(uint32_t fmt_id, bool is_444)
{
	...
	_fmt_write_reg(fmt_id, (void *)disp_fmt, DISP_FMT_REG_FORMAT_422_CTRL,
		       true);
...
/*show color func is for DGI, now saved this api temply.*/
/*the api which not mix or mix sub video and two layers osd*/
/*will replace by display mix hw*/
...
int32_t fmt_hal_set_dummy(uint32_t fmt_id, bool en)
{
	...
	disp_fmt->sw_fmt_base->rField.add_dummy = en;
...
void fmt_hal_config_sec_register(uint32_t fmt_id, struct cmdq_pkt *pkt)
{
	...
	if (fmt_id > VDOUT_FMT) {
		FMT_LOG_E("%s invalid fmt_id%d\n",
			__func__, fmt_id);
		return;
		}
	if (pkt == NULL) {
		FMT_LOG_E("%s invalid pkt%p\n",
			__func__, pkt);
		return;
		}
	_fmt_mutex_lock();
	...
	for (idx = 0; idx < HAL_DISP_FMT_MAIN_REG_NUM; idx++) {
		if (IS_REG_SET(*disp_fmt->reg_mode, idx))
			cmdq_pkt_write_value_addr(pkt,
			(fmt.disp_fmt_phy_base[fmt_id] + idx * 4),
			disp_fmt->sw_fmt_base->au4Reg[idx],
			0xffffffff);
	}
}
#endif
...
int32_t fmt_hal_clock_on_off(uint32_t fmt_id, bool on)
{
	...
	if (fmt_id == VDOUT_FMT) {
		...
		if (id == DISP_FMT1) {
			...
			if (on) {
				/*need to open disp_main pm before enable
				 * disp_sub pm
				 */
				disp_clock_enable(DISP_CLK_DISPFMT3, on);
...
int32_t fmt_hal_init(struct fmt_init_pararm *param)
{
...
#ifdef DISP_GCE_SUPPORT
	fmt.disp_fmt_phy_base[0] = 0x15001000;
	fmt.disp_fmt_phy_base[1] = 0x15008000;
#endif

	fmt.vdout_fmt.sw_fmt_base =
		kmalloc(sizeof(union vdout_fmt_union_t), GFP_KERNEL);
	fmt.sub_vdout_fmt.sw_fmt_base =
		kmalloc(sizeof(union vdout_fmt_union_t), GFP_KERNEL);
	fmt.main_disp_fmt.sw_fmt_base =
		kmalloc(sizeof(union disp_fmt_union_t), GFP_KERNEL);
	fmt.sub_disp_fmt.sw_fmt_base =
		kmalloc(sizeof(union disp_fmt_union_t), GFP_KERNEL);
	...
	if (fmt.vdout_fmt.reg_mode == 0 || fmt.main_disp_fmt.reg_mode == 0 ||
	...
	fmt.sub_vdout_fmt.hw_fmt_base =
		fmt.vdout_fmt_reg_base + SUB_VDOUT_FMT_OFFSET;
	_fmt_reset_fmt_info(DISP_FMT_MAIN);
	_fmt_reset_fmt_info(DISP_FMT_SUB);
	...
	spin_lock_init(&fmt.main_disp_fmt.fmt_shadow_lock);
	spin_lock_init(&fmt.sub_disp_fmt.fmt_shadow_lock);

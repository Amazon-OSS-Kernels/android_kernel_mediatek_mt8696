...
static atomic_t disp_adl_wakeup_thread;
...
void disp_adl_thread_wakeup(uint8_t path)
{
	...
	atomic_set(&disp_adl_wakeup_thread, 1);
...
static int disp_adl_routine(void *data)
{
	...
	while (!kthread_should_stop()) {
		wait_ret = wait_event_interruptible(disp_adl_thread_wq,
			atomic_read(&disp_adl_wakeup_thread));
		if (wait_ret)
			adl_info("wait adl routine error\n");
		atomic_set(&disp_adl_wakeup_thread, 0);
...
int adl_client_ctx_init(enum ADL_CLIT_REG_ID clit)
{
	...
	adl_info("adl clt[%d] 0x%x 0x%p 0x%llx 0x%p 0x%x\n", clit,
	...
	void **va, dma_addr_t *pa)
{
	...
	adl_info("clt[%d] get buffer 0x%p 0x%llx\n",
		clit, *va, *pa);
	return 0;
}
...
int disp_filmg_config_adl_table(struct adl_src_tbl *adl_tbl)
{
	...
	adl_info("clt[%d][%d] va:0x%p mva:0x%llx\n", clit, hw_client,
...
int disp_adl_main(void)
{
	...
	for (hw_clt = ADL_DSYS_CLITA; hw_clt < ADL_CLIT_REG_MAX; hw_clt++) {
		...
		adl_dbg("clt[%d][%d] va:0x%p mva:0x%llx\n", clit, hw_clt,
			dst_va, dst_mva);
		switch (clit) {
		case DV_ADL_V_MAIN:
			//memset(dst_va, 0, ADL_LUT_BUFFER_SIZE);
...
int disp_adl_client_status(void)
{
	...
	for (i = 0 ; i < ADL_CLIT_REG_MAX; i++) {
		...
		adl_printf("mem 0x%p 0x%llx 0x%x 0x%p\n",

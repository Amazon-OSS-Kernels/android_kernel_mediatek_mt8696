...
struct mtk_disp_log {
	...
	struct mmp_disp_event disp_mmp_event[MMP_EVENTS_MAX];
#endif
};
...
enum DISP_LOG_MASK {
	...
	DISP_LOG_VRR_MASK = (1 << 7),
	DISP_LOG_VRR_IRQ = (1 << 8),
};

#define DISP_LOG_LEVEL_DEBUG1 (DISP_LOG_MSG_MASK)
#define DISP_LOG_LEVEL_DEBUG2 (DISP_LOG_MSG_MASK | DISP_LOG_FUNC_MASK)
#define DISP_LOG_LEVEL_DEBUG3                                                  \
	(DISP_LOG_MSG_MASK | DISP_LOG_FUNC_MASK | DISP_LOG_SYNC_MASK |         \
	 DISP_LOG_MMP_MASK)
#define DISP_LOG_LEVEL_DEBUG4                                                  \
	(DISP_LOG_MSG_MASK | DISP_LOG_FUNC_MASK | DISP_LOG_IRQ_MASK |          \
	 DISP_LOG_SYNC_MASK | DISP_LOG_MMP_MASK | DISP_LOG_MGR_MASK |          \
	 DISP_LOG_HW_MASK)

#define DISP_LOG_LEVEL_DEBUG5 (DISP_LOG_VRR_MASK)

#define DISP_LOG_LEVEL_DEBUG6 (DISP_LOG_VRR_IRQ)
...
#define DISP_LOG_I(fmt, args...)                                               \
	dev_info(disp_log.dev, "[" LOG_TAG "]" fmt, ##args)
#define DISP_LOG_N(fmt, args...)                                               \
	dev_notice(disp_log.dev, "[" LOG_TAG "]" fmt, ##args)
#define DISP_LOG_W(fmt, args...)                                               \
	dev_warn(disp_log.dev, "[" LOG_TAG "]warn: " fmt, ##args)
#define DISP_LOG_E(fmt, args...)                                               \
	dev_err(disp_log.dev, "[" LOG_TAG "]error: " fmt, ##args)
...
#define DISP_LOG_VRR(fmt, args...)                          \
	do {                                                    \
		if (DISP_LOG_VRR_MASK & disp_log.mgr_level)         \
			DISP_LOG_I(fmt, ##args);						\
	} while (0)

#define DISP_LOG_VRR_IRQ(fmt, args...)                      \
	do {                                                    \
		if (DISP_LOG_VRR_IRQ & disp_log.mgr_level)			\
			DISP_LOG_I(fmt, ##args);						\
	} while (0)
...
#define DISP_MMP_STRING(event, str, data1, data2)                              \
	mmprofile_log_meta_string_ex(disp_log.mmp_event[event].value,          \
				     MMPROFILE_FLAG_PULSE, data1, data2, str)
#define DISP_MMP_STRUCT(event, pStruct, struct_type)                           \
	{                                                                      \
		struct mmp_metadata_structure_t meta_data;                     \
		meta_data.data1 = 0;                                           \
		meta_data.data2 = 0;                                           \
		strcpy(meta_data.struct_name, #struct_type);                   \
		meta_data.struct_size = sizeof(struct_type);                   \
		meta_data.p_data = (void *)(pStruct);                          \
		mmprofile_log_meta_structure(                                  \
					 disp_log.disp_mmp_event[event].value, \
					 MMPROFILE_FLAG_PULSE,             \
					 &meta_data);                      \
	}

#else
static inline void DISP_MMP(enum MMP_DISP_EVENT event,
			    enum MMP_DISP_EVENT_TYPE type, uint64_t data1,
			    uint64_t data2)
{
}

static inline void DISP_MMP_DUMP(enum MMP_DISP_EVENT event, unsigned long addr,
				 uint32_t size, enum DISP_HW_COLOR_FORMAT fmt,

...
struct fb_info *fb_info_for_buffer_free;
				  ...
				  struct fb_info *info)
{
	...
	if (ret != 0) {
		MTKFB_INFO("%s get hw mgr info fail\n", __func__);
		if (pConfig != NULL)
			vfree(pConfig);
		pConfig = NULL;
		return -1;
	...
	pConfig->buffer_info[0].src_fmt =
		(var->blue.offset == 0) ? DISP_HW_COLOR_FORMAT_BGRA8888
	...
	pConfig->buffer_info[0].crop.width = var->xres;
	pConfig->buffer_info[0].crop.height = var->yres;
...
static int mtkfb_check_var(struct fb_var_screeninfo *var, struct fb_info *fbi)
{
	...
	MTKFB_LOG_DBG(
		"%s, xres=%u, yres=%u, xres_virtual=%u, yres_virtual=%u,\n",
		__func__, var->xres, var->yres, var->xres_virtual,
		var->yres_virtual);
	MTKFB_LOG_DBG("xoffset=%u, yoffset=%u, bits_per_pixel=%u\n", var->xoffset,
	...
	MTKFB_LOG_DBG("fbdev->fb_size_in_byte=0x%08lx\n", fbdev->fb_size_in_byte);
	...
	if (bpp == 16) {
		...
		ASSERT((var->red.offset == 16 + var->blue.offset) ||
		       (var->blue.offset == 16 + var->red.offset));
...
static void mtkfb_blank_suspend(struct fb_info *info)
{
	...
	const struct HDMI_DRIVER *hdmitxdev = NULL;

	mtkfb_dev = (struct mtkfb_device *)info->par;
	MTKFB_LOG("enter early_suspend\n");

	hdmitxdev = HDMI_GetDriver();
	if (hdmitxdev != NULL) {
		hdmiret = hdmitxdev->suspend();
		/* keep recovery mode display power on, same as kara */
		if (hdmiret == HDMI_INTERNAL_SUSPEND_IN_RECOVERY_OK)
...
static void mtkfb_blank_resume(struct fb_info *info)
{
	...
	/* change back to karat defalut status after reocvery mode resume */
	if (hdmiret == HDMI_INTERNAL_RESUME_IN_RECOVERY_OK)
...
static int mtkfb_allocate_framebuffer(phys_addr_t pa_start, phys_addr_t pa_end,
				      ...
				      struct device *dev)
{
	...
	*va = (unsigned long)__va(pa_start);
	MTKFB_LOG_DBG("disphal_allocate_fb, pa=%pa, va=0x%lx\n", &pa_start, *va);

	bus_addr = (unsigned long)phys_to_dma(dev, pa_start);

	MTKFB_LOG_DBG("disphal_allocate_fb, mva=0x%lx\n", bus_addr);

	*mva = bus_addr;

	MTKFB_LOG_DBG("disphal_allocate_fb, bus=0x%lx\n", *mva);
...
void mtkfb_free_reserved_fb(void)
{
	...
	if (framebuffer_base) {
		free_reserved_area(
			(void *)__va(framebuffer_base),
			(void *)__va(framebuffer_base + framebuffer_size - 1),
			-1, "fb");
		framebuffer_base = 0;
		//after setting inout 10 buffers, kernel will free this fb buffer, but user
		//still can get this fb buffer, user will read/wirte fb buffer via screen
		//base address and mmap fb buffer via smem_start address and smem_len.
		// so it sets fb pa, va address and len to null when free fb buffer.
		if (fb_info_for_buffer_free) {
			fb_info_for_buffer_free->screen_base = 0;
			fb_info_for_buffer_free->fix.smem_len = 0;
			fb_info_for_buffer_free->fix.smem_start = 0;
		}
	}
}
...
static void _parse_tag_videolfb(struct device *dev)
{
	...
	MTKFB_INFO("[DT][videolfb]ic_version  = %d\n", g_ic_version);
	MTKFB_INFO("[DT][videolfb]qms  = %d\n", g_qms);

	is_videofb_parse_done = 1;
}

#ifdef CONFIG_HDMI_BLACK
static int mtkfb_suspend_notifier(struct notifier_block *nb,
	unsigned long action, void *data)
{
	int ret = 0;

	if (action == PM_SUSPEND_PREPARE) {
		/* this flag is necessary to avoid execute      */
		/* disable behavior many times                   */
		/* and, in late resume flow, it will be useful  */
		if (is_display_off)
			return 0;
		is_display_off = true;
		ret = disp_hw_mgr_deep_suspend();
		if (ret != 0)
			MTKFB_INFO("mtkfb suspend failed\n");
	}
	return NOTIFY_DONE;
}
#endif
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	state++;
#ifdef CONFIG_MTK_IOMMU
	max_larb_num = of_count_phandle_with_args(pdev->dev.of_node,
						  "mediatek,larb", NULL);
	for (i = 0; i < max_larb_num; i++) {
		larb_node =
			of_parse_phandle(pdev->dev.of_node, "mediatek,larb", i);
		if (!larb_node) {
			ret = -EINVAL;
			goto cleanup;
		}
	...
	/*add for deep suspend*/
#ifdef CONFIG_HDMI_BLACK
	pm_notifier(mtkfb_suspend_notifier, 0);
#endif

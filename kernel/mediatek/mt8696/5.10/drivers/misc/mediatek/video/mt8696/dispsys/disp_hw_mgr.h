...
#define DISP_QMS_SUPPORT

#ifdef DISP_QMS_SUPPORT
#define VRR_PREC 1000
#endif
...
struct vrr_mode {
	bool vrr_en;
	uint8_t vrr_mode; // 0-adaptive, 1 force
	uint8_t next_tfr; //src tfr
	uint32_t mvrr;
	uint32_t mvrr_delat_limit; // for cineamvrr mvrrdelta enable case
	int mvrr_fraction_value; //mvrr fraction value, decide use mvrr or mvrr+1 to min mvrrerr
	int mvrr_f_ratio; //mvrr floor ratio, for fraction mvrr
	int mvrr_c_ratio; //mvrr ceiling ration, for fraction mvrr
	int mvrr_delat_step; // how many step mvrr increase need, for cinemavrr
	uint32_t trigger_start_line;
	uint32_t start_line_mode_vtotal;
	uint32_t brr_vtotal;
	uint32_t current_vtotal;
	struct mutex vrr_mutex;
};
#endif
...
#endif
#ifdef DISP_QMS_SUPPORT
	enum DISP_MODULE_ENUM vrr_enable[DISP_MODULE_NUM];
#endif
...
struct tee_mmu {
	...
	struct sg_table *sgt;
	union disp_hw_dma_buffer buffer_addr;
};
...
#endif
#ifdef DISP_QMS_SUPPORT
	int (*vrr_enable)(struct vrr_mode *vrr_info);
#endif
...
struct disp_hw_manager {
	...
	#ifdef DISP_QMS_SUPPORT
	struct task_struct *vrr_task;
	wait_queue_head_t vrr_wq;
	atomic_t vrr_flag;
	#endif
...
#ifdef DISP_QMS_SUPPORT
extern uint64_t fmt_active_irq_cnt;
extern bool vrr_start_line_mode;
extern bool vrr_timer_check;
extern uint32_t vdp_qms_fps[2];
extern uint32_t g_qms;
...
extern bool disp_hdr_prejudge_dovi_output(bool is_dovi,
bool is_hdr10p);
#endif
#endif

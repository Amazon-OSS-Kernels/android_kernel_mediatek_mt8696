...
const char *disp_get_clk_name(enum DISP_HW_CLK disp_clk)
{
	...
	case DISP_CLK_DGI:
		return "dgi";
	case DISP_CLK_FAKE_ENG:
		return "fake_eng";
	case DISP_CLK_FMT_TG:
		return "fmt_tg";
	case DISP_CLK_OSD_FHD:
		return "osd_fhd";
	case DISP_CLK_OSD_FBDC:
		return "osd_fbdc";
	case DISP_CLK_OSD_FHD_SIM2MIU:
		return "osd_fhd_sim2miu";
	case DISP_CLK_FHD_GFX_FE_FIFO:
		return "fhd_gfx_fe_fifo";
	case DISP_CLK_FHD_HDR_GFX_FE:
		return "fhd_hdr_gfx_fe";
	case DISP_CLK_HDR_ADL:
		return "hdr_adl";
	case DISP_CLK_DISP_MIX:
		return "disp_mix";
	case DISP_CLK_HDR_VDO_BE:
		return "hdr_vdo_be";
	case DISP_CLK_HDR_BE_FIFO:
		return "hdr_be_fifo";
	case DISP_CLK_HDMI_TX_PIXEL:
		return "hdmi_tx_pixel";
	case DISP_CLK_HDMI_TX_PIXEL_D2:
		return "hdmi_tx_pixel_d2";


	case DISP_CLK_HDMI_METADATA:
		return "hdmi_metadata";
	case DISP_CLK_W2D:
		return "w2d";
	case DISP_CLK_P2I:
		return "p2i";
	...
	case DISP_CLK_OSD_UHD:
		return "osd_uhd";
	case DISP_CLK_UHD_GFX_FE_FIFO:
		return "uhd_gfx_fe_fifo";
	case DISP_CLK_UHD_HDR_GFX_FE:
		return "uhd_hdr_gfx_fe";

	case DISP_CLK_VDO3:
		return "vdo3";
	case DISP_CLK_DISPFMT3:
		return "dispfmt3";
	case DISP_CLK_M_VDO_FE_FIFO:
		return "m_vdo_fe_fifo";
	case DISP_CLK_M_FILM_GRAIN:
		return "m_film_grain";
	case DISP_CLK_M_HDR_VDO_FE:
		return "m_hdr_vdo_fe";
	case DISP_CLK_M_VDO_FE_ADL:
		return "m_vdo_fe_adl";
	case DISP_CLK_R2R:
		return "r2r";
	...
	case DISP_CLK_DISPFMT4:
		return "dispfmt4";
	case DISP_CLK_S_VDO_FE_FIFO:
		return "s_vdo_fe_fifo";
	case DISP_CLK_S_FILM_GRAIN:
		return "s_film_grain";
	case DISP_CLK_S_HDR_VDO_FE:
		return "s_hdr_vdo_fe";
...
static enum DISP_PM_TYPE disp_get_pm_type(enum DISP_HW_CLK hw_clk)
{
	...
	case DISP_CLK_VDOIN:
	case DISP_CLK_VM:
		type = DISP_PM_MMSYS_COM;
		break;

	case DISP_CLK_OSD_UHD:
	case DISP_CLK_UHD_GFX_FE_FIFO:
	case DISP_CLK_UHD_HDR_GFX_FE:
		type = DISP_PM_OSD_UHD;
		break;
...
static int disp_enable_disable_pm(enum DISP_HW_CLK hw_clk, bool en)
{
	...
	if (en && pm_info[type].pm_clk_cnt == 0 && pm_info[type].pm_dev) {
		...
		DISP_LOG_D("enable pm= %d ret %d\n", type, ret);
	} else if (!en && pm_info[type].pm_clk_cnt == 0 &&
		   pm_info[type].pm_dev) {
		DISP_LOG_D("disbale pm= %d\n", type);
...
{
	if (!dev || larb_id >= DISP_SMI_LARB_NUM || larb_id < 0) {
...
int disp_clock_smi_larb_en(enum DISP_SMI_LARB_ID larb_id, bool en)
{
	if (larb_id >= DISP_SMI_LARB_NUM) {
		DISP_LOG_E("%s id err, larb_id=%d\n", __func__, larb_id);
		return -1;
	}
	if (!smi_larb_dev[larb_id]) {
		DISP_LOG_E(
			"%s can not find larb dev,id=%d\n", __func__, larb_id);
		return -1;
	}
	DISP_LOG_D("%s id=%d, en=%d\n", __func__, larb_id, en);

	if (en)
		pm_runtime_get_sync(smi_larb_dev[larb_id]);
		/*mtk_smi_larb_get(smi_larb_dev[larb_id]);*/
	else
		pm_runtime_put_sync(smi_larb_dev[larb_id]);
...
int disp_clock_set_pll(enum DISP_HW_CLK hw_clk, unsigned int rate)
{
	...
	DISP_LOG_I("%s %s - %d\n", __func__, disp_get_clk_name(hw_clk),
		   rate);
...
static int mtk_disp_clk_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		DISP_LOG_D("%s get clk fail, ret= 0x%x\n", __func__, ret);
		return ret;
	}

	DISP_LOG_D("%s, pm type=%d\n", __func__, type);
	...
	if (type == DISP_PM_MMSYS_TOP) {
		/*
		 *
		 * for mmsys_top power, when it probes, power domain will
		 * close this power, but it hopes not to be turned off from lk
		 * to kernel stage or display will work abnomally. so turn on
		 * vdout_sys clock at probing stage, it will turn on
		 * mmsys_top power. And it did not need to turn on vdout_sys
		 * clock at pmx inited.
		 */
		disp_clock_enable(DISP_CLK_VDOUT_SYS, true);
...
static struct platform_driver mtk_disp_clk_driver = {
	...
	.driver = {
		.name = "mtk-disp-clk", .of_match_table = mtk_disp_clk_of_ids,
	} };

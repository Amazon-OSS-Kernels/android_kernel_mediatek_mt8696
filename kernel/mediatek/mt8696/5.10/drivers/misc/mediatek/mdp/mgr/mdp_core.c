...
#define MAX_MDP_TASK 5

struct global_mdp_task_struct global_mdp_task;
static int mdp_task_taskcount;
...
/* for MMP use. */
enum MDP_TASK_STATUS mdp_core_copy_mmp_data(
	struct mdp_task_struct *pTask,
	struct mmp_mdp_task_struct *pMmpData)
{
#if IS_ENABLED(CONFIG_MTK_CLI_DEBUG_SUPPORT)
	...
	memcpy(pMmpData->taskString, pTask->mdp_command.taskString,
		sizeof(pTask->mdp_command.taskString));
	memcpy(&pMmpData->mdp_command,
		&pTask->mdp_command, sizeof(pTask->mdp_command));
...
enum MDP_TASK_STATUS mdp_core_set_mmp_tag(
	struct mdp_task_struct *pTask, enum MMP_DISP_EVENT event)
{
#if IS_ENABLED(CONFIG_MTK_CLI_DEBUG_SUPPORT)
	if (!mdp_cli_get()->enable_mmp_debug)
		return MDP_TASK_STATUS_OK;
...
void mdp_core_init_global_mdp_task(void)
{
	...
	mutex_init(
		&global_mdp_task.acquire_temp_buffer_task_list.mutex_lock);
	INIT_LIST_HEAD(
		&global_mdp_task.acquire_temp_buffer_task_list.list);
	...
	init_waitqueue_head(
		&global_mdp_task.mdp_core_wait_imgresz_hw_idle_queue);
	init_waitqueue_head(
		&global_mdp_task.mdp_core_wait_imgresz_ticket_queue);
	init_waitqueue_head(
		&global_mdp_task.mdp_core_wait_task_done_queue);
	init_waitqueue_head(
		&global_mdp_task.mdp_core_start_wait_task0_queue);
	init_waitqueue_head(
		&global_mdp_task.mdp_core_start_wait_task1_queue);
	init_waitqueue_head(
		&global_mdp_task.mdp_core_wait_done_task0_queue);
	init_waitqueue_head(
		&global_mdp_task.mdp_core_wait_done_task1_queue);
	init_waitqueue_head(
...
struct mdp_task_struct *mdp_core_get_mdp_task(void)
{
	...
	if (list_empty(&global_mdp_task.free_task_list.list)) {
		...
		if (mdp_task_taskcount > MAX_MDP_TASK) {
			MDP_ERR("mdp_task_taskcount >MAX_MDP_TASK failed\n");
			return NULL;
		}
	...
	pTask = list_first_entry(&global_mdp_task.free_task_list.list,
 ...
 * release task fence
 */
enum MDP_TASK_STATUS mdp_core_put_mdp_task(
	struct mdp_task_struct *pTask)
{

	if (mdp_proc_get_time_in_us(pTask->time.start,
	    pTask->time.releaseFence) > 16670) {
		...
		size = snprintf(show_buffer, sizeof(show_buffer),
			...
			mdp_proc_get_time_in_us(pTask->time.start,
	...
	mutex_lock(&global_mdp_task.free_task_list.mutex_lock);
	mdp_buffer_release_ion_handle(&pTask->mdp_command.src_buffer,
		&pTask->src_ion_handle);
	mdp_buffer_release_ion_handle(&pTask->mdp_command.dst_buffer,
		&pTask->dst_ion_handle);
	mutex_unlock(&global_mdp_task.free_task_list.mutex_lock);
...
enum MDP_TASK_STATUS mdp_core_wait_hw_ready(
	struct mdp_task_struct *pTask)
{
	if (pTask == NULL) {
		MDP_ERR("function:%s invalid param\n", __func__);
...
enum MDP_TASK_STATUS mdp_core_wait_buffer_ready(
	struct mdp_task_struct *pTask)
{
	enum MDP_TASK_STATUS status = MDP_TASK_STATUS_OK;

	do {
		status = mdp_fence_wait_fence_release(
			pTask->mdp_command.src_buffer.wait_fence_fd);
		if (status != MDP_TASK_STATUS_OK) {
			MDP_ERR("wait src buffer fence fd error:%d\n", status);
			break;
		}
		status = mdp_fence_wait_fence_release(
			pTask->mdp_command.dst_buffer.wait_fence_fd);
		if (status != MDP_TASK_STATUS_OK) {
...
enum MDP_TASK_STATUS mdp_core_trigger_hw_work(
	struct mdp_task_struct *pTask)
{
	if (pTask == NULL) {
		MDP_ERR("function:%s invalid param\n", __func__);
 ...
 */
enum MDP_TASK_STATUS mdp_core_wait_task_done(
	struct mdp_task_struct *pTask, int hwid)
{
	int ret = 0;
	enum MDP_TASK_STATUS status = MDP_TASK_STATUS_OK;
	unsigned int msec = MDP_WAIT_TASK_TIME_MS;
	...
	if (pTask->mdp_command.src_buffer.buffer_info.memory_type == DP_MEMORY_SECURE)
		msec = MDP_WAIT_TASK_TIME_MS_TZ;
	else
		msec = MDP_WAIT_TASK_TIME_MS;

	if (hwid == 0) {
		ret = wait_event_timeout(global_mdp_task.mdp_core_wait_done_task0_queue,
		...
		ret = wait_event_timeout(global_mdp_task.mdp_core_wait_done_task1_queue,
			pTask->current_state == MDP_TASK_STATE_DONE ||
			pTask->current_state == MDP_TASK_STATE_ERROR ||
			pTask->current_state == MDP_TASK_STATE_NR_OUTPUT_RELEASED,
			msecs_to_jiffies(msec));
...
int mdp_core_wait_task0(void *ignore)
{
	struct mdp_task_struct *pTask;
	DEFINE_WAIT_FUNC(wait, woken_wake_function);

	add_wait_queue(
		&global_mdp_task.mdp_core_start_wait_task0_queue, &wait);
	do {
		pTask = mdp_imgresz_get_mdp_task(0);
		if (pTask == NULL) {
			wait_woken(&wait,
				TASK_INTERRUPTIBLE,
		...
		mdp_core_wait_task_done(pTask, 0);
...
int mdp_core_wait_task1(void *ignore)
{
	struct mdp_task_struct *pTask;
	DEFINE_WAIT_FUNC(wait, woken_wake_function);

	add_wait_queue(
		&global_mdp_task.mdp_core_start_wait_task1_queue, &wait);
	do {
		pTask = mdp_imgresz_get_mdp_task(1);
		if (pTask == NULL) {
			wait_woken(&wait,
				TASK_INTERRUPTIBLE,
				MAX_SCHEDULE_TIMEOUT);
		...
		mdp_core_wait_task_done(pTask, 1);
	} while (1);
...
void mdp_core_release_task(struct mdp_task_struct *pTask)
{
	enum MDP_TASK_STATUS status = MDP_TASK_STATUS_OK;
	if (pTask == NULL) {
...
void mdp_core_trigger_wait_task_done(struct work_struct *work)
{
	...
	MDP_LOG("mdp exec time:%lld\n",
		mdp_proc_get_time_in_us(pTask->time.trigger,
			pTask->time.gotIRQ));
...
int32_t mdp_core_handle_task(struct mdp_task_struct *pTask)
{
	...
	do {
		...
		queue_work(trigger_wq[MDP_TRIGGER_WORK_QUEUE_TYPE_IMGRESZ_HW0],
			&pTask->task_work);
	} while (0);
 ...
 */
enum MDP_TASK_STATUS mdp_core_handle_exec_command(
	struct mdp_command_struct *pCommand)
{
	enum MDP_TASK_STATUS status = MDP_TASK_STATUS_OK;
	struct mdp_task_struct *pTask;
	...
	status = mdp_buffer_convert_fd(&pTask->mdp_command.src_buffer,
		&pTask->src_ion_handle);
	if (status != MDP_TASK_STATUS_OK) {
	...
	status = mdp_buffer_convert_fd(&pTask->mdp_command.dst_buffer,
		&pTask->dst_ion_handle);
	if (status != MDP_TASK_STATUS_OK) {
...
void mdp_core_dump_command(struct mdp_command_struct *pCommand)
{
	...
	MDP_LOG("DpCommand: src buffer: fd[%d] y[0x%x] u[0x%x] v[0x%x]\n",
	...
	MDP_LOG("ylen[0x%x] clen[0x%x] ylen_len[%d] clen_len[%d]\n",
	...
	MDP_LOG("DpCommand: dst buffer: fd[%d] y[0x%x] u[0x%x] v[0x%x]\n",

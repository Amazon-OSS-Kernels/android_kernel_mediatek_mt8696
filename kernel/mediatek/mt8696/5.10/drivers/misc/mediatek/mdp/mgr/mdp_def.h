 ...
 * we need 1 fence since only 1 hardware work and input buffer &
 * output buffer is released at same time.
 *
 * NR only have 1 hardware, but for 3D NR,
 * input buffer is released at end of this frame.
 * but output buffer is useful to next frame.
 * so output buffer is released at end of next frame.
...
enum MDP_TIMELINE_TYPE {
	...
	/* for nr & imgresz+rot input buffer &
	 * output buffer are not release at same time
	 */
	/* release when current frame done */
	MDP_TIMELINE_TYPE_NR_INPUT = 3,
	/* release when next frame done */
	MDP_TIMELINE_TYPE_NR_OUTPUT = 4,
	/* this is imgresz input buffer */
	MDP_TIMELINE_TYPE_IMGRESZ_ROT_INPUT = 5,
	/* this is rot output buffer */
	MDP_TIMELINE_TYPE_IMGRESZ_ROT_OUTPUT = 6,
...
enum MDP_TASK_STATE {
	...
	/* got a new mdp_task_struct to store
	 * mdp_command_struct from userspace
	 */
	MDP_TASK_STATE_STARTED,
	...
	/* fence is created, mdp task is
	 * summited to hardware driver. wait for done
	 */
	MDP_TASK_STATE_WORKING,
	MDP_TASK_STATE_DONE, /* mdp task is done. */
	/* nr task output buffer is released */
	MDP_TASK_STATE_NR_OUTPUT_RELEASED,
...
#define MDP_WAIT_TASK_TIME_MS_TZ 2000
...
struct global_mdp_task_struct {
	/* free task put to this list.
	 * when task is used, remove from this list.
	 */
	struct task_list_struct free_task_list;

	/* acquire task list  task need to allocate
	 * temp buffer put on this list.
	 ...
	 * task need to wait a idle hardware put on this list.
	 * ensure execute sequence
	...
	/* nr prepare task list, task need to wait nr hardware put on this list,
	 * ensure execute sequence
	 * when prepared, remove from this list
	 */
	struct task_list_struct nr_prepare_task_list;

	/* rot prepare task list, task need to wait rot
	 * hardware put on this list, ensure execute sequence
	 */
	...
	/* waitqueue: sleep wait for aquire a imgresz ticket,
	 * wait on main thread
	 */
	...
	/* waitqueue: start wait task0 */
	wait_queue_head_t mdp_core_start_wait_task0_queue;

	/* waitqueue: start wait task1 */
	wait_queue_head_t mdp_core_start_wait_task1_queue;

	/* waitqueue: wait done task0 */
	wait_queue_head_t mdp_core_wait_done_task0_queue;

	/* waitqueue: wait done task1 */
	wait_queue_head_t mdp_core_wait_done_task1_queue;
...
struct mdp_ion_struct {
	...
	struct dma_buf_attachment *attach[2];
...
struct mdp_task_struct {
	...
	/* mdp_task_struct will put to list */
	struct list_head list_entry;
	/* input data from userspace */
	struct mdp_command_struct mdp_command;
	/* store ion handle convert from ion fd. */
	struct mdp_ion_struct src_ion_handle;
	/* store ion handle convert from ion fd. */
	struct mdp_ion_struct dst_ion_handle;
	/* record task time */
	struct task_time_item_struct time;

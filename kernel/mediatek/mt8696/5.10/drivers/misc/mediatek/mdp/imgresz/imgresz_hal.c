...
#define IMG_RESZ_RECORD_CB			(0x1 << 15)
#define IMG_RESZ_RECORD_Y			(0x1 << 16)
...
#define IMG_RESZ_LINE_BUFFER_LEN_MASK		(0X1F << 24)
...
#define RW_IMG_RESZ_SRC_Y_ADDR_BASE1_8696		0x004
#define RW_IMG_RESZ_SRC_Y_ADDR_BASE2_8696		0x008

#define RW_IMG_RESZ_SRC_CB_ADDR_BASE1_8696		0x00C
#define RW_IMG_RESZ_SRC_CB_ADDR_BASE2_8696		0x010
#define RW_IMG_RESZ_SRC_CR_ADDR_BASE1_8696		0x014
#define RW_IMG_RESZ_SRC_CR_ADDR_BASE2_8696		0x018

#define RW_IMG_RESZ_TGT_Y_ADDR_BASE_8696		0x01C
#define RW_IMG_RESZ_TGT_C_ADDR_BASE_8696		0x020
...
#define RW_IMG_RESZ_TMP_ADDR_BASE_8696		0x024
#define RW_IMG_RESZ_Y_PRELOAD_OW_ADDR_BASE_8696	0x028
#define RW_IMG_RESZ_C_PRELOAD_OW_ADDR_BASE_8696	0x02C
...
#define IMG_RESZ_OSD_CSC_MODE_MSK		0x60
#define IMG_RESZ_OSD_CSC_ARGB8888		(0x3 << 5)
#define IMG_RESZ_OSD_CSC_ARGB1555		(0x2 << 5)
#define IMG_RESZ_OSD_CSC_ARGB4444		(0x1 << 5)
#define IMG_RESZ_OSD_CSC_ARGB0565		(0x0 << 5)
...
/*1: make ufo line buffer ready after imgresz done  0: off*/
...
#define RW_IMG_RESZ_UFO_Y_ADDR_8696			0x034
#define RW_IMG_RESZ_UFO_Y_LEN_ADDR_8696		0x038
#define RW_IMG_RESZ_UFO_C_ADDR_8696			0x03C
#define	RW_IMG_RESZ_UFO_C_LEN_ADDR_8696		0x040
...
static void
imgresz_hal_set_dram_burstlimit(void __iomem *base, int limit)
{
	u32 reg;

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_MEM_IF_MODE);

	reg &= ~(IMG_RESZ_DRAM_BURST_LIMIT_1 | IMG_RESZ_DRAM_BURST_LIMIT_2 |
			IMG_RESZ_DRAM_BURST_LIMIT_4 |
			IMG_RESZ_DRAM_BURST_LIMIT_8 |
			IMG_RESZ_DRAM_BURST_LIMIT_16);

	reg |= ((limit & 0x1f) << 8);

	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_MEM_IF_MODE);
}

void
imgresz_hal_set_wr_gmc_burstlimit(void __iomem *base, int limit)
{
	u32 reg;

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_VENC_SKIP);
	reg &= IMG_RESZ_WR_BURST_LIMIT_CLEAR;
	reg |= (limit << IMG_RESZ_WR_BURST_SHIFT);
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_VENC_SKIP);
}

void
imgresz_hal_set_rd_gmc_burstlimit(void __iomem *base, int limit)
{
	u32 reg;
...
void imgresz_hal_print_reg(void __iomem *base, void __iomem *base_sec)
{
	int temp = 0, dump_sz = 144;
	...
	for (temp = 0; temp < dump_sz; temp += 4) {
		pr_info("[0x%x]0x%8x   0x%8x   0x%8x   0x%8x\n",
			temp * 4,
	...
	for (temp = 0; temp < dump_sz; temp += 4) {
		pr_info("[0x%x]0x%8x   0x%8x   0x%8x   0x%8x\n",
			temp * 4,
			readl_relaxed(base_sec + temp * 4),
			readl_relaxed(base_sec + temp * 4 + 0x4),
			readl_relaxed(base_sec + temp * 4 + 0x8),
			readl_relaxed(base_sec + temp * 4 + 0xc));
	}
}

void imgresz_hal_trigger_hw(void __iomem *base,
	void __iomem *base_2nd, bool ufo)
{
	u32 reg;

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_START);
	reg |= IMG_RESZ_ACTIVATE;
	imgresz_writel(reg, base + RW_IMG_RESZ_START);

	if (base_2nd) {
		reg = readl_relaxed(base_2nd + RW_IMG_RESZ_START);
		reg |= IMG_RESZ_ACTIVATE;
		writel(reg, base_2nd + RW_IMG_RESZ_START);
	}
...
int
imgresz_hal_set_resz_mode(void __iomem *base,
			enum imgresz_scale_mode reszmode,
			  enum imgresz_buf_main_format_t mainformat)
{
	u32 reg;

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_TYPE);
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_TYPE);
...
void imgresz_hal_burst_enable(void __iomem *base,
...
void
imgresz_hal_set_src_buf_addr(void __iomem *base,
	void __iomem *base_sec, uint32_t y_buf_addr,
	uint32_t cb_buf_addr, uint32_t cr_buf_addr)
{
	if (imgresz_cur_chip_ver <= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(y_buf_addr >> 4,
			base + RW_IMG_RESZ_SRC_Y_ADDR_BASE1);
		imgresz_writel_relaxed(cb_buf_addr >> 4,
			base + RW_IMG_RESZ_SRC_CB_ADDR_BASE1);
		imgresz_writel_relaxed(cr_buf_addr >> 4,
			base + RW_IMG_RESZ_SRC_CR_ADDR_BASE1);
	} else {
		imgresz_writel_relaxed(y_buf_addr >> 4,
			base_sec + RW_IMG_RESZ_SRC_Y_ADDR_BASE1_8696);
		imgresz_writel_relaxed(cb_buf_addr >> 4,
			base_sec + RW_IMG_RESZ_SRC_CB_ADDR_BASE1_8696);
		imgresz_writel_relaxed(cr_buf_addr >> 4,
			base_sec + RW_IMG_RESZ_SRC_CR_ADDR_BASE1_8696);
	}
}
			       ...
			       const struct imgresz_buf_format *src_buf_format)
{
	...
	resz_type = imgresz_readl_relaxed(base + RW_IMG_RESZ_TYPE);
	osd_setting = imgresz_readl_relaxed(base +
		RW_IMG_RESZ_OSD_MODE_SETTING);
	...
	case IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR:
		...
		if (src_buf_format->h_sample[0] <= 1
			|| src_buf_format->v_sample[0] <= 1
			|| src_buf_format->h_sample[1] <= 1
			|| src_buf_format->v_sample[1] <= 1
			|| src_buf_format->h_sample[2] <= 1
			|| src_buf_format->v_sample[2] <= 1)
			pr_info("sample (%d,%d,%d,%d,%d,%d)\n",
						src_buf_format->h_sample[0],
						src_buf_format->v_sample[0],
						src_buf_format->h_sample[1],
						src_buf_format->v_sample[1],
						src_buf_format->h_sample[2],
						src_buf_format->v_sample[2]);
		/* Set Sample Factor */
		reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_JPG_MODE);
		reg &= 0xFFFFF000;
		...
		imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_JPG_MODE);
		break;
	...
	imgresz_writel_relaxed(resz_type, base + RW_IMG_RESZ_TYPE);
	imgresz_writel_relaxed(osd_setting, base +
		RW_IMG_RESZ_OSD_MODE_SETTING);
...
void
imgresz_hal_set_src_buf_pitch(void __iomem *base,
	uint32_t buf_width, const struct imgresz_buf_format *src_buf_format)
{
	uint32_t buf_width_y = 0, buf_width_c = 0;

	if (src_buf_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR) {
		buf_width_y = buf_width;
		/* For 411, 420 or 444 jpg */
		if ((src_buf_format->h_sample[1] <
			src_buf_format->h_sample[0]) &&
		    (src_buf_format->h_sample[2] <
		    src_buf_format->h_sample[0])) {
			if ((src_buf_format->h_sample[1] != 0) &&
				((src_buf_format->h_sample[0] / src_buf_format->h_sample[1]) != 0))
				buf_width_c = IMGALIGN(buf_width /
							  (src_buf_format->h_sample[0] /
							  src_buf_format->h_sample[1]), 16);
		} else {
			buf_width_c = buf_width;
		}
		if (buf_width_y == 0 || buf_width_c == 0)
			pr_info("width_y[%d] or width_c[%d] is 0\n", buf_width_y, buf_width_c);

		imgresz_writel_relaxed((((buf_width_c >> 4) & 0x3FFF) << 12) |
			(buf_width_y >> 4), base + RW_IMG_RESZ_SRC_BUF_LEN);
	} else {
		if (buf_width == 0)
			pr_info("buf_width[%d] is 0\n", buf_width);

		imgresz_writel_relaxed((((buf_width >> 4) & 0x3FFF) << 12) |
			(buf_width >> 4), base + RW_IMG_RESZ_SRC_BUF_LEN);
	}
}

void
imgresz_hal_set_src_rowbuf_height(void __iomem *base,
	uint32_t row_buf_hei, const struct imgresz_buf_format *src_buf_format,
				  bool rm_rpr_racing_mode)
{
	...
	if (row_buf_hei == 0) {
		...
		if (src_buf_format->mainformat ==
			IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR) {
			if (src_buf_format->v_sample[0] != 0) {
				...
				if (row_hei_cb == 0 || row_hei_cr == 0)
					pr_info("row_hei_cb[%d]  row_hei_cr[%d] maybe 0\n",
								row_hei_cb, row_hei_cr);
			}
		}

		if (row_hei_y == 0)
			pr_info("row_hei_y[%d] maybe 0\n", row_hei_y);

		imgresz_writel_relaxed(row_hei_y, base +
			RW_IMG_RESZ_LINE_NUM_IN_Y_COLOR_BUF);
		imgresz_writel_relaxed(row_hei_cb, base +
			RW_IMG_RESZ_LINE_NUM_IN_CB_COLOR_BUF);
		imgresz_writel_relaxed(row_hei_cr, base +
			RW_IMG_RESZ_LINE_NUM_IN_CR_COLOR_BUF);
	}
}

void imgresz_hal_set_src_pic_wid_hei(void __iomem *base,
	uint32_t wid_y, uint32_t wid_c,
	uint32_t wid_cr, uint32_t hei_y, uint32_t hei_c, uint32_t hei_cr)
{
	imgresz_writel_relaxed(((wid_y & 0xFFFF) << 16) | (hei_y & 0xFFFF),
			       base + RW_IMG_RESZ_SRC_SIZE_Y);
	imgresz_writel_relaxed(((wid_c & 0xFFFF) << 16) | (hei_c & 0xFFFF),
			       base + RW_IMG_RESZ_SRC_SIZE_CB);
	imgresz_writel_relaxed(((wid_cr & 0xFFFF) << 16) | (hei_cr & 0xFFFF),
			       base + RW_IMG_RESZ_SRC_SIZE_CR);
}

int imgresz_hal_set_src_pic_offset(void __iomem *base,
			struct imgresz_scale_data *scaledata)
{
	...
	case IMGRESZ_BUF_MAIN_FORMAT_Y_C:
		...
		break;
	case IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR:
		if ((src_format->h_sample[1] != 0) &&
			(src_format->v_sample[1] != 0)) {
			h_sample = src_format->h_sample[0] /
				src_format->h_sample[1];
			v_sample = src_format->v_sample[0] /
				src_format->v_sample[1];
			if (h_sample != 0)
		...
		if ((src_format->h_sample[2] != 0) &&
			(src_format->v_sample[2] != 0)) {
			h_sample = src_format->h_sample[0] /
				src_format->h_sample[2];
			v_sample = src_format->v_sample[0] /
				src_format->v_sample[2];
			if (h_sample != 0)
	...
	if (imgresz_cur_chip_ver >= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(((x_ofs1 & 0xFFFF) << 16) |
			(y_ofs1 & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_Y);
		imgresz_writel_relaxed(((x_ofs2 & 0xFFFF) << 16) |
			(y_ofs2 & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_CB);
		imgresz_writel_relaxed(((x_ofs3 & 0xFFFF) << 16) |
			(y_ofs3 & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_CR);
	} else {
		imgresz_writel_relaxed(((x_ofs1 & 0xFFF) << 12) |
			(y_ofs1 & 0xFFF), base + RW_IMG_RESZ_SRC_OFFSET_Y);
		imgresz_writel_relaxed(((x_ofs2 & 0xFFF) << 12) |
			(y_ofs2 & 0xFFF), base + RW_IMG_RESZ_SRC_OFFSET_CB);
		imgresz_writel_relaxed(((x_ofs3 & 0xFFF) << 12) |
			(y_ofs3 & 0xFFF), base + RW_IMG_RESZ_SRC_OFFSET_CR);
	}
	return 0;
}
...
void
imgresz_hal_set_src_offset_vdo_partition(void __iomem *base,
				unsigned int y_h_offset,
				unsigned int y_v_offset,
				unsigned int c_h_offset)
{
	imgresz_writel_relaxed(((y_h_offset & 0xFFFF) << 16) |
		(y_v_offset & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_Y);
	imgresz_writel_relaxed(((c_h_offset & 0xFFFF) << 16) |
		((y_v_offset >> 1) & 0xFFFF), base + RW_IMG_RESZ_SRC_OFFSET_CB);
}


void imgresz_hal_set_src_rm_rpr(void __iomem *base,
			bool rpr_mode, bool rpr_racing)
{
	u32 reg;

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_RPR);
	if (rpr_mode)
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_RPR);
}

void
imgresz_hal_set_src_first_row(void __iomem *base, bool firstrow)
{
	u32 reg;
...
void
imgresz_hal_set_vdo_cbcr_swap(void __iomem *base, bool cbcr_swap)
{
	u32 reg;
			       ...
			       const struct imgresz_buf_format *dst_format)
{
	...
	osd_setting = imgresz_readl_relaxed(base +
		RW_IMG_RESZ_OSD_MODE_SETTING);
	addr_swap_setting = imgresz_readl_relaxed(base +
		RW_IMG_RESZ_MEM_IF_MODE);
	csc_setting = imgresz_readl_relaxed(base +
		RW_IMG_RESZ_OSD_CSC_SETTING);
	...
	case IMGRESZ_BUF_MAIN_FORMAT_Y_C:
		...
		reg &= ~(IMG_RESZ_420_OUT | IMG_RESZ_422_OUT |
				IMG_RESZ_444_OUT);
	...
	case IMGRESZ_BUF_MAIN_FORMAT_AYUV:
		...
		case IMGRESZ_BUF_MAIN_FORMAT_ARGB:
			...
			writel_relaxed(0x132, base +
				RW_IMG_RESZ_OSD_CSC_COEF11);
			writel_relaxed(0x259, base +
				RW_IMG_RESZ_OSD_CSC_COEF12);
			writel_relaxed(0x75, base +
				RW_IMG_RESZ_OSD_CSC_COEF13);
			writel_relaxed(0x1F50, base +
				RW_IMG_RESZ_OSD_CSC_COEF21);
			writel_relaxed(0x1EA5, base +
				RW_IMG_RESZ_OSD_CSC_COEF22);
			writel_relaxed(0x20B, base +
				RW_IMG_RESZ_OSD_CSC_COEF23);
			writel_relaxed(0x20B, base +
				RW_IMG_RESZ_OSD_CSC_COEF31);
			writel_relaxed(0x1E4A, base +
				RW_IMG_RESZ_OSD_CSC_COEF32);
			writel_relaxed(0x1FAB, base +
				RW_IMG_RESZ_OSD_CSC_COEF33);
			break;
		default:
			break;
		}
	...
	case IMGRESZ_BUF_MAIN_FORMAT_ARGB:
		osd_setting &= ~(IMG_RESZ_OSD_OUTPUT_RGB565 |
			IMG_RESZ_OSD_OUTPUT_ARGB1555 |
				 IMG_RESZ_OSD_OUTPUT_ARGB4444 |
				 IMG_RESZ_OSD_OUTPUT_ARGB8888);
		...
		case IMGRESZ_BUF_MAIN_FORMAT_AYUV:
			...
			csc_setting &= ~IMG_RESZ_OSD_CSC_YOUT_A16;
			csc_setting &= ~IMG_RESZ_OSD_CSC_COUT_A128;
			writel_relaxed(0x400, base +
				RW_IMG_RESZ_OSD_CSC_COEF11);
			writel_relaxed(0x0, base +
				RW_IMG_RESZ_OSD_CSC_COEF12);
			writel_relaxed(0x57C, base +
				RW_IMG_RESZ_OSD_CSC_COEF13);
			writel_relaxed(0x400, base +
				RW_IMG_RESZ_OSD_CSC_COEF21);
			writel_relaxed(0x1EA8, base +
				RW_IMG_RESZ_OSD_CSC_COEF22);
			writel_relaxed(0x1D35, base +
				RW_IMG_RESZ_OSD_CSC_COEF23);
			writel_relaxed(0x400, base +
				RW_IMG_RESZ_OSD_CSC_COEF31);
			writel_relaxed(0x6EE, base +
				RW_IMG_RESZ_OSD_CSC_COEF32);
		...
		case IMGRESZ_BUF_MAIN_FORMAT_Y_C:
			...
			/*
			 *always set ARGB8888 no matter which output color
			 *mode when YC->ARGB one phase
			 */
			osd_setting &= ~(IMG_RESZ_OSD_OUTPUT_RGB565 |
					IMG_RESZ_OSD_OUTPUT_ARGB1555 |
					 IMG_RESZ_OSD_OUTPUT_ARGB4444 |
			...
			csc_setting &= ~IMG_RESZ_OSD_CSC_MODE_MSK;
			switch (dst_format->argb_format) {
			case IMGRESZ_ARGB_FORMAT_0565:
				csc_setting |= IMG_RESZ_OSD_CSC_ARGB0565;
				break;
			case IMGRESZ_ARGB_FORMAT_1555:
				csc_setting |= IMG_RESZ_OSD_CSC_ARGB1555;
				break;
			case IMGRESZ_ARGB_FORMAT_4444:
				csc_setting |= IMG_RESZ_OSD_CSC_ARGB4444;
				break;
			case IMGRESZ_ARGB_FORMAT_8888:
				csc_setting |= IMG_RESZ_OSD_CSC_ARGB8888;
				break;
			}
			csc_setting |= IMG_RESZ_OSD_CSC_ENABLE;
	...
	imgresz_writel_relaxed(osd_setting, base +
		RW_IMG_RESZ_OSD_MODE_SETTING);
	imgresz_writel_relaxed(addr_swap_setting, base +
		RW_IMG_RESZ_MEM_IF_MODE);
	imgresz_writel_relaxed(csc_setting, base +
		RW_IMG_RESZ_OSD_CSC_SETTING);

	return 0;
}

void imgresz_hal_set_dst_buf_addr(void __iomem *base,
	void __iomem *base_sec, uint32_t addr_y, uint32_t addr_c)
{
	if (imgresz_cur_chip_ver <= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(addr_y >> 4,
			base + RW_IMG_RESZ_TGT_Y_ADDR_BASE);
		imgresz_writel_relaxed(addr_c >> 4,
			base + RW_IMG_RESZ_TGT_C_ADDR_BASE);
	} else {
		imgresz_writel_relaxed(addr_y >> 4,
			base_sec + RW_IMG_RESZ_TGT_Y_ADDR_BASE_8696);
		imgresz_writel_relaxed(addr_c >> 4,
			base_sec + RW_IMG_RESZ_TGT_C_ADDR_BASE_8696);
	}
}

void imgresz_hal_set_dst_buf_pitch(void __iomem *base,
				uint32_t buf_width)
{
	u32 reg;

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_TGT_BUF_LEN);
	reg = (reg & 0xFF000000) |
		(((buf_width >> 4) & 0x3FFF) << 12) |
		(buf_width >> 4);
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_TGT_BUF_LEN);
}

void imgresz_hal_set_dst_pic_wid_hei(void __iomem *base,
				uint32_t width, uint32_t height)
{
	imgresz_writel_relaxed(((width & 0xFFFF) << 16) | (height & 0xFFFF),
...
{
	if (imgresz_cur_chip_ver >= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(((x_offset & 0xFFFF) << 16) |
			(y_offset & 0xFFFF), base + RW_IMG_RESZ_TGT_OFFSET);
	} else {
		imgresz_writel_relaxed(((x_offset & 0xFFF) << 12) |
			(y_offset & 0xFFF), base + RW_IMG_RESZ_TGT_OFFSET);
	}
}

static void imgresz_hal_set_linebuflen_ext16(void __iomem *base,
								bool enable)
{
	u32 reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_TGT_BUF_LEN);
...
static void imgresz_hal_set_sram_linebuflen(void __iomem *base,
						uint32_t linebuf)
{
	u32 mask = ~IMG_RESZ_LINE_BUFFER_LEN_MASK;
	u32 reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_TGT_BUF_LEN);
...
{
	u32 reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_LINEB_ECO);

	imgresz_writel_relaxed(reg | IMG_RESZ_UFO_LINEB_ECO, base +
		RW_IMG_RESZ_UFO_LINEB_ECO);
}
 ...
 */
static int imgresz_hal_calc_linebuflen(bool ufo, bool onephase,
					bool osdmode, bool dst_blk,
				       ...
				       unsigned int linebuflen)
{
	...
	if ((dst_width - cnt * linebuflen * 32 < 16) && (dst_width - cnt *
		linebuflen * 32 > 0)	&& linebuflen > 1) {
		loginfo(IMGRESZ_LOG_UFO,
			"[ImgResz] The last part wid too small:%u. SrcW:%u",
			(dst_width - cnt * linebuflen*32), src_width);
		loginfo(IMGRESZ_LOG_UFO,
			" DstW:%u, Count:%u, curline:%u\n", dst_width,
				cnt, linebuflen);
	}
	...
	loginfo(IMGRESZ_LOG_UFO,
		"[ImgResz] srcW:%u dstW:%u lineB:%u Hw partial NUM:%d",
		src_width, dst_width, linebuflen, cnt);
	loginfo(IMGRESZ_LOG_UFO,
		" last part:%u, ufo:%d one phase %d(%d)\n",
 ...
 * the linebuflen is 0.
 * Add this function for hal_if.c trying to adjust v-partition
 *division if it meet
 * HW limitation.
 *
 * Return FAIL: HW limitation,xCalcLineBuflen below will return 0.
 */
bool imgresz_hal_survey_linebuflen_is_ok(bool ufo,
			bool onephase, bool dst_blk, uint32_t src_width,
			uint32_t dst_width, unsigned int linebuflen)
{
			   ...
			   bool osd_mode, bool dst_blk, bool dst_10bit)
{
	...
	if (!(one_phase && osd_mode))
		linebuflen = imgresz_hal_calc_linebuflen(ufo, one_phase,
			osd_mode, dst_blk, src_width, dst_width, linebuflen);
	else {
		/* ufo2vdo one phase formula version6 */
		linebuflen = imgresz_hal_calc_linebuflen(ufo, one_phase,
		osd_mode, dst_blk, readl_relaxed(base +
			RW_IMG_RESZ_SRC_SIZE_CB) >> 16,
				dst_width, ufo ? 0x10 : 0x8);
	}
...
void imgresz_hal_set_tempbuf(void __iomem *base,
		void __iomem *base_sec, uint32_t addr)
{
	u32 reg;
	/* Temp buffer always No-iommu. */
	if (imgresz_cur_chip_ver <= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(addr >> 4, base +
			RW_IMG_RESZ_TMP_ADDR_BASE);
	} else {
		imgresz_writel_relaxed(addr >> 4, base_sec +
			RW_IMG_RESZ_TMP_ADDR_BASE_8696);
	}
...
{
	imgresz_writel_relaxed(IMG_RESZ_TRACKING_WITH_JPG_HW,
		       base + RW_IMG_RESZ_INTERFACE_SWITCH);
}

void imgresz_hal_jpg_preload_enable(void __iomem *base)
{
	u32 reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_JPG_MODE);
...
void imgresz_hal_jpg_cbcr_pad(void __iomem *base,
			bool cb_exist, bool cr_exist)
{
	u32 reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_TYPE);
...
void imgresz_hal_jpg_preload_buf(void __iomem *base,
					void __iomem *base_sec,
						uint32_t addr_y,
						uint32_t addr_c)
{
	if (imgresz_cur_chip_ver <= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(addr_y >> 4, base +
			RW_IMG_RESZ_Y_PRELOAD_OW_ADDR_BASE);
		imgresz_writel_relaxed(addr_c >> 4, base +
			RW_IMG_RESZ_C_PRELOAD_OW_ADDR_BASE);
	} else {
		imgresz_writel_relaxed(addr_y >> 4, base_sec +
			RW_IMG_RESZ_Y_PRELOAD_OW_ADDR_BASE_8696);
		imgresz_writel_relaxed(addr_c >> 4, base_sec +
			RW_IMG_RESZ_C_PRELOAD_OW_ADDR_BASE_8696);
	}
}

void imgresz_hal_set_scaling_type(void __iomem *base,
					unsigned int type)
{
	u32 reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_OSD_MODE_SETTING);
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_OSD_MODE_SETTING);
}
			   ...
			   unsigned int offset, unsigned int factor)
{
	...
	if (0 == (factor & 0xFC000000)) {
		imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_H8TAPS_FAC_Y);
		imgresz_writel_relaxed(offset, base +
			RW_IMG_RESZ_H8TAP_OFSET_Y);
	} else {
...
{
	return imgresz_readl_relaxed(base + RW_IMG_RESZ_H8TAPS_FAC_Y);
}
			    ...
			    unsigned int offset, unsigned int factor)
{
	...
	if (0 == (factor & 0xFC000000)) {
		imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_H8TAPS_FAC_CB);
		imgresz_writel_relaxed(offset, base +
			RW_IMG_RESZ_H8TAP_OFSET_CB);
	} else {
...
{
	return imgresz_readl_relaxed(base + RW_IMG_RESZ_H8TAPS_FAC_CB);
}
			    ...
			    unsigned int offset, unsigned int factor)
{
	...
	if (0 == (factor & 0xFC000000)) {
		imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_H8TAPS_FAC_CR);
		imgresz_writel_relaxed(offset, base +
			RW_IMG_RESZ_H8TAP_OFSET_CR);
	} else {
...
imgresz_hal_coeff_set_h8_coeffs(void __iomem *base,
				enum imgresz_scale_factor factor)
{
	imgresz_writel_relaxed(h_filtercoeff[factor][0], base +
		RW_IMG_RESZ_H_COEF0);
	imgresz_writel_relaxed(h_filtercoeff[factor][1], base +
		RW_IMG_RESZ_H_COEF1);
	imgresz_writel_relaxed(h_filtercoeff[factor][2], base +
		RW_IMG_RESZ_H_COEF2);
	imgresz_writel_relaxed(h_filtercoeff[factor][3], base +
		RW_IMG_RESZ_H_COEF3);
	imgresz_writel_relaxed(h_filtercoeff[factor][4], base +
		RW_IMG_RESZ_H_COEF4);
	imgresz_writel_relaxed(h_filtercoeff[factor][5], base +
		RW_IMG_RESZ_H_COEF5);
	imgresz_writel_relaxed(h_filtercoeff[factor][6], base +
		RW_IMG_RESZ_H_COEF6);
	imgresz_writel_relaxed(h_filtercoeff[factor][7], base +
		RW_IMG_RESZ_H_COEF7);
	imgresz_writel_relaxed(h_filtercoeff[factor][8], base +
		RW_IMG_RESZ_H_COEF8);
	imgresz_writel_relaxed(h_filtercoeff[factor][9], base +
		RW_IMG_RESZ_H_COEF9);
	imgresz_writel_relaxed(h_filtercoeff[factor][10], base +
		RW_IMG_RESZ_H_COEF10);
	imgresz_writel_relaxed(h_filtercoeff[factor][11], base +
		RW_IMG_RESZ_H_COEF11);
	imgresz_writel_relaxed(h_filtercoeff[factor][12], base +
		RW_IMG_RESZ_H_COEF12);
	imgresz_writel_relaxed(h_filtercoeff[factor][13], base +
		RW_IMG_RESZ_H_COEF13);
	imgresz_writel_relaxed(h_filtercoeff[factor][14], base +
		RW_IMG_RESZ_H_COEF14);
	imgresz_writel_relaxed(h_filtercoeff[factor][15], base +
		RW_IMG_RESZ_H_COEF15);
	imgresz_writel_relaxed(h_filtercoeff[factor][16], base +
		RW_IMG_RESZ_H_COEF16);
	imgresz_writel_relaxed(h_filtercoeff[factor][17], base +
		RW_IMG_RESZ_H_COEF17);
}

static void
imgresz_hal_coeff_set_v4_coeffs(void __iomem *base,
				enum imgresz_scale_factor factor)
{
	imgresz_writel_relaxed(v_filtercoeff[factor][0], base +
		RW_IMG_RESZ_V_COEF0);
	imgresz_writel_relaxed(v_filtercoeff[factor][1], base +
		RW_IMG_RESZ_V_COEF1);
	imgresz_writel_relaxed(v_filtercoeff[factor][2], base +
		RW_IMG_RESZ_V_COEF2);
	imgresz_writel_relaxed(v_filtercoeff[factor][3], base +
		RW_IMG_RESZ_V_COEF3);
	imgresz_writel_relaxed(v_filtercoeff[factor][4], base +
		RW_IMG_RESZ_V_COEF4);
	imgresz_writel_relaxed(v_filtercoeff[factor][5], base +
		RW_IMG_RESZ_V_COEF5);
	imgresz_writel_relaxed(v_filtercoeff[factor][6], base +
		RW_IMG_RESZ_V_COEF6);
	imgresz_writel_relaxed(v_filtercoeff[factor][7], base +
		RW_IMG_RESZ_V_COEF7);
	imgresz_writel_relaxed(v_filtercoeff[factor][8], base +
		RW_IMG_RESZ_V_COEF8);
}

static enum imgresz_scale_factor
imgresz_hal_coeff_get_factor(uint32_t src_size, uint32_t dst_size)
{
	uint32_t scale_ratio = 0;

	if (src_size != 0)
		scale_ratio = dst_size * 10000 / src_size;
			    ...
			    unsigned int factor)
{
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_HSA_SCL_Y);
}

static void
imgresz_hal_coeff_set_hsa_cb(void __iomem *base,
			unsigned int offset, unsigned int factor)
{
	u32 reg = factor & 0x00000FFF;

	reg += ((offset & 0x000007FF) << 12);
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_HSA_SCL_CB);
}

static void
imgresz_hal_coeff_set_hsa_cr(void __iomem *base,
			unsigned int offset, unsigned int factor)
{
	u32 reg = factor & 0x00000FFF;

	reg += ((offset & 0x000007FF) << 12);
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_HSA_SCL_CR);
}
			   ...
			   unsigned int factor)
{
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_V4TAPS_SCL_Y);
	imgresz_writel_relaxed(offset, base + RW_IMG_RESZ_V4TAP_OFSET_Y);
			    ...
			    unsigned int  factor)
{
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_V4TAPS_SCL_CB);
	imgresz_writel_relaxed(offset, base + RW_IMG_RESZ_V4TAP_OFSET_C);
			    ...
			    unsigned int factor)
{
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_V4TAPS_SCL_CR);

	return 0;
}

static int
imgresz_hal_coeff_set_vscale_y(void __iomem *base,
			unsigned int offset, unsigned int factor, bool scale_up)
{
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_V_SCL_Y);

	return 0;
}

static int
imgresz_hal_coeff_set_vscale_cb(void __iomem *base,
		unsigned int offset, unsigned int factor, bool scale_up)
{
	u32 reg = factor & 0x000007FF;
	...
	reg += ((offset & 0x000007FF) << 12) + ((scale_up & 0x1) << 11);
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_V_SCL_CB);

	return 0;
}

static int
imgresz_hal_coeff_set_vscale_cr(void __iomem *base,
			unsigned int offset, unsigned int factor, bool scale_up)
{
	u32 reg = factor & 0x000007FF;
	...
	reg += ((offset & 0x000007FF) << 12) + ((scale_up & 0x1) << 11);
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_V_SCL_CR);
...
{
	/* Y */
	imgresz_hal_coeff_set_h8_y(base, hal_info->h8_offset_y,
	hal_info->h8_factor_y);
	imgresz_hal_coeff_set_hsa_y(base, hal_info->hsa_offset_y,
		hal_info->hsa_factor_y);
	if ((resample_method == IMGRESZ_RESAMPLE_METHOD_8_TAP) ||
	    (src_buf->pic_width <= dst_buf->pic_width)) {
		imgresz_hal_coeff_set_h8_coeffs(base,
			imgresz_hal_coeff_get_factor(src_buf->pic_width,
			dst_buf->pic_width));
	}
	...
	/* Cb */
	imgresz_hal_coeff_set_h8_cb(base, hal_info->h8_offset_cb,
	hal_info->h8_factor_cb);
	imgresz_hal_coeff_set_hsa_cb(base, hal_info->hsa_offset_cb,
		hal_info->hsa_factor_cb);

	if (src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_Y_C)
		return 0;

	/* Cr */
	imgresz_hal_coeff_set_h8_cr(base, hal_info->h8_offset_cr,
	hal_info->h8_factor_cr);
	imgresz_hal_coeff_set_hsa_cr(base, hal_info->hsa_offset_cr,
		hal_info->hsa_factor_cr);
			       ...
			       const struct imgresz_hal_info *hal_info)
{
	...
	case IMGRESZ_RESAMPLE_METHOD_4_TAP:
		imgresz_hal_coeff_set_v4_y(base, hal_info->v4_offset_y,
			hal_info->v4_factor_y);
		if ((src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_ARGB) ||
		    (src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_AYUV) ||
		    (src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_INDEX))
			break;
		imgresz_hal_coeff_set_v4_cb(base, hal_info->v4_offset_cb,
			hal_info->v4_factor_cb);
		if (src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_Y_C)
			break;
		imgresz_hal_coeff_set_v4_cr(base, hal_info->v4_offset_cr,
			hal_info->v4_factor_cr);
		imgresz_hal_coeff_set_v4_coeffs(base,
			imgresz_hal_coeff_get_factor(src_buf->pic_height,
			dst_buf->pic_height));
		break;
	...
	case IMGRESZ_RESAMPLE_METHOD_M_TAP:
		...
		imgresz_hal_coeff_set_vscale_cb(base, hal_info->vm_offset_cb,
				hal_info->vm_factor_cb,
				hal_info->vm_scale_up_cb);
		if (src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_Y_C)
			break;
		imgresz_hal_coeff_set_vscale_cr(base, hal_info->vm_offset_cr,
				hal_info->vm_factor_cr,
				hal_info->vm_scale_up_cr);
		break;
	default:
		return -EINVAL;
	}
...
{
	return imgresz_readl_relaxed(base + RW_IMG_RESZ_V4TAPS_SCL_Y);
}

void
imgresz_hal_coeff_h8tap_vdo_partition_offset(void
				__iomem *base, unsigned int tg_offset,
				unsigned int hfactor_y, unsigned int hfactor_c)
{
	if ((tg_offset == 0) && (hfactor_y == 0)) {
		/*just for src_w&h == tg_w&h*/
		imgresz_writel_relaxed(0, base + RW_IMG_RESZ_H8TAP_OFSET_Y);
		imgresz_writel_relaxed(0, base + RW_IMG_RESZ_H8TAP_OFSET_CB);
	} else {
		imgresz_writel_relaxed(((tg_offset * hfactor_y) & 0x3ffff) +
			0xc0000, base + RW_IMG_RESZ_H8TAP_OFSET_Y);
		imgresz_writel_relaxed(((tg_offset / 2 * hfactor_c) &
			0x3ffff) + 0xc0000, base + RW_IMG_RESZ_H8TAP_OFSET_CB);
	}
}
...
imgresz_hal_coeff_v4tap_vdo_partition_offset(
			void __iomem *base, unsigned int part1_offset)
{
	imgresz_writel_relaxed(part1_offset, base + RW_IMG_RESZ_V4TAP_OFSET_Y);
	imgresz_writel_relaxed(part1_offset / 2, base +
		RW_IMG_RESZ_V4TAP_OFSET_C);
}

void imgresz_hal_set_src_pic_wid_hei_vdo_partition(
	void __iomem *base, unsigned int y_width,
	unsigned int c_width, unsigned int height)
{
	imgresz_writel_relaxed(((y_width & 0xFFFF) << 16) |
		(height & 0xFFFF), base + RW_IMG_RESZ_SRC_SIZE_Y);
	imgresz_writel_relaxed(((c_width & 0xFFFF) << 16) |
		((height / 2) & 0xFFFF), base + RW_IMG_RESZ_SRC_SIZE_CB);
}

void
imgresz_hal_coeff_set_rpr_H_factor(void __iomem *base,
				uint32_t src_width, uint32_t dst_width)
{
	...
	if (dst_width == 0)
		pr_info("dst_width[%d]is 0\n", dst_width);

	/* UXL and UXR are independent of row, so compute once only */
	/* numerator part */
	UXR = ((((src_width - dst_width) << 1)) << (4 + m));
	/* complete iUxR init by dividing by H with rounding to nearest int */
	...
	imgresz_hal_coeff_set_h8_y(base, offset, factor);
	imgresz_hal_coeff_set_h8_cb(base, offset, factor);
	imgresz_hal_coeff_set_h8_coeffs(base, FACTOR_RM);
}

void imgresz_hal_coeff_set_rpr_V_factor(void __iomem *base,
				uint32_t src_height, uint32_t dst_height)
{
	unsigned int factor, offset;
	int n = 0;
	int temp;
...
{
	return imgresz_readl_relaxed(base + RO_IMG_RESZ_CHECK_SUM_REG);
}
...
u32 imgresz_checksum_read(void __iomem *base)
{
	...
	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_FLIP);
	reg &= ~(0x1 << 5);
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_FLIP);

	return imgresz_readl_relaxed(base + RW_IMG_RESZ_READ_CHECKSUM);
}

bool imgresz_hal_is_done(void __iomem *base)
{
	return imgresz_readl_relaxed(base + RO_IMG_RESZ_DONE) & 0x1;
}
...
int imgresz_hal_grace_reset(void __iomem *base)
{
	...
	if (readl_poll_timeout_atomic(base + RO_IMG_RESZ_DONE,
		tmp, tmp & 0x2, 10, 100000))
		return -EAGAIN;
...
void imgresz_hal_set_argb_one_phase(void __iomem *base,
	void __iomem *base_2nd)
{
	u32 i, value;
	bool ufo = readl_relaxed(base + RW_IMG_RESZ_UFO_POWER) &
		IMG_RESZ_UFO_ON;

	/* set ch2 as ch1 */
	for (i = 0; i < 256; i++) {
		value = readl_relaxed(base + i * 4);
		writel_relaxed(value, base_2nd + i * 4);
	}

	value = readl_relaxed(base_2nd + RW_IMG_RESZ_START) &
		(~IMG_RESZ_WAIT_WR_DONE);
	writel_relaxed(value, base_2nd + RW_IMG_RESZ_START);

	/* ch2 enable chroma */
	value = readl_relaxed(base_2nd + RW_IMG_RESZ_VENC_SKIP) |
	IMG_RESZ_ENABLE_CHROMA;
	writel_relaxed(value, base_2nd + RW_IMG_RESZ_VENC_SKIP);

	/* ch1 disable chroma */
	value = readl_relaxed(base + RW_IMG_RESZ_JPG_MODE) &
	~(IMG_RESZ_RECORD_CR | IMG_RESZ_RECORD_CB);
	writel_relaxed(value, base + RW_IMG_RESZ_JPG_MODE);

	value = readl_relaxed(base + RW_IMG_RESZ_TYPE) |
		IMG_RESZ_ONE_PHASE_CSC | IMG_RESZ_ARGB_ONE_PHASE;
	writel_relaxed(value, base + RW_IMG_RESZ_TYPE);
	value = readl_relaxed(base_2nd + RW_IMG_RESZ_TYPE) |
		IMG_RESZ_ONE_PHASE_CSC | IMG_RESZ_ARGB_ONE_PHASE;
	writel_relaxed(value, base_2nd + RW_IMG_RESZ_TYPE);

	/* ch2 line-buf-len is 2*ch1 line-buf-len. */
	value = readl_relaxed(base + RW_IMG_RESZ_TGT_BUF_LEN);
	value = ((value & IMG_RESZ_LINE_BUFFER_LEN_MASK) * 2) |
		(value & (~IMG_RESZ_LINE_BUFFER_LEN_MASK));
	writel_relaxed(value, base_2nd + RW_IMG_RESZ_TGT_BUF_LEN);

	if (ufo) {
		value = readl_relaxed(base_2nd + RW_IMG_RESZ_UFO_CFG) |
			IMG_RESZ_CHROMA;
		writel_relaxed(value, base_2nd + RW_IMG_RESZ_UFO_CFG);

		/* ufo C page height need half */
		value = readl_relaxed(base_2nd + RW_IMG_RESZ_UFO_PAGE_SZ);
		value = (value & 0xffff0000) >> 1 | (value & 0x0000ffff);
		writel_relaxed(value, base_2nd + RW_IMG_RESZ_UFO_PAGE_SZ);
	}

}

void imgresz_ufo_partition_set_start_point(void __iomem *base,
					unsigned int x, unsigned int y)
{
	y &= 0xfffffff8; //8ALIGN
	imgresz_writel_relaxed(x|(y << 16), base + RW_IMG_RESZ_UFO_START_POINT);
}

void imgresz_ufo_pagesz(void __iomem *base, uint32_t srcwid,
	uint32_t srchei, uint32_t dstwid)
{
	unsigned int lenbuf = 512;
	u32 page_width = 0, page_height = 0;
	...
	if (page_width == 0 || page_height == 0)
		pr_info("width[%d] or height[%d] is 0\n", page_width, page_height);

	imgresz_writel_relaxed((page_height << 16) | page_width,
...
{
	u32 u4ufopower = readl_relaxed(base + RW_IMG_RESZ_UFO_POWER);

	u4ufopower |= IMG_RESZ_UFO_ON | IMG_RESZ_BITS_POWER |
		IMG_RESZ_LEN_POWER | IMG_RESZ_DRAM_CLK | IMG_RESZ_UFO_CLK;
...
void imgresz_ufo_config(void __iomem *base, enum imgresz_ufo_type type)
{
	...
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_UFO_CFG);
}

void imgresz_ufo_picsz(void __iomem *base, uint32_t buf_width,
					uint32_t buf_height)
{
	buf_height = (buf_height + 7) / 8 * 8;

	if (buf_width == 0 || buf_height == 0)
		pr_info("width[%d] or height[%d] maybe 0\n", buf_width, buf_height);

	imgresz_writel_relaxed(buf_width | (buf_height << 16),
		       base + RW_IMG_RESZ_UFO_PIC_SZ);
}

void imgresz_ufo_src_buf(void __iomem *base, void __iomem *base_sec,
				uint32_t y_addr, uint32_t c_addr)
{
	loginfo(IMGRESZ_LOG_REG, "[Reg] W 0x21c:%pad, 0x224:%pad\n",
		&y_addr, &c_addr);
	if (imgresz_cur_chip_ver <= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(y_addr, base + RW_IMG_RESZ_UFO_Y_ADDR);
		imgresz_writel_relaxed(c_addr, base + RW_IMG_RESZ_UFO_C_ADDR);
	} else {
		imgresz_writel_relaxed(y_addr, base_sec +
			RW_IMG_RESZ_UFO_Y_ADDR_8696);
		imgresz_writel_relaxed(c_addr, base_sec +
			RW_IMG_RESZ_UFO_C_ADDR_8696);
	}
}

void imgresz_ufo_srclen_buf(void __iomem *base, void __iomem *base_sec,
				uint32_t y_len_addr, uint32_t c_len_addr)
{
	if (imgresz_cur_chip_ver <= IMGRESZ_CURR_CHIP_VER_8695) {
		imgresz_writel_relaxed(y_len_addr, base +
			RW_IMG_RESZ_UFO_Y_LEN_ADDR);
		imgresz_writel_relaxed(c_len_addr, base +
			RW_IMG_RESZ_UFO_C_LEN_ADDR);
}
	else {
		imgresz_writel_relaxed(y_len_addr, base_sec +
			RW_IMG_RESZ_UFO_Y_LEN_ADDR_8696);
		imgresz_writel_relaxed(c_len_addr, base_sec +
			RW_IMG_RESZ_UFO_C_LEN_ADDR_8696);
	}
}
...
void imgresz_ufo_10bit_jump_mode_enable(void __iomem *base)
{
	...
	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_JUMPMODE);
	reg |= IMG_RESZ_UFO_JUMPMODE_EN;
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_UFO_JUMPMODE);
}

/*
 *husky es2:instead ufo idle polling to fix
 *when imgresz done but ufo not done issue
 */
void imgresz_ufo_idle_int_on(void __iomem *base)
{
	u32 reg;

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_POWER);
	reg |= IMG_RESZ_UFO_INT_ON | IMG_RESZ_UFO_LINEB_READY;
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_UFO_POWER);
}
...
int imgresz_ufo_vdo_v_partition_offset_update(
	void __iomem *base, void __iomem *base_sec, uint32_t src_buf_wid,
	uint32_t part1_src_bgn)
{
	u32 reg, c_offset;
	u32 tmp;
	bool fg8bit;
	...
	if (imgresz_cur_chip_ver <= IMGRESZ_CURR_CHIP_VER_8695) {
		reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_Y_LEN_ADDR);
		reg += tmp;
		imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_UFO_Y_LEN_ADDR);

		reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_C_LEN_ADDR);
		reg += tmp / 2;
		imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_UFO_C_LEN_ADDR);
	} else {
		reg = imgresz_readl_relaxed(base_sec +
			RW_IMG_RESZ_UFO_Y_LEN_ADDR_8696);
		reg += tmp;
		imgresz_writel_relaxed(reg, base_sec +
			RW_IMG_RESZ_UFO_Y_LEN_ADDR_8696);
	...
	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_CFG);
	fg8bit = !((reg >> 1) & 0x3);
	...
	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_Y_ADDR);
	reg += tmp;
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_UFO_Y_ADDR);

	reg = imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_C_ADDR);
	reg += tmp / 2;
	imgresz_writel_relaxed(reg, base + RW_IMG_RESZ_UFO_C_ADDR);
...
void imgresz_debug_vdo_partition_setting(void __iomem *base)
{
	pr_info("src_width_y(0x40)  :0x%8x 0x%8x\n",
		(imgresz_readl_relaxed(base +
		RW_IMG_RESZ_SRC_SIZE_Y) & 0xffff0000) >> 16,
		imgresz_readl_relaxed(base + RW_IMG_RESZ_SRC_SIZE_Y) & 0xffff);
	pr_info("src_width_c(0x44)  :0x%8x 0x%8x\n",
		(imgresz_readl_relaxed(base +
		RW_IMG_RESZ_SRC_SIZE_CB) & 0xffff0000) >> 16,
		imgresz_readl_relaxed(base +
		RW_IMG_RESZ_SRC_SIZE_CB) & 0xffff);
	pr_info("tg_y (0x4c)        :0x%8x 0x%8x\n",
		(imgresz_readl_relaxed(base +
		RW_IMG_RESZ_TGT_SIZE) & 0xffff0000) >> 16,
		imgresz_readl_relaxed(base +
		RW_IMG_RESZ_TGT_SIZE) & 0xffff);
	pr_info("Page_start (0x218) :0x%8x 0x%8x\n",
		(imgresz_readl_relaxed(base +
		RW_IMG_RESZ_UFO_START_POINT) & 0xffff0000) >> 16,
		imgresz_readl_relaxed(base +
		RW_IMG_RESZ_UFO_START_POINT) & 0xffff);

	if (imgresz_cur_chip_ver >= IMGRESZ_CURR_CHIP_VER_8695) {
		pr_info("rd_h_offset_y(0x54):0x%8x 0x%8x\n",
			(imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_Y) & 0xffff0000) >> 16,
			imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_Y) & 0xffff);
		pr_info("rd_h_offset_c(0x58):0x%8x 0x%8x\n",
			(imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_CB) & 0xffff0000) >> 16,
			imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_CB) & 0xffff);
		pr_info("tg_offset  (0x60)  :0x%8x 0x%8x\n",
			(imgresz_readl_relaxed(base +
			RW_IMG_RESZ_TGT_OFFSET) & 0xffff0000) >> 16,
			imgresz_readl_relaxed(base +
			RW_IMG_RESZ_TGT_OFFSET) & 0xffff);
	} else {
		pr_info("rd_h_offset_y(0x54):0x%8x 0x%8x\n",
			(imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_Y) & 0xfff000) >> 12,
			imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_Y) & 0xfff);
		pr_info("rd_h_offset_c(0x58):0x%8x 0x%8x\n",
			(imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_CB) & 0xfff000) >> 12,
			imgresz_readl_relaxed(base +
			RW_IMG_RESZ_SRC_OFFSET_CB) & 0xfff);
	    pr_info("tg_offset  (0x60)  :0x%8x 0x%8x\n",
			(imgresz_readl_relaxed(base +
			RW_IMG_RESZ_TGT_OFFSET) & 0xfff000) >> 12,
			imgresz_readl_relaxed(base +
			RW_IMG_RESZ_TGT_OFFSET) & 0xfff);
	}
	pr_info("H_factor_y (0x64)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_H8TAPS_FAC_Y) &
		0xffffff);
	pr_info("H_factor_c (0x68)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_H8TAPS_FAC_CB) &
		0xffffff);
	pr_info("V_factor_y (0x88)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_V4TAPS_SCL_Y));
	pr_info("V_factor_c (0x8c)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_V4TAPS_SCL_CB));
	pr_info("H_offset_y (0xc8)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_H8TAP_OFSET_Y));
	pr_info("H_offset_c (0xcc)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_H8TAP_OFSET_CB));
	pr_info("V_offset_y (0xd4)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_V4TAP_OFSET_Y));
	pr_info("V_offset_c (0xd8)  :0x%8x\n",
		imgresz_readl_relaxed(base + RW_IMG_RESZ_V4TAP_OFSET_C));
}

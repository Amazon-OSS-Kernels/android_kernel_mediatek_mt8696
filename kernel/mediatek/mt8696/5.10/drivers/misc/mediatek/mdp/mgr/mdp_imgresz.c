...
int mdp_imgresz_copy_buffer(struct mdp_task_struct *pTask)
{
	struct mdp_ion_struct *ion_hdl = &pTask->dst_ion_handle;
	struct mdp_buffer_struct *buf = &pTask->mdp_command.dst_buffer;
	char *tempBuffer = NULL;
	int width = 0;
	int height = 0;
	char *pVA[5] = {NULL};
	int idx = 0;
	int bufferSize = buf->width * buf->height * 3 / 2;

	tempBuffer = vmalloc(bufferSize);
	if (tempBuffer == NULL) {
		MDP_ERR("malloc tempbuffer fail\n");
		goto err_malloc_fail;
	}

	MDP_LOG("copy buffer width:%d pitch:%d, offset:0x%x\n",
		buf->width, buf->y_pitch, buf->buffer_info.cb_offset);
	pVA[0] = (char *)mdp_buffer_get_va(ion_hdl);
	if (!pVA[0])
		goto err_invalid_va;

	pVA[1] = pVA[0] + buf->buffer_info.cb_offset;

	for (height = 0; height < buf->height; height++)
		for (width = 0; width < buf->width; width++) {
			idx = width + height * buf->width;
			tempBuffer[idx] = *(pVA[0] + width + height * buf->y_pitch);
		}

	for (height = 0; height < buf->height / 2; height++)
		for (width = 0; width < buf->width; width++) {
			idx = buf->height * buf->width + width + height * buf->width;
			tempBuffer[idx] = *(pVA[1] + width + height * buf->y_pitch);
		}

	memcpy(pVA[0], tempBuffer, bufferSize);
	mdp_buffer_free_va(ion_hdl, pVA[0]);
	vfree(tempBuffer);
	return 0;
...
void mdp_imgresz_mgr_task_clk(struct mdp_task_struct *pTask,
	bool enable_clock)
{
	/*
	...
	enum imgresz_ticket_fun_type type = mdp_imgresz_get_function_type(
		pTask->mdp_command.src_buffer.color_format,
		pTask->mdp_command.src_buffer.width,
		pTask->mdp_command.src_buffer.height,
		pTask->mdp_command.dst_buffer.width,
		pTask->mdp_command.dst_buffer.height,
...
{
	struct task_struct *pThread, *pThread0, *pThread1;
	struct sched_param param = {2};

	pThread = kthread_run(mdp_imgresz_dispatch_hw, NULL,
		"mdp_imgresz_hw_dispatch_management");
	if (IS_ERR(pThread)) {
		MDP_ERR("create thread failed\n");
		return MDP_TASK_STATUS_CREATE_IMGRESZ_THREAD_FAIL;
	}
	/* adjust thread priority. */
	sched_setscheduler(pThread, SCHED_RR, &param);

	pThread0 = kthread_run(mdp_core_wait_task0, NULL,
		"mdp_core_wait_task0_management");
	if (IS_ERR(pThread0)) {
		MDP_ERR("create thread failed\n");
		return MDP_TASK_STATUS_CREATE_IMGRESZ_THREAD_FAIL;
	}
	sched_setscheduler(pThread0, SCHED_RR, &param);

	pThread1 = kthread_run(mdp_core_wait_task1, NULL,
		"mdp_core_wait_task1_management");
	if (IS_ERR(pThread1)) {
		MDP_ERR("create thread failed\n");
		return MDP_TASK_STATUS_CREATE_IMGRESZ_THREAD_FAIL;
	}
 ...
 */
enum imgresz_src_color_mode mdp_imgresz_map_src_format(
	enum DP_COLOR_ENUM color_format)
{
 ...
 */
enum imgresz_dst_color_mode mdp_imgresz_map_dst_format(
	enum DP_COLOR_ENUM color_format)
{
	/* TODO: we need to map more color format */
	switch (color_format) {
...
enum imgresz_ufo_type mdp_imgresz_get_ufo_format(
	enum DP_COLOR_ENUM color_format)
{
	if (DP_COLOR_GET_10BIT(color_format) &&
		DP_COLOR_GET_UFP_ENABLE(color_format))
 ...
 */
enum imgresz_ticket_fun_type mdp_imgresz_get_function_type(
	enum DP_COLOR_ENUM format,
	int src_width,
	int src_height,
	int dst_width,
	int dst_height,
	enum imgresz_mem_enum mem_type)
{
	enum imgresz_src_color_mode src_mode = IMGRESZ_SRC_COL_MD_NONE;
	...
	src_mode = mdp_imgresz_map_src_format(format);
	if ((src_mode == IMGRESZ_SRC_COL_MD_420_BLK) ||
	    (src_mode == IMGRESZ_SRC_COL_MD_420_RS)) {
		if ((src_width >= 1920) &&
		    (src_height >= 1080) &&
		    (dst_width >= 1920) &&
		    (dst_height >= 1080)) {
			MDP_LOG("use 2 hw to convert\n");
			return IMGRESZ_FUN_UFO_2HW;
		}
	}

	if (mem_type == IMGRESZ_MEM_SECURE) {
		MDP_LOG("SVP occupied 2 hw\n");
		return IMGRESZ_FUN_SECURE;
	}
...
enum MDP_TASK_STATUS mdp_imgresz_check_param(
	struct imgresz_src_buf_info *p_task_src_buffer,
	struct imgresz_dst_buf_info *p_task_dst_buffer)
{
	enum MDP_TASK_STATUS status = MDP_TASK_STATUS_OK;
	uint32_t size = 0;
	uint32_t buffersize = 0;

	if (p_task_src_buffer->pic_width > 8192 ||
	    p_task_src_buffer->pic_height > 8192 ||
	    p_task_dst_buffer->pic_width > 8192 ||
	    p_task_dst_buffer->pic_height > 8192 ||
	    p_task_src_buffer->pic_width < 4 ||
	    p_task_src_buffer->pic_height < 4 ||
	    p_task_dst_buffer->pic_width < 4 ||
	    p_task_dst_buffer->pic_height < 4) {
		MDP_ERR("unsupported mdp size: src [%dx%d] dst [%dx%d]\n",
			p_task_src_buffer->pic_width, p_task_src_buffer->pic_height,
			p_task_dst_buffer->pic_width, p_task_dst_buffer->pic_height);
		status = MDP_TASK_STATUS_INVALID_PARAM;
	}

	if ((p_task_src_buffer->src_mode == IMGRESZ_SRC_COL_MD_420_BLK) ||
	    (p_task_src_buffer->src_mode == IMGRESZ_SRC_COL_MD_420_RS)) {
		size = p_task_src_buffer->buf_width *
			   p_task_src_buffer->buf_height * 3 / 2;
		buffersize = p_task_src_buffer->buf_info.buffersize[0];

		if (buffersize < size) {
			MDP_ERR("[%d] err : src size[0x%x] src buffersize[%dx%d]\n",
				__LINE__, p_task_src_buffer->buf_info.buffersize[0],
				p_task_src_buffer->buf_width,
				p_task_src_buffer->buf_height);
			status = MDP_TASK_STATUS_INVALID_PARAM;
		}
	}

	if ((p_task_dst_buffer->dst_mode == IMGRESZ_DST_COL_MD_420_BLK) ||
	    (p_task_dst_buffer->dst_mode == IMGRESZ_DST_COL_MD_420_RS)) {
		size = p_task_dst_buffer->buf_width *
			   p_task_dst_buffer->buf_height * 3 / 2;
		buffersize = p_task_dst_buffer->buf_info.buffersize[0];

		if (buffersize < size) {
			MDP_ERR("[%d] err : dst size[0x%x] dst buffersize[%dx%d]\n",
				__LINE__, p_task_dst_buffer->buf_info.buffersize[0],
				p_task_dst_buffer->buf_width,
				p_task_dst_buffer->buf_height);
			status = MDP_TASK_STATUS_INVALID_PARAM;
		}
	}
	return status;
...
enum MDP_TASK_STATUS mdp_imgresz_fill_task(struct mdp_task_struct *pTask)
{
	...
	/* color format */
	p_task_src_buffer->src_mode =
		mdp_imgresz_map_src_format(p_command_buffer->color_format);
	if (p_task_src_buffer->src_mode == IMGRESZ_SRC_COL_MD_NONE) {
		...
		case DP_COLOR_YV12:
			...
			p_command_buffer->u_buffer_address =
				p_command_buffer->v_buffer_address;
		...
		default:
			MDP_ERR("3 plane unsupport mdp format:0x%x\n",
				p_command_buffer->color_format);
			status =
				MDP_TASK_STATUS_UNSUPPORTED_IMGRESZ_SRC_FORMAT;
			break;
		}
	...
	p_task_src_buffer->buf_info.mem_type =
	(enum imgresz_mem_enum)p_command_buffer->buffer_info.memory_type;

	p_task_src_buffer->buf_info.fd[0] =
		p_command_buffer->buffer_info.memory_type ==
		DP_MEMORY_SECURE ?
		p_command_buffer->buffer_info.secureHandle :
		p_command_buffer->buffer_info.fd;

	p_task_src_buffer->buf_info.y_offset =
		p_command_buffer->buffer_info.y_offset;
	p_task_src_buffer->buf_info.cb_offset =
		p_command_buffer->buffer_info.cb_offset;
	p_task_src_buffer->buf_info.cr_offset =
		p_command_buffer->buffer_info.cr_offset;
	p_task_src_buffer->buf_info.srcmetadata_offset =
		p_command_buffer->buffer_info.srcMetadata_offset;
	p_task_src_buffer->buf_info.metadata_size =
		p_command_buffer->buffer_info.Metadata_size;

	p_task_src_buffer->buf_info.buffersize[0] =
		p_command_buffer->buffer_info.buffersize[0];

	p_task_src_buffer->y_buf_addr =
		p_command_buffer->y_buffer_address; /* y buffer */
	p_task_src_buffer->cb_buf_addr =
		p_command_buffer->u_buffer_address; /* u buffer */
	/* v buffer (not used)*/
	p_task_src_buffer->cr_buf_addr =
		p_command_buffer->v_buffer_address;
	p_task_src_buffer->buf_width =
		p_command_buffer->y_pitch; /* pitch */
	/* buffer height align value */
	p_task_src_buffer->buf_height =
		p_command_buffer->buffer_height;
	p_task_src_buffer->pic_width =
		p_command_buffer->width; /* width */
	p_task_src_buffer->pic_height =
		p_command_buffer->height; /* height */
	p_task_src_buffer->pic_x_offset =
		p_command_buffer->pic_x_offset; /* default 0 */
	p_task_src_buffer->pic_y_offset =
		p_command_buffer->pic_y_offset; /* default 0 */

	p_task_src_buffer->interlaced =
		DP_COLOR_GET_INTERLACED_MODE(
			p_command_buffer->color_format);
	p_task_src_buffer->topfield =
		(p_command_buffer->interlace_format == eTop_Field);
	p_task_src_buffer->bottomfield =
		(p_command_buffer->interlace_format == eBottom_Field);
	...
	if (p_task_dst_buffer->dst_mode == IMGRESZ_DST_COL_MD_NONE) {
		MDP_ERR("unsupported dst mdp format:0x%x\n",
			p_command_buffer->color_format);
		status = MDP_TASK_STATUS_UNSUPPORTED_IMGRESZ_DST_FORMAT;
		return status;
	}

	p_task_dst_buffer->buf_info.mem_type =
	(enum imgresz_mem_enum)p_command_buffer->buffer_info.memory_type;

	p_task_dst_buffer->buf_info.fd[0] =
		p_command_buffer->buffer_info.memory_type ==
			DP_MEMORY_SECURE ?
			p_command_buffer->buffer_info.secureHandle :
			p_command_buffer->buffer_info.fd;

	p_task_dst_buffer->buf_info.y_offset =
		p_command_buffer->buffer_info.y_offset;
	p_task_dst_buffer->buf_info.cb_offset =
		p_command_buffer->buffer_info.cb_offset;
	p_task_dst_buffer->buf_info.cr_offset =
		p_command_buffer->buffer_info.cr_offset;
	p_task_dst_buffer->buf_info.dstmetadata_offset =
		p_command_buffer->buffer_info.dstMetadata_offset;

	p_task_dst_buffer->buf_info.buffersize[0] =
		p_command_buffer->buffer_info.buffersize[0];

	p_task_dst_buffer->y_buf_addr =
		p_command_buffer->y_buffer_address;
	p_task_dst_buffer->c_buf_addr =
		p_command_buffer->u_buffer_address;
	p_task_dst_buffer->pic_width =
		p_command_buffer->width;
	p_task_dst_buffer->pic_height =
		p_command_buffer->height;
	p_task_dst_buffer->buf_width =
		p_command_buffer->y_pitch;
	p_task_dst_buffer->buf_height =
		p_command_buffer->buffer_height;
	p_task_dst_buffer->pic_x_offset =
		p_command_buffer->pic_x_offset;
	p_task_dst_buffer->pic_y_offset =
		p_command_buffer->pic_y_offset;
	p_task_dst_buffer->cbcr_swap =
		(p_command_buffer->color_format == DP_COLOR_NV21) ?
		!need_cbcr_swap : need_cbcr_swap;

	status = mdp_imgresz_check_param(p_task_src_buffer, p_task_dst_buffer);
...
bool mdp_imgresz_get_ticket(struct mdp_task_struct *pTask,
	IMGRESZ_TICKET *pTicket)
{
	enum imgresz_ticket_fun_type type;

	type = mdp_imgresz_get_function_type(
		pTask->mdp_command.src_buffer.color_format,
		pTask->mdp_command.src_buffer.width,
		pTask->mdp_command.src_buffer.height,
		pTask->mdp_command.dst_buffer.width,
...
struct mdp_task_struct *mdp_imgresz_get_mdp_task(int i)
{
	return imgresz_get_cbdata(TICKET_FROM_HWID(i));
}
 ...
 */
enum MDP_TASK_STATUS mdp_imgresz_wait_hardware_ready(
	struct mdp_task_struct *pTask)
{
	...
	add_wait_queue(
		&global_mdp_task.mdp_core_wait_imgresz_ticket_queue, &wait);
 ...
 */
enum MDP_TASK_STATUS mdp_imgresz_trigger_hw_work(
	struct mdp_task_struct *pTask)
{
	enum MDP_TASK_STATUS status = MDP_TASK_STATUS_OK;
	...
	do {
		...
		/* set scale mode */
		ret = imgresz_set_scale_mode(
			pTask->task.imgresz_task.ticket, IMGRESZ_FRAME_SCALE);
		if (ret < 0) {
		...
		ret = imgresz_set_src_bufinfo(pTask->task.imgresz_task.ticket,
			&pTask->task.imgresz_task.src_buffer);
		if (ret < 0) {
		...
		ret = imgresz_set_dst_bufinfo(pTask->task.imgresz_task.ticket,
			&pTask->task.imgresz_task.dst_buffer);
		if (ret < 0) {
		...
		if (pTask->task.imgresz_task.ticket == TICKET_FROM_HWID(0))
			wake_up(&global_mdp_task.mdp_core_start_wait_task0_queue);
		else if (pTask->task.imgresz_task.ticket == TICKET_FROM_HWID(1))
			wake_up(&global_mdp_task.mdp_core_start_wait_task1_queue);

		mdp_core_set_mmp_tag(pTask, MMP_DISP_MDP_SCALE_ASYNC);
		ret = imgresz_trigger_scale_async(
			pTask->task.imgresz_task.ticket, async_mode);
		if (ret < 0) {
 ...
 * so we need to add lock to ensure only one thread
 * is getting ticket at one time.
 */
bool mdp_imgresz_is_hardware_ready(
	enum imgresz_ticket_fun_type function_type,
	IMGRESZ_TICKET *pTicket)
{
...
bool mdp_imgresz_normal_list_first_task_filled(void)
{
	...
	mutex_lock(
		&global_mdp_task.imgresz_normal_prepare_task_list.mutex_lock);
	if (!list_empty(
		&global_mdp_task.imgresz_normal_prepare_task_list.list)) {
		pTask = list_first_entry(
			&global_mdp_task.imgresz_normal_prepare_task_list.list,
...
bool mdp_imgresz_ufo_list_first_task_filled(void)
{
	...
	if (!list_empty(&global_mdp_task.imgresz_ufo_prepare_task_list.list)) {
		pTask = list_first_entry(
			&global_mdp_task.imgresz_ufo_prepare_task_list.list,
			struct mdp_task_struct, list_entry);
		ufo_list_filled =
			(pTask->current_state == MDP_TASK_STATE_FILLED);
	}
...
bool mdp_imgresz_ufo_2_hw_list_first_task_filled(void)
{
	...
	mutex_lock(
		&global_mdp_task.imgresz_ufo_2_hw_prepare_task_list.mutex_lock);
	if (!list_empty(
		&global_mdp_task.imgresz_ufo_2_hw_prepare_task_list.list)) {
		pTask = list_first_entry(
		&global_mdp_task.imgresz_ufo_2_hw_prepare_task_list.list,
		struct mdp_task_struct, list_entry);

		ufo_2_hw_list_filled = (pTask->current_state ==
			MDP_TASK_STATE_FILLED);
	}
	mutex_unlock(
		&global_mdp_task.imgresz_ufo_2_hw_prepare_task_list.mutex_lock);
...
bool mdp_imgresz_set_type(enum imgresz_ticket_fun_type *pType,
	enum imgresz_ticket_fun_type value)
{
	*pType = value;
	return true;
}
...
int32_t mdp_imgresz_dispatch_hw(void *ignore)
{
	...
	add_wait_queue(
		&global_mdp_task.imgresz_wait_trigger_list_not_empty, &wait);
	do {
		/* find a task in trigger buffer list */
		spin_lock_irqsave(
			&global_mdp_task.trigger_imgresz_task_list.spin_lock,
			flags);
		pTask = list_first_entry_or_null(
		...
		if ((pTask == NULL) ||
			!mdp_imgresz_get_ticket(
				pTask, &pTask->task.imgresz_task.ticket)) {
			wait_woken(&wait,
				TASK_INTERRUPTIBLE,
	...
	remove_wait_queue(
		&global_mdp_task.imgresz_wait_trigger_list_not_empty,
		&wait);

	return 0;
}

int mdp_imgresz_handle_irq(IMGRESZ_TICKET ti,
	enum imgresz_cb_state hw_state, void *privdata)
{
	struct mdp_task_struct *pTask = (struct mdp_task_struct *)privdata;
	struct mdp_buffer_struct *dst_buf = &pTask->mdp_command.dst_buffer;

	mdp_core_set_mmp_tag(pTask, MMP_DISP_MDP_HW_DONE);
	pTask->time.gotIRQ = sched_clock();

	if (dst_buf->profile == DP_PROFILE_JPEG &&
	    dst_buf->width != dst_buf->y_pitch) {
		/* Root-Cause: Golden data is a buffer which width equals to pitch,
		 * imgresz has hw limitation which output buffer pitch need 16-align
		 * this will led to compare failure.
		 *
		 * Solution: Alloc a temp buffer whose width equals to pitch.
		 * Copy output buffer data to temp buffer
		 * to eliminate garbage caused by alignment,
		 * and then copy temp back to output buffer.
		 */
		mdp_imgresz_copy_buffer(pTask);
	}

	mdp_release_frame_count++;
	pTask->current_state = MDP_TASK_STATE_DONE;
	if (TICKET_TO_HWID(ti) == 0)
		wake_up(&global_mdp_task.mdp_core_wait_done_task0_queue);
	else if (TICKET_TO_HWID(ti) == 1)
		wake_up(&global_mdp_task.mdp_core_wait_done_task1_queue);
...
{
#if IS_ENABLED(CONFIG_MTK_CLI_DEBUG_SUPPORT)
	...
	MDP_ERR("IMGREZ_SRC: buffer type[%d->%s] fd[0x%08x]",
		pTask->task.imgresz_task.src_buffer.buf_info.mem_type,
		(pTask->task.imgresz_task.src_buffer.buf_info.mem_type !=
			IMGRESZ_MEM_SECURE) ? "ion" : "secure",
		pTask->task.imgresz_task.src_buffer.buf_info.fd[0]);

	MDP_ERR(" offset:y[0x%x] cb[0x%x] cr[0x%x] ylen[0x%x] clen[0x%x]\n",
	...
	MDP_ERR("IMGREZ_SRC: format[%d] Y[0x%x] CB[0x%x] CR[0x%x]",
	...
	MDP_ERR(" W[%u] H[%u] YPitch[%u] bufferHeight[%u]\n",
	...
	MDP_ERR("IMGRESZ_SRC: xOffset[%d] yOffset[%d] interlace[%d]",
	...
	MDP_ERR(" ylen_len[%u] clen_len[%u] ufo_jump[%d]\n",
	...
	MDP_ERR("IMGREZ_DST: buffer type[%d->%s] fd[0x%08x]",
		pTask->task.imgresz_task.dst_buffer.buf_info.mem_type,
		(pTask->task.imgresz_task.dst_buffer.buf_info.mem_type ==
			IMGRESZ_MEM_ION) ? "ion" : "secure",
		pTask->task.imgresz_task.dst_buffer.buf_info.fd[0]);
	...
	MDP_ERR("IMGREZ_DST: format[%d] Y[0x%x] CB[0x%x]",
	...
	MDP_ERR(" YPitch[%u] W[%u] WCb[%u] WCr[%u] H[%u]\n",

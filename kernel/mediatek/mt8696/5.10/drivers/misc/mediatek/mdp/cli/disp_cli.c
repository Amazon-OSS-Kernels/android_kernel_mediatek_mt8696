...
int32_t cli_register(char *module_name, struct CLI_EXEC_T item[])
{
	...
	for (i = 0; item[i].function != NULL; i++) {
		struct module_item_struct *pItem =
			vmalloc(sizeof(struct module_item_struct));
		...
		if (snprintf(pItem->module_name, sizeof(pItem->module_name),
				"%s", module_name) < 0) {
...
enum CLI_STATUS cli_core_fill_items(struct cli_items_struct *pItem)
{
	...
	list_for_each_entry(pTempItem, &gCliInfo.gCliListHead, list) {
		for (i = 0; i < pItem->module_count; i++) {
			if (strcmp(pTempItem->module_name,
				pItem->module[i].module_name) == 0) {
				module_index = i;
				break;
			}
		}
		if (i == pItem->module_count) { /* find new module */
			...
			if (pItem->module_count >=  MAX_MODULE_COUNT) {
				CLI_ERR("module count is too large\n");
		...
		/* find module index */
		pUserItem = &(pItem->module[module_index].items[
				pItem->module[module_index].item_count]);
		size = snprintf(pUserItem->name, sizeof(pUserItem->name),
			"%s", pTempItem->module_case.name);
		if (size < 0) {
			CLI_ERR("copy string fail:%d\n", __LINE__);
			return CLI_STATUS_OK;
		}
		size = snprintf(pUserItem->module_name,
			sizeof(pUserItem->module_name),
			"%s", pTempItem->module_name);
		if (size < 0) {
			CLI_ERR("copy string fail:%d\n", __LINE__);
			return CLI_STATUS_OK;
		}
		size = snprintf(pUserItem->detail,
			sizeof(pUserItem->detail), "%s",
			pTempItem->module_case.detail);
		if (size < 0) {
			CLI_ERR("copy string fail:%d\n", __LINE__);
			return CLI_STATUS_OK;
		}
...
enum CLI_STATUS cli_parse_command(char *pCommand, int *pWordCount,
		char *argv[], int size)
{
	int i = 0;
	char *pBeg = pCommand;
...
enum CLI_STATUS cli_handle_command(struct cli_exec_command_struct *pCommand)
{
	...
	for (i = 0; i < MAX_PARAM_COUNT; i++) {
		...
		if (argv[i] == NULL) {
			/* allocat fail, need to release
			 * all allcated buffer and return
			 */
			while (--i >= 0)
				vfree(argv[i]);
	...
	list_for_each_entry(pTempItem, &gCliInfo.gCliListHead, list) {
		if (strcmp(pTempItem->module_name,
				pCommand->item.module_name) == 0 &&
			strcmp(pTempItem->module_case.name,
				pCommand->item.name) == 0) {
			/* since pCommand->param_string is parse from userspace,
			 * param_string may not end with \0
			 * kernel needs to make sure pCommand->param_string
			 * ends with '\0' before use it.
			 */
			pCommand->param_string[MAX_LONG_STRING_LEN - 1] = '\0';

			/* find the execute item. parse & execute. */
			status = cli_parse_command(pCommand->param_string,
				&argc, argv, MAX_STRING_LEN);
			if (status != 0) {
				CLI_ERR("cli_parse_command fail:%d\n", status);
				break;
			}
			status = pTempItem->module_case.function(argc,
				(const char **)argv);
			if (status != 0) {
...
static long cli_ioctl(struct file *pFile, unsigned int code,
	unsigned long param)
{
	enum CLI_STATUS status = CLI_STATUS_OK;
	struct cli_items_struct *pItems = NULL;
	struct cli_exec_command_struct *pCommand = NULL;
	...
	case CLI_GET_ITMES:
		...
		if (status != CLI_STATUS_OK)
			break;
		if (copy_to_user((void *)param, (void *)pItems,
			sizeof(struct cli_items_struct))) {
			CLI_ERR("copy to user fail\n");
			status = CLI_STATUS_COPY_TO_USER_FAIL;
		}
		break;
	case CLI_EXEC_COMMAND:
		if (copy_from_user(pCommand, (void *)param,
			sizeof(struct cli_exec_command_struct))) {
...
static long cli_ioctl_compat(struct file *pFile,

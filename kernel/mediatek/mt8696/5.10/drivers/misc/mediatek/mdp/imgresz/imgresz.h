...
#define IMGRESZ_TEE_ENABLE 1

#define TZ_TA_IMGRESZ_UUID "88da5a2c-22c1-88dc-49a8-24f9bdc58890"
...
#else
#define IMGRESZ_TEE_ENABLE 0
#endif

#if IS_ENABLED(CONFIG_MTK_IMGRZ_TEST)
#if IMGRESZ_TEE_ENABLE
extern KREE_SECUREMEM_HANDLE imgresz_test_sec_src_mem;
extern KREE_SECUREMEM_HANDLE imgresz_test_sec_dst_mem;
#endif
extern dma_addr_t imgresz_test_buffer_pa;
extern unsigned int imgresz_test_buffer_size;
extern unsigned long imgresz_test_buffer_va;
extern void imgresz_debug_init(struct device *dev);
#endif
extern enum imgresz_chip_ver imgresz_cur_chip_ver;

#define IMGRESZ_TICKET	int

enum imgresz_status {
	IMGRESZ_STATUS_OK = 0,
	IMGRESZ_STATUS_ERROR = 1,
};
...
struct imgresz_buffer_info {
	...
	//hdr metadata info
	uint32_t srcmetadata_offset;
	uint32_t dstmetadata_offset;
...
struct imgresz_src_buf_info {
	 ...
	 *    If you'd like to use 32bit va, please
	 *    call imgresz_legacy_iommu_enable.
	 * 2) all the dma_addr_t below are the same.
	 */
	uint32_t y_buf_addr;
	uint32_t cb_buf_addr;
	uint32_t cr_buf_addr; /* Only needed by jpg. */
	struct imgresz_buffer_info buf_info;
	...
	bool topfield;
	bool bottomfield;
	/* Only for jpg compoent factor ratio */
	struct imgresz_jpg_comp_sample jpg_comp;
...
typedef int (*imgresz_callback_fun)(IMGRESZ_TICKET ti,
	enum imgresz_cb_state hw_state,
	void *privdata);
 ...
 */
int imgresz_set_scale_mode(IMGRESZ_TICKET ti,
		enum imgresz_scale_mode scalemd);
int imgresz_set_src_bufinfo(IMGRESZ_TICKET ti,
		struct imgresz_src_buf_info *srcbufinfo);
int imgresz_set_dst_bufinfo(IMGRESZ_TICKET ti,
		struct imgresz_dst_buf_info *dstbufinfo);
int imgresz_set_jpeg_info(IMGRESZ_TICKET ti,
		struct imgresz_jpg_info *jpeginfo);
int imgresz_set_rm_info(IMGRESZ_TICKET ti,
		struct imgresz_rm_info *rminfo);
 ...
 * please don't add so many code in the callback
 *function as it is in irq context.
 */
int imgresz_set_callback_fun
	(IMGRESZ_TICKET ti, imgresz_callback_fun fun, void *privdata);
 ...
 *
 * Note that: don't call this in arm64.
 *if this is enable, use va in "dma_addr_t".
 */
int imgresz_legacy_iommu_enable
			(IMGRESZ_TICKET ti, phys_addr_t pgtablebase,

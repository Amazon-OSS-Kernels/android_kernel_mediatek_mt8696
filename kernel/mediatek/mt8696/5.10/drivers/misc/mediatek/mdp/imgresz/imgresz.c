...
#if IS_ENABLED(CONFIG_MTK_IMGRZ_TEST)
static KREE_SESSION_HANDLE imgresz_mem_session;
#endif
#endif

struct device *imgresz_dev;
...
static bool imgresz_valid_ticket(const IMGRESZ_TICKET ti)
{
	return ((ti >= 0) &&
		(!!(ti & (0x1 << 28))) &&
		(TICKET_TO_HWID(ti) < IMGRESZ_MAX_NR));
}
...
void *imgresz_get_cbdata(const IMGRESZ_TICKET ti)
{
...
struct imgresz_data *imgresz_get_data_2nd(struct imgresz_data *data)
{
	if (imgresz_cur_chip_ver >= IMGRESZ_CURR_CHIP_VER_8695) {
		if (data->scale_data.ufo_h_partition ||
		    ...
		    data->scale_data.funtype == IMGRESZ_FUN_2HW)
	...
	if (imgresz_log_level & IMGRESZ_LOG_SIMLOG) {
		...
		for (; hwid < IMGRESZ_MAX_NR; hwid++) {
			if (addr >= imgresz_inst_data[hwid]->base &&
				...
				pr_info("[ImgResz]HWrite(%d + 'h%lx, 32'h%x);\n",
...
u32 imgresz_readl_relaxed(void __iomem *addr)
{
	...
	if (imgresz_log_level & IMGRESZ_LOG_SIMLOG) {
		...
		for (; hwid < IMGRESZ_MAX_NR; hwid++) {
			if (addr >= imgresz_inst_data[hwid]->base &&
			    addr < imgresz_inst_data[hwid]->base + 0x1000) {
				pr_info("[ImgResz]HRead(%d + 'h%lx, 32'h%x);\n",
				hwid, addr - imgresz_inst_data[hwid]->base,
				value);
				break;
			}
...
imgresz_ticket_get_8695(enum imgresz_ticket_fun_type type)
{
	...
	if (type == IMGRESZ_FUN_UFO_2HW ||
	    ...
	    type == IMGRESZ_FUN_SECURE ||
	    type == IMGRESZ_FUN_2HW)
...
int imgresz_ticket_put_8695(IMGRESZ_TICKET ti)
{
	...
	data->cb_info.fun_cb = NULL;
	data->cb_info.cb_data = NULL;
	 ...
	 */
	if (data->scale_data.ufo_h_partition ||
	    data->scale_data.funtype == IMGRESZ_FUN_SECURE ||
	    data->scale_data.funtype == IMGRESZ_FUN_2HW) {
...
struct device *imgresz_get_dev(void)
{
	return imgresz_dev;
}
			...
			struct imgresz_src_buf_info *srcbuf)
{
	...
	if (srcbuf->buf_info.mem_type == IMGRESZ_MEM_PHY ||
		srcbuf->buf_info.mem_type == IMGRESZ_MEM_MVA) {
			...
			struct imgresz_dst_buf_info *dstbuf)
{
	...
	if (dstbuf->buf_info.mem_type == IMGRESZ_MEM_PHY ||
		dstbuf->buf_info.mem_type == IMGRESZ_MEM_MVA) {
...
static int _imgresz_hal_if_do_scale(struct imgresz_data *data)
{
	...
	struct imgresz_data *imgrsz_data = NULL;
	void __iomem *base = data->base;
	void __iomem *base_2nd = NULL;
	void __iomem *base_2nd_secbase = NULL;
	...
	imgresz_hal_set_dst_buf_addr(base, data->base_sec, dst_buf->y_buf_addr,
				     dst_buf->c_buf_addr);
	imgresz_hal_set_dst_buf_pitch(base, dst_buf->buf_width);
	...
	if (scaledata->one_phase) {
		imgrsz_data = imgresz_get_data_2nd(data);
		if (imgrsz_data == NULL)
			return -ENODEV;
		base_2nd = imgrsz_data->base;
		base_2nd_secbase =  imgrsz_data->base_sec;
	...
	if (imgresz_log_level & IMGRESZ_LOG_SHOWREG) {
		loginfo(IMGRESZ_LOG_SHOWREG, "Begin to trigger hw: %d\n",
			data->hw_id);
		imgresz_hal_print_reg(base, data->base_sec);
		if (scaledata->one_phase && base_2nd)
			imgresz_hal_print_reg(base_2nd, base_2nd_secbase);
	}
...
static int imgresz_cal_v_factor(struct imgresz_scale_data *scaledata)
{
	...
	unsigned int factor = 0, offset = 0;
...
static bool imgresz_ticket_is_done(struct imgresz_data *data)
{
	...
	spin_lock_irqsave(state_lock, flags);
	if (data->scale_data.ufo_v_partition ||
	    data->scale_data.ufo_h_partition ||
	    data->scale_data.one_phase ||
	    data->scale_data.funtype == IMGRESZ_FUN_2HW) {
...
static
irqreturn_t __attribute__((unused)) imgresz_irq_top_half
			(int irq, void *dev_id)
{
	struct imgresz_data *data = (struct imgresz_data *)dev_id;
	struct imgresz_scale_data *scaledata = &data->scale_data;

	if (scaledata->src_buf.buf_info.mem_type == IMGRESZ_MEM_SECURE)
		return IRQ_HANDLED;
			...
			(int irq, void *dev_id)
{
	...
	if (imgresz_log_level & IMGRESZ_LOG_DONEREG)
		imgresz_hal_print_reg(data->base, data->base_sec);

	if (data->state != IMGRESZ_STATE_BUSY) {
		logwarn("%s, id:%d, state:%d, type:%d\n", __func__,
			data->hw_id, data->state,
			scaledata->src_buf.buf_info.mem_type);
		return IRQ_HANDLED;
	}
	...
	if (scaledata->ufo_h_partition ||
	    ...
	    scaledata->one_phase ||
	    scaledata->funtype == IMGRESZ_FUN_2HW) {
...
int imgresz_trigger_scale_async(IMGRESZ_TICKET ti, bool async)
{
	...
	unsigned long flags;
	struct imgresz_tz_data *data_tz = NULL, *data_tz_2nd = NULL;
	enum imgresz_state tzstate = IMGRESZ_STATE_DONE;
	enum imgresz_state tzstate_2nd = IMGRESZ_STATE_DONE;
#endif
	struct imgresz_data *data = NULL, *data_2nd = NULL;
	 ...
	 *msecs_to_jiffies(IMGRESZ_HW_TIMEOUT));
	 */
	...
	if (scaledata->src_buf.buf_info.mem_type == IMGRESZ_MEM_SECURE) {
		...
		spin_lock_irqsave(state_lock, flags);
		if (data_tz)
			tzstate = data_tz->state;
		if (scaledata->ufo_v_partition ||
		    scaledata->ufo_h_partition ||
		    scaledata->funtype == IMGRESZ_FUN_2HW) {
			if (data_tz_2nd)
				tzstate_2nd = data_tz_2nd->state;
		}
		spin_unlock_irqrestore(state_lock, flags);
		...
		if (scaledata->ufo_v_partition ||
		    ...
		    scaledata->one_phase ||
		    scaledata->funtype == IMGRESZ_FUN_2HW) {
			if ((tzstate != IMGRESZ_STATE_DONE) ||
			    (tzstate_2nd != IMGRESZ_STATE_DONE))
				logwarn("[%s] tzstate[%d], tzstate_2nd[%d]\n",
					__func__, tzstate, tzstate_2nd);
		} else {
			if (tzstate != IMGRESZ_STATE_DONE)
				logwarn("[%s] tzstate[%d]\n", __func__, tzstate);
		}
	...
	if ((scaledata->ufo_v_partition ||
	    scaledata->ufo_h_partition ||
	    scaledata->funtype == IMGRESZ_FUN_2HW) && data_2nd) {
		imgresz_hal_HW_init(data_2nd->base);
	...
	if (!async) {
		wait_event_timeout(data->wait_hw_done_queue,
...
static int imgresz_probe(struct platform_device *pdev)
{
	...
	if (devm_request_threaded_irq(dev, data->irq, imgresz_irq_top_half,
				      imgresz_irq_handle, IRQF_SHARED,
	...
	data->hw_timeout = msecs_to_jiffies(IMGRESZ_HW_TIMEOUT);
	/*
...
#if IS_ENABLED(CONFIG_MTK_IMGRZ_TEST)
	if (data->hw_id == 0) {
#if IS_ENABLED(CONFIG_MTK_IMGRZ_TEST_PA)
		...
		if (of_address_to_resource(np, 0, &res1))
			logwarn("imgresz of_address_to_resource failed\n");
		imgresz_test_buffer_pa = (unsigned int)res1.start;
		imgresz_test_buffer_size = (unsigned int)resource_size(&res1);
		imgresz_test_buffer_va =
			(unsigned long)ioremap_wc((phys_addr_t)
					imgresz_test_buffer_pa,
					imgresz_test_buffer_size);
#else
		imgresz_test_buffer_size = 128*1024*1024;
		imgresz_test_buffer_va = (unsigned long)dma_alloc_coherent
			(&pdev->dev, imgresz_test_buffer_size,
			(dma_addr_t *)&imgresz_test_buffer_pa, GFP_KERNEL);
#endif
		logwarn("test region va:0x%lx, pa:0x%pad, size:%u\n",
			imgresz_test_buffer_va, &imgresz_test_buffer_pa,
			imgresz_test_buffer_size);
...
static enum imgresz_status imgresz_sec_create_session(void)
{
	...
	if (imgresz_ta_session == 0) {
		ret = KREE_CreateSession(TZ_TA_IMGRESZ_UUID,
			&imgresz_ta_session);
		if (ret != TZ_RESULT_SUCCESS) {

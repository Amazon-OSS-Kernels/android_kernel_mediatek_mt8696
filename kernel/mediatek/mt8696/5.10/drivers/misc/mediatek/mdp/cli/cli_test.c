...
int cli_test_param(int argc, const char **argv)
{
	...
	for (i = 0; i < argc; i++)
		pr_debug("%s argv[%d] is %s\n", __func__, i, argv[i]);
	return 0;
}

struct CLI_EXEC_T items[] = {
	{"init",  NULL, init_cli_test,
		NULL,  "cli Test: Init cli test", CLI_GUEST},
	{"testparam", NULL, cli_test_param,
		NULL, "cli Test: test param", CLI_GUEST},

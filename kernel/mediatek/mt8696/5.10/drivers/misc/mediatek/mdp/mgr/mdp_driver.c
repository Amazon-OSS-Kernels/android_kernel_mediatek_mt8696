...
static long mdp_ioctl(struct file *pFile,
	unsigned int code, unsigned long param)
{
	...
	if (mdp_driver_is_suspend()) {
		pr_notice_ratelimited("HW is suspend, bypass\n");
		return 0;
	}
	...
	case MDP_IOCTL_EXEC_COMMAND:
		if (copy_from_user(&command,
			(void *)param, sizeof(struct mdp_command_struct))) {
			pr_notice_ratelimited(
				"copy MDP_IOCTL_EXEC_COMMAND info failed\n");
			return MDP_TASK_STATUS_COPY_FROM_USER_FAIL;
		}
		mdp_config_frame_count++;

		/* force taskString last char = \0 to
		 * ensure taskString is a string type.
		 */
		...
		if (status != MDP_TASK_STATUS_OK)
			pr_notice_ratelimited(
				"exec MDP_IOCTL_EXEC_COMMAND error:%d\n",
				status);
	...
	default:
		pr_notice_ratelimited(
			"undefined ioctl:0x%x kernel:0x%lx\n",
				code, (unsigned long)MDP_IOCTL_EXEC_COMMAND);
		pr_notice_ratelimited(
			"(mdp_command_struct): %zu, (mdp_buffer_struct): %zu\n",
			sizeof(struct mdp_command_struct),
			sizeof(struct mdp_buffer_struct));
		return MDP_TASK_STATUS_UNDEFINED_IOCTL;
	}

	if (copy_to_user((void *)param, (void *)&command,
		sizeof(struct mdp_command_struct))) {
		pr_notice_ratelimited("copy to user fail\n");
...
static long mdp_ioctl_compat(struct file *pFile,
	unsigned int code, unsigned long param)
{
	switch (code) {
	case MDP_IOCTL_EXEC_COMMAND:
	...
	default:
		pr_notice_ratelimited(
			"undefined compat ioctl:0x%x, kernel:0x%lx, size:%ld\n",
			code, MDP_IOCTL_EXEC_COMMAND,
			sizeof(struct mdp_command_struct));
		pr_notice_ratelimited(
			"(mdp_command_struct): %ld, (mdp_buffer_struct): %ld\n",
			sizeof(struct mdp_command_struct),
			sizeof(struct mdp_buffer_struct));
		pr_notice_ratelimited(
			"(DPCOLOR_ENUM): %ld, (DpInterlaceFormat): %ld\n",
...
void mdp_shutdown(struct platform_device *pdev)
{
	...
	if (status < 0) {
		/* Note: in this case, HW may access dram
		 * while power off disp_m domain.
		 * This may cause HW hang.
		 */
		MDP_ERR("%s wait hw idle timeout", __func__);
	}
}
...
static const struct proc_ops error_fop = {
	.proc_open = mdp_proc_print_open_error,
	.proc_read = seq_read,
	.proc_release = single_release,
};

static const struct proc_ops record_fop = {
	.proc_open = mdp_proc_print_open_record,
	.proc_read = seq_read,
	.proc_release = single_release,
};
...
static int mtk_mdp_device_probe(struct platform_device *pdev)
{
	...
	mdp_cdev->ops = &fop;
	ret = cdev_add(mdp_cdev, mdp_devno, 1);
	if (ret)
		return ret;
...
enum MDP_TASK_STATUS mdp_driver_create_fs(void)
{
	...
	do {
		...
		if (gMdpProcEntry == NULL) {
			pr_notice("MDP_ERR: creat /proc/mdp dir fail\n");
			return MDP_TASK_STATUS_CREATE_MDP_DIR_FAIL;
		}

		pDir = proc_create("error", 0640, gMdpProcEntry, &error_fop);
		if (!pDir) {
			pr_notice("MDP_ERR: create /proc/mdp/error fail\n");
			return MDP_TASK_STATUS_CREATE_MDP_ERROR_FAIL;
		}
		MDP_LOG("create /proc/mdp/error success\n");

		pDir = proc_create("record", 0640, gMdpProcEntry, &record_fop);
		if (!pDir) {
			pr_notice("MDP_ERR: create /proc/mdp/record fail\n");
...
static int __init mdp_init(void)
{
	...
	if (status != MDP_TASK_STATUS_OK)
		pr_notice("init mdp device fail:%d\n", status);

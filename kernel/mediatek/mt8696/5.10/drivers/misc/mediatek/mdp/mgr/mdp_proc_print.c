...
enum MDP_TASK_STATUS mdp_proc_add_error(char *fmt, ...)
{
	...
	va_start(arg, fmt);
	global_mdp_proc_print_list.error->count +=
		vsnprintf(global_mdp_proc_print_list.error->printBuffer +
			global_mdp_proc_print_list.error->count,
		len, fmt, arg);
	va_end(arg);
...
enum MDP_TASK_STATUS mdp_proc_add_time_item(
	struct mdp_task_struct *pTask)
{
	...
	if (snprintf(pTime->taskString, sizeof(pTime->taskString),
		"%s", pTask->mdp_command.taskString) < 0)
		MDP_ERR("print buffer fail\n");

	mutex_lock(&global_mdp_proc_print_list.record->lock);
	if (global_mdp_proc_print_list.record->record_count >
		SHOW_RECORD_COUNT) {
		struct task_time_item_struct *pTempTime;

		pTempTime = list_first_entry(
			&global_mdp_proc_print_list.record->listHead,
			struct task_time_item_struct, list);
		list_del_init(&pTempTime->list);
		vfree(pTempTime);
		global_mdp_proc_print_list.record->record_count--;
	}
	list_add_tail(&pTime->list,
		&global_mdp_proc_print_list.record->listHead);
	global_mdp_proc_print_list.record->record_count++;
...
int mdp_proc_print_show_record(struct seq_file *seq, void *v)
{
	...
	mutex_lock(&global_mdp_proc_print_list.record->lock);
	list_for_each_entry(pTime,
		&global_mdp_proc_print_list.record->listHead,
		list) {
		seq_printf(seq,
			"task %s start[0x%llx] [%lld]us trigger[0x%llx] [%lld]us",
				pTime->taskString, pTime->start,
				mdp_proc_get_time_in_us(pTime->start,
					pTime->trigger),
				pTime->trigger,
				mdp_proc_get_time_in_us(pTime->trigger,
					pTime->gotIRQ));
		seq_printf(seq,
			" gotIRQ[0x%llx] [%lld]us release[0x%llx] [%lld]us done[0x%llx]",
			pTime->gotIRQ,
			mdp_proc_get_time_in_us(pTime->gotIRQ,
				pTime->releaseFence),
			pTime->releaseFence,
			mdp_proc_get_time_in_us(pTime->releaseFence,
				pTime->done),
			pTime->done);
...
{
	global_mdp_proc_print_list.record = vmalloc(
		sizeof(struct mdp_proc_record_struct));
	if (global_mdp_proc_print_list.record == NULL) {
		MDP_ERR("create mdp_proc_record_struct fail\n");
		return MDP_TASK_STATUS_ALLOCATE_TIME_ITEM_FAIL;
	}
	memset(global_mdp_proc_print_list.record, 0,
		sizeof(struct mdp_proc_record_struct));

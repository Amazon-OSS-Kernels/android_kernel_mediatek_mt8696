...
#include "imx766dualomipiraw_Sensor.h"
#include "imx766dualo_eeprom.h"
#include "imx766dualo_ana_gain_table.h"
#include "imx766dualo_Sensor_setting.h"
...
#define imx766dualo_table_write_cmos_sensor_8(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)

#define IMX766DUALO_EEPROM_READ_ID  0xA2
#define IMX766DUALO_EEPROM_WRITE_ID 0xA3

#define PFX "IMX766DUALO_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX766DUALO_SENSOR_ID,
	...
	.normal_video = { /* Reg_K_4096x2304_30FPS */
		.pclk = 2246400000,
		.linelength = 15616,
		.framelength = 4792,
		.startx = 0,
	...
	.custom1 = { /* Reg_S_4096x2304_30FPS seamless K*/
		.pclk = 2246400000,
		.linelength = 15616,
		.framelength = 4794,
		.startx = 0,
	...
	.custom5 = { /* Reg_R1_1536x1152_24FPS */
		.pclk = 1416000000,
		.linelength = 26448,
		.framelength = 2228,
		.startx = 0,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[15] = {
	...
	/* custom4 */
	{8192, 6144,    0,  768, 8192, 4608, 4096, 2304,
	  512,    0, 3072, 2304,    0,    0, 4096, 2304},
	/* custom5 */
	{8192, 6144,    0,  768, 8192, 4608, 2048, 1152,
	  256,    0, 1536, 1152,    0,    0, 1536, 1152},
	/* custom6 */
	{8192, 6144,    0,    0, 8192, 6144, 4096, 3072,
		0,    0, 4096, 3072,    0,    0, 4096, 3072},
	/* custom7 */
...
static struct SENSOR_VC_INFO2_STRUCT SENSOR_VC_INFO2[15] = {
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00, //preivew
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x1000, 0x300},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0x1000, 0x300},
		},
		1
	},
	{
	...
	},
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00, //normal_video
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x1000, 0x240},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0x1000, 0x240},
		},
		1
	},
	{
	...
	},
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00, //custom3
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x1000, 0xc00},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0x1000, 0xc00},
		},
		1
	},
	{
		0x04, 0x0a, 0x00, 0x08, 0x40, 0x00, //custom4
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0xc00, 0x240},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0xc00, 0x240},
		},
		1
	},
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00, //custom5
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x600, 0x120},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0x600, 0x120},
		},
		1
	},
	{
		...
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0x1000, 0xc00},
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x800, 0x600},
		},
		1
	},
	{
		...
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0x1000, 0x900},
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x800, 0x480},
		},
		1
	},
	{
...
static kal_uint16 imx766dualo_QSC_setting[3072 * 2];
...
static void imx766dualo_get_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
static void imx766dualo_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
		MUINT32 regNum, kal_uint16 *regDa)
{
	imx766dualo_table_write_cmos_sensor_8(ctx, regDa, regNum*2);
}
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, IMX766DUALO_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
...
static void read_sensor_Cali(struct subdrv_ctx *ctx)
{
	...
	if ((otp_data[0] == SENSOR_ID_L) &&
		...
		for (idx = 0; idx < QSC_SIZE; idx++) {
			...
			imx766dualo_QSC_setting[2 * idx] = sensor_qsc;
			imx766dualo_QSC_setting[2 * idx + 1] =
...
static void write_sensor_QSC(struct subdrv_ctx *ctx)
{
	...
	if ((otp_flag == OTP_QSC_CUSTOM) || (otp_flag == OTP_QSC_INTERNAL)) {
		imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_QSC_setting,
		sizeof(imx766dualo_QSC_setting) / sizeof(kal_uint16));
	}
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter, kal_bool gph)
{
	...
	if (shutter >
		...
		write_cmos_sensor_8(ctx, 0x3128,
		...
		write_cmos_sensor_8(ctx, 0x3128, read_cmos_sensor_16(ctx, 0x3128) & 0xf8);
		write_frame_len(ctx, ctx->frame_length);
		ctx->current_ae_effective_frame = 2;
...
#ifdef USE_GAIN_TABLE
	reg_gain = imx766dualo_gain_reg[IMX766DUALO_GAIN_TABLE_SIZE - 1];
	for (i = 0; i < IMX766DUALO_GAIN_TABLE_SIZE; i++) {
		if (gain_value <= imx766dualo_gain_ratio[i]) {
			reg_gain = imx766dualo_gain_reg[i];
			break;
		}
	}
#else
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_init_setting,
		sizeof(imx766dualo_init_setting)/sizeof(kal_uint16));
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_preview_setting,
		sizeof(imx766dualo_preview_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_capture_30_setting,
		sizeof(imx766dualo_capture_30_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_normal_video_setting,
		sizeof(imx766dualo_normal_video_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_hs_video_setting,
		sizeof(imx766dualo_hs_video_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_slim_video_setting,
		sizeof(imx766dualo_slim_video_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom1_setting,
		sizeof(imx766dualo_custom1_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom2_setting,
		sizeof(imx766dualo_custom2_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom3_setting,
		sizeof(imx766dualo_custom3_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom4_setting,
		sizeof(imx766dualo_custom4_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom5_setting,
		sizeof(imx766dualo_custom5_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom6_setting,
		sizeof(imx766dualo_custom6_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom7_setting,
		sizeof(imx766dualo_custom7_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom8_setting,
		sizeof(imx766dualo_custom8_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom9_setting,
		sizeof(imx766dualo_custom9_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_custom10_setting,
		sizeof(imx766dualo_custom10_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
		...
		enum SENSOR_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM4:
	{
		unsigned short imx766dualo_seamless_custom4[] = {
			0x0340, 0x09,
			0x0341, 0x5C,
			0x0224, 0x00,
			0x0225, 0xFC,
		...
		FMC_GPH_START;
		imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_seamless_custom4,
				sizeof(imx766dualo_seamless_custom4) / sizeof(kal_uint16));
	...
	case SENSOR_SCENARIO_ID_NORMAL_VIDEO:
	{
		unsigned short imx766dualo_seamless_normal_video[] = {
			0x0340, 0x12,
			0x0341, 0xBA,
			0x0224, 0x01,
			0x0225, 0xF4,
		...
		FMC_GPH_START;
		imx766dualo_table_write_cmos_sensor_8(ctx, imx766dualo_seamless_normal_video,
				sizeof(imx766dualo_seamless_normal_video) / sizeof(kal_uint16));
				...
				UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx766dualo_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx766dualo_ana_gain_table,
			sizeof(imx766dualo_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		imx766dualo_get_pdaf_reg_setting(ctx, (*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		imx766dualo_set_pdaf_reg_setting(ctx, (*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
...
const struct subdrv_entry imx766dualo_mipi_raw_entry = {
	.name = "imx766dualo_mipi_raw",
	.id = IMX766DUALO_SENSOR_ID,
	.pw_seq = pw_seq,

...
#include "imx766omipiraw_Sensor.h"
#include "imx766o_eeprom.h"
#include "imx766o_ana_gain_table.h"
#include "imx766o_Sensor_setting.h"
...
#define imx766o_table_write_cmos_sensor_8(...) subdrv_i2c_wr_regs_u8(__VA_ARGS__)

#define IMX766O_EEPROM_READ_ID  0xA0
#define IMX766O_EEPROM_WRITE_ID 0xA1

#define PFX "IMX766O_camera_sensor"
...
static struct SENSOR_VC_INFO2_STRUCT SENSOR_VC_INFO2[16] = {
	...
	},
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00, //custom5
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x800, 0x120},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0x800, 0x120},
		},
		1
	},
	{
		0x01, 0x0a, 0x00, 0x08, 0x40, 0x00, //custom6
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x5a0, 0x438},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0x5a0, 0x438},
		},
		1
	},
	{
		0x03, 0x0a, 0x00, 0x08, 0x40, 0x00, //custom7
		{
			...
			{VC_PDAF_STATS_NE_PIX_1, 0x00, 0x30, 0x5c0, 0x114},
			{VC_PDAF_STATS_NE_PIX_2, 0x00, 0x31, 0x5c0, 0x114},
		},
		1
	},
	{
...
static kal_uint16 imx766o_QSC_setting[3072 * 2];
...
static void imx766o_get_pdaf_reg_setting(struct subdrv_ctx *ctx,
...
static void imx766o_set_pdaf_reg_setting(struct subdrv_ctx *ctx,
		MUINT32 regNum, kal_uint16 *regDa)
{
	imx766o_table_write_cmos_sensor_8(ctx, regDa, regNum*2);
}
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
	...
	adaptor_i2c_rd_u8(ctx->i2c_client, IMX766O_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
	return get_byte;
}
...
static void read_sensor_Cali(struct subdrv_ctx *ctx)
{
	...
	if ((otp_data[0] == SENSOR_ID_L) &&
		...
		for (idx = 0; idx < QSC_SIZE; idx++) {
			...
			imx766o_QSC_setting[2 * idx] = sensor_qsc;
			imx766o_QSC_setting[2 * idx + 1] =
...
static void write_sensor_QSC(struct subdrv_ctx *ctx)
{
	...
	if ((otp_flag == OTP_QSC_CUSTOM) || (otp_flag == OTP_QSC_INTERNAL)) {
		imx766o_table_write_cmos_sensor_8(ctx, imx766o_QSC_setting,
		sizeof(imx766o_QSC_setting) / sizeof(kal_uint16));
	}
...
#ifdef USE_GAIN_TABLE
	reg_gain = imx766o_gain_reg[IMX766O_GAIN_TABLE_SIZE - 1];
	for (i = 0; i < IMX766O_GAIN_TABLE_SIZE; i++) {
		if (gain_value <= imx766o_gain_ratio[i]) {
			reg_gain = imx766o_gain_reg[i];
			break;
		}
	}
#else
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_init_setting,
		sizeof(imx766o_init_setting)/sizeof(kal_uint16));
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_preview_setting,
		sizeof(imx766o_preview_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_capture_30_setting,
		sizeof(imx766o_capture_30_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_normal_video_setting,
		sizeof(imx766o_normal_video_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_hs_video_setting,
		sizeof(imx766o_hs_video_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_slim_video_setting,
		sizeof(imx766o_slim_video_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom1_setting,
		sizeof(imx766o_custom1_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom2_setting,
		sizeof(imx766o_custom2_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom3_setting,
		sizeof(imx766o_custom3_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom4_setting,
		sizeof(imx766o_custom4_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom5_setting,
		sizeof(imx766o_custom5_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom6_setting,
		sizeof(imx766o_custom6_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom7_setting,
		sizeof(imx766o_custom7_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom8_setting,
		sizeof(imx766o_custom8_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom9_setting,
		sizeof(imx766o_custom9_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom10_setting,
		sizeof(imx766o_custom10_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
...
{
	LOG_INF("E\n");
	imx766o_table_write_cmos_sensor_8(ctx, imx766o_custom11_setting,
		sizeof(imx766o_custom11_setting)/sizeof(kal_uint16));
	set_mirror_flip(ctx, ctx->mirror);
	if (otp_flag == OTP_QSC_NONE) {
		LOG_INF("OTP no QSC Data, close qsc register");
		write_cmos_sensor_8(ctx, 0x32D2, 0x00);
		...
		enum SENSOR_SCENARIO_ID_ENUM scenario_id, uint32_t *ae_ctrl)
{
	...
	case SENSOR_SCENARIO_ID_CUSTOM4:
	{
		unsigned short imx766o_seamless_custom4[] = {
			0x0340, 0x09,
			0x0341, 0x5C,
			0x0224, 0x00,
			0x0225, 0xFC,
		...
		FMC_GPH_START;
		imx766o_table_write_cmos_sensor_8(ctx, imx766o_seamless_custom4,
				sizeof(imx766o_seamless_custom4) / sizeof(kal_uint16));
	...
	case SENSOR_SCENARIO_ID_NORMAL_VIDEO:
	{
		unsigned short imx766o_seamless_normal_video[] = {
			0x0340, 0x12,
			0x0341, 0xBA,
			0x0224, 0x01,
			0x0225, 0xF4,
		...
		FMC_GPH_START;
		imx766o_table_write_cmos_sensor_8(ctx, imx766o_seamless_normal_video,
				sizeof(imx766o_seamless_normal_video) / sizeof(kal_uint16));
				...
				UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx766o_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx766o_ana_gain_table,
			sizeof(imx766o_ana_gain_table));
		}
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		imx766o_get_pdaf_reg_setting(ctx, (*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		imx766o_set_pdaf_reg_setting(ctx, (*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
...
const struct subdrv_entry imx766o_mipi_raw_entry = {
	.name = "imx766o_mipi_raw",
	.id = IMX766O_SENSOR_ID,
	.pw_seq = pw_seq,

...
#include "imx214submipiraw_Sensor.h"
#include "imx214sub_ana_gain_table.h"
...
#define PFX "IMX214SUB_camera_sensor"
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(ctx, 0x0016) << 8) |
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("IMX214SUB i2c write id: 0x%x, sensor id: 0x%x\n",
					ctx->i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("IMX214SUB Read sensor id fail, write id:0x%x id: 0x%x\n",
				ctx->i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx214sub_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx214sub_ana_gain_table,
			sizeof(imx214sub_ana_gain_table));
		}
		break;
...
const struct subdrv_entry imx214sub_mipi_raw_entry = {
	.name = "imx214sub_mipi_raw",
	.id = IMX214_SENSOR_ID,
	.pw_seq = pw_seq,

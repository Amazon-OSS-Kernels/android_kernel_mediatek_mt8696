...
#define IMX766O_I2C_SPEED       100
#define IMX766O_MAX_OFFSET      0x8000
...
static DEFINE_MUTEX(gimx766o_eeprom_mutex);

static bool selective_read_eeprom(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data)
{
	if (addr > IMX766O_MAX_OFFSET)
		return false;

	if (adaptor_i2c_rd_u8(ctx->i2c_client,
			IMX766O_EEPROM_READ_ID >> 1, addr, data) < 0) {
		return false;
	}
	return true;
}

static bool read_imx766o_eeprom(struct subdrv_ctx *ctx, kal_uint16 addr, BYTE *data, int size)
{
	int i = 0;
	int offset = addr;
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(struct subdrv_ctx *ctx)
{
	...
	mutex_lock(&gimx766o_eeprom_mutex);
	if (pinfo == NULL) {
		read_imx766o_eeprom(ctx, 0x1, read_data, 4);
		if (((read_data[3] << 24) |
			 (read_data[2] << 16) |
			 (read_data[1] << 8) |
	...
	mutex_unlock(&gimx766o_eeprom_mutex);
...
{
	static BYTE IMX766O_LRC_data[LRC_SIZE] = { 0 };
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info(ctx);

	LOG_INF("read imx766o LRC, addr = %d, size = %u\n",
		pinfo->LRC_addr, pinfo->LRC_size);

	mutex_lock(&gimx766o_eeprom_mutex);
	if ((readed_size == 0) &&
		read_imx766o_eeprom(ctx, pinfo->LRC_addr,
				   IMX766O_LRC_data, pinfo->LRC_size)) {
		readed_size = pinfo->LRC_size;
	}
	mutex_unlock(&gimx766o_eeprom_mutex);

	memcpy(data, IMX766O_LRC_data, pinfo->LRC_size);
	return readed_size;
}

unsigned int read_imx766o_DCC(struct subdrv_ctx *ctx, BYTE *data)
{
	static BYTE IMX766O_DCC_data[DCC_SIZE] = { 0 };
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info(ctx);

	LOG_INF("read imx766o DCC, addr = %d, size = %u\n",
		pinfo->DCC_addr, pinfo->DCC_size);

	mutex_lock(&gimx766o_eeprom_mutex);
	if ((readed_size == 0) &&
		read_imx766o_eeprom(ctx, pinfo->DCC_addr,
				   IMX766O_DCC_data, pinfo->DCC_size)) {
		readed_size = pinfo->DCC_size;
	}
	mutex_unlock(&gimx766o_eeprom_mutex);

	memcpy(data, IMX766O_DCC_data, pinfo->DCC_size);
	return readed_size;
}

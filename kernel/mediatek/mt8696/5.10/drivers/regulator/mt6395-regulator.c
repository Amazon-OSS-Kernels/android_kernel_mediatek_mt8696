...
static int __regmap_write_bits(struct regmap *map, unsigned int reg,
...
static int mt6395_regs_show(struct mt6395_chip *chip, struct seq_file *s)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6395_regaddr); i++) {
		ret = __regmap_read(chip->regmap, mt6395_regaddr[i], &data);
		if (ret < 0)
			continue;
...
static int mt6395_data_show(struct mt6395_chip *chip, struct seq_file *s)
{
	...
	ret = __regmap_read(chip->regmap, chip->dbg_regaddr, &data);
	if (ret < 0)
		return ret;
				...
				size_t count, loff_t *ppos)
{
	...
	case MT6395_DBG_DATA:
		...
		ret = __regmap_write(chip->regmap, chip->dbg_regaddr, param);
		if (ret < 0)
...
static int mt6395_init_debugfs(struct mt6395_chip *chip)
{
	...
	size = snprintf(dirname, len + 8, "mt6395-%s", dev_name(chip->dev));
	if (size < 0 || (unsigned int)size >= (unsigned int)(len + 8)) {
		pr_err("exported device name length %i >= %u or < 0", size,
		       (unsigned int)(len + 8));
		return -EPERM;
	}
...
static int mt6395_regulator_get_status(struct regulator_dev *rdev)
{
	...
	ret = __regmap_read(rdev->regmap, rdesc->enable_reg, &regval);
	if (ret < 0) {
...
static unsigned int mt6395_regulator_get_mode(struct regulator_dev *rdev)
{
	...
	ret = __regmap_read(rdev->regmap, MT6395_SLEEP_MODE_CTRL, &regval);
	if (ret < 0)
		return ret;
	...
	ret = __regmap_read(rdev->regmap, MT6395_BUCK_MODE_CTRL, &regval);
	if (ret < 0)
		return ret;
...
static int mt6395_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	ret = __regmap_read(rdev->regmap, rdesc->vsel_reg, &regval);
	if (ret < 0) {
		dev_info(&rdev->dev, "%s get volt fail(%d)\n", __func__, ret);
		return ret;
	}

	regval &= rdev->desc->vsel_mask;
	regval >>= ffs(rdev->desc->vsel_mask) - 1;
...
static const struct regulator_ops mt6395_regulator_ops = {
	...
	.get_voltage_sel = mt6395_get_voltage_sel,
...
static int __mt6395_irq_handler(struct mt6395_chip *chip)
{
	...
	ret = __regmap_read(chip->regmap, MT6395_INT_STATUS, &event);
	if (ret < 0)
		goto err;
	ret = __regmap_read(chip->regmap, MT6395_INT_ENABLE, &mask);
	if (ret < 0)
		goto err;
	ret = __regmap_write_bits(chip->regmap, MT6395_INT_STATUS,
...
static int mt6395_register_irq(struct mt6395_chip *chip)
{
	...
	__regmap_write_bits(chip->regmap, MT6395_INT_STATUS,
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (csram_base == NULL) {
		dev_info(&client->dev, "[%s] ioremap failed\n", __func__);
		return -ENOMEM;
	}
	csram_write(OFFS_KR, 0);
#endif

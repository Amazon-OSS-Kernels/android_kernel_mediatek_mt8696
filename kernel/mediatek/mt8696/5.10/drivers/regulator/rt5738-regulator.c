			   ...
			   struct rt5738_regulator_info *info)
{
	...
	if (!np) {
		RT5738_INFO("%s cant find node (0x%02x)\n",
...
static void rt5738_i2c_shutdown(struct i2c_client *i2c)
{
	struct rt5738_regulator_info *info = i2c_get_clientdata(i2c);
	int ret = 0;
	...
	ret = regmap_write(info->regmap, RT5738_REG_CTRL4, 0x08);
	if (ret < 0)
		dev_info(&i2c->dev, "[%#x] Failed to enable power off sequence. %d\n", ret);
}

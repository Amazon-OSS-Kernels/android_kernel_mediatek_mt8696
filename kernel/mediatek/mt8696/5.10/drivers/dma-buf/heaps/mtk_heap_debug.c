				    ...
				    void *priv)
{
	...
	dmabuf_dump(s, "\tinode:%-8lu size:0x%-8lx count:%-2ld cache_sg:%-1d exp:%s\tname:%s\n",
...
static void dma_heap_dump_vmas(struct task_struct *t, void *priv)
{
	...
	while (vma && (mapcount < t->mm->map_count)) {
		...
		dmabuf_dump(s,
			    "\t\tinode:%lu sz:0x%lx va:%08lx-%08lx map_sz:0x%lx prot:%c%c%c%c off:%08llx node:%s\n",
			      ...
			      unsigned int fd)
{
	...
	dmabuf_dump(s, "\tpid:%-8d\t%-8d\t%-14zu\t%-8lu\t%-8ld\t%-8s\n",
	...
	if (fd_rb_check)
		dmabuf_dump(s, "add: fd:%d, inode:%u\n",
			    new_node->fd_val, new_node->inode);
	...
	for_each_process(p) {
		...
		if (!RB_EMPTY_ROOT(&fddata.fd_root)) {
			...
			while (tmp_rb) {
				...
				dmabuf_dump(s, "clear: fd:%d, inode:%u\n",
		...
		dmabuf_dump(s, "\tpid:%d(%s) Total size:%lu KB, err:%d\n\n",
			    p->pid, p->comm,
		...
		if (fd_rb_check)
			for (tmp_rb = rb_first(&fddata.fd_root);
				...
				fd_entry =
					rb_entry(tmp_rb, struct fd_rb_s, node);
				dmabuf_dump(s, "dump: fd:%d, inode:%u\n",
					    fd_entry->fd_val, fd_entry->inode);
			}
		...
		while (tmp_rb) {
			...
			if (fd_rb_check)
				dmabuf_dump(s, "erase: fd:%d, inode:%u\n",
					    fd_entry->fd_val, fd_entry->inode);
			rb_erase(tmp_rb, &fddata.fd_root);
			kfree(fd_entry);
			tmp_rb = rb_first(&fddata.fd_root);
				 ...
				 int flag)
{
	if (!heap) {
		...
		dmabuf_dump(s, "dmabuf total:%lu KB\n", (total_sz*4)/PAGE_SIZE);
		return;
	}

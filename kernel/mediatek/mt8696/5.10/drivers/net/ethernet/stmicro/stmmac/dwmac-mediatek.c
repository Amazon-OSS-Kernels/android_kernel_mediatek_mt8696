...
/* Peri Configuration register for mt8188 */
#define MT8188_PERI_ETH_CTRL0		0xfd0
#define MT8188_EXT_PHY			1
#define MT8188_INTER_PHY		0
#define MT8188_RMII_CLK_EXT_TXC		0
#define MT8188_RMII_CLK_EXT_RXC		1
#define MT8188_RMII_CLK_INT_RXC		2
#define MT8188_RGMII_TXC_PHASE		1
#define MT8188_RMII_CLK_SRC_SEL		GENMASK(28, 27)
#define MT8188_ETH_INTF_SEL		GENMASK(26, 24)
#define MT8188_RGMII_TXC_PHASE_CTRL	GENMASK(23, 22)
#define MT8188_EXT_PHY_MODE		BIT(21)
#define MT8188_DLY_GTXC_INV		BIT(12)
#define MT8188_DLY_GTXC_FINE_EN		BIT(11)
#define MT8188_DLY_GTXC_FINE_STG	GENMASK(10, 6)
#define MT8188_DLY_GTXC_COARSE_EN	BIT(5)
#define MT8188_DLY_GTXC_COARSE_STG	GENMASK(4, 0)
#define MT8188_DLY_STAGE		7
#define MT8188_RGMII_TXC_PHASE_CTRL_ENABLE	BIT(22)

#define MT8188_PERI_ETH_CTRL1		0xfd4
#define MT8188_DLY_RXC_INV		BIT(25)
#define MT8188_DLY_RXC_FINE_EN		BIT(24)
#define MT8188_DLY_RXC_FINE_STG		GENMASK(23, 19)
#define MT8188_DLY_RXC_COARSE_EN	BIT(18)
#define MT8188_DLY_RXC_COARSE_STG	GENMASK(17, 13)
#define MT8188_DLY_TXC_INV		BIT(12)
#define MT8188_DLY_TXC_FINE_EN		BIT(11)
#define MT8188_DLY_TXC_FINE_STG		GENMASK(10, 6)
#define MT8188_DLY_TXC_COARSE_EN	BIT(5)
#define MT8188_DLY_TXC_COARSE_STG	GENMASK(4, 0)

#define MT8188_PERI_ETH_CTRL2		0xfd8
#define MT8188_DLY_INT_RXC_INV		BIT(25)
#define MT8188_DLY_INT_RXC_FINE_EN	BIT(24)
#define MT8188_DLY_INT_RXC_FINE_STG	GENMASK(23, 19)
#define MT8188_DLY_INT_RXC_COARSE_EN	BIT(18)
#define MT8188_DLY_INT_RXC_COARSE_STG	GENMASK(17, 13)
#define MT8188_DLY_INT_TXC_INV		BIT(12)
#define MT8188_DLY_INT_TXC_FINE_EN	BIT(11)
#define MT8188_DLY_INT_TXC_FINE_STG	GENMASK(10, 6)
#define MT8188_DLY_INT_TXC_COARSE_EN	BIT(5)
#define MT8188_DLY_INT_TXC_COARSE_STG	GENMASK(4, 0)
...
static const char * const mt8188_dwmac_clk_l[] = {
...
static bool __loopback_test(struct stmmac_priv *priv, int num,
			    int delay, bool log)
{
			   ...
			   char *buf)
{
	...
	struct mediatek_dwmac_plat_data *plat = priv->plat->bsp_priv;
	int len = 0;
			    ...
			    size_t count)
{
	...
	if (!strncmp(buf, "er", 2) &&
		...
		plat->smt_result = false;
		if (!__loopback_test(priv, reg, 1000000, true))
			plat->smt_result = true;
	} else if (!strncmp(buf, "phyloop", 7) &&
		   (sscanf(buf + 7, "%d", &reg) == 1)) {
		phy_loopback(phy_dev, reg);
	} else if (!strncmp(buf, "carrier", 7) &&
		   (sscanf(buf + 7, "%d", &reg) == 1)) {
		if (reg)
...
static void fix_mac_speed(void *priv, unsigned int speed)
{
	...
	if (speed == SPEED_1000)
		regmap_update_bits(priv_plat->peri_regmap, MT8188_PERI_ETH_CTRL0,
				   MT8188_RGMII_TXC_PHASE_CTRL | MT8188_DLY_GTXC_COARSE_EN
				   | MT8188_DLY_GTXC_COARSE_STG,
				   MT8188_RGMII_TXC_PHASE_CTRL_ENABLE);
	else
		regmap_update_bits(priv_plat->peri_regmap, MT8188_PERI_ETH_CTRL0,
				   MT8188_RGMII_TXC_PHASE_CTRL | MT8188_DLY_GTXC_COARSE_EN
				   | MT8188_DLY_GTXC_COARSE_STG,
				   MT8188_DLY_GTXC_COARSE_EN | MT8188_DLY_STAGE);
}
...
static int mt8188_set_interface(struct mediatek_dwmac_plat_data *plat)
{
	int rmii_clk = MT8188_RMII_CLK_INT_RXC;
	u32 intf_val = 0;

	/* select phy interface in top control domain */
	switch (plat->phy_mode) {
	case PHY_INTERFACE_MODE_MII:
		intf_val |= FIELD_PREP(MT8188_ETH_INTF_SEL, PHY_INTF_MII);
		break;
	case PHY_INTERFACE_MODE_RMII:
		if (plat->rmii_clk_from_mac)
			rmii_clk = MT8188_RMII_CLK_INT_RXC;
		else
			rmii_clk = plat->rmii_rxc ? MT8188_RMII_CLK_EXT_RXC :
						    MT8188_RMII_CLK_EXT_TXC;
		intf_val |= FIELD_PREP(MT8188_ETH_INTF_SEL, PHY_INTF_RMII);
		intf_val |= FIELD_PREP(MT8188_RMII_CLK_SRC_SEL, rmii_clk);
		break;
	case PHY_INTERFACE_MODE_RGMII:
	case PHY_INTERFACE_MODE_RGMII_TXID:
	case PHY_INTERFACE_MODE_RGMII_RXID:
	case PHY_INTERFACE_MODE_RGMII_ID:
		intf_val |= FIELD_PREP(MT8188_ETH_INTF_SEL, PHY_INTF_RGMII);
		intf_val |= FIELD_PREP(MT8188_RGMII_TXC_PHASE_CTRL, MT8188_RGMII_TXC_PHASE);
		break;
	default:
		dev_err(plat->dev, "phy interface not supported\n");
		return -EINVAL;
	...
	intf_val |= FIELD_PREP(MT8188_EXT_PHY_MODE, MT8188_EXT_PHY);
	regmap_write(plat->peri_regmap, MT8188_PERI_ETH_CTRL0, intf_val);

	return 0;
}

static int mt8188_set_delay(struct mediatek_dwmac_plat_data *plat)
{
	return 0;
}

static const struct mediatek_dwmac_variant mt8188_gmac_variant = {
	.dwmac_set_phy_interface = mt8188_set_interface,
	.dwmac_set_delay = mt8188_set_delay,
	.clk_list = mt8188_dwmac_clk_l,
	.num_clks = ARRAY_SIZE(mt8188_dwmac_clk_l),
	.dma_bit_mask = 35,
	.rx_delay_max = 2800,
	.tx_delay_max = 2800,
};
...
static int mediatek_dwmac_config_dt(struct mediatek_dwmac_plat_data *plat)
{
	...
	plat->peri_regmap = syscon_regmap_lookup_by_phandle(plat->np, "mediatek,infracfg_ao");
...
static int mediatek_dwmac_probe(struct platform_device *pdev)
{
	...
	plat_dat->tso_en = of_property_read_bool(priv_plat->np, "snps,tso");
	plat_dat->init = mediatek_dwmac_init;
	plat_dat->exit = mediatek_dwmac_exit;
	plat_dat->fix_mac_speed = fix_mac_speed;

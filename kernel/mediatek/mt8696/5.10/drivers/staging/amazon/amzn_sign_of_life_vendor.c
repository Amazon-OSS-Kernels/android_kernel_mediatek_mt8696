...
#include <linux/amzn_sign_of_life_vendor.h>
...
bool wdt_status_cmdline = false;
bool rc_exp_type_cmdline = false;
static int __init rc_wdt_status_setup (char *str){
	if (strncmp(str, "0", 1) == 0)
		wdt_status_cmdline = true;
	return 1;
}

early_param("rc_wdt_status", rc_wdt_status_setup);

static int __init rc_exp_type_setup (char *str){
	if (strncmp(str, "0", 1) == 0)
		rc_exp_type_cmdline = true;
	return 1;
}

early_param("rc_exp_type", rc_exp_type_setup);
...
static int (vendor_read_boot_reason)(life_cycle_reason_t *boot_reason)
{
	...
	else if (vendor_breason & (1<<WD_SOL_COLD_BOOT_POWER_SUPPLY) || (rc_exp_type_cmdline && wdt_status_cmdline))

...
#if (IS_ENABLED(CONFIG_MTK_ION_DEBUG))
#define ION_OOM_DUMP_INTERVAL         (2000)  /* unit: ms */
unsigned long long last_ion_oom_time;/* ms */
#endif
...
static void ion_dump_heap_size_all(void)
{
	struct ion_device *dev = internal_dev;
	struct ion_heap *heap;
	...
	plist_for_each_entry(heap, &dev->heaps, node) {
		spin_lock(&heap->stat_lock);
		pr_info("----------------------------------------------------\n");
		pr_info("%16.s: buf_num %llu, total_buf_size %lluM\n",
			heap->name, heap->num_of_buffers, (heap->num_of_alloc_bytes / SZ_1M));
		pr_info("%16.s: alloc_bytes_wm %lluM, free_list_size %lluM\n",
			heap->name, (heap->alloc_bytes_wm / SZ_1M),
			(heap->free_list_size / SZ_1M));
		pr_info("----------------------------------------------------\n");
		spin_unlock(&heap->stat_lock);
			  ...
			  unsigned long nb_val, void *nb_freed)
{
	...
	if (oom_time - last_ion_oom_time < ION_OOM_DUMP_INTERVAL) {
		last_ion_oom_time = oom_time;
		pr_info("%s: less than %dms since last dump, return\n",
			__func__, ION_OOM_DUMP_INTERVAL);
		return 0;
	}

	ion_dump_heap_size_all();
...
int __ion_device_add_heap(struct ion_heap *heap, struct module *owner)
{
	...
	if (heap->ops->debug_show) {
		debugfs_create_file("buffer_debug", 0644, heap_root, heap,
				    &debug_heap_fops);
#if IS_ENABLED(CONFIG_PROC_FS)
		heap->proc_dir = proc_mkdir(heap->name, dev->proc_root);
		if (heap->proc_dir)
			proc_create_data("buffer_debug", S_IFREG | 0644,
...
/*Print the ion heap size for debug.*/
#if (IS_ENABLED(CONFIG_MTK_ION_DEBUG))
	ion_oom_nb_status = register_oom_notifier(&ion_oom_nb);
	if (ion_oom_nb_status)
		pr_info("register ion oom notifier failed:%d\n", ion_oom_nb_status);
#endif

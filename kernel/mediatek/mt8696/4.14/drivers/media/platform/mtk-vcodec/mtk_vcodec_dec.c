...
static int *get_general_buffer(struct mtk_vcodec_ctx *ctx, int fd)
{
	int *va = 0;
	int i = 0;
	struct dma_buf *dmabuf = dma_buf_get(fd);

	mtk_v4l2_debug(2, "get dmabuf %p", dmabuf);
	if (dmabuf) {
		for (i = 0; i < MAX_GEN_BUF_CNT; ++i) {
			if (dmabuf == ctx->dma_buf_list[i].dmabuf) {
				va = ctx->dma_buf_list[i].va;
				mtk_v4l2_debug(2, "reuse va %p at %d", va, i);
				dma_buf_put(dmabuf);
				break;
			}
		}
	...
	if (va == 0) {
		...
		mtk_v4l2_debug(2, "map new va %p", va);
		//save va and dmabuf
		if (va) {
			for (i = 0; i < MAX_GEN_BUF_CNT; ++i) {
				if (ctx->dma_buf_list[i].va == 0) {
					...
					mtk_v4l2_debug(2, "save general buf va %p dmabuf %p at %d",
						va, dmabuf, i);
					break;
				}
			}
...
void mtk_vdec_process_put_fb_job(struct mtk_vcodec_ctx *ctx)
{
	...
	src_vb2_v4l2 = container_of(src_buf,
	 ...
	 */
	if (src_buf && src_buf_info->lastframe != NON_EOS)
		goto unlock;
...
static void mtk_vdec_flush_decoder(struct mtk_vcodec_ctx *ctx)
{
	...
	for (i = 0; i < MAX_GEN_BUF_CNT; ++i) {
		if (ctx->dma_buf_list[i].va && ctx->dma_buf_list[i].dmabuf) {
			...
			*dma_va = -1;
			dma_buf_kunmap(ctx->dma_buf_list[i].dmabuf, 0, dma_va);
			dma_buf_end_cpu_access(ctx->dma_buf_list[i].dmabuf,
				DMA_TO_DEVICE);
			dma_buf_put(ctx->dma_buf_list[i].dmabuf);
		}
	}
...
int mtk_vdec_put_fb(struct mtk_vcodec_ctx *ctx, int type)
{
	...
	if (type != -1 && !(ctx->use_fence || ctx->input_driven))
	...
	if (src_buf_info->lastframe == EOS && type == -1) {
...
static void mtk_vdec_worker(struct work_struct *work)
{
	...
	if (!ctx->input_driven) {
		...
		if (dst_buf_info->dma_meta_buf != 0)
			pfb->dma_meta_buf = dst_buf_info->dma_meta_buf;
		pfb->status = 0;

		mtk_v4l2_debug(1,
				"id=%d Framebuf  pfb=%p VA=%p Y_DMA=%pad C_DMA=%pad ion_buffer=(%p %p) Size=%zx, dma_general_buf = %p, general_buf_fd = %d, dma_meta_buf = %p, meta_user_fd =%d",
			dst_buf->index, pfb,
			...
			dst_buf_info->dma_meta_buf,
			dst_buf_info->meta_user_fd);
	}
...
void mtk_vcodec_dec_empty_queues(struct file *file, struct mtk_vcodec_ctx *ctx)
{
	...
	while ((dst_buf = v4l2_m2m_dst_buf_remove(ctx->m2m_ctx))) {
		...
		if (mtkbuf != NULL) {
			...
			if (mtkbuf->dma_meta_buf != 0) {
				mtk_v4l2_debug(4,
					"[%d] meta_user_fd = %d, buf = %p",
					ctx->id,
					mtkbuf->meta_user_fd,
					mtkbuf->dma_meta_buf);
				dma_buf_put(mtkbuf->dma_meta_buf);
				mtkbuf->dma_meta_buf = 0;
			}
		}
							...
							struct v4l2_buffer *buf)
{
	...
	if (buf->index >= vq->num_buffers) {
		mtk_v4l2_debug(1, "[%d] buffer index %d out of range %d",
	...
	if (buf->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		if (mtkbuf->general_user_fd != -1) {
			...
			dma_va = get_general_buffer(ctx,
				mtkbuf->general_user_fd);
			if (dma_va != 0 && *dma_va == 1 &&
				ctx->dec_params.svp_mode == 0) {
				ctx->use_fence = true;
				mtkbuf->general_dma_va = dma_va;
			} else if (dma_va != 0)
				*dma_va = -1;
		}
		mtkbuf->meta_user_fd = -1; //default value is -1
		if (mtkbuf->general_user_fd != -1 && dma_va != 0) {
			// metaBufferfd should be
			//the 2st member in struct of general buffer
			metaBufferfd = *(dma_va + 1);
			if (metaBufferfd > 0)
				mtkbuf->meta_user_fd = metaBufferfd;
			mtk_v4l2_debug(1, "[%d] id=%d FB (%d) dma_va=%p, metaBufferfd=%d, meta_user_fd = %d",
				ctx->id, buf->index,
				buf->length, dma_va,
...
static int vb2ops_vdec_buf_prepare(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->type != V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		if (mtkbuf->meta_user_fd > 0) {
			...
			if (IS_ERR(mtkbuf->frame_buffer.dma_meta_buf)) {
				mtk_v4l2_err("%s dma_meta_buf is err 0x%p, meta_user_fd:%d\n",
					__func__,
					mtkbuf->frame_buffer.dma_meta_buf,
					mtkbuf->meta_user_fd);
				mtk_vdec_queue_error_event(ctx);
			...
			mtkbuf->frame_buffer.dma_meta_addr =
				sg_dma_address(sgt->sgl);
			dma_buf_unmap_attachment(buf_att, sgt, DMA_TO_DEVICE);
			dma_buf_detach(mtkbuf->frame_buffer.dma_meta_buf,
				buf_att);
		} else
	...
	if (!(mtkbuf->flags & NO_CAHCE_CLEAN) &&
		...
		if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
			...
			flush_dma_buffer(sgt, ctx, vb->planes[0].bytesused,
				DMA_TO_DEVICE);
...
static void vb2ops_vdec_stop_streaming(struct vb2_queue *q)
{
	...
	if (q->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		while ((src_buf = v4l2_m2m_src_buf_remove(ctx->m2m_ctx))) {
			...
			if (buf->vb2_buf.state == VB2_BUF_STATE_DONE) {
				//in case of NEED_MORE_OUTPUT,
				//vb2_buf.state has been set as DONE in worker
				continue;
			}
			v4l2_m2m_buf_done(to_vb2_v4l2_buffer(src_buf),
				VB2_BUF_STATE_ERROR);
...
static int mtk_vdec_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_QUEUED_FRAMEBUF_COUNT:
		...
		break;
	default:
		mtk_v4l2_debug(2, "ctrl-id=%x not support!", ctrl->id);
		return -EINVAL;
	}
...
static const struct v4l2_ctrl_config mtk_interlacing_fieldseq_ctrl = {
	.ops = &mtk_vcodec_dec_ctrl_ops,
	.id = V4L2_CID_MPEG_MTK_INTERLACING_FIELD_SEQ,
	.name = "MTK Query Interlacing FieldSeq",
	.type = V4L2_CTRL_TYPE_BOOLEAN,
	.min = 0,
	.max = 1,
	.step = 1,
...
int mtk_vcodec_dec_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	ctrl = v4l2_ctrl_new_custom(&ctx->ctrl_hdl,
				&mtk_interlacing_fieldseq_ctrl, NULL);
	if (ctrl)
		ctrl->flags |= V4L2_CTRL_FLAG_VOLATILE;

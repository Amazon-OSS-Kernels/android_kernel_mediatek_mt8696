...
{
	int ret = 0;
#ifndef FPGA_PWRCLK_API_DISABLE//yinbo  temp
	...
	pm->chip_node = of_find_compatible_node(NULL,
		NULL, "mediatek,mt8696-vcodec-enc");//yinbo
	...
	pm->MT_VENC_SMI = devm_clk_get(&pdev->dev, "MT_VENC_SMI");
	if (IS_ERR(pm->MT_VENC_SMI)) {
		mtk_v4l2_err("[VCODEC][ERROR] Unable to devm_clk_get MT_VENC_SMI\n");
		ret = PTR_ERR(pm->MT_VENC_SMI);
	}

	pm->MT_VENC_CON = devm_clk_get(&pdev->dev, "MT_VENC_CON");
	if (IS_ERR(pm->MT_VENC_CON)) {
		mtk_v4l2_err("[VCODEC][ERROR] Unable to devm_clk_get MT_VENC_CON\n");
		ret = PTR_ERR(pm->MT_VENC_CON);
	}
...
//	smi_bus_prepare_enable(SMI_LARB7, "VENC");
	...
	ret = clk_prepare_enable(pm->MT_VENC_CON);
	if (ret)
		mtk_v4l2_err("clk_prepare_enable MT_VENC_CON fail %d", ret);

	ret = clk_prepare_enable(pm->MT_VENC_SMI);
	if (ret)
		mtk_v4l2_err("clk_prepare_enable MT_VENC_SMI fail %d", ret);
...
#ifndef FPGA_PWRCLK_API_DISABLE
	struct mtk_vcodec_pm *pm = &ctx->dev->pm;
	clk_disable_unprepare(pm->MT_VENC_CON);
	clk_disable_unprepare(pm->MT_VENC_SMI);

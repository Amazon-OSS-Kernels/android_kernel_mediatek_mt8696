...
irqreturn_t mtk_vcodec_dec_irq_handler(int irq, void *priv)
{
	...
	if (ctx->dec_params.svp_mode != OPEN_TEE) {
		vdec_misc_addr = dev->dec_reg_base[VDEC_MISC] +
		...
		if (irq == dev->dec_irq[0]) {
			if (ctx->use_fence && ctx->p_timeline_obj)
				timeline_inc(ctx->p_timeline_obj, 1);
			...
			writel((readl(vdec_misc_addr) | MTK_VDEC_IRQ_CFG),
				dev->dec_reg_base[VDEC_MISC] +
				MTK_VDEC_IRQ_CFG_REG);
			writel((readl(vdec_misc_addr) & ~MTK_VDEC_IRQ_CLR),
				dev->dec_reg_base[VDEC_MISC] +
				MTK_VDEC_IRQ_CFG_REG);
		} else if (irq == dev->dec_line_cnt_irq) {
			void __iomem *vdec_ufo_enc_addr = NULL;

			dec_done_status = MTK_VDEC_IRQ_STATUS_LINE_REACHED;
			ctx->irq_status = dec_done_status;

			/* clear interrupt */
			vdec_ufo_enc_addr = dev->dec_reg_base[VDEC_SYS] +
				0x800 + MTK_VDEC_LINE_CNT_IRQ_CFG_REG;
			writel((readl(vdec_ufo_enc_addr) |
				MTK_VDEC_LINE_CNT_IRQ_CFG),
				vdec_ufo_enc_addr);
			writel((readl(vdec_ufo_enc_addr) &
				~MTK_VDEC_LINE_CNT_IRQ_CFG),
				vdec_ufo_enc_addr);
...
irqreturn_t mtk_vcodec_lat_dec_irq_handler(int irq, void *priv)
{
	...
	if (ctx->dec_params.svp_mode != OPEN_TEE) {
		vdec_misc_addr = dev->dec_reg_base[VDEC_LAT_MISC] +
			MTK_VDEC_IRQ_CFG_REG;
	...
	struct mtk_vcodec_dev *dev)
{
	...
	for (i = 0; i < MTK_VDEC_HW_NUM; i++) {
		...
		if (i == MTK_VDEC_CORE)
			ret = devm_request_irq(&pdev->dev, dev->dec_irq[i],
				mtk_vcodec_dec_irq_handler, IRQF_TRIGGER_HIGH |
	...
	dev->dec_line_cnt_irq = platform_get_irq(pdev, 2);
	mtk_v4l2_debug(0, "line_cnt_irq %d", dev->dec_line_cnt_irq);
	ret = devm_request_irq(&pdev->dev, dev->dec_line_cnt_irq,
		mtk_vcodec_dec_irq_handler,
		IRQF_TRIGGER_HIGH | IRQF_SHARED,
		pdev->name, dev);
	if (ret) {
		mtk_v4l2_debug(1, "Failed to install dev->dec_line_cnt_irq %d (%d)",
				dev->dec_line_cnt_irq,
				ret);
	}
	disable_irq(dev->dec_line_cnt_irq);
#endif
	return 0;

...
static int mtk_iommu_hw_init(const struct mtk_iommu_data *data)
{
	...
	if (ret) {
		dev_info(data->dev, "Failed to enable iommu apb(%d)\n", ret);
		return ret;
	}
...
static int mtk_iommu_probe(struct platform_device *pdev)
{
	...
	if (PTR_ERR(data->apb) == -ENOENT)
		data->apb = NULL;
	else if (IS_ERR(data->apb))
...
static int __maybe_unused mtk_iommu_suspend(struct device *dev)
{
	...
	int cmd = IOMMU_ATF_SET_COMMAND(0, 1, IOMMU_ATF_SECURITY_BACKUP);
	struct arm_smccc_res res;

	arm_smccc_smc(MTK_M4U_DEBUG_DUMP, cmd, 0, 0, 0, 0, 0, 0, &res);
...
static int __maybe_unused mtk_iommu_resume(struct device *dev)
{
	...
	void __iomem *base = data->base;
	int ret;
	int cmd = IOMMU_ATF_SET_COMMAND(0, 1, IOMMU_ATF_SECURITY_RESTORE);
	struct arm_smccc_res res;
...
static const struct mtk_iommu_resv_iova_region mt8696_iommu_rsv_list[2] = {
	{	.iova_base = 0,
		.iova_size = 0,
		.type = IOMMU_RESV_DIRECT,
		.get_resv_data = mtk_get_disp_dm_region,
...
static const struct mtk_iommu_plat_data mt8696_data = {
	.m4u_plat = M4U_MT8696,
	.spec_device_comp = "mediatek,mtk-pseudo-m4u",
	.spec_cnt = ARRAY_SIZE(mt8696_iommu_rsv_list),
	.spec_region = &mt8696_iommu_rsv_list[0],
...
static const struct of_device_id mtk_iommu_of_ids[] = {
	...
	{ .compatible = "mediatek,mt8696-m4u", .data = &mt8696_data},
	{}
};

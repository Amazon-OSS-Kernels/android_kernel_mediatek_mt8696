...
/* specific for mt8695,mt8696 larb8 */
static void mtk_smi_larb_config_port_mt8696(struct device *dev)
{
	struct mtk_smi_larb *larb = dev_get_drvdata(dev);
	unsigned int port_nr_larb[] = {4, 10, 0, 9, 11, 15, 17, 7, 2};
	int cmd = IOMMU_ATF_SET_COMMAND(0, 0, IOMMU_ATF_SET_SMI_SEC_LARB);
	...
	for (i = 0; i < port_nr_larb[larb->larbid]; i++) {
		int mmu_en = !!(*larb->mmu & BIT(i));

		/* only fisrt config need change seting from lk
		 * or disable mmu also need reconfig.
		 */
		if (larb->first_config || !mmu_en)
			arm_smccc_smc(MTK_M4U_DEBUG_DUMP, cmd,
				(larb->larbid << 5) | i, mmu_en << 16,
				0, 0, 0, 0, &res);
	}
	larb->first_config = false;
}
...
int mtk_smi_init_setting(void)
{
	...
	for (i = 0; i < 8 ; i++) {
		if (i == 2)
			continue;
		pm_runtime_get_sync(gmtk_larb[i]->smi.dev);
	}

	addr = gmtk_larb[0]->base;
	writel_relaxed(0x2, addr + 0x200);
	writel_relaxed(0x20, addr + 0x204);

	addr = gmtk_larb[1]->base;
	writel_relaxed(0x20, addr + 0x200);
	writel_relaxed(0x10, addr + 0x204);
	writel_relaxed(0x4, addr + 0x208);
	writel_relaxed(0x4, addr + 0x20c);
	writel_relaxed(0x4, addr + 0x210);
	writel_relaxed(0x4, addr + 0x214);
	writel_relaxed(0x8, addr + 0x218);
	...
	writel_relaxed(0x2, addr + 0x200); /* dobly */
	writel_relaxed(0xc, addr + 0x204);

	addr = gmtk_larb[5]->base;
	writel_relaxed(0x4, addr + 0x200);
	writel_relaxed(0x4, addr + 0x204);
	writel_relaxed(0xa, addr + 0x208);
	writel_relaxed(0xa, addr + 0x20c);
	writel_relaxed(0x5, addr + 0x210);
	writel_relaxed(0x5, addr + 0x214);
	writel_relaxed(0x5, addr + 0x218);
	writel_relaxed(0xa, addr + 0x21c);

	writel_relaxed(0x10, addr + 0x224);
	writel_relaxed(0x6, addr + 0x228);
	writel_relaxed(0x10, addr + 0x22c);

	addr = gmtk_larb[6]->base;
	writel_relaxed(0x4, addr + 0x200);
	writel_relaxed(0x4, addr + 0x204);
	writel_relaxed(0xa, addr + 0x208);
	writel_relaxed(0x5, addr + 0x20c);
	writel_relaxed(0xa, addr + 0x210);
	writel_relaxed(0xa, addr + 0x214);
	writel_relaxed(0x5, addr + 0x218);
	writel_relaxed(0x5, addr + 0x21c);

	writel_relaxed(0x10, addr + 0x228);
	writel_relaxed(0x6, addr + 0x22c);
	writel_relaxed(0x10, addr + 0x230);
	...
	writel_relaxed(0x1466, addr + 0x104); /* normal video.*/
	writel_relaxed(0x1634, addr  + 0x108);
	writel_relaxed(0x110d, addr  + 0x114);
	writel_relaxed(0x140a, addr + 0x118);
	writel_relaxed(0x140a, addr + 0x11c);
	writel_relaxed(0x1139, addr + 0x120);
	writel(0x4014, addr + SMI_BUS_SEL);

	for (i = 7; i >= 0; i--) {
		if (i == 2)
			continue;
		pm_runtime_put_sync(gmtk_larb[i]->smi.dev);
	}
...
static const struct mtk_smi_larb_gen mtk_smi_larb_mt8696 = {
	.need_larbid = true,
	.config_port = mtk_smi_larb_config_port_mt8696,
};

		      ...
		      struct sg_table *sg_table)
{
	...
	if (!m4u_buf_info) {
		M4U_ERR("%s-%d error: can NOT fine buf-info mva %lx.",
	...
	if (!table) {
		M4U_ERR("%s-%d could not find the table from mva 0x%x\n",
				__func__, __LINE__, MVA);
		M4U_ERR("dealloc mod=%s,va=0x%lx,sz=0x%x MVA(0x%x-0x%x)\n",
...
#ifdef M4U_TEE_SERVICE_ENABLE
static int __init mtk_pseudo_sec_init(void)
{
	/* init the sec_mem_size to 400M to avoid build error. */
	unsigned int sec_mem_size = 400 * 0x100000;

	pseudo_m4u_session_init();
	pseudo_m4u_sec_init(0, 1, &sec_mem_size);

	return 0;
}

late_initcall_sync(mtk_pseudo_sec_init);
#endif

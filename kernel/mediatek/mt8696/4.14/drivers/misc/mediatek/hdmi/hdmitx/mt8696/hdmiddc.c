	...
	unsigned char wr_data)
{
	...
	if ((bReadByteHdmiGRL(HDCP2X_DDCM_STATUS) &
		...
		if ((bReadByteHdmiGRL(DDC_CTRL) & 0xFF) == (RX_ID << 1))
			TX_DEF_LOG(
		"[HDMI][DDC][1x]err_w:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
			bReadByteHdmiGRL(DDC_CTRL),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
		else
			HDMI_HDCP_LOG(
		"[HDMI][DDC]err_w:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
			bReadByteHdmiGRL(DDC_CTRL),
			bReadByteHdmiGRL(HPD_DDC_STATUS),
			bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
		...
		for (i = 0; i < loop_counter; i++) {
			...
			if ((bReadByteHdmiGRL(HDCP2X_DDCM_STATUS) &
				...
				if ((bReadByteHdmiGRL(DDC_CTRL) &
					...
					(RX_ID << 1))
					TX_DEF_LOG(
		"[HDMI][DDC][1x]err_r:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
					bReadByteHdmiGRL(DDC_CTRL),
					bReadByteHdmiGRL(HPD_DDC_STATUS),
					bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
				else
					HDMI_HDCP_LOG(
			"[HDMI][DDC]err_r:0xc10=0x%08x,0xc60=0x%08x,0xc68=0x%08x\n",
					bReadByteHdmiGRL(DDC_CTRL),
					bReadByteHdmiGRL(HPD_DDC_STATUS),
					bReadByteHdmiGRL(HDCP2X_DDCM_STATUS));
				if (bReadByteHdmiGRL(HDCP2X_DDCM_STATUS) &

...
"%s:e_deep_color_bit = %d; hdmi_boot_colordepth = %d\n",
	...
	if ((_stAvdAVInfo.e_resolution == hdmi_boot_res) &&
		...
		&& ((!vIsDviMode()) == _HdmiSinkAvCap.b_sink_support_hdmi_mode)) {
		TX_DEF_LOG(
...
#if IS_ENABLED(CONFIG_COMPAT)
#if 0
static int compat_get_hdmitx_audio_para(
COMPAT_HDMITX_AUDIO_PARA __user *data32,
...
struct HDMI_EDID_T __user *data)
{
	...
	err |= get_user(u,
		&data->ui4_sink_cea_FP_SUP_3D_resolution);
	err |= put_user(u,
		&data32->ui4_sink_cea_FP_SUP_3D_resolution);

	err |= get_user(u, &data->ui4_sink_cea_TOB_SUP_3D_resolution);
	err |= put_user(u,
		&data32->ui4_sink_cea_TOB_SUP_3D_resolution);

	err |= get_user(u, &data->ui4_sink_cea_SBS_SUP_3D_resolution);
	err |= put_user(u,
		&data32->ui4_sink_cea_SBS_SUP_3D_resolution);
	...
	case COMPAT_MTK_HDMI_GET_EDID:
		{
			...
			HDMI_DRV_DBG(">> COMPAT_MTK_HDMI_GET_EDID\n");
			/* userspace passed argument */
			data32 = compat_ptr(arg);
			//data = compat_alloc_user_space(sizeof(*data));
	...
	case COMPAT_MTK_HDMI_HDCP_KEY:
		{
			...
			HDMI_DRV_DBG("COMPAT_MTK_HDMI_HDCP_KEY! arg:%ld\n",
				arg);
			break;
		}
				      ...
				      size_t count)
{
	...
	if (strncmp(buf, "dbglevel:", 9) == 0) {
		...
		if (ret == 1) {
			sprintf(hdmi_debug_normal_buffer,
				"log old=0x%x\n", hdmidrv_log_on);
			...
			snprintf(hdmi_debug_normal_buffer,
				sizeof(hdmi_debug_normal_buffer),
				"error,buf=%s\n", buf);
			TX_DEF_LOG("%s,%d,%s\n",
				__func__, __LINE__,
				 ...
				 struct device_attribute *devattr, char *buf)
{
	...
	sprintf(hdmi_hdr_status_buffer +
...
static DEVICE_ATTR(hdmi_suspend_enable,
	0644, hdmi_suspend_show_enable,
	hdmi_suspend_store_enable);
static DEVICE_ATTR(hdmi_debug_normal,
	0644, hdmi_debug_normal_show,
	hdmi_debug_normal_store);
static DEVICE_ATTR(hdmi_hdr_status,
	0644, hdmi_hdr_status_show,
	hdmi_hdr_status_store);
static DEVICE_ATTR(hdmi_common_status,
	0644, hdmi_common_status_show,
	hdmi_common_status_store);
...
static int hdmi_probe(struct platform_device *pdev)
{
	...
	device_create_file((struct device *)class_dev,

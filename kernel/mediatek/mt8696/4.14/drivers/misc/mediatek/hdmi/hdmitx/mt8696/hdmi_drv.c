...
{
	if (hdmi_boot_res >= 0 && hdmi_boot_res <
	...
	enum HDMI_VIDEO_RESOLUTION vformat)
{
	...
	if ((vformat == HDMI_VIDEO_720x480i_60Hz) ||
		...
		if (hdmi_p2icksel == 0) {
			clk_prepare(hdmi_ref_clock[MMSYS_HDMI_P2I]);
			clk_enable(hdmi_ref_clock[MMSYS_HDMI_P2I]);
			hdmi_p2icksel = 1;
		}
	} else {
		if (hdmi_p2icksel == 1) {

...
bool isKsvLegal(unsigned char ksv[HDCP_AKSV_COUNT])
{
	...
	TX_DEF_LOG(
		"[HDCP] %s,err ksv is:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x\n",
...
bool fgCompareRi(void)
{
	...
	}
	{
		_bHdcpStatus = SV_FAIL;
		TX_DEF_LOG(
			"[HDCP][1.x]Rx_Ri=0x%x%x Tx_Ri=0x%x%x\n",
			bHDCPBuf[0],
			bHDCPBuf[1], bHDCPBuf[2], bHDCPBuf[3]);
		return FALSE;
	}
...
void HdcpService(enum HDCP_CTRL_STATE_T e_hdcp_state)
{
	...
	if (hdcp_key_load_status == 0) {
		pr_info("hdmi hdcp key not loaded disable hdcp")
		pr_info(" device may be non-secure===>\n");
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
		...
		if (fgIsHDCPCtrlTimeOut()) {
			if (i4SharedInfo(SI_HDMI_RECEIVER_STATUS) !=
				...
				if (hdcp_err_0x30_flag == 1) {
					...
					vWriteIoHdmiAnaMsk(HDMI_1_CFG_0, RG_HDMITX21_DRV_EN, RG_HDMITX21_DRV_EN);
				}
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) || defined(CONFIG_OPTEE))
			TX_DEF_LOG(
		"[HDMI]CONFIG_TEE_SUPPORT fgCaHDMILoadROM\n");
			fgCaHDMILoadROM();
#else
...
int hdmi_tx_get_Bksv(char *Bksv, int *count, int *depth)
{
	...
	if (_bDevice_Count <= 9) {
		...
		{
			TX_DEF_LOG(
				"[HDCP] !!!Error!! TX dowstream over 9 = %d\n",
				   _bDevice_Count);
		}

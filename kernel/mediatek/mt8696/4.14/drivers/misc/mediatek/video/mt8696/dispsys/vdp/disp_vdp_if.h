...
#define VDP_REC_LINE_LOG (1 << 13)
#define VDP_FG_LOG (1 << 14)
...
/*Error code define */
#define VDP_OK 0
#define VDP_ALREADY_START 1
#define VDP_ALREADY_STOP 2
#define VDP_FAIL -1
...
#define vdp_printf(level, string, args...)                                     \
	do {                                                                   \
...
extern bool main_fmt_first;
extern bool sub_fmt_first;
...
struct video_buffer_info {
	...
	bool is_hdr10plus_synced;
	bool is_metadata_async;
	bool metadata_already_set;
	bool is_enter_async_mode;
	...
	bool is_film_grain;
	struct mtk_vdp_film_gain_md_t film_grain_info;
	 ...
	 */
	struct mtk_disp_buffer video_disp_buffer;
	struct disp_hw_common_info common_info;

	unsigned long timestap1;
	unsigned long timestap2;
	struct timespec ts;
...
struct video_layer_info {
	...
	enum HDR10_TYPE_ENUM hdr10_type;
	struct video_buffer_info *videobuf;
#ifdef DISP_GCE_SUPPORT
	struct cmdq_pkt *pkt;
	bool new_trigger;
	bool buffer_sec;
#endif
	bool allm_en; /*record the allm status*/

	/*save ion_fd for last frame*/
	int ion_fd;

	bool dispsys_shadow_enable; /* dispsys shadwo enable flag. */
};

struct video_playback_info {
	enum HDMI_VIDEO_RESOLUTION curr_res;
};

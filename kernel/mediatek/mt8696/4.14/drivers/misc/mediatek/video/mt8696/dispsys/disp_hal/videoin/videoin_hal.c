...
int videoin_hal_set_h(int h_active, int h_total, bool is_444,
	int is_16_packet, enum VIDEO_BIT_MODE bit_mode)
{
	int pixel_cnt = 0;
	...
	vWriteVIDEOIN(VIDEO_IN_H_CNTEND,
		(unit_size - 1) << 16 | (unit_size - 1));
	/* vWriteVIDEOINMsk(VIDEO_IN_H_BACKUP, unit_size << 16, 0xfff << 16); */
	vWriteVIDEOIN(VIDEO_IN_H_BACKUP,
		unit_size << 16 | 0x54E3);  /*for 8695 test*/
	if (is_444)
...
int videoin_hal_set_v(int v_active, enum VIDEOIN_YCbCr_FORMAT format)
{
	...
	if (format == VIDEOIN_FORMAT_420)
		vWriteVIDEOIN(VIDEO_IN_ACTIVE_LINE,
		(v_active / 2 - 1) << 16 | (v_active - 1));
	else
		vWriteVIDEOIN(VIDEO_IN_ACTIVE_LINE,
		(v_active - 1) << 16 | (v_active - 1));

	if (format == VIDEOIN_FORMAT_444)
		vWriteVIDEOINMsk(VIDEO_IN_C2_ACTIVE, (v_active - 1) << 16,
		0xfff << 16);
...
{
	if (format == VIDEOIN_FORMAT_420) {
		vWriteVIDEOINMsk(VIDEO_IN_CONFIG, CONFIG_420_MODE,
			CONFIG_420_MODE);
		vWriteVIDEOINMsk(VIDEO_IN_INPUT_CTRL, VIDEO_IN_444_MODE,
			VIDEO_IN_444_MODE);
...
int videoin_hal_set_channel_select(enum VIDEO_CHN_SEL y_sel,
	enum VIDEO_CHN_SEL cb_sel, enum VIDEO_CHN_SEL cr_sel)
{
...
int videoin_hal_set_active_zone(int h_active_start, int v_odd_active_start,
	int v_event_active_start)
{
	vWriteVIDEOIN(VIDEO_IN_H_PKCNT, h_active_start);
	vWriteVIDEOIN(VIDEO_IN_V_PKCNT,
		(v_event_active_start << 16) | v_odd_active_start);
	return 0;
}
...
int videoin_hal_demode_enable(bool en)
{
	if (en) {
		vWriteVIDEOINMsk(VIDEO_IN_CONFIG, CONFIG_DE_MODE,
			CONFIG_DE_MODE);
		vWriteVIDEOINMsk(VIDEO_IN_REQ_OUT, VIDEO_IN_IDX_MODE,
			VIDEO_IN_IDX_MODE);
		vWriteVIDEOINMsk(VIDEO_IN_REQ_OUT, VIDEO_IN_ACT_SEL,
			VIDEO_IN_ACT_SEL);
		vWriteVIDEOINMsk(VIDEO_IN_REQ_OUT, VIDEO_IN_END_SEL,
			VIDEO_IN_END_SEL);
	} else {
...
{
	if (packet_16bit_en)
		vWriteVIDEOINMsk(VIDEO_IN_CONFIG, CONFIG_16_PACKET,
			CONFIG_16_PACKET);
	else
		vWriteVIDEOINMsk(VIDEO_IN_CONFIG, 0, CONFIG_16_PACKET);

	if (mode == VIDEOIN_BITMODE_10)
		vWriteVIDEOINMsk(VIDEO_IN_CONFIG, CONFIG_10_BIT,
			CONFIG_PACKET_MODE);
	else if (mode == VIDEOIN_BITMODE_12)
		vWriteVIDEOINMsk(VIDEO_IN_CONFIG, CONFIG_12_BIT,
			CONFIG_PACKET_MODE);
	else
		vWriteVIDEOINMsk(VIDEO_IN_CONFIG, CONFIG_8_BIT,
			CONFIG_PACKET_MODE);

...
uint32_t *y_addr_va[MAX_VDO_ID];
dma_addr_t y_addr_pa[MAX_VDO_ID];

uint32_t *c_addr_va[MAX_VDO_ID];
dma_addr_t c_addr_pa[MAX_VDO_ID];

uint32_t *vin_y_addr_va;
dma_addr_t vin_y_addr_pa;

uint32_t *vin_c_addr_va;
dma_addr_t vin_c_addr_pa;


uint32_t *y_len_addr_va[MAX_VDO_ID];
dma_addr_t y_len_addr_pa[MAX_VDO_ID];

uint32_t *c_len_addr_va[MAX_VDO_ID];
dma_addr_t c_len_addr_pa[MAX_VDO_ID];

uint32_t *graphic_addr_va[MAX_VDO_ID];
dma_addr_t graphic_addr_pa[MAX_VDO_ID];

uint32_t *graphic_header_va[MAX_VDO_ID];
dma_addr_t graphic_header_pa[MAX_VDO_ID];

static bool allocated[MAX_VDO_ID];
static bool vin_allocated;
...
void disp_dovi_idk_dump_frame_end(void)
{
	if (dump_file_opened) {
		...
		dma_free_coherent(dovi_dev, idk_dump_size,
			y_idk_dump_addr_va, y_idk_dump_addr_pa);
		dma_free_coherent(dovi_dev, idk_dump_size,
			cb_idk_dump_addr_va,
			cb_idk_dump_addr_pa);
		dma_free_coherent(dovi_dev, idk_dump_size,
...
void disp_dovi_idk_dump_frame_start(uint32_t file_id)
{
	if (!dump_file_opened) {
		...
		y_idk_dump_addr_va =
		    dma_alloc_coherent(dovi_dev, idk_dump_size,
		    &y_idk_dump_addr_pa, GFP_KERNEL);
		cb_idk_dump_addr_va =
		    dma_alloc_coherent(dovi_dev, idk_dump_size,
		    &cb_idk_dump_addr_pa, GFP_KERNEL);
		cr_idk_dump_addr_va =
		    dma_alloc_coherent(dovi_dev, idk_dump_size,
		    &cr_idk_dump_addr_pa, GFP_KERNEL);
		graphic_idk_load_addr_va =
...
void disp_dovi_alloc_graphic_buffer(void)
{
	if (!graphic_buf_allocated) {
		...
		if (dovi_sdk_test && !dovi_sdk_file_id) {
			sdk_alloc_size = 1920 * 1088 * 4;
			graphic_size = sdk_alloc_size;
		}
		...
		graphic_header_idk_load_addr_va =
		    ...
		    graphic_header_size,
		    &graphic_header_idk_dump_load_pa, GFP_KERNEL);
	...
	unsigned char *buff, uint32_t len)
{
	...
	if (file_name != NULL && *file_name != '\0') {
		ret = sprintf(fileName, "/sdcard/dovi/%s",
		file_name);
		if (ret <= 0)
			return;
	}
...
void disp_dovi_load_buffer(char *file_name,
	unsigned char *buff, uint32_t len)
{
	mm_segment_t fs;
	struct file *fp = NULL;
			     ...
			     unsigned char *buff, uint32_t len)
{
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_dm_core%u_reg.%04u.bin",
		core_id, frame_num);
	if (ret <= 0)
		return;
			     ...
			     unsigned char *buff, uint32_t len)
{
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_dm_core%u_lut.%04u.bin",
		core_id, frame_num);
	if (ret <= 0)
		return;
...
void disp_dovi_dump_hw_setting(unsigned int core_id,
			       unsigned int frame_num,
			       unsigned char *buff, uint32_t len)
{
	char fileName[MAX_FILE_NAME];
	...
	memset(fileName, 0, MAX_FILE_NAME);
	ret = sprintf(fileName, "tmp_all_hw_setting%u_lut.%04u.bin",
		core_id, frame_num);
	if (ret <= 0)
		return;
...
void disp_dovi_dump_rpu(bool dump_enable,
	unsigned char *buff, uint32_t rpu_len)
{
			...
			enum VIDEO_BIT_MODE bpp)
{
	...
	if (!allocated) {
		...
		dma_alloc_coherent(dovi_dev,
		dump_size,
		&y_addr_pa, GFP_KERNEL);

		cb_addr_va =
		dma_alloc_coherent(dovi_dev,
		dump_size,
		&cb_addr_pa, GFP_KERNEL);

		cr_addr_va =
		dma_alloc_coherent(dovi_dev,
		dump_size,
		&cr_addr_pa, GFP_KERNEL);
	...
	if (enable) {
		videoin_hal_update_addr(y_addr_pa, cb_addr_pa,
			cr_addr_pa, is_444);
		vdout_sys_hal_videoin_source_sel(src);
		videoin_hal_set_h(width, htotal, is_444,
			is_16_packet, bit_mode);
		...
		videoin_hal_set_channel_select(SEL_Y_CHN,
			cb_sel, cr_sel);
		videoin_hal_set_bitmode(bit_mode,
			is_16_packet);
		videoin_hal_demode_enable(false);
		videoin_hal_set_active_zone(h_start,
			v_odd_start, v_even_start);
...
void disp_dovi_show_input_buf_status(uint32_t vdp_id)
{
	...
	dovi_default("y_addr_va %p 0x%x\n", y_addr_va[vdp_id],
		(uint32_t) y_addr_pa[vdp_id]);
	dovi_default("cb_addr_va %p 0x%x\n", c_addr_va[vdp_id],
		(uint32_t) c_addr_pa[vdp_id]);
...
void disp_dovi_alloc_input_buf(uint32_t vdp_id, bool enable)
{
	...
	if (!vin_allocated && enable) {
		vin_y_addr_va =
		    dma_alloc_coherent(dovi_dev, vin_size,
		    &vin_y_addr_pa, GFP_KERNEL);
		vin_c_addr_va =
		    dma_alloc_coherent(dovi_dev, vin_size,
		    &vin_c_addr_pa, GFP_KERNEL);
		vin_allocated = 1;

	} else if (vin_allocated && !enable) {
		dma_free_coherent(dovi_dev, vin_size,
			vin_y_addr_va, vin_y_addr_pa);
		dma_free_coherent(dovi_dev, vin_size,
			vin_c_addr_va, vin_c_addr_pa);
		vin_allocated = 0;
	}
	...
	if (!allocated[vdp_id] && enable) {
		y_addr_va[vdp_id] =
		    ...
		    &y_addr_pa[vdp_id], GFP_KERNEL);

		c_addr_va[vdp_id] =
		    dma_alloc_coherent(dovi_dev, video_size,
		    &c_addr_pa[vdp_id], GFP_KERNEL);

		y_len_addr_va[vdp_id] =
			dma_alloc_coherent(dovi_dev,
			video_size / 256,
			&y_len_addr_pa[vdp_id],
			GFP_KERNEL);

		c_len_addr_va[vdp_id] =
			dma_alloc_coherent(dovi_dev,
			video_size / 256,
			&c_len_addr_pa[vdp_id],
			GFP_KERNEL);
		...
		dma_free_coherent(dovi_dev, video_size,
			y_addr_va[vdp_id], y_addr_pa[vdp_id]);
		dma_free_coherent(dovi_dev, video_size,
			c_addr_va[vdp_id], c_addr_pa[vdp_id]);
...
void disp_dovi_load_video_pattern(uint32_t vdp_id,
	uint32_t pattern, enum dovi_clr_fmt color_fmt)
{
	int ret = 0;
	...
	if (allocated[vdp_id]) {
		...
		char graphic_file_name[MAX_FILE_NAME];
		...
		memset((void *)&clr_fmt, 0,
			sizeof(struct vdp_hal_color_format));
		...
		//load video pattern
		ret = sprintf(y_file_name, "/sdcard/dovi/%d/%s",
			pattern_idx[pattern], "pic_0_Y.out");
		if (ret <= 0)
			return;
		ret = sprintf(c_file_name, "/sdcard/vdp/%d/%s",
			pattern_idx[pattern], "pic_0_CbCr.out");
		if (ret <= 0)
			return;

		disp_dovi_load_buffer(y_file_name,
			(unsigned char *)y_addr_va[vdp_id], file_size);

		disp_dovi_load_buffer(c_file_name,
				      (unsigned char *)c_addr_va[vdp_id],
				      file_size / 2);
		//load gfx pattern
		file_size = (src_width * src_height * 4);
		ret = sprintf(graphic_file_name, "/sdcard/dovi/%d/%s",
			pattern_idx[pattern], "argb.bin");
		if (ret <= 0)
			return;
		disp_dovi_load_buffer(graphic_file_name,
			(uint8_t *)graphic_addr_va,
			file_size);
...
void disp_dovi_enable_vdp(uint32_t vdp_id, uint32_t pattern)
{
	...
	fmt_hal_set_pixel_factor(DISP_FMT_SUB,
		src_width, h_factor, 1);
	...
	vdp_hal_set_disp_config(vdp_id, &src_region,
		&out_region);

...
/* region config */
struct OSD_REGION_NODE_T _rAllRgnNode[OSD_RGN_REG_NUM];
/* plane config */
BOOL _afgPlaneEnableStatus[OSD_PLANE_MAX_NUM];
/* scaler config */
static struct OSD_SCALE_INFO _arOsdScaleInfo[OSD_SCALER_MAX_NUM];
...
void osd_region_init(void)
{
	...
	for (i4Index = 0; i4Index < OSD_RGN_REG_NUM; i4Index++)
		memset(&_rAllRgnNode, 0, sizeof(struct OSD_REGION_NODE_T));
...
INT32 osd_get_free_region(UINT32 u4Plane, UINT32 *u4region)
{
	...
	for (; u4Index < u4MaxRgnByPlane; u4Index++) {
		if (_rAllRgnNode[u4Index].eOsdRgnState == OSD_RGN_STATE_IDLE) {
			*u4region = u4Index;
			_rAllRgnNode[u4Index].eOsdRgnState =
...
void osd_set_region_state(UINT32 u4Plane)
{
	...
	for (; u4Index < u4MaxRgnByPlane; u4Index++) {
		if (_rAllRgnNode[u4Index].eOsdRgnState ==
		    OSD_RGN_STATE_ACTIVE) {
			_rAllRgnNode[u4Index].eOsdRgnState = OSD_RGN_STATE_USED;
			OSD_PRINTF(
				OSD_CONFIG_SW_LOG,
				"[Osd]used region V_addr=0x%lx P_addr=0x%x u4Index=%d\n",
				_rAllRgnNode[u4Index].pu4_vAddr,
				_rAllRgnNode[u4Index].pu4_pAddr,
				u4Index);
		} else if (_rAllRgnNode[u4Index].eOsdRgnState ==
			   OSD_RGN_STATE_USED) {
			_rAllRgnNode[u4Index].eOsdRgnState =
...
static int osd_base_reconfig_all(uint32_t plane, uint32_t path,
				 enum HDMI_VIDEO_RESOLUTION osd_res_mode)
{
	unsigned int mix_layer_id;
	enum DISP_SOF_TYPE start_type;
	enum DISP_SOF_TYPE end_type;
	enum FMT_SOF_TYPE type_start;
	enum FMT_SOF_TYPE type_end;
	int sof_start = 0;
	int sof_end = 0;
	...
	if (plane == OSD_PLANE_1) {
		mix_layer_id = DISP_PATH_UHD_OSD; /*UHD*/
		start_type = DISP_SOF_14_UHD_OSD_STA;
		end_type = DISP_SOF_14_UHD_OSD_END;
		type_start = FMT_SOF_14_UHD_OSD_STA;
		type_end = FMT_SOF_14_UHD_OSD_END;
	} else {
		mix_layer_id = DISP_PATH_FHD_OSD; /*FHD*/
		start_type = DISP_SOF_17_FHD_OSD_STA;
		end_type = DISP_SOF_17_FHD_OSD_END;
		type_start = FMT_SOF_17_FHD_OSD_STA;
		type_end = FMT_SOF_17_FHD_OSD_END;
	}

	//sof setting
	disp_path_get_sof_info(start_type, end_type, &sof_start, &sof_end);
	fmt_hal_set_sof(type_start, type_end, sof_start, sof_end);

	disp_path_get_active_zone(mix_layer_id, osd_res_mode, &osd_hstart,
				  &osd_vodd, &osd_veven);


	_OSD_BASE_SetScrnHStartOsd(plane, osd_hstart);
...
static int osd_get_res_total_size(struct osd_resolution_change *res_chg,
				  UINT32 u4Path, UINT32 *pu4HTotal,
				  UINT32 *pu4VTotal)
{
...
#if 0
static int osd_sc_reconfig_all(uint32_t plane,
				struct osd_resolution_change *osd_res)
{
...
INT32 osd_reset_plane_region_state(UINT32 u4Plane)
{
	...
	OSD_PRINTF(OSD_CONFIG_SW_LOG,
			"%s Plane=0x%x,Index=%d\n",
			__func__, u4Plane, u4Index);
	for (; u4Index < u4MaxRgnByPlane; u4Index++) {
		if (_rAllRgnNode[u4Index].eOsdRgnState !=
		    OSD_RGN_STATE_READ_DONE) {
			_rAllRgnNode[u4Index].eOsdRgnState = OSD_RGN_STATE_IDLE;
...
INT32 OSD_RGN_Get(UINT32 u4Region, INT32 i4Cmd, UINT32 *pu4Value)
{
	...
	if (u4Region >= OSD_RGN_REG_NUM) {
		OSD_LOG_I("%s: u4Region is %d, Invalid\n",
			__func__, u4Region);
		return -(INT32)OSD_RET_INV_ARG;
	}
#if 0
...
INT32 OSD_RGN_Set(UINT32 u4Region, INT32 i4Cmd, UINT32 u4Value)
{
	...
	case OSD_RGN_BMP_PITCH:
		IGNORE_RET(_OSD_RGN_SetLineSize(u4Region,
			(u4Value >> 4) & 0x7ff));
		break;
...
INT32 OSD_RGN_Create_EX(UINT32 u4Region, UINT32 u4SrcBufWidth,
			UINT32 u4SrcBufHeight, UINT32 u4SrcDispX,
			UINT32 u4SrcDispY, UINT32 s_width,
			UINT32 s_height, unsigned int pvBitmap,
			UINT32 eColorMode, UINT32 u4BmpPitch, UINT32 u4DispX,
			UINT32 u4DispY, UINT32 u4DispW, UINT32 u4DispH,
			UINT32 ui4Plane, uint32_t index, UINT32 u4AlphaEn,
			uint32_t alpha, uint32_t fmt_order, bool is_pvric)
{
	INT32 i4Ret;
	UINT32 u4VStep;
	UINT32 u4HStep;
	...
	OSD_PRINTF(OSD_CONFIG_SW_LOG,
		"Create region: region=%d,index =%d, addr=0x%x,CM=%d,u4BmpPitch=%d\n",
		u4Region, index, pvBitmap, eColorMode, u4BmpPitch);
	OSD_PRINTF(OSD_CONFIG_SW_LOG,
		"Create region: src buffer w=%d,src buffer h=%d is_pvric=%d\n",
		   u4SrcBufWidth, u4SrcBufHeight, is_pvric);
	OSD_PRINTF(OSD_CONFIG_SW_LOG,
		"Create region: w=%d,h=%d,ix=%d, iy=%d, ox=%d,oy=%d,ow=%d,oh=%d\n",
		   s_width, s_height, u4SrcDispX, u4SrcDispY,
		   u4DispX, u4DispY, u4DispW, u4DispH);

	if ((u4Region >= OSD_RGN_REG_NUM) || (pvBitmap == 0) ||
	    ((pvBitmap & 0xf) != 0) || (s_width == 0) || (s_height == 0) ||
	    (u4DispW == 0) || (u4DispH == 0) || ((u4BmpPitch & 0xf) != 0) ||
	    ((u4BmpPitch >> 15) != 0)) {
		OSD_PRINTF(
		OSD_CONFIG_SW_LOG, "region Attr Err pvbitmap=%x!!\n", pvBitmap);
		OSD_PRINTF(
		OSD_CONFIG_SW_LOG,
			"Create region: region=%d,index =%d, addr=0x%x,CM=%d,u4BmpPitch=%d\n",
			u4Region, index, pvBitmap, eColorMode, u4BmpPitch);
		OSD_PRINTF(
		OSD_CONFIG_SW_LOG,
		"Create region: w=%d,h=%d,ox=%d,oy=%d,ow=%d,oh=%d\n",
			  s_width, s_height, u4DispX, u4DispY, u4DispW,
			  u4DispH);
		return -(INT32)OSD_RET_INV_ARG;
	}
	...
	if (is_pvric) {
		pvBitmap +=
		((((u4SrcBufWidth * u4SrcBufHeight) >> 6)
		+ 255) >> 8) << 7;
		if (((u4SrcDispX + s_width) % 32) != 0) {
			u4Temp = ALIGN((u4SrcDispX + s_width), 32);
			s_width = u4Temp - u4SrcDispX;
		}
	}
#if 0
	//for 8695 issue
	...
	i4Ret = _OSD_RGN_SetFbdcEnable(u4Region, index, is_pvric);
	if (is_pvric)
		i4Ret = _OSD_RGN_SetPixelFormat(u4Region, index, (uint32_t)0xc);
	else
		i4Ret = _OSD_RGN_SetPixelFormat(u4Region, index, (uint32_t)0x0);
	 ...
	 *i4Ret = OSD_RGN_Set(u4Region, (INT32) OSD_RGN_BMP_PITCH,
	 *u4BmpPitch);
	 */
	//don't use core scale function
	_OSD_RGN_SetLineSize(u4Region, index, (u4BmpPitch >> 4) & 0x7ff);
	u4HStep = (s_width == u4DispW) ? 0x1000 : ((s_width << 12) / u4DispW);
	i4Ret = _OSD_RGN_SetHStep(u4Region, index, 0x1000);
	u4VStep = (s_height == u4DispH) ? 0x1000 : ((s_height << 12) / u4DispH);
	i4Ret = _OSD_RGN_SetVStep(u4Region, index, 0x1000);
...
int osd_create_multi_region(UINT32 u4Region, UINT32 u4SrcDispX,
			    UINT32 u4SrcDispY, UINT32 s_width, UINT32 s_height,
			    unsigned int pvBitmap, UINT32 eColorMode,
			    UINT32 u4BmpPitch, UINT32 u4DispX, UINT32 u4DispY,
			    UINT32 u4DispW, UINT32 u4DispH, UINT32 ui4Plane,
			    uint32_t index)
{
...
/************************************************************************/
int i4Osd_BaseSetFmt(uint32_t plane, uint32_t u4Path,
		     struct osd_resolution_change *res_chg, bool fgFlip)
{
	uint32_t u4HSize, u4VSize, u4HTotal, u4VTotal;
	uint32_t u4DisplayMode;
	uint32_t osd_hstart, osd_veven;

	u4DisplayMode = res_chg->osd_res_mode;
	OSDDBG("%s u4Path=%d, u4DM=%d, fgFlip=%d\n",
		__func__, u4Path, u4DisplayMode, fgFlip);
	...
	IGNORE_RET(_OSD_BASE_SetOsdFrameEndEvent(plane, true));
	IGNORE_RET(_OSD_BASE_SetAutoSwEn(plane, 1));
	IGNORE_RET(_OSD_BASE_SetOsdChkSumEn(plane, true));
	//IGNORE_RET(_OSD_BASE_SetPvricValue0ColorCh3(plane, 1));
	...
	if (plane == OSD_PLANE_2) { /*plane cnt 4ms before osd vsync*/
		if (res_chg->frequency == 60)
			IGNORE_RET(osd_pla_set_sync_threshold(
				plane, u4VTotal * 76 / 100));
		else if (res_chg->frequency == 50)
			IGNORE_RET(osd_pla_set_sync_threshold(
				plane, u4VTotal * 4 / 5));
		else if (res_chg->frequency == 30)
			IGNORE_RET(osd_pla_set_sync_threshold(
				plane, u4VTotal * 88 / 100));
		else /*less than 30*/
			IGNORE_RET(osd_pla_set_sync_threshold(
				plane, u4VTotal * 90 / 100));

		osd_plane_set_sync_threshold_update(plane, 1);
	}
	_OSD_BASE_GetScrnHStartOsd(plane, &osd_hstart);
	_OSD_BASE_GetScrnVStartBotMain(plane, &osd_veven);
	_OSD_BASE_SetFrameEndEventValueH(1, u4HSize);
	_OSD_BASE_SetFrameEndEventValueV(1, u4VSize);
#if 0
	if (plane == OSD_PLANE_1) {
...
INT32 OSDSetColorSpace(UINT8 ui1ClrSpa)
{
	...
	if (ui1ClrSpa == OSD_COLORSPACE_601) {
		...
		for (u4Plane = OSD_PLANE_1; u4Plane < OSD_PLANE_MAX_NUM;
		     u4Plane++)
			_OSD_PLA_SetYCbCr709En((UINT32)u4Plane, 0);
	} else {
		_fgOsdYCbCr709 = true;
...
INT32 OSD_PLA_Update(uint32_t plane, struct cmdq_pkt *pkt)
{
	uint32_t update;

	if (_OSD_PLA_GetUpdateStatus(plane)) {
		/*OSDDBG("[Osd %ld]%s plane =  0x%x\n ",
		 *__func__, vsync_cnt, plane);
		 */
		_OSD_PLA_Update(plane, pkt);
		_OSD_PLA_SetUpdateStatus(plane, false);
	}

	osd_plane_get_extend_update(plane, &update);
	if (update) {
		osd_plane_update_extend(plane, pkt);
		osd_plane_set_extend_update(plane, false);
	}

	osd_plane_get_pvric_update(plane, &update);
	if (update) {
		osd_plane_update_pvric(plane, pkt);
		osd_plane_set_pvric_update(plane, false);
	}

	osd_plane_get_faker_header_update(plane, &update);
	if (update) {
		osd_plane_update_faker_header(plane, pkt);
		osd_plane_set_faker_header_update(plane, false);
	}
	osd_plane_get_sync_threshold_update(plane, &update);
	if (update) {
		osd_plane_update_index(plane, pkt, 0x39);
		osd_plane_set_sync_threshold_update(plane, false);
	}

	osd_plane_get_burst8_update(plane, &update);
	if (update) {
		//burst8 config at oofset_0x1ac bit28/29
		osd_plane_update_index(plane, pkt, 0x2b);
		osd_plane_set_burst8_update(plane, false);
	}
	return (INT32)OSD_RET_OK;
}
...
void osd_alpha_detect_enable(UINT32 u4Plane, BOOL fgEnable,
...
INT32 OsdFlipPlaneRightNow(UINT32 u4Plane, BOOL fgValidReg, UINT32 u4Region,
			   unsigned int idx)
{
	OSD_VERIFY_PLANE(u4Plane);
...
INT32 OSD_SC_CheckCapability(UINT32 u4SrcW, UINT32 u4SrcH, UINT32 u4DstW,
			     UINT32 u4DstH)
{
	return OSD_RET_OK;
}
...
INT32 osd_sc_check_same_scaler_info(UINT32 u4Scaler, UINT32 u4SrcWidth,
				    UINT32 u4SrcHeight, UINT32 uDstX,
				    UINT32 uDstY, UINT32 u4DstWidth,
				    UINT32 u4DstHeight)
{
...
INT32 OSD_SC_Scale(UINT32 u4Scaler, UINT32 u4Enable, UINT32 u4SrcWidth,
		   UINT32 u4SrcHeight, UINT32 uDstX, UINT32 uDstY,
		   UINT32 u4DstWidth, UINT32 u4DstHeight)
{
	UINT32 u4Tmp = 0, u4MaxWidth = 0, u4MaxHeight = 0, fgPrgs = 0,
	       u4Path = OSD_MAIN_PATH;
	...
	OSD_PRINTF(OSD_CONFIG_SW_LOG,
		   "%s:id=%d,en=%d,src=(%d,%d),dst=(%d,%d)\n",
		   __func__, u4Scaler, u4Enable, u4SrcWidth, u4SrcHeight,
		   u4DstWidth, u4DstHeight);
	...
	if ((u4MaxWidth == 0) || (u4MaxHeight) == 0) {
		...
		u4MaxHeight = 0x438;
		if (u4Path == OSD_MAIN_PATH) {
			IGNORE_RET(_OSD_BASE_SetScrnHSizeMain(u4Scaler,
							      u4MaxWidth));
			IGNORE_RET(_OSD_BASE_SetScrnVSizeMain(u4Scaler,
							      u4MaxHeight));
		}
	}
	...
	case (UINT32)OSD_SCALER_1:
		IGNORE_RET(_OSD_BASE_SetOsdHStart(u4Scaler, uDstX));
		IGNORE_RET(_OSD_BASE_SetOsdVStart(u4Scaler, uDstY));
		IGNORE_RET(_OSD_BASE_GetOsdPrgs(u4Scaler, &fgPrgs));
		IGNORE_RET(_OSD_BASE_GetOsdHStart(u4Scaler, &u4Tmp));
		u4MaxWidth -= u4Tmp;
		IGNORE_RET(_OSD_BASE_GetOsdVStart(u4Scaler, &u4Tmp));
		u4MaxHeight -= u4Tmp;
		break;
	...
	if (u4SrcWidth == u4DstWidth) {
		...
		if (u4SrcWidth < u4DstWidth) {
			...
			if (_afgCutEdge[u4Scaler]) {
				IGNORE_RET(_OSD_SC_SetHuscAlphaEdgeElt(u4Scaler,
								       1));
				IGNORE_RET(_OSD_SC_SetHdscAlphaEdgeElt(u4Scaler,
								       0));
			}

			u4Tmp = ((u4SrcWidth - 1) << OSD_SC_STEP_BIT) /
				(u4DstWidth - 1);
			...
			u4Tmp = ((u4DstWidth << OSD_SC_STEP_BIT) / u4SrcWidth) +
				(((u4DstWidth << OSD_SC_STEP_BIT) % u4SrcWidth)
					 ? 1
					 : 0);
	...
	/*support interlace mode,just need enable hw P2I*/
	/*if (!fgPrgs)*/
	/*u4DstHeight >>= 1;*/
	...
	if (u4SrcHeight == u4DstHeight) {
		...
		if (u4SrcHeight <= u4DstHeight) {
			...
			if (_afgCutEdge[u4Scaler]) {
				IGNORE_RET(_OSD_SC_SetVuscAlphaEdgeElt(u4Scaler,
								       1));
				IGNORE_RET(_OSD_SC_SetVdscAlphaEdgeElt(u4Scaler,
								       0));
			}
			...
			if (fgPrgs) {
				...
				IGNORE_RET(_OSD_SC_SetVscOfstTop(u4Scaler,
								 u4VTopOfst));
				IGNORE_RET(_OSD_SC_SetVscOfstBot(u4Scaler,
								 u4VBotOfst));
			}
			...
			if (fgPrgs) {
				u4Tmp = ((u4DstHeight << OSD_SC_STEP_BIT) /
					 u4SrcHeight) +
					(((u4DstHeight << OSD_SC_STEP_BIT) %
					  u4SrcHeight)
						 ? 1
						 : 0);
...
INT32 OSD_SC_Update(uint32_t plane, struct cmdq_pkt *pkt)
{
	if (_OSD_SC_GetUpdateStatus(plane)) {
		_OSD_SC_Update(plane, pkt);

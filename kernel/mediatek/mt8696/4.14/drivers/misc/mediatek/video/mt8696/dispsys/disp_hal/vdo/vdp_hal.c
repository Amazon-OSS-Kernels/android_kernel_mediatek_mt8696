...
/*gce support*/
#ifdef DISP_GCE_SUPPORT
struct cmdq_pkt *vdo_cmdq_pkt[VDP_MAX];
struct vdo_phy_register vdo_phy_addr[VDP_MAX];
#endif
...
void vdp_hal_init(unsigned char vdp_id)
{
	...
	if (vdp_id == 0) {
		vdo_phy_addr[vdp_id].vdo_base = 0x15001400;
		vdo_phy_addr[vdp_id].ufo_base = 0x15001200;
		vdo_phy_addr[vdp_id].scl_base = 0x15001100;
		}
	else if (vdp_id == 1) {
		vdo_phy_addr[vdp_id].vdo_base = 0x15008400;
		vdo_phy_addr[vdp_id].ufo_base = 0x15008200;
		vdo_phy_addr[vdp_id].scl_base = 0x15008100;
		}
#endif
...
/*For GCE enable, no shadow ,no trriger*/
#ifndef DISP_GCE_SUPPORT
	if (vdp_id == 0) {

		/*trigger vdo*/
		WriteREG32Msk_Cmdq(NULL,
			(uintptr_t)(disp_vdo_reg_base[0] +
			HAL_VDO3_OFST + 0x14),
			(1 << 31), (1 << 31));
		/*trigger ufo*/
		WriteREG32Msk_Cmdq(NULL,
			(uintptr_t)(disp_vdo_reg_base[0] +
			HAL_VDO3_UFO_OFST + 0x40),
			(1 << 2), (1 << 2));
	} else if (vdp_id == 1) {
		/*trigger vdo*/
		WriteREG32Msk_Cmdq(NULL,
			(uintptr_t)(disp_vdo_reg_base[1] +
			HAL_VDO3_OFST + 0x14),
			(1 << 31), (1 << 31));
		/*trigger ufo*/
		WriteREG32Msk_Cmdq(NULL,
...
void vdp_hal_isr(bool update_main, bool update_sub)
{
	...
	if (update_main) {
		while (1) {
			#ifdef DISP_GCE_SUPPORT
			if (vdo_cmdq_pkt[0] == NULL) {
				DISP_LOG_E("%s invalid gce packet\n",
					__func__);
				break;
			}
			#endif
		...
		for (idx = 0; idx < HAL_VDO_REG_NUM; idx++) {
			if (IS_REG_SET(vdo_sw_reg[0].vdo_reg_mode,
				       REG_MASK(idx))) {
				#ifdef DISP_GCE_SUPPORT

				cmdq_pkt_write_value_addr(vdo_cmdq_pkt[0],
				vdo_phy_addr[0].vdo_base + 4 * idx,
		...
		for (idx = 0; idx < HAL_VDO_UFOD_REG_NUM; idx++) {
			if (IS_REG_SET(vdo_sw_reg[0].vdo_ufo_reg_mode,
				       REG_MASK(idx)))
				#ifdef DISP_GCE_SUPPORT
				cmdq_pkt_write_value_addr(vdo_cmdq_pkt[0],
				vdo_phy_addr[0].ufo_base + 4 * idx,
		...
		for (idx = 0; idx < HAL_VDO_SCL_REG_NUM; idx++) {
			if (IS_REG_SET(vdo_sw_reg[0].vdo_scl_reg_mode,
				       REG_MASK(idx)))
				#ifdef DISP_GCE_SUPPORT
				cmdq_pkt_write_value_addr(vdo_cmdq_pkt[0],
				vdo_phy_addr[0].scl_base + 4 * idx,
	...
	if (update_sub) {
		while (1) {
			...
			#ifdef DISP_GCE_SUPPORT
			if (vdo_cmdq_pkt[1] == NULL) {
				DISP_LOG_E("invalid gce packet\n");
				break;
			}
			#endif
		/* update vdo4 hw reg */
		for (idx = 0; idx < HAL_VDO_REG_NUM; idx++) {
			if (IS_REG_SET(vdo_sw_reg[1].vdo_reg_mode,
				       REG_MASK(idx)))
				#ifdef DISP_GCE_SUPPORT
				cmdq_pkt_write_value_addr(vdo_cmdq_pkt[1],
				vdo_phy_addr[1].vdo_base + 4 * idx,
		...
		for (idx = 0; idx < HAL_VDO_UFOD_REG_NUM; idx++) {
			if (IS_REG_SET(vdo_sw_reg[1].vdo_ufo_reg_mode,
				       REG_MASK(idx)))
				#ifdef DISP_GCE_SUPPORT
				cmdq_pkt_write_value_addr(vdo_cmdq_pkt[1],
				vdo_phy_addr[1].ufo_base + 4 * idx,
		...
		for (idx = 0; idx < HAL_VDO_SCL_REG_NUM; idx++) {
			if (IS_REG_SET(vdo_sw_reg[1].vdo_scl_reg_mode,
				       REG_MASK(idx)))
				#ifdef DISP_GCE_SUPPORT
				cmdq_pkt_write_value_addr(vdo_cmdq_pkt[1],
				vdo_phy_addr[1].scl_base + 4 * idx,
...
#ifdef DISP_GCE_SUPPORT
void vdp_hal_config_sec_buffer(unsigned char vdp_id,
	struct vdp_hal_fb_addr *addr)
{
	if (vdp_id > VDP_MAX) {
		DISP_LOG_E("%s invalid vdp id%d\n", __func__, vdp_id);
		return;
		}
	/*main video*/
	if (vdp_id == 0) {
		cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[0],
			vdo_phy_addr[0].ufo_base,
			addr->sec_handle,
			CMDQ_IWC_H_2_MVA,
			addr->addr_y,
			addr->buffer_size,
			M4U_PORT_VDO3_UFOD_Y0);
		cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[0],
			vdo_phy_addr[0].ufo_base + 4,
			addr->sec_handle,
			CMDQ_IWC_H_2_MVA,
			addr->addr_c,
			addr->buffer_size,
			M4U_PORT_VDO3_UFOD_C0);
		if (addr->addr_y_len != addr->addr_y)
			cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[0],
				vdo_phy_addr[0].ufo_base + 0x10,
				addr->sec_handle,
				CMDQ_IWC_H_2_MVA,
				addr->addr_y_len,
				addr->buffer_size,
				M4U_PORT_VDO3_UFOD_Y_LENGTH);
		if (addr->addr_c_len != addr->addr_y)
			cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[0],
				vdo_phy_addr[0].ufo_base + 0x14,
				addr->sec_handle,
				CMDQ_IWC_H_2_MVA,
				addr->addr_c_len,
				addr->buffer_size,
				M4U_PORT_VDO3_UFOD_C_LENGTH);
	}
	/*sub video*/
	if (vdp_id == 1) {
		cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[1],
			vdo_phy_addr[1].ufo_base,
			addr->sec_handle,
			CMDQ_IWC_H_2_MVA,
			addr->addr_y,
			addr->buffer_size,
			M4U_PORT_VDO4_UFOD_Y0);
		cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[1],
			vdo_phy_addr[1].ufo_base + 4,
			addr->sec_handle,
			CMDQ_IWC_H_2_MVA,
			addr->addr_c,
			addr->buffer_size,
			M4U_PORT_VDO4_UFOD_C0);
		if (addr->addr_y_len != addr->addr_y)
			cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[1],
				vdo_phy_addr[1].ufo_base + 0x10,
				addr->sec_handle,
				CMDQ_IWC_H_2_MVA,
				addr->addr_y_len,
				addr->buffer_size,
				M4U_PORT_VDO4_UFOD_Y_LENGTH);
		if (addr->addr_c_len != addr->addr_y)
			cmdq_sec_pkt_write_reg(vdo_cmdq_pkt[1],
				vdo_phy_addr[1].ufo_base + 0x14,
				addr->sec_handle,
				CMDQ_IWC_H_2_MVA,
				addr->addr_c_len,
				addr->buffer_size,
				M4U_PORT_VDO4_UFOD_C_LENGTH);
		}
}
#endif
void vdp_hal_set_enable(uint8_t vdp_id, uint8_t enable)
{
	...
	if ((vdp_id != VDP_1) && (vdp_id != VDP_2)) {
		DISP_LOG_E("%s get invalid vdp id%d", __func__, vdp_id);
		return;
	}
	...
	if (vdp_id == 0) {
		...
		p_ufo_sw_reg->field.ULTRA_TH = 0x7A;
		p_ufo_sw_reg->field.C_ULTRA_EN = 1;
		p_ufo_sw_reg->field.C_ULTRA_TH = 0x7C;
		/*pre-ultra*/
		p_ufo_sw_reg->field.PRE_ULTRA_EN = 1;
		p_ufo_sw_reg->field.PRE_ULTRA_TH = 0x78;
		p_ufo_sw_reg->field.C_PRE_ULTRA_EN = 1;
		p_ufo_sw_reg->field.C_PRE_ULTRA_TH = 0x7A;
	} else {
		/*sub video*/
		p_ufo_sw_reg->field.ULTRA_EN = 1;
		p_ufo_sw_reg->field.ULTRA_TH = 0x7A;
	...
	REG_SET(*p_ufo_reg_mode,
		REG_MASK(0x40 / 4) | REG_MASK(0x1c / 4) | REG_MASK(0x20 / 4));
...
void vdp_hal_set_src_size(uint8_t vdp_id, uint32_t width, uint32_t height)
{
	...
	p_ufo_sw_reg->field.H_WIDTH =
		(((width >= 3840) ? 4096 : width) + 1) / 2; /* picw */

	p_ufo_sw_reg->field.V_HEIGTH = (height + 3) / 4 * 4; /* picHigh */
	...
	REG_SET(*p_ufo_reg_mode,
		REG_MASK(0x08 / 4) | REG_MASK(0x0c / 4) | REG_MASK(0x18 / 4));
				  ...
				  struct vdp_hal_color_format *p_clr_fmt)
{
	...
	REG_SET(*p_ufo_reg_mode, REG_MASK(0x18 / 4) | REG_MASK(0x1C / 4));
	REG_SET(*p_vdp_reg_mode, REG_MASK(0x30 / 4));
}

void vdp_hal_set_disp_config(uint8_t vdp_id, struct vdp_hal_region *p_src,
			     struct vdp_hal_region *p_out)
{
	union vdo_hal_union *p_vdp_sw_reg = NULL;
	struct vdp_hal_disp_info *p_disp = NULL;
	uint64_t *p_vdp_reg_mode = NULL;
	uint32_t dw_need;
	uint32_t scl_factor;
	...
	p_vdp_sw_reg->field.VSCALE =
		((p_src->height * scl_factor) / p_out->height);
	...
	if (p_src->y == 0) {
		...
		p_vdp_sw_reg->field.Video_Opt8 = 0x1; /* 0x7c[8] */
	} else {
	...
	REG_SET(*p_vdp_reg_mode,
		REG_MASK(0x10 / 4) | REG_MASK(0x14 / 4) | REG_MASK(0x20 / 4) |
			REG_MASK(0xE0 / 4) | REG_MASK(0x24 / 4) |
			REG_MASK(0x2C / 4) | REG_MASK(0x7C / 4));
	...
	p_disp->v_odd_end =
		p_disp->v_odd_start + p_out->height - 1;	/* 0x420A4 */
	p_disp->v_even_start = v_even_start[vdp_id] + p_out->y; /* 0x420A8 */
	p_disp->v_even_end =
		p_disp->v_even_start + p_out->height - 1; /* 0x420A8 */
	if (h_down_scl_on == 0)
		p_disp->h_factor = ((p_src->width * h_scl_factor) /
				    p_out->width); /* 0x420B0 */
...
void vdp_hal_set_dsd_config(uint8_t vdp_id, uint16_t src_height,
			    uint16_t dst_height, uint16_t res_height)
{
	uint32_t scl_factor;
	union vdo_hal_union *p_vdp_sw_reg = NULL;
	uint64_t *p_vdp_reg_mode = NULL;
	...
	if ((res_height == 480) && (src_height > 1080)) /*4k to 480p dsd*/
		p_vdp_sw_reg->field.VSCALE =
			((src_height * scl_factor) / (dst_height * 4));
	else
		p_vdp_sw_reg->field.VSCALE =
			((src_height * scl_factor) / (dst_height * 2));
...
void vdp_hal_set_ufo_addr(uint8_t vdp_id, struct vdp_hal_fb_addr *p_fb_addr)
{
	...
	REG_SET(*p_ufo_reg_mode, REG_MASK(0x00 / 4) | REG_MASK(0x04 / 4) |
					 REG_MASK(0x10 / 4) |
					 REG_MASK(0x14 / 4));
}
			   ...
			   struct vdp_hal_region *p_dst)
{
	...
	if (p_src->x != p_dst->x || p_src->y != p_dst->y ||
	    p_src->width != p_dst->width || p_src->height != p_dst->height) {
		ret = 0;
	}
	return ret;
}
			    ...
			    struct vdp_hal_fb_addr *p_dst)
{
	...
	if (p_src->addr_y != p_dst->addr_y || p_src->addr_c != p_dst->addr_c ||
...
{
	struct vdp_hal_data_info *p_data =
		NULL; /* record last HAL config info */
	struct vdp_hal_color_format *p_clr_fmt =
		NULL; /* record last HAL config color format info */
	struct vdp_hal_fb_info *p_cur_fb =
		NULL; /* current config buffer info */
	uint8_t vdp_idx = 1;
	int ret = 0;
...
void vdp_hal_config_timing(unsigned char vdp_id,
			   const struct disp_hw_resolution *res_info, int h_sta,
			   int v_sta_odd, int v_sta_even)
{
	int ret = 0;
...
void vdp_hal_get_info(unsigned char vdp_id, struct vdp_hal_data_info *vdp_hal,
...
void vdp_hal_set_info(unsigned char vdp_id, struct vdp_hal_data_info *vdp_hal,
		      struct vdp_hal_disp_info *vdp_disp,
		      struct vdo_sw_shadow *vdp_sw_shadow)
{
	int ret = 0;
...
#ifdef DISP_GCE_SUPPORT
void vdp_hal_get_cmdq_pkt(struct cmdq_pkt *pkt_main,
	struct cmdq_pkt *pkt_sub)
{
	vdo_cmdq_pkt[0] = pkt_main;
	vdo_cmdq_pkt[1] = pkt_sub;
}
#endif

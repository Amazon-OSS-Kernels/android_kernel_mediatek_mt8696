...
static void dovi_process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "log_level:", 10) == 0) {
		...
		p = (char *)opt + 10;
		STR_CVT(&p, &level, uint32_t, goto Error);
		STR_CVT(&p, &enable, uint32_t, goto Error);
		if (enable)
		...
		p = (char *)opt + 11;
		STR_CVT(&p, &test_case_id, uint32_t, goto Error);
		STR_CVT(&p, &out_format, uint32_t, goto Error);
		...
		p = (char *)opt + 8;
		STR_CVT(&p, &test_case_id, uint32_t, goto Error);
		STR_CVT(&p, &use_ll, uint32_t, goto Error);
		STR_CVT(&p, &ll_rgb_desired, uint32_t, goto Error);
		...
		p = (char *)opt + 13;
		STR_CVT(&p, &force_pri_mode, uint32_t, goto Error);
		STR_CVT(&p, &pri_mode, uint32_t, goto Error);
		...
		p = (char *)opt + 9;
		STR_CVT(&p, &force_g_format, uint32_t, goto Error);
		STR_CVT(&p, &g_format, uint32_t, goto Error);
		...
		STR_CVT(&p, &enable, uint32_t, goto Error);
		STR_CVT(&p, &src, uint32_t, goto Error);
		STR_CVT(&p, &fmt, uint32_t, goto Error);
		STR_CVT(&p, &bpp, uint32_t, goto Error);
		...
		p = (char *)opt + 6;
		STR_CVT(&p, &vdp_id, uint32_t, goto Error);
		STR_CVT(&p, &enable, uint32_t, goto Error);
		...
		p = (char *)opt + 5;
		STR_CVT(&p, &vdp_id, uint32_t, goto Error);
		STR_CVT(&p, &pattern, uint32_t, goto Error);
		STR_CVT(&p, &color_fmt, uint32_t, goto Error);
		...
		STR_CVT(&p, &cmd, uint32_t, goto Error);
		STR_CVT(&p, &data, uint32_t, goto Error);
		...
		p = (char *)opt + 7;
		STR_CVT(&p, &tv_type, uint32_t, goto Error);
		...
		p = (char *)opt + 8;
		STR_CVT(&p, &layer_info, uint32_t, goto Error);
		STR_CVT(&p, &rpu_id, uint32_t, goto Error);
		...
		p = (char *)opt + 9;
		STR_CVT(&p, &id, uint32_t, goto Error);
		...
		p = (char *)opt + 5;
		STR_CVT(&p, &subv_type_val, uint32_t, goto Error);
		...
		p = (char *)opt + 7;
		STR_CVT(&p, &id, uint32_t, goto Error);
		STR_CVT(&p, &width, uint32_t, goto Error);
		STR_CVT(&p, &height, uint32_t, goto Error);
		...
		p = (char *)opt + 4;
		STR_CVT(&p, &slt_case, uint32_t, goto Error);
		...
		p = (char *)opt + 8;
		STR_CVT(&p, &case_id, uint32_t, goto Error);
		...
		p = (char *)opt + 8;
		STR_CVT(&p, &timing, uint32_t, goto Error);
		...
		p = (char *)opt + 6;
		STR_CVT(&p, &black_en, uint32_t, goto Error);
		STR_CVT(&p, &black_cnt, uint32_t, goto Error);
		...
		p = (char *)opt + 9;
		STR_CVT(&p, &out_res, uint32_t, goto Error);
		STR_CVT(&p, &out_format, uint32_t, goto Error);
		STR_CVT(&p, &ll_on, uint32_t, goto Error);
		STR_CVT(&p, &ll_rgb, uint32_t, goto Error);
		STR_CVT(&p, &layer_on[0], uint32_t, goto Error);
		STR_CVT(&p, &case_id[0], uint32_t, goto Error);
		STR_CVT(&p, &layer_on[1], uint32_t, goto Error);
		STR_CVT(&p, &case_id[1], uint32_t, goto Error);
		STR_CVT(&p, &layer_on[2], uint32_t, goto Error);
		STR_CVT(&p, &case_id[2], uint32_t, goto Error);
		STR_CVT(&p, &layer_on[3], uint32_t, goto Error);
		STR_CVT(&p, &case_id[3], uint32_t, goto Error);

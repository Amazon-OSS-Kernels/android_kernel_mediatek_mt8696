...
enum cfd_status cfd_sec_test_case(uint32_t layer_id, uint32_t test_no,
	uint32_t mode)
{
	enum cfd_status status = CFD_STATUS_OK;
	uint32_t addr = 0;
	...
	uint32_t count = 0;
	uint32_t idx = 0;
	struct cfd_share_memory_info_t *p_share_mem = cfd_share_mem[layer_id];
	uint32_t size = sizeof(struct cfd_share_memory_info_t_ar);
	...
	if (status != CFD_STATUS_OK)
		cfd_error("cfd set test case fail[%d]\n", status);

	// dump output
	if (p_share_mem != NULL) {
		p_share_mem->dbg_id = test_no;
		cfd_default("cfd client share mem [%d][%p]\n",
			layer_id, p_share_mem);
		// register
		count = p_share_mem->cfd_hw_output.stRegHdrTable.u32Depth;
		cfd_default("cfd hw output HDR cmd[%d]\n", count);
		...
		cfd_default("cfd hw output SDR cmd[%d]\n", count);
		for (idx = 0; idx < count; idx++) {
			addr =
		...
		cfd_default("cfd output eotf lut[%d]\n",
			size);
		...
		cfd_default("cfd output ootf lut[%d]\n",
			size);
		...
		cfd_default("cfd output oetf lut[%d]\n",
			size);
		if (size > 0) {
			// call adl
			// prepare adl table
		...
		cfd_default("cfd output tmo lut[%d]\n",
			size);

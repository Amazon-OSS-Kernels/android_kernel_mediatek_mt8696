...
static struct ion_handle *vdp_ion_import_handle(struct ion_client *client,
						int fd)
{
...
static size_t vdp_ion_phys_mmu_addr(struct ion_client *client,
	...
	do {
		...
		list_for_each_entry_safe(buf, temp, &(layer_info->buf_list),
			...
			if (layer_info->release_timeline_idx >
				...
				vdp_printf(
					VDP_AVSYNC_LOG,
					"rel_tl_idx %d, rel_idx %d\n",
					layer_info->release_timeline_idx,
					layer_info->release_idx);
				/*free ion handle */
				vdp_ion_free_handle(vdp_ion_client,
						    buf->ion_hnd);
...
int disp_vdp_get_mva(int ion_fd, struct video_buffer_info *buf_info)
{
	...
	if (is_Y_C_independent) {
		...
		buf_info->ion_hnd2 = vdp_ion_import_handle(
			vdp_ion_client, (ion_fd & 0xFFFF0000) >> 16);
	} else
		buf_info->ion_hnd =
			vdp_ion_import_handle(vdp_ion_client, ion_fd);
					...
					struct video_buffer_info *buf_info)
{
	...
	vdp_printf(VDP_FG_LOG, "disp_fg offset %d len %d %d\n",
	...
	if (!config->secruity_en) { /* normal display buffer */
		...
		memcpy(&film_grain_info, &config->film_grain_info,
		       sizeof(struct mtk_disp_film_grain_md_t));

		/* get film grain info from tz */
		fg_sec_get_params(&film_grain_info);

		memcpy(buf_info->film_grain_info.buff,
		       film_grain_info.addr,
		       film_grain_info.len);
	}
}
		    ...
		    struct disp_hw_common_info *info)
{
		...
		if (!config->secruity_en) { /* normal display buffer */
			...
			if (ret_cfd == VDP_OK) {
				buf_info->hdr_info.metadata_info.dovi_metadata
					.sec_handle =
				hdr10_plus_svp_handle.sec_new_handle_from_tz;
				buf_info->hdr_info.metadata_info
					.dovi_metadata.len =
				hdr10_plus_svp_handle.len_from_tz;
				buf_info->hdr_info.metadata_info
	...
	vdp_printf(
		VDP_FLOW_LOG,
		"vdo%d ion=%d hnd=0x%p   0x%X (0x%X 0x%X 0x%X 0x%X)\n",
		buf_info->layer_id, buf_info->ion_fd, buf_info->ion_hnd,
		  buf_info->src_phy_addr, buf_info->ofst_y,
		buf_info->ofst_c, buf_info->ofst_y_len,
		buf_info->ofst_c_len);

	vdp_printf(
		VDP_AVSYNC_LOG,
		"vdo%d pts=%lld rel=%d cur=%d release %d",
		buf_info->layer_id, config->pts,

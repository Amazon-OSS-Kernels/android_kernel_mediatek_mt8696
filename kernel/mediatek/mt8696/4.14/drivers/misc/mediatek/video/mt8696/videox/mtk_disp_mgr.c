...
{
	int ret = 0;
	struct mtk_disp_config *config = NULL;

	config = vmalloc(sizeof(struct mtk_disp_config));
	if (config == NULL)
		return -1;
				      ...
				      unsigned long arg)
{
	...
	struct compat_mtk_disp_config *compat_config = NULL;

	compat_config = vmalloc(sizeof(struct compat_mtk_disp_config));
	if (compat_config == NULL)
		return -1;
	memset(compat_config, 0, sizeof(struct compat_mtk_disp_config));

	if (cmd == COMPAT_MTK_DISP_IOCTL_SET_INPUT_BUFFER) {
		int ret = 0;
		void __user *argp = (void __user *)arg;

		if (copy_from_user(compat_config, argp,
		...
		ret = copy_to_user(argp, compat_config,
				   sizeof(struct compat_mtk_disp_config));

...
s32 cmdq_util_error_save_lst(const char *format, va_list args)
{
	...
	if (size < 0) {
		cmdq_err("snprintf return error:%d", size);
		return size;
	} else if ((u32)size >= (CMDQ_FIRST_ERR_SIZE - util.err.length))
...
static int cmdq_util_record_print(struct seq_file *seq, void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(util.record); i++) {
		...
		hw_time_rem = CMDQ_TICK_TO_US_U32(hw_time);
...
#if 0 /*!IS_ENABLED(CONFIG_FPGA_EARLY_PORTING)*/
/* tmp mark this for build pass */

	...
	struct cmdq_sec_task *task)
{
	...
	else if (iwc_cmd != CMD_CMDQ_TL_PATH_RES_ALLOCATE)
		cmdq_err("send null task to session reply!");
...
static int cmdq_sec_mbox_startup(struct mbox_chan *chan)
{
	...
	if (len < 0) {
		cmdq_err("snprintf return error:%d", len);
		return len;
	} else if ((size_t)len >= sizeof(name))
		cmdq_err("size:%d over buf size:%d",
			len,
			sizeof(name));
	cmdq_log("snprintf len:%dname size:%d", len, sizeof(name));

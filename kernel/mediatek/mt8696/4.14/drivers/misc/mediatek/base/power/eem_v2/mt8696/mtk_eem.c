...
static unsigned int ctrl_SN_Enable;
...
static int eem_dump_proc_show(struct seq_file *m, void *v)
{
	...
	struct eemsn_det *det;
#endif
	int *val = (int *)&eemsn_log->efuse_devinfo;
	unsigned int devinfo_num =
	sizeof(struct eemsn_devinfo) / sizeof(unsigned int);
	...
	for (j = 0; j < devinfo_num; j++)
		seq_printf(m, "[%d]M_HW_RES%d\t= 0x%08X\n",
			j, j, val[j]);


	for (k = 0; k < NR_EEMSN_DET; k++) {
		seq_printf(m, "id:%d, vf_tbl_det pi_vf_num:%d\n",
		k, eemsn_log->vf_tbl_det[k].pi_vf_num);
		if (eemsn_log->vf_tbl_det[k].pi_vf_num <= NR_PI_VF)
			for (j = 0; j < eemsn_log->vf_tbl_det[k].pi_vf_num; j++)
				seq_printf(m, "idx:%d, f:%d, v:0x%x\n",
				j, eemsn_log->vf_tbl_det[k].pi_freq_tbl[j],
				eemsn_log->vf_tbl_det[k].pi_volt_tbl[j]);
	}
...
static int eem_status_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "bank = %d, feature:0x%x, - (",
		   det->det_id, det->features);

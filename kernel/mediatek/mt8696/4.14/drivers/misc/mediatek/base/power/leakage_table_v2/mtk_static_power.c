...
#ifdef CONFIG_POWER_TABLE_MT8696
...
{
	int i, j;
	struct sptab_s *tab[MAX_TABLE_SIZE] = {};
	struct sptab_s *tab1, *tab2, *tspt;
	...
	for (i = 0; i < spower_raw->table_size; i++) {
		...
		for (j = 0; j < MTK_SPOWER_MAX; j++) {
			...
			if (devinfo_domain & BIT(j)) {
				c[i] = (sptab_lookup(&(all_tab[j]->tab_raw[i]),
						     voltage, degree));
				 #if 0
				c[i] += (temp *
					...
					10;
				 #endif
				/* there total core lkg in default lkg table */
				/* SPOWER_INFO("table[%d] lkg %d\n",i, c[i]); */
			}
		}

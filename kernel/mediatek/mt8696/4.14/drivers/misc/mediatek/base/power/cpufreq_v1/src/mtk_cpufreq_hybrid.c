...
#define DVFS_D_LEN		(4) /* # of cmd + arg0 + arg1 + ... */
	...
	case IPI_DVFS_INIT_PTBL:
		...
		cpufreq_ver
		("I'd like to initialize mcupm pvt_tbl,	segment code = %d\n",
...
static int _mt_dvfsp_pdrv_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node,
			"vsram", &vsram_separated);
	if (ret) {
		tag_pr_info("Cant find vsram info\n");
		vsram_separated = 0;
	}
	...
	if (vsram_separated) {
		...
		for (i = 0; i < 3; i++) {
			named_gpio[i] = of_get_named_gpio(pdev->dev.of_node, "brd-gpios", i);
			if (!gpio_is_valid(named_gpio[i]))
				named_gpio[i] = -EINVAL;

			ret = devm_gpio_request(dev, named_gpio[i], NULL);
			if (ret)
				dev_err(dev, "failed to get GPIO%d\n", i);
			else
				brd_value[i] = gpio_get_value(named_gpio[i]);

			gpio_free(named_gpio[i]);
		}

		/* 011 for EVT
		 if ((brd_value[0] == 0) && (brd_value[1] == 1) && (brd_value[2] == 1)) */
		/* Detect GPIO21 for now, as pull down value might not be accurate for GPIO19/20 to do correct detection EVT:0, HVT:1
		   For DVT and beyond, EE now doesn't have plan to change Vproc_sram to separate, for this logic can reuse the detection.
		   If we want to change back to 3 pin detection logic, EE needs to change the pull down resistor value */
		if (brd_value[0] == 0) {
			vsram_separated = 0;
		}
		tag_pr_info("vsram_seperated = %d", vsram_separated);
	}
...
int cpuhvfs_set_init_ptbl(void)
{
	...
	/* seg code */
	cdvfs_d.u.set_fv.arg[0] = vsram_separated;
...
int cpuhvfs_set_iccs_freq(enum mt_cpu_dvfs_id id, unsigned int freq)
{
	...
	cluster = (id == MT_CPU_DVFS_LL) ? DVFS_CLUSTER_LL : DVFS_CLUSTER_B;
#else
	cluster = (id == MT_CPU_DVFS_LL) ? 0 : 2;
#endif
#else
	cluster = 0;
#endif

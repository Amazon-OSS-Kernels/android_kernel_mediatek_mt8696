...
#if defined(CONFIG_MTK_LCM) || defined(CONFIG_MTK_FB_SUPPORT_ASSERTION_LAYER)
...
static void ke_gen_process_msg(void)
{
	...
	memset(data, 0x0, AEE_PROCESS_NAME_LENGTH);
	strncpy(data, aed_dev.kerec.lastlog->process_path,
			AEE_PROCESS_NAME_LENGTH - 1);
...
static void ke_gen_backtrace_msg(void)
{
	...
	memset(data, 0x0, AEE_BACKTRACE_LENGTH);
	strncpy(data, aed_dev.kerec.lastlog->backtrace, AEE_BACKTRACE_LENGTH - 1);
	rep_msg->len = strlen(data) + 1;
}

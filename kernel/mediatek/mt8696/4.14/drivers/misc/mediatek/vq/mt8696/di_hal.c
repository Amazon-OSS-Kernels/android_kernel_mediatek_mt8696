...
#define DI_SD_AREA_SIZE (TIMING_SIZE_720_576_H * TIMING_SIZE_720_576_V)
#define DI_HD_AREA_SIZE (TIMING_SIZE_1280_720_H * TIMING_SIZE_1280_720_V)
#define DI_FHD_AREA_SIZE (TIMING_SIZE_1920_1080_H * TIMING_SIZE_1920_1080_V)
...
#define FMT_ACTIVE_720_480_H_END \
	(FMT_ACTIVE_720_480_H_START + (FMT_SIZE_720_480_H - 1))
#define FMT_ACTIVE_720_480_V_START      45	/* resolved */
#define FMT_ACTIVE_720_480_V_END \
	(FMT_ACTIVE_720_480_V_START + (FMT_SIZE_720_480_V - 1))
#define FMT_ACTIVE_720_576_H_START      165	/* resolved */
#define FMT_ACTIVE_720_576_H_END \
	(FMT_ACTIVE_720_576_H_START + (FMT_SIZE_720_576_H - 1))
#define FMT_ACTIVE_720_576_V_START      45	/* resolved */
#define FMT_ACTIVE_720_576_V_END \
	(FMT_ACTIVE_720_576_V_START + (FMT_SIZE_720_576_V - 1))
#define FMT_ACTIVE_1280_720_H_START     185
#define FMT_ACTIVE_1280_720_H_END \
	(FMT_ACTIVE_1280_720_H_START + (FMT_SIZE_1280_720_H - 1))
#define FMT_ACTIVE_1280_720_V_START     24
#define FMT_ACTIVE_1280_720_V_END \
	(FMT_ACTIVE_1280_720_V_START + (FMT_SIZE_1280_720_V - 1))
#define FMT_ACTIVE_1920_1080_H_START    193
#define FMT_ACTIVE_1920_1080_H_END \
	(FMT_ACTIVE_1920_1080_H_START + (FMT_SIZE_1920_1080_H - 1))
#define FMT_ACTIVE_1920_1080_V_START    32
#define FMT_ACTIVE_1920_1080_V_END \
	(FMT_ACTIVE_1920_1080_V_START + (FMT_SIZE_1920_1080_V - 1))

#define MA8F_BUFFER_ALIGN 2048
/* additional 128x6 bits */
#define MA8F_PER_BUF_SIZE (((768 * 576 / 2 / 8 / 2 + 6 * 128 / 8) + \
			    MA8F_BUFFER_ALIGN - 1) & ~(MA8F_BUFFER_ALIGN - 1))
#define MA8F_BUFFER_TOTAL_SIZE (MA8F_PER_BUF_SIZE * 2 * (4 + 1) + \
				(MA8F_BUFFER_ALIGN - 1))
...
#define NR_MESS_FILTER_TYPE_VALUE(level) \
	(NR_MESS_FILTER_TYPE_CNT * level / VQ_NR_LEVEL_MAX)
#define NR_MESS_FILTER_THL_VALUE(level) \
	((NR_MESS_FILTER_THL_CNT / VQ_NR_LEVEL_MAX) * level)
#define NR_BLENDING_LEVEL_CNT                   15
#define NR_BLENDING_LEVEL_VALUE(level) \
	(NR_BLENDING_LEVEL_CNT * level / VQ_NR_LEVEL_MAX)
...
unsigned int di_dbg_level = (1 << DI_LOG_ERROR);
...
static struct VDO_START_LINE_T vdo1_420_startline[] = {
	{
		0x07fb07fa, 0x07fd07fc,
		0x80800000, 0x2060a0e0,
		0x07fa07f9, 0x07fe07fd
	},
	{
		0x07fa07f9, 0x07fd07fc,
		0x00000000, 0x20602060,
		0x07fa07f9, 0x07fd07fc
	},
	{
		0x07fd07fc, 0x07ff07fe,
		0x80800000, 0xa0e02060,
		0x07fc07fb, 0x07fe07fd
	},
	{
		0x07fb07fa, 0x07ff07fe,
		0x80800000, 0xa0e02060,
		0x07fa07f9, 0x07fe07fd
	}
};
...
static struct VDO_START_LINE_T vdo1_422_startline[] = {
	{
		0x07fb07fa, 0x07fb07fa,
		0x80800000, 0x80800000,
		0x07fa07f9, 0x07fa07f9
	},
	{
		0x07fa07f9, 0x07fa07f9,
	...
	{
		0x07fd07fc, 0x07fb07fa,
		0x80800000, 0x00008080,
		0x07fc07fb, 0x07fc07fb
	}
};
#else
/*vdo2 startline*/
...
static int MTK_DI_IonToAddr(unsigned int ion_fd, unsigned int *pu4Addr,
			    unsigned int *pu4Size)
{
	...
	DI_Printf(DI_LOG_FLOW,
		  "[ADDR] ion fd[0x%x] handle[0x%x] address[0x%x] size[%d]",
...
int di_hal_request_irq(struct device_node *np, struct di_info *di)
{
	...
	DI_Printf(DI_LOG_FLOW, " wrch_frame_end_irq value is %d",
		  di_init.wrch_frame_end_irq);
	if (di_init.wrch_frame_end_irq == 0)
		DI_Printf(DI_LOG_FLOW,
			  " get wrch_frame_end_irq from dts fail\n");

	di_init.di_vsync_irq = irq_of_parse_and_map(np, 1);
	DI_Printf(DI_LOG_FLOW, " di_vsync_irq value is %d",
		  di_init.di_vsync_irq);
	...
	DI_Printf(DI_LOG_FLOW, " di_disp_end_irq value is %d",
		  di_init.di_disp_end_irq);
...
int di_hal_request_irq(struct platform_device *pdev, struct di_info *di)
{
	...
	di->wrch_frame_end_irq =
		platform_get_irq(pdev, MTK_VQ_IRQ_WRCH_FRAME_END);
	...
	di->di_under_run_irq = platform_get_irq(pdev, MTK_VQ_IRQ_DI_UNDER_RUN);
	if (di->di_under_run_irq < 0) {
		dev_notice(dev, "[DI] get di_under_run_irq irq err\n");
		return di->di_under_run_irq;
	}
	DI_Printf(DI_LOG_FLOW, "di_under_run_irq value is %d\n",
		  di->di_under_run_irq);
...
static int _di_parse_reg_dev_node(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, VQ_COMPATIBLE_NAME);
	if (np == NULL) {
	...
	di_init.mmsys_config_reg_base =
		(uintptr_t) of_iomap(np, MTK_VQ_REG_MMSYS_CONFIG);
	if (IS_ERR((void *)di_init.mmsys_config_reg_base)) {
		DI_Printf(DI_LOG_ERROR, " mmsys config va addr error 0x%lx\n",
			  di_init.mmsys_config_reg_base);
		return DI_RET_ERR_PARAM;
	}
	DI_Printf(DI_LOG_FLOW, "mmsys config reg base=0x%lx\n",
		  di_init.mmsys_config_reg_base);

	/*vdout sys, 0x14001500 */
	di_init.vdout_sys_reg_base =
				(uintptr_t) of_iomap(np, MTK_VQ_REG_VDOUT);
	if (IS_ERR((void *)di_init.vdout_sys_reg_base)) {
		DI_Printf(DI_LOG_ERROR, "vdout sys va addr error 0x%lx\n",
			  di_init.vdout_sys_reg_base);
		return DI_RET_ERR_PARAM;
	}
	DI_Printf(DI_LOG_FLOW, "vdout sys reg base=0x%lx\n",
		  di_init.vdout_sys_reg_base);

	/*disp top, 15000000 */
	di_init.disp_top_reg_base = (uintptr_t) of_iomap(np,
							 MTK_VQ_REG_DISP_TOP);
	...
	/*dispfmt, 0x1500e000 */
	di_init.dispfmt_reg_base = (uintptr_t) of_iomap(np,
	...
	/*wch, 0x1500f000 */
	di_init.wch_reg_base = (uintptr_t) of_iomap(np, MTK_VQ_REG_WC);
...
{
	struct FMT_TIMING_INFO_T *timing;

	if (prParam->timing_type >= VQ_TIMING_TYPE_MAX) {
		DI_Printf(DI_LOG_ERROR, "%s enable=%d", __func__,
		  prParam->nr_enable);
		return DI_RET_ERR_PARAM;
	}

	timing = &_arFmtTimingInfo[prParam->timing_type];

	DI_Printf(DI_LOG_FLOW, "%s enable=%d", __func__,
		  prParam->nr_enable);
	if (prParam->nr_enable) {
		/* 0x010 */
		...
		/* 0x084 */
#if VQ_WH_TIMING
		DISPFMT_W_VSYNC_END(timing->start_v + prParam->height - 1);
#else
		DISPFMT_W_VSYNC_END(timing->end_v);
#endif
		DISPFMT_W_VSYNC_START(timing->start_v - 1);
		...
		/* 0x088 */
#if VQ_WH_TIMING
		DISPFMT_W_HOR_END(timing->start_h +
				  prParam->width - 1 + 7);
#else
		DISPFMT_W_HOR_END(timing->end_h + 7);
#endif
		DISPFMT_W_HOR_START(timing->start_h + 7);

		/* 0x08C */
#if VQ_WH_TIMING
		DISPFMT_W_VO_END(timing->start_v + prParam->height - 1);
#else
		DISPFMT_W_VO_END(timing->end_v);
#endif
		DISPFMT_W_VO_START(timing->start_v);

		/* 0x090 */
#if VQ_WH_TIMING
		DISPFMT_W_VE_END(timing->start_v + prParam->height - 1);
#else
		DISPFMT_W_VE_END(timing->end_v);
#endif
		DISPFMT_W_VE_START(timing->start_v);
		...
		DI_Printf(DI_LOG_FLOW, "%s nr 0x010 disable", __func__);
...
static int di_dispfmt_config_timingtype(struct DISPFMT_PARAM_T *prParam)
{
	...
	if (prParam->timing_type >= VQ_TIMING_TYPE_MAX) {
		DI_Printf(DI_LOG_ERROR, "di_dispfmt_config_nr enable=%d",
		  prParam->nr_enable);
		return DI_RET_ERR_PARAM;
	}
	...
	/* 0x000 */
	DISPFMT_W_HOR_VALID_STAR(timing->start_h + 7);
#if VQ_WH_TIMING
	DISPFMT_W_HOR_VALID_END(timing->start_h + prParam->width -
				1 + 7);
#else
	DISPFMT_W_HOR_VALID_END(timing->end_h + 7);
#endif

	/* 0x004 */
#if VQ_WH_TIMING
	DISPFMT_W_NR_IN_VSYNC_END(timing->start_v +
				  prParam->height - 1);
#else
	DISPFMT_W_NR_IN_VSYNC_END(timing->end_v);
#endif
	DISPFMT_W_NR_IN_VSYNC_START(timing->start_v - 1);
	...
	/* 0x008 */
	DISPFMT_W_HOR_NR_VALID_STAR(timing->start_h + 7);
#if VQ_WH_TIMING
	DISPFMT_W_HOR_NR_VALID_END(timing->start_h +
				   prParam->width - 1 + 7);
#else
	DISPFMT_W_HOR_NR_VALID_END(timing->end_h + 7);
#endif

	/* 0x00C */
	DISPFMT_W_NR_IN_HOR_STAT(timing->start_h + 7);
#if VQ_WH_TIMING
	DISPFMT_W_NR_IN_HOR_END(timing->start_h + prParam->width -
				1 + 7);
#else
	DISPFMT_W_NR_IN_HOR_END(timing->end_h + 7);
#endif

	/* 0x014 */

	DI_Printf(DI_LOG_FLOW, "prParam->pattern.enable:%d, type:%d,type:%d",
		  prParam->pattern.enable, prParam->pattern.type,
		  prParam->pattern.width);
...
#else
	DISPFMT_W_PXLLEN(timing->size_h);
#endif
	...
	/* 0x0A0 */
#if VQ_WH_TIMING
	DISPFMT_W_HACTEND(timing->start_h + prParam->width - 1);
#else
	DISPFMT_W_HACTEND(timing->end_h);
#endif
	DISPFMT_W_HACTBGN(timing->start_h);

	/* 0x0A4 */
#if VQ_WH_TIMING
	DISPFMT_W_VOACTEND(timing->start_v + prParam->height - 1);
#else
	DISPFMT_W_VOACTEND(timing->end_v);
#endif
	DISPFMT_W_VOACTBGN(timing->start_v);
	DISPFMT_W_HIDE_OST(0);

	/* 0x0A8 */
#if VQ_WH_TIMING
	DISPFMT_W_VEACTEND(timing->start_v + prParam->height - 1);
#else
	DISPFMT_W_VEACTEND(timing->end_v);
#endif
	DISPFMT_W_VEACTBGN(timing->start_v);
	...
	/* 0x0D0 */
	DISPFMT_W_0D0_11_0(timing->total_pixel_v);
	DISPFMT_W_0D0_28_16(timing->total_pixel_h);
	DISPFMT_W_0D0_31_31(1);

	/* 0x0D4 */
	DISPFMT_W_V_TOTAL(timing->total_pixel_v);
	DISPFMT_W_H_TOTAL(timing->total_pixel_h);
	DISPFMT_W_ADJ_T(1);
	...
	DI_Printf(DI_LOG_FLOW, "dispfmt h265, h265_enable[0x%x]",
		  prParam->h265_enable);
...
static int di_dispfmt_trigger(struct DISPFMT_PARAM_T *prParam)
{
	...
	while (1) {
		if (wait_event_interruptible_timeout
		    (di_init.di_event_irq_handle,
		     atomic_read(&di_init.di_event_Irq_flag),
		     HZ / 10) == 0) {

			di_init.di_event_irq_timeout_count++;

			if (di_init.di_event_irq_timeout_count == 1)
				DI_Printf(DI_LOG_ERROR,
				  "timeout St %d Flag %d count %d",
...
{
	bool is_fhd = (prParam->timing_type > VQ_TIMING_TYPE_720P);

	unsigned int align_width = prParam->align_width;
	unsigned int height = prParam->height;

	if (prParam->di_mode != VDO_FRAME_MODE) {
		/* fix ALPS05495790, there is no 8tap support for mt8696 */
		if (prParam->timing_type >= VQ_TIMING_TYPE_480P) {
			/* 1080p no 8 tap */
			/* 0x14c[25:24] y c 8 tap,full hd no 8tap */
	...
	if (prParam->di_mode != VDO_FRAME_MODE) {
		...
		if (prParam->timing_type > VQ_TIMING_TYPE_720P)
			/* 0x478[7]  4tap for full hd 1080 */
			DI_VDO_W_F_CTRL(0x00000000);
		else
	...
	if (prParam->area_size <= DI_SD_AREA_SIZE)
	...
	DI_VDO_W_HD_MEM_1920((is_fhd ? 1 : 0));
	DI_VDO_W_DW_NEED_HD((align_width / 4));	/* check */
#else
	DI_VDO_W_HD_MEM(((timing->size_h > 720) ? 1 : 0));
	DI_VDO_W_HD_MEM_1920(((timing->size_h > 1280) ? 1 : 0));
	DI_VDO_W_DW_NEED_HD((timing->size_h / 4));
#endif
...
static int di_vdo_config_srcsize(unsigned int width, unsigned int height,
				 enum VQ_COLOR_FMT src_fmt)
{
	...
	/* 0x4e0[20] //check */
	DI_VDO_W_HD_LINE_MODE(((height >= 720) ? 1 : 0));
...
static int di_vdo_config_hwsram(enum VDO_DI_MODE di_mode, unsigned int width)
{
	...
	if (width <= 720) {
		...
		/*for keep brace */
		DI_Printf(DI_LOG_ERROR,
			  "[E] vdo sram config fail, w = %d, di = %d", width,
			  di_mode);
	}

	DI_Printf(DI_LOG_FLOW, "[F] vdo sram config to %d, w = %d, di = %d",
		  u4Hw_Sram_Util, width,
		  di_mode);
				    ...
				    enum VQ_COLOR_FMT src_fmt)
{
	...
	enum VDO_YUV420_START_LINE_MODE vdo1_420_startline_mode;
	enum VDO_YUV422_START_LINE_MODE vdo1_422_startline_mode;
	static struct VDO_START_LINE_T *start_line;
	...
	if (src_fmt == VQ_COLOR_FMT_420BLK || src_fmt == VQ_COLOR_FMT_420SCL) {
		if (
			...
			vdo1_420_startline_mode = VDO_YUV420_START_LINE_MODE1;
			DI_Printf(DI_LOG_ERROR,
				  "[E] stl fail src %d w %d di %d vfl %d",
				  src_fmt, width, di_mode, eVFilterMode);
		}

		start_line = &vdo1_420_startline[vdo1_420_startline_mode];
	} else if (src_fmt == VQ_COLOR_FMT_422BLK ||
		src_fmt == VQ_COLOR_FMT_422SCL) {
		...
		start_line = &vdo1_422_startline[vdo1_420_startline_mode];
	}

	startline_y = start_line->startline_y;
	startline_c = start_line->startline_c;
	sub_startline_y = start_line->sub_startline_y;
	sub_startline_c = start_line->sub_startline_c;
	alter_startline_y = start_line->alter_startline_y;
	alter_startline_c = start_line->alter_startline_c;
	...
	DI_Printf(DI_LOG_FLOW,
		  "stl success srcf %d w %d di %d vfl %d 420 %d 422 %d",
		  src_fmt, width, di_mode, eVFilterMode,
		  vdo1_420_startline_mode,
...
static void di_vdo2_config_startline(struct DI_VDO_PARAM_T *prParam)
{
	...
	unsigned int alter_startline_y = 0;
	unsigned int alter_startline_c = 0;
	enum VDO_YUV420_START_LINE_MODE vdo_420_startline_mode;
	enum VDO_YUV422_START_LINE_MODE vdo_422_startline_mode;
	struct VDO_START_LINE_T vdo2_startline;

	if (prParam->src_fmt == VQ_COLOR_FMT_422BLK ||
	    prParam->src_fmt == VQ_COLOR_FMT_422SCL) {
		if (prParam->di_mode == VDO_FRAME_MODE)
...
static int di_vdo_config_ma8f(unsigned char bIsOn, unsigned int AddrW,
			      unsigned int Addr1,
			      unsigned int Addr2)
{
	return DI_RET_OK;
...
static int di_vdo_config_deint_wxyza(unsigned int *pu4AddrY,
				     unsigned int *pu4AddrC)
{
...
static void di_vdo_config_wxyza_buffer(struct DI_VDO_PARAM_T *prParam)
{
	...
	if (prParam->top_field_first_en) {
		...
		if (!prParam->cur_top_enable) {
			...
			DI_Printf(DI_LOG_FLOW,
				  "bottom_first,cur is bottom field");
			addry[0] = prParam->addr_y_next;
			addry[1] = prParam->addr_y_next;
			addry[2] = prParam->addr_y_curr;
			addry[3] = prParam->addr_y_curr;
	...
	for (idx = 0; idx < 5; ++idx)
		DI_Printf(DI_LOG_FLOW,
			  "wxyza[%d]addry:0x%x,addrc:0x%x", idx, addry[idx],
			  addrc[idx]);
...
static int di_vdo_config_threshold(unsigned short u2CtThrehold,
				   unsigned short u2MovingThrehold,
...
static int di_vdo_config_certical_chroma_detect(unsigned char bIsOn,
...
{
	if ((enableBuffer == 1) && (di_init.fusionbuf_va == 0) &&
	    (di_init.fusionbuf_mva == 0)) {

		di_init.fusionbuf_va =
			dma_alloc_coherent(di_init.pm_dev,
					  MA8F_BUFFER_TOTAL_SIZE,
					  &di_init.fusionbuf_mva,
					  GFP_KERNEL);

		DI_Printf(DI_LOG_FLOW,
			  "Fusion alloc pm_dev %p Va 0x%p Mva 0x%x size 0x%x",
			  di_init.pm_dev, di_init.fusionbuf_va,
			  (uint32_t) di_init.fusionbuf_mva,
			  MA8F_BUFFER_TOTAL_SIZE);
		...
		DI_Printf(DI_LOG_FLOW,
			  "[D] FusionBuf free success, Va = 0x%p, Mva= 0x%x",
			  di_init.fusionbuf_va,
			  (uint32_t) di_init.fusionbuf_mva);
		...
		DI_Printf(DI_LOG_FLOW,
			  "FusionBuf Enable %d Va 0x%p Mva 0x%x",
			  enableBuffer,
			  di_init.fusionbuf_va,
			  (uint32_t) di_init.fusionbuf_mva);
	}
...
static int di_vdo_config_fusion_drammode(unsigned char fusion_enable,
					 unsigned int AddrW)
{
	...
	if (fusion_enable) {
		...
		/* 0x874[27:0] */
		DI_VDO_W_da_flag_waddr_hi_limit(
			((AddrW + MA8F_BUFFER_TOTAL_SIZE)) / 16);
...
int di_vdo_config_set_param(struct DI_VDO_PARAM_T *prParam)
{
	...
	di_vdo_config_srcsize(prParam->align_width, prParam->height,
			      prParam->src_fmt);

	di_vdo_config_hwsram(prParam->di_mode, prParam->align_width);

#if DI_VDO_SUPPORT_FUSION
	di_vdo1_config_startline(prParam->di_mode, prParam->align_width,
				 VDO_VERTICAL_FILTER_4TAP,
				 prParam->src_fmt);
#else
...
static void di_wch_config_set_block_param(struct DI_WC_PARAM_T *prParam)
{
	...
	DI_Printf(DI_LOG_FLOW,
		  "width=%d, height= %d timing_type=%d act_hcnt=%d",
		  prParam->width, prParam->height, prParam->timing_type,
		  act_hcnt);
...
static int di_wch_config_set_param(struct DI_WC_PARAM_T *prParam)
{
	...
	if (prParam->dst_fmt == VQ_COLOR_FMT_420BLK) {
		...
		DI_Printf(DI_LOG_ERROR, "[E] not support dst color %d",
			  prParam->dst_fmt);
		return DI_RET_ERR_PARAM;
	}
...
#else
	DI_WC_W_ACTLINE((timing->size_v - 1));	/* 0x00C[11:0] */
#endif
...
#if 1				/* VQ_WH_TIMING */
	if ((prParam->dst_fmt == VQ_COLOR_FMT_420BLK) ||
	    (prParam->dst_fmt == VQ_COLOR_FMT_420SCL)) {
		/*for keep brace */
		/* 0x014[27:16] */
		DI_WC_W_DW_NEED_C_LINE(((prParam->height / 2) - 1));
		...
		/*for keep brace */
		/* 0x014[27:16] */
		DI_WC_W_DW_NEED_C_LINE((prParam->height - 1));
	}

	DI_WC_W_DW_NEED_Y_LINE((prParam->height - 1));	/* 0x014[11:0] */
#else
	DI_WC_W_DW_NEED_C_LINE((timing->size_v - 1));	/* 0x014[27:16] */
	DI_WC_W_DW_NEED_Y_LINE((timing->size_v - 1));	/* 0x014[11:0] */
#endif
...
#else
	DI_WC_W_HACTCNT(timing->size_h);	/* 0x034[28:16] */
#endif
...
#else
	DI_WC_W_CHCNT((timing->size_h / 16 - 1));	/* 0x038[25:16] */
	DI_WC_W_YHCNT((timing->size_h / 16 - 1));	/* 0x038[9:0] */
#endif
...
#else
	DI_WC_W_bghsize_dw((timing->size_h / 16));	/* 0x03C[25:16] */
#endif
...
int di_hal_config(struct mtk_vq_config_info *config_info)
{
	...
	nr_enable = (config->mnr_level || config->bnr_level) ? 1 : 0;
	...
	if ((config->src_align_width <= TIMING_SIZE_720_480_H) &&
		...
		DI_Printf(DI_LOG_ERROR,
			  "[E] not support width = %d, height = %d",
	...
	if (config->src_fmt > VQ_COLOR_FMT_422SCL) {
		DI_Printf(DI_LOG_ERROR, "[E] not support src color format %d",
			  config->src_fmt);
		return DI_RET_ERR_PARAM;
	}

	if (config->dst_fmt > VQ_COLOR_FMT_422SCL) {
		DI_Printf(DI_LOG_ERROR, "[E] not support dst color format %d",
			  config->dst_fmt);
		return DI_RET_ERR_PARAM;
	}

	DI_Printf(DI_LOG_FLOW,
		  "Timing %d(%dx%d)->(%dx%d) (%d %d %d %d) (%d %d) (%d %d)",
		  timing_type,
		  config->src_width,
		  config->src_height,
	...
	if (config->vq_mode == VQ_DI_STANDALONE)
		...
		rDispfmtParam.nr_enable = nr_enable;
	...
	case VQ_DI_MODE_FIELD:
		...
		break;
	default:
		DI_Printf(DI_LOG_ERROR, "[E] map dimode fail, input %d",
			  config->di_mode);
		return DI_RET_ERR_PARAM;
	}
	...
	DI_Printf(DI_LOG_FLOW,
		  "map DiMode %d to vdo mode %d src_mva[0x%x]", config->di_mode,
		  rVdoParam.di_mode, config_info->src_mva[0]);

	rVdoParam.src_fmt = config->src_fmt;
	rVdoParam.cur_top_enable = (config->cur_field == VQ_FIELD_TYPE_TOP);
	...
	if (config_info->src_mva[0]) {
		rVdoParam.addr_y_prev = config_info->src_mva[0] +
					config->src_ofset_y_len[0];
		rVdoParam.addr_cbcr_prev = config_info->src_mva[0] +
					   config->src_ofset_c_len[0];

		rVdoParam.addr_y_curr = config_info->src_mva[1] +
					config->src_ofset_y_len[1];
		rVdoParam.addr_cbcr_curr = config_info->src_mva[1] +
					   config->src_ofset_c_len[1];

		rVdoParam.addr_y_next = config_info->src_mva[2] +
					config->src_ofset_y_len[2];
		rVdoParam.addr_cbcr_next = config_info->src_mva[2] +
					   config->src_ofset_c_len[2];
	}
	rVdoParam.h265_enable = config->h265_enable;
	rVdoParam.area_size = rVdoParam.width * rVdoParam.height;
	/* fix ALPS05495790, timing_type is random value before assigned */
	rVdoParam.timing_type = timing_type;
	...
	rWcParam.output_addr_y = 0;
	rWcParam.output_addr_cbcr = 0;
	if (config_info->dst_mva) {
		rWcParam.output_addr_y = config_info->dst_mva;
		rWcParam.output_addr_cbcr = config_info->dst_mva +
					    config->dst_ofset_c_len;
		di_init.wch_va_y = config_info->dst_va;
		di_init.wch_va_c = config_info->dst_va +
				   config->dst_ofset_c_len;
	}
	di_init.count++;
...
int di_hal_power_on(struct di_info *di)
{
	...
	DI_Printf(DI_LOG_FLOW, "%s", __func__);
	if (di == NULL)
		di = &di_init;
	...
	DI_Printf(DI_LOG_FLOW,
		  "clk_set_parent CLK_TOP_DI_SEL[%d]->CLK_TOP_TVDPLL_D2[%d]",

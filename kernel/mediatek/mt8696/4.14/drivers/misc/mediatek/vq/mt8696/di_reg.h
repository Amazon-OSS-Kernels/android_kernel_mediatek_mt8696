...
#define REG_R(dAddr)		readl((void *)dAddr)
#define REG_W(reg, val)		writel(val, (void *)(reg))
#define REG_R_BITS(reg, bits, shift)          (((REG_R(reg))>>(shift))&(bits))
#define REG_W_BITS(reg, bits, shift, val) \
(REG_W((reg), (((REG_R((reg)))&(~((UINT32)(bits)<<(shift)))) |\
	       (((UINT32)(val)&(UINT32)(bits))<<(shift)))))
#define REG_W_MASK(reg, val, mask) \
(REG_W(reg, (((REG_R(reg)) & (~((unsigned int)(mask)))) |\
				((unsigned int)(val)))))

/* mmsys_config 0x14000000 */
#define DISPSYSCFG_DISP_TOP_DI_REG	(di_init.mmsys_config_reg_base)
...
#define DISPSYSCFG_DISP_PCLK_REG               (di_init.vdout_sys_reg_base)

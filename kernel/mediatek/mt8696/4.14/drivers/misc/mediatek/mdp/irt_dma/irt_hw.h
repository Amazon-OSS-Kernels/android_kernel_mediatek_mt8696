...
#define INT_CLR_IRT_DMA                 (0x1 << 25)
#define DISP_TOP_INT_STATUS             (0x04)
#define INT_STATUS_IRT_DMA              (0x1 << 25)
...
/*[IRT_DMA_TIRG] - 0x00*/
#define IRT_DMA_FLD_RG_5351_MODE_EN(x) \
	(((x) << 24)&(IRT_DMA_MSK_RG_5351_MODE_EN))
#define IRT_DMA_FLD_RG_5351_MODE_SEL(x) \
	(((x) << 20)&(IRT_DMA_MSK_RG_5351_MODE_SEL))
#define IRT_DMA_FLD_RG_SCAN_LINE(x) \
	(((x) << 16)&(IRT_DMA_MSK_RG_SCAN_LINE))
#define IRT_DMA_FLD_RG_DONE_SEL(x) \
	(((x) << 12)&(IRT_DMA_MSK_RG_DONE_SEL))
#define IRT_DMA_FLD_RG_DMA_MODE(x) \
	(((x) << 4)&(IRT_DMA_MSK_RG_DMA_MODE))
#define IRT_DMA_FLD_RG_BLOCK_BURST_EN(x) \
	(((x) << 1)&(IRT_DMA_MSK_RG_BLOCK_BURST_EN))
#define IRT_DMA_FLD_RG_START(x) \
	(((x) << 0)&(IRT_DMA_MSK_RG_START))

/*[IRT_DMA_CTRL] - 0x04*/
#define IRT_DMA_FLD_RG_DRAM_CLK_EN(x) \
	(((x) << 31)&(IRT_DMA_MSK_RG_DRAM_CLK_EN))
#define IRT_DMA_FLD_RG_MON_SEL(x) \
	(((x) << 4)&(IRT_DMA_MSK_RG_MON_SEL))
#define IRT_DMA_FLD_RG_RESET_B(x) \
	(((x) << 0)&(IRT_DMA_MSK_RG_RESET_B))

/*[IRT_DMA_HVSIZE] - 0x08*/
#define IRT_DMA_FLD_RG_VSIZE(x) \
	(((x) << 16)&(IRT_DMA_MSK_RG_VSIZE))
#define IRT_DMA_FLD_RG_HSIZE(x) \
	(((x) << 0)&(IRT_DMA_MSK_RG_HSIZE))

/*[IRT_DMA_10BIT_CTL] - 0x60*/
#define IRT_DMA_FLD_DITHER_MODE_ENABLE(x) \
	(((x) << 7)&(IRT_DMA_MSK_DITHER_MODE_ENABLE))
#define IRT_DMA_FLD_DITHER_MODE_SEL(x) \
	(((x) << 4)&(IRT_DMA_MSK_DITHER_MODE_SEL))
#define IRT_DMA_FLD_RG_WR_GARBAGE_CANCEL(x) \
	(((x) << 3)&(IRT_DMA_MSK_RG_WR_GARBAGE_CANCEL))
#define IRT_DMA_FLD_RG_10BIT_MODE_SEL(x) \
	(((x) << 2)&(IRT_DMA_MSK_RG_10BIT_MODE_SEL))
#define IRT_DMA_FLD_RG_10BIT_ROTATE_EN(x) \
	(((x) << 1)&(IRT_DMA_MSK_RG_10BIT_ROTATE_EN))
#define IRT_DMA_FLD_RG_SOURCE_10BIT_EN(x) \
	(((x) << 0)&(IRT_DMA_MSK_RG_SOURCE_10BIT_EN))

#define IRT_DMA_READ32(addr)                   (readl((void *)addr))
#define IRT_DMA_WRITE32(addr, value) \

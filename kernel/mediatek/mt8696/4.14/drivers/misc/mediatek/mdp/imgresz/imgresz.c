...
static KREE_SESSION_HANDLE imgresz_sharemem_session;
static KREE_SHAREDMEM_HANDLE imgresz_tz_mem_handle[IMGRESZ_MAX_NR];

#ifdef CONFIG_MTK_IMGRZ_TEST
...
static const struct mtk_imgresz_compatible mt8696_compat = {
	.imgresz_curr_chip_ver = IMGRESZ_CURR_CHIP_VER_8696,
};
...
static
struct imgresz_data *imgresz_get_data_2nd(struct imgresz_data *data)
{
	if (imgresz_cur_chip_ver >= IMGRESZ_CURR_CHIP_VER_8695) {
		if (data->scale_data.ufo_h_partition ||
		    data->scale_data.one_phase)
			return imgresz_get_data(TICKET_FROM_HWID(1));
	} else {
		if (data->scale_data.ufo_v_partition)
			return imgresz_get_data(TICKET_FROM_HWID(3));
	}
	return NULL;
}
...
static void imgresz_tz_data_init(struct imgresz_data *data,
					struct imgresz_data *data_2nd)
{
	...
	if (data_2nd != NULL) {
		data_tz_2nd = imgresz_g_tz_data
			(TICKET_FROM_HWID(data_2nd->hw_id));
...
void imgresz_writel_relaxed(u32 value, void __iomem *addr)
{
	if (imgresz_log_level & IMGRESZ_LOG_SIMLOG) {
		unsigned int hwid = 0;

		for (; hwid < IMGRESZ_MAX_NR; hwid++) {
			if (addr >= imgresz_inst_data[hwid]->base &&
			    addr < imgresz_inst_data[hwid]->base + 0x1000) {
				pr_info("[ImgResz]HWrite(%d + 'h%x, 32'h%x);\n",
				hwid, addr - imgresz_inst_data[hwid]->base,
				value);
				break;
			}
		}
...
void imgresz_writel(u32 value, void __iomem *addr)
{
	if (imgresz_log_level & IMGRESZ_LOG_SIMLOG) {
		unsigned int hwid = 0;
...
u32 imgresz_readl_relaxed(void __iomem *addr)
{
	u32 value = readl_relaxed(addr);
	...
	if (imgresz_log_level & IMGRESZ_LOG_SIMLOG) {
		...
		for (; hwid < IMGRESZ_MAX_NR; hwid++) {
			if (addr >= imgresz_inst_data[hwid]->base &&
			    addr < imgresz_inst_data[hwid]->base + 0x1000) {
				pr_info("[ImgResz]HRead(%d + 'h%x, 32'h%x);\n",
				hwid, addr - imgresz_inst_data[hwid]->base,
				value);
				break;
			}
...
int imgresz_clk_on(unsigned int hw_id)
{
	...
	pm_runtime_get_sync(data->smi_larb_dev);
...
void imgresz_clk_off(unsigned int hw_id)
{
	...
	pm_runtime_put_sync(data->smi_larb_dev);
}

IMGRESZ_TICKET
imgresz_ticket_get_8695(enum imgresz_ticket_fun_type type)
{
	...
	for (i = 0; i < imgresz_hw_num; i++) {
		if (BIT(i) & tryhwid) {
			...
			if (data->state == IMGRESZ_STATE_IDLE) {
				/* someone get the resource,busy now */
				...
				if (type == IMGRESZ_FUN_UFO_2HW ||
					type == IMGRESZ_FUN_ONEPHASE_2HW) {
					data_2nd = imgresz_get_data
						(TICKET_FROM_HWID(1));
					if (data_2nd->state ==
						IMGRESZ_STATE_IDLE) {
						data_2nd->state =
							IMGRESZ_STATE_BUSY;
						imgresz_scale_data_init
						(&data_2nd->scale_data);
						data_2nd->scale_data.funtype =
							type;
	...
	if (imgresz_log_level & IMGRESZ_LOG_CMD) {
		...
		pr_notice_ratelimited
			("[Imgresz]HW busy(Req%d), HW0:%u-%u; HW1:%u-%u\n",
...
IMGRESZ_TICKET
imgresz_ticket_get_8697(enum imgresz_ticket_fun_type type)
{
			...
			if (data->state == IMGRESZ_STATE_IDLE) {
				...
				if (hw2ufo) {
					if (data_2nd->state ==
						...
						data_2nd->scale_data.funtype
							= type;
						data->scale_data.ufo_v_partition
							= true;
						data->scale_data.ufo_page0
							= true;
						data_2nd->time_start = 0;
						data_2nd->time_end = 0;
					} else {
	...
	if (imgresz_log_level & IMGRESZ_LOG_CMD) {
		...
		pr_notice_ratelimited
			("[Imgresz]HW busy(Req%d), HW0:%d-%d; HW1:%d-%d\n",
			type, hwstatus[0], fun[0], hwstatus[1], fun[1]);
		pr_notice_ratelimited
			("[Imgresz]HW2:%d-%d,HW3:%d-%d\n",
...
int imgresz_ticket_put_8695(IMGRESZ_TICKET ti)
{
	 ...
	 * change the hw3 status in HW2 spinlock
	 as it only check hw2 lock in ticket_get.
	 */
	if (data->scale_data.ufo_h_partition) {
...
IMGRESZ_TICKET imgresz_ticket_get(enum imgresz_ticket_fun_type type)
{
	...
	case IMGRESZ_CURR_CHIP_VER_8696:
...
int imgresz_set_scale_mode(IMGRESZ_TICKET ti, enum
			imgresz_scale_mode scalemd)
{
	struct imgresz_data *data;
...
static int
imgresz_src_buf_format_init(struct imgresz_src_buf_info *srcbufinfo,
...
static int
imgresz_dst_buf_format_init(struct imgresz_dst_buf_info *dstbufinfo,
...
int
imgresz_set_src_bufinfo(IMGRESZ_TICKET ti,
			struct imgresz_src_buf_info *srcbuf)
{
	struct imgresz_data *data;
	...
	if (srcbuf->buf_info.mem_type == IMGRZ_MEM_PHY ||
		srcbuf->buf_info.mem_type == IMGRZ_MEM_MVA) {
		...
		data->scale_data.src_buf.y_buf_addr =
			srcbuf->buf_info.y_offset;
		data->scale_data.src_buf.cb_buf_addr =
			srcbuf->buf_info.cb_offset;
		data->scale_data.src_buf.cr_buf_addr =
			srcbuf->buf_info.cr_offset;
		data->scale_data.src_buf.ufo_ylen_buf =
			srcbuf->buf_info.ylen_offset;
		data->scale_data.src_buf.ufo_clen_buf =
			srcbuf->buf_info.clen_offset;
	}

	/* UFO2HW must be enabled while src is ufo and src must be >= FHD. */
	#if 0
	if (data->scale_data.funtype == IMGRESZ_FUN_UFO_2HW &&
	    (!imgresz_src_is_ufo(srcbuf->ufo_type) ||
	    srcbuf->pic_width * srcbuf->pic_height < 2073600)) {
		logwarn("UFO2HW only support >= FHD\n");
		return -EINVAL;
	}
	#endif
	return imgresz_src_buf_format_init(srcbuf,
	&data->scale_data.src_format);
}

int
imgresz_set_dst_bufinfo(IMGRESZ_TICKET ti,
			struct imgresz_dst_buf_info *dstbuf)
{
	struct imgresz_data *data;
	...
	if (dstbuf->buf_info.mem_type == IMGRZ_MEM_PHY ||
		dstbuf->buf_info.mem_type == IMGRZ_MEM_MVA) {
		WARN_ON_ONCE(!!(dstbuf->buf_info.y_offset & 0xf));
		WARN_ON_ONCE(!!(dstbuf->buf_info.cb_offset & 0xf));
		data->scale_data.dst_buf.y_buf_addr =
			dstbuf->buf_info.y_offset;
		data->scale_data.dst_buf.c_buf_addr =
			dstbuf->buf_info.cb_offset;
	}
	return imgresz_dst_buf_format_init(dstbuf,
		&data->scale_data.dst_format);
}

int imgresz_set_jpeg_info(IMGRESZ_TICKET ti,
			struct imgresz_jpg_info *jpeginfo)
{
	return 0;
}

int imgresz_set_rm_info(IMGRESZ_TICKET ti,
...
int imgresz_legacy_iommu_enable(IMGRESZ_TICKET ti,
				phys_addr_t pgtablebase,
				bool rd_iommu_en, bool wr_iommu_en)
{
	return 0;
}

int imgresz_set_callback_fun(IMGRESZ_TICKET ti,
		imgresz_callback_fun fun, void *privdata)
{
	struct imgresz_data *data;
...
#define fgsrc_hei_8_is_even(hei)	\
	(!((hei) & 1) && (!!((hei) & 0x8)) == (!!((hei) & 0x7)))
...
static int imgresz_ufo_v_partition(struct imgresz_data *data)
{
	...
	osd_mode = (dst_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_ARGB ||
		dst_format->mainformat ==
		IMGRESZ_BUF_MAIN_FORMAT_AYUV) ? true : false;
	...
	while (!fgsrc_hei_8_is_even(part0_src_end) || (part0_dst_end & 0x1)) {
		...
		part0_dst_end = (xfactor == 0x40000)
			? part0_src_end : (part0_src_end + 1);
	...
	do {
		...
		part1_dst_bgn = (xfactor == 0x40000)
			? part1_src_bgn : (part1_src_bgn + 1);
		part1_dst_bgn = (part1_dst_bgn << 18) / xfactor;

		/* whether the linebuf is 0. this is not allowed. */
		linebuflenok = imgresz_hal_survey_linebuflen_is_ok
				(true, false, dst_blk,
				src_buf->pic_width, dst_buf->pic_width,	16);

		/* If linebuf is not OK, we have to adjust the part1_src_bgn. */
		if (!linebuflenok)
			linebuflenok = fgsrc_hei_8_is_even
			(src_buf->pic_height - part1_src_bgn);

		/* C len offset, should 16 bytes align. */
		c_lenoffsetok = !((src_buf->buf_width
		* part1_src_bgn / 4 / 64 / 2) & 0xf);

		/*
		 * loginfo("try part1bgn %d part1ds
		 t %d linebufok %d c_lenoffsetok %d\n",
				part1_src_bgn, part1_dst_bgn,
	...
	if (part0_src_end >= src_buf->pic_height
	    ...
	    part0_dst_end >= dst_buf->pic_height
	    || part1_dst_bgn >= dst_buf->pic_height) {
		logwarn(IMGRESZ_LOG_WARNING,
			"2part fail-hei, src:0x%zx(0x%x-0x%x),dst:0x%zx(0x%x-0x%x)\n",
	...
	if (part0_src_end < part1_src_bgn || part0_dst_end < part1_dst_bgn) {
		loginfo("2part fail, src:(0x%x-0x%x),dst:(0x%x-0x%x)\n",
			part0_src_end, part1_src_bgn,
			part0_dst_end, part1_dst_bgn);
		return -EPERM;
	}
	...
	if (scaledata->ufo_page0) { /* the top part. */
		loginfo(IMGRESZ_LOG_UFO,
			...
			part1_src_bgn, src_buf->pic_height,
			part1_dst_bgn, dst_buf->pic_height,
			(((src_buf->pic_height + 1) << 18)
			/ xfactor == dst_buf->pic_height));
		...
		imgresz_ufo_vdo_v_partition_offset_update(base,
						src_buf->buf_width,
							  part1_src_bgn);
		imgresz_ufo_pagesz(base, src_buf->pic_width,
				   src_buf->pic_height - part1_src_bgn,
...
static int imgresz_ufo_h_partition_2hw(struct imgresz_data *data)
{
	...
	bool osd_mode = (dst_format->mainformat ==
	...
	if ((src_buf->pic_height == dst_buf->pic_height)
		...
		do {
			...
			if (!fgLinebufok) {
				loginfo(IMGRESZ_LOG_UFO,
					"fgLinebufok:%d,tg_x1:%d x0_end:%d,u4Count:%d\n",
	...
	if (scaledata->ufo_page0) {
		imgresz_hal_set_src_pic_wid_hei(base, x0_end,
			src_buf->pic_height, src_format);
		imgresz_hal_set_dst_pic_wid_hei(base, tg_x1,
			dst_buf->pic_height);
		...
		if ((src_buf->pic_width - x1) >= 2 *
			(src_buf->pic_width / 2 - x1_c))
		...
		loginfo(IMGRESZ_LOG_UFO,
			...
			x0_end, x1, x1_c, src_buf->pic_width,
			cur_src_wid, tg_x1, dst_buf->pic_width);
		...
		imgresz_ufo_pagesz(base, cur_src_wid, src_buf->pic_height,
			dst_buf->pic_width - tg_x1);
	}

	if ((src_buf->pic_height == dst_buf->pic_height) &&
		(src_buf->pic_width == src_buf->pic_width))
		imgresz_hal_coeff_h8tap_vdo_partition_offset(base, 0, 0, 0);
...
static int _imgresz_hal_if_do_scale(struct imgresz_data *data)
{
	...
	imgresz_hal_set_resz_mode(base, scaledata->scale_mode,
					src_format->mainformat);

	/* 2. Set Resample Method to HW */
	imgresz_hal_set_resample_method(base, scaledata->h_method,
					scaledata->v_method);
	...
	if (imgresz_src_is_ufo(src_buf->ufo_type)) {
		...
		imgresz_ufo_picsz(base,
			src_buf->buf_width, src_buf->buf_height);
		imgresz_ufo_src_buf(base, data->base_sec,
			src_buf->y_buf_addr, src_buf->cb_buf_addr);
		...
		if (src_buf->ufo_type != IMGRESZ_UFO_10BIT_COMPACT_UNCOMPRESS)
			imgresz_ufo_srclen_buf(base, data->base_sec,
			src_buf->ufo_ylen_buf, src_buf->ufo_clen_buf);
		imgresz_ufo_partition_set_start_point(base,
			src_buf->pic_x_offset, src_buf->pic_y_offset);
	}
	...
	if (scaledata->scale_mode == IMGRESZ_FRAME_SCALE) {
		...
		if (!imgresz_src_is_ufo(src_buf->ufo_type))
			imgresz_hal_set_src_buf_addr(base, data->base_sec,
			src_buf->y_buf_addr, src_buf->cb_buf_addr,
			src_buf->cr_buf_addr);

		if (rm_info->racing_mode)
			src_buf->buf_height = 0x20;
		if (src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR)
	...
	imgresz_hal_set_dst_buf_addr(base, data->base_sec, dst_buf->y_buf_addr,
				     dst_buf->c_buf_addr);
	imgresz_hal_set_dst_buf_pitch(base,
		dst_format->bit10 ? dst_buf->buf_width*4/5
		: dst_buf->buf_width);
	imgresz_hal_set_dst_pic_wid_hei(base, partition->dst_w,
		dst_buf->pic_height);
	imgresz_hal_set_dst_pic_offset(base,
		dst_buf->pic_x_offset + partition->dst_x_offset,
	...
	if (rm_info->rpr_mode) {
		...
		imgresz_hal_coeff_set_h_factor(base, src_buf,
			dst_buf, src_format,
				scaledata->h_method, hal_info);
		imgresz_hal_coeff_set_v_factor(base, src_buf,
			dst_buf, src_format,
	...
	else if (scaledata->ufo_h_partition &&
	imgresz_src_is_ufo(src_buf->ufo_type))
	...
	if (scaledata->scale_mode == IMGRESZ_PARTIAL_SCALE || rm_info->rpr_mode)
		imgresz_hal_set_tempbuf(base, data->base_sec,
		scaledata->partial_info.temp_buf_addr);
	...
	if (rm_info->racing_mode) {
		...
		imgresz_hal_jpg_component(base, jpg->y_exist,
			jpg->cb_exist, jpg->cr_exist);

		if (jpg_picmode ||
			src_format->mainformat ==
			IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR)
			imgresz_hal_jpg_component_ex
			(base, jpg->y_exist,
			jpg->cb_exist, jpg->cr_exist, false);
	...
	if (jpg->preload) {
		 ...
		 */
		imgresz_hal_jpg_preload_buf(base, data->base_sec,
				dst_buf->y_buf_addr, dst_buf->c_buf_addr);
	}
	...
	if (scaledata->one_phase) {
		base_2nd = imgresz_get_data_2nd(data)->base;
		if (dst_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_ARGB)
			imgresz_hal_set_argb_one_phase(base, base_2nd);
	}
	...
	if (imgresz_log_level & IMGRESZ_LOG_SHOWREG) {
		...
		if (scaledata->one_phase && base_2nd)
			imgresz_hal_print_reg(base_2nd);
	}

	/* 9. Trigger HW */
	imgresz_hal_trigger_hw(base, base_2nd,
		imgresz_src_is_ufo(src_buf->ufo_type));
...
static void
imgresz_decide_resample_method
...
static int imgresz_cal_h_factor(struct imgresz_scale_data *scaledata)
{
	...
	if ((resample_method == IMGRESZ_RESAMPLE_METHOD_8_TAP) ||
		...
		factor = (2048 * dst_width_y +
			(src_width_y >> 1)) / src_width_y;
	...
	if ((resample_method == IMGRESZ_RESAMPLE_METHOD_8_TAP) ||
		...
		factor = (2048 * dst_width_cb +
			(src_width_cb >> 1)) / src_width_cb;
	...
	if ((resample_method == IMGRESZ_RESAMPLE_METHOD_8_TAP) ||
		...
		factor = (2048 * dst_width_cr +
			(src_width_cr >> 1)) / src_width_cr;
...
static int imgresz_cal_v_factor(struct imgresz_scale_data *scaledata)
{
	...
	case IMGRESZ_RESAMPLE_METHOD_4_TAP:
		/* Y */
		hal_info->v4_factor_y = (0x40000 * src_height_y
		+ (dst_height_y >> 1)) /
				dst_height_y;
		...
		if ((src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_ARGB) ||
			...
			break;
		/* Cb */
		hal_info->v4_factor_cb = (0x40000 * src_height_cb
		+ (dst_height_cb >> 1)) /
				dst_height_cb;
		...
		if (src_format->mainformat == IMGRESZ_BUF_MAIN_FORMAT_Y_C)
			break;
		/* Cr */
		hal_info->v4_factor_cr = (0x40000 * src_height_cr
		+ (dst_height_cr >> 1)) /
				dst_height_cr;
...
static int
imgresz_ufo_partition_init(struct imgresz_scale_data *scaledata)
{
	struct imgresz_buf_format *src_format = &scaledata->src_format;
	struct imgresz_partition_info *partition = &scaledata->partition;
	...
	case IMGRESZ_BUF_MAIN_FORMAT_Y_CB_CR:
		if ((src_format->h_sample[1] != 0) &&
			...
			partition->src_h_c = partition->src_h_y
				* src_format->v_sample[1]
				/ src_format->v_sample[0];
			/*
			 *For jpeg picture mode, prevent
			 *source height 401 come
			 *two interrupt (Y interrupt and C interrupt)
			 */
		...
		if ((src_format->h_sample[2] != 0) &&
			...
			partition->src_w_cr = partition->src_w_y
				* src_format->h_sample[2] /
				src_format->h_sample[0];
			partition->src_h_cr = partition->src_h_y
...
static int
imgresz_cal_ufo_h_partition_2hw
		(struct imgresz_scale_data *scaledata,
		struct imgresz_scale_data *scaledata_2nd)
{
	...
	loginfo(IMGRESZ_LOG_UFO,
		"p0: 0~%d->0~%d; p1:%d(c:%d)+%d(y:%d,c:%d)->%d+%d\n",
...
static void
imgresz_prepare_param(struct imgresz_data *data,
			struct imgresz_data *data_2nd)
{
	struct imgresz_scale_data *scaledata = &data->scale_data;
...
static bool imgresz_ticket_is_done(struct imgresz_data *data)
{
	...
	if (data->scale_data.ufo_v_partition || data->scale_data.ufo_h_partition
		...
		data_2nd = imgresz_get_data(TICKET_FROM_HWID(1));
			...
			data_2nd->state == IMGRESZ_STATE_DONE);
	} else {
		done = (data->state == IMGRESZ_STATE_DONE);
	}
	spin_unlock_irqrestore(state_lock, flags);
...
static
irqreturn_t __attribute__((unused)) imgresz_irq_handle
			(int irq, void *dev_id)
{
...
int imgresz_trigger_scale_async(IMGRESZ_TICKET ti, bool async)
{
	...
	#endif
	struct imgresz_data *data = NULL, *data_2nd = NULL;
	struct imgresz_scale_data *scaledata = NULL;
	int ret = 0;
	...
	data_2nd = imgresz_get_data_2nd(data);

	loginfo(IMGRESZ_LOG_CMD,
		"HW %d Md:%d src Md%d Res(%u-%u-%u) a(%pad-%pad)\n",
		data->hw_id, scaledata->scale_mode,
		scaledata->src_buf.src_mode,
		scaledata->src_buf.buf_width, scaledata->src_buf.pic_width,
		scaledata->src_buf.pic_height,
		&scaledata->src_buf.y_buf_addr,
		&scaledata->src_buf.cb_buf_addr);
	loginfo(IMGRESZ_LOG_CMD,
		"ufo-%d(part-%d,%d) op:%d;DstM %d Res(%u-%u-%u) a(%pad-%pad)\n",
		scaledata->src_buf.ufo_type, scaledata->ufo_h_partition,
		scaledata->ufo_v_partition,
		scaledata->one_phase, scaledata->dst_buf.dst_mode,
		scaledata->dst_buf.buf_width, scaledata->dst_buf.pic_width,
		scaledata->dst_buf.pic_height,
	 ...
	 *msecs_to_jiffies(IMGRZ_HW_TIMEOUT));
	 */
	...
	if (scaledata->src_buf.buf_info.mem_type == IMGRZ_MEM_SECURE) {
#if 1
		uint32_t size = sizeof(struct imgresz_tz_data);
		...
		param_type = TZ_ParamTypes1(TZPT_MEMREF_INOUT);
		param[0].memref.handle =
			imgresz_tz_mem_handle[data->hw_id];
		...
		if (scaledata->ufo_v_partition || scaledata->ufo_h_partition
			...
			param[1].memref.handle =
				imgresz_tz_mem_handle[data_2nd->hw_id];
			param[1].memref.offset = 0;
			param[1].memref.size = size;

			param_type = TZ_ParamTypes2(TZPT_MEMREF_INOUT,
					TZPT_MEMREF_INOUT);
		}
		...
		if (scaledata->ufo_v_partition || scaledata->ufo_h_partition
			|| scaledata->one_phase)
			devm_free_irq(&data_2nd->pdev->dev,
			data_2nd->irq, (void *)data_2nd);
		ret = KREE_TeeServiceCall(imgresz_ta_session,
			IMGRESZ_TZ_CMD_TRIGGER, param_type, param);
		/* request normal world irq */
		if (devm_request_irq(&data->pdev->dev, data->irq,
			imgresz_irq_handle, 0,
				dev_name(&data->pdev->dev), (void *)data)) {
			dev_notice(&data->pdev->dev,
				"Failed @ IRQ-%d Request\n", data->irq);
			return -ENODEV;
		}
		if (scaledata->ufo_v_partition || scaledata->ufo_h_partition
			|| scaledata->one_phase)
			if (devm_request_irq(&data_2nd->pdev->dev,
				data_2nd->irq, imgresz_irq_handle, 0,
					dev_name(&data_2nd->pdev->dev),
					(void *)data_2nd)) {
				dev_notice(&data_2nd->pdev->dev,
					"Failed @ IRQ-%d Request\n",
					data_2nd->irq);
				return -ENODEV;
			}
		...
		if (scaledata->ufo_v_partition || scaledata->ufo_h_partition
			...
			devm_free_irq(&data_2nd->pdev->dev,
			data_2nd->irq, (void *)data_2nd);
		param_type = TZ_ParamTypes3(TZPT_MEM_INPUT,
			TZPT_MEM_INPUT, TZPT_VALUE_OUTPUT);
		ret = KREE_TeeServiceCall(imgresz_ta_session,
			IMGRESZ_TZ_CMD_TRIGGER, param_type, param);
		...
		/* only use hw0's state lock*/
		state_lock = &(imgresz_get_data
		(TICKET_FROM_HWID(0))->state_lock);
		spin_lock_irqsave(state_lock, flags);
		data->state = data_tz->state;
		if (scaledata->ufo_v_partition || scaledata->ufo_h_partition)
			data_2nd->state = data_tz_2nd->state;
		spin_unlock_irqrestore(state_lock, flags);
		if (data->state == IMGRESZ_STATE_DONE) {
			if (data->cb_info.fun_cb)
				data->cb_info.fun_cb
				(TICKET_FROM_HWID(data->hw_id),
	...
	if (!async) {
		if (wait_event_timeout(data->wait_hw_done_queue,
			imgresz_ticket_is_done(data),	data->hw_timeout))
			goto DONE;
		goto TIMEOUT;
	}
...
int
imgresz_agent_trustzone_enable(IMGRESZ_TICKET ti, bool svp_enable)
{
	return 0;
}
...
static const struct of_device_id mtk_imgresz_of_match[] = {
	...
	{.compatible = "mediatek,mt8696-imgresz", .data = &mt8696_compat},
	{} /* NULL */
};
...
static int imgresz_probe(struct platform_device *pdev)
{
	...
	data->base_sec = devm_ioremap_resource(dev, res);
	if (IS_ERR(data->base_sec))
		return PTR_ERR(data->base_sec);
	...
	imgresz_hal_set_chip_ver
		(((struct mtk_imgresz_compatible *)
		(of_id->data))->imgresz_curr_chip_ver);

	ret = of_property_read_u32(dev->of_node,
		"mediatek,imgreszid", &data->hw_id);
	if (ret) {
	...
	if ((!larb_pdev) || (!larb_pdev->dev.driver)) {
		logwarn("imgresz is earlier than SMI\n");
		return -EPROBE_DEFER;
	}
...
#if IMGRZ_TEE_ENABLE
	imgresz_tz_inst_data[data->hw_id] =
	devm_kzalloc(dev, sizeof(struct imgresz_tz_data), GFP_KERNEL);
	if (!data)
		return -ENOMEM;
#endif
...
#ifdef CONFIG_MTK_IMGRZ_TEST
	if (data->hw_id == 0) {
#ifdef CONFIG_MTK_IMGRZ_TEST_PA
		...
		imgrz_test_buffer_va =
			(unsigned long)ioremap_wc((phys_addr_t)
					imgrz_test_buffer_pa,
					imgrz_test_buffer_size);
#else
		imgrz_test_buffer_size = 128*1024*1024;
		imgrz_test_buffer_va = (unsigned long)dma_alloc_coherent
			(&pdev->dev, imgrz_test_buffer_size,
			(dma_addr_t *)&imgrz_test_buffer_pa, GFP_KERNEL);
#endif
		logwarn("test region va:0x%lx, pa:0x%pad, size:%u\n",
			imgrz_test_buffer_va, &imgrz_test_buffer_pa,
			imgrz_test_buffer_size);
		imgresz_debug_init(dev);
	}
#endif
...
#if IMGRZ_TEE_ENABLE
static enum imgresz_status imgresz_sec_create_session(void)
{
	enum imgresz_status status = IMGRESZ_STATUS_OK;
	int ret;

	if (imgresz_ta_session == 0) {
		ret = KREE_CreateSession(TZ_TA_IMGRZ_UUID,
			&imgresz_ta_session);
		if (ret != TZ_RESULT_SUCCESS) {
			logwarn("create tz_session fail:%d\n", ret);
			status = IMGRESZ_STATUS_ERROR;
		}
	}

	if (status != IMGRESZ_STATUS_OK)
		logwarn("create session fail:%d\n", status);
	else
		logwarn("create session imgresz_ta_session[0x%X]\n",
		imgresz_ta_session);

	return status;
}

enum imgresz_status imgresz_sec_create_share_mem_session(void)
{
	enum imgresz_status status = IMGRESZ_STATUS_OK;
	int ret;

	if (imgresz_sharemem_session == 0) {
		ret = KREE_CreateSession(TZ_TA_MEM_UUID,
			&imgresz_sharemem_session);
		if (ret != TZ_RESULT_SUCCESS) {
			logwarn("createtz_mem_session fail:%d\n", ret);
			status = IMGRESZ_STATUS_ERROR;
		}
	}
	...
	if (status != IMGRESZ_STATUS_OK)
		...
		logwarn("create session imgrez_tz_mem_session[0x%X]\n",
		imgresz_sharemem_session);

	return status;
}

enum imgresz_status imgresz_sec_create_share_mem_handle(void *mem,
	KREE_SHAREDMEM_HANDLE *handle)
{
	enum imgresz_status status = IMGRESZ_STATUS_OK;
	struct KREE_SHAREDMEM_PARAM imgresz_param;
	int ret;

	imgresz_param.buffer = mem;
	imgresz_param.size = sizeof(struct imgresz_tz_data);

	if (*handle == 0) {
		ret = KREE_RegisterSharedmem(imgresz_sharemem_session,
			handle, &imgresz_param);
		if (ret != TZ_RESULT_SUCCESS) {
			status = IMGRESZ_STATUS_ERROR;
		}
	}
	if (status != IMGRESZ_STATUS_OK)
		logwarn("create imgresz_tz_mem_handle fail\n");
	else {
		logwarn(
			"create imgresz_tz_mem_handle,pointer 0x%p\n", mem);
	}

	return status;
}

static int __init mtk_imgresz_sec_init(void)
{
	int hw_id = 0, ret = 0;
	enum imgresz_status status = IMGRESZ_STATUS_OK;

	status = imgresz_sec_create_session();
	if (status != IMGRESZ_STATUS_OK)
		return -EINVAL;
	status = imgresz_sec_create_share_mem_session();
	if (status != IMGRESZ_STATUS_OK)
		return -EINVAL;
	for (hw_id = 0; hw_id < IMGRESZ_MAX_NR; hw_id++) {
		status = imgresz_sec_create_share_mem_handle(
			(void *)imgresz_tz_inst_data[hw_id],
			&imgresz_tz_mem_handle[hw_id]);
		if (status != IMGRESZ_STATUS_OK)
			return -EINVAL;
	}
#ifdef CONFIG_MTK_IMGRZ_TEST
	...
	ret = KREE_AllocSecurechunkmem(imgresz_mem_session,
		&imgrz_test_sec_src_mem, 1024, 3840*2176*4);
	if (ret != TZ_RESULT_SUCCESS) {
		logwarn("KREE_AllocSecurechunkmem src,ret = %x\n", ret);
		return ret;
	}
	ret = KREE_AllocSecurechunkmem(imgresz_mem_session,
		&imgrz_test_sec_dst_mem, 1024, 3840*2176*4);
	if (ret != TZ_RESULT_SUCCESS) {
...
late_initcall_sync(mtk_imgresz_sec_init);
#endif

module_param_named(imgresz_log_level,
imgresz_log_level, int, 0644);

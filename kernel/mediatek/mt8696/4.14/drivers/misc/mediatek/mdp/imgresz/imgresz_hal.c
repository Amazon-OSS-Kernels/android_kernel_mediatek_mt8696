...
void imgresz_hal_print_reg(void __iomem *base)
{
	...
	/* UFO need more reg. */
	if (imgresz_readl_relaxed(base + RW_IMG_RESZ_UFO_POWER) &
		IMG_RESZ_UFO_ON)
		dump_sz += 13 * 4;
	...
	for (temp = 0; temp < dump_sz; temp += 4) {
		pr_info("[%p]0x%8x   0x%8x   0x%8x   0x%8x\n",
			base + temp * 4,
			imgresz_readl_relaxed(base + temp * 4),
			imgresz_readl_relaxed(base + temp * 4 + 0x4),
			imgresz_readl_relaxed(base + temp * 4 + 0x8),
			imgresz_readl_relaxed(base + temp * 4 + 0xc));
	}
}
...
void imgresz_ufo_trigger(void __iomem *base)
{
	...
	while (imgresz_readl_relaxed(base +
		RO_IMG_RESZ_STATUS_MONITOR_REG) & IMG_RESZ_RD_REQ)
		break;

	imgresz_writel(IMG_RESZ_UFO_TRIG, base + RW_IMG_RESZ_UFO_TRIG);
}
#endif
 ...
 *when imgrz done but ufo not done issue
 */

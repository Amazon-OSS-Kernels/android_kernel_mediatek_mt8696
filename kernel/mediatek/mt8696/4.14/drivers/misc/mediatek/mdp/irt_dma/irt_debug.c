...
bool irtAllocSecureBuffer(KREE_SESSION_HANDLE session,
	KREE_SECUREMEM_HANDLE *handle, int size)
{
	TZ_RESULT ret = TZ_RESULT_SUCCESS;

	ret = KREE_AllocSecurechunkmemWithTag(
		session, handle, 512, size, "irt");
...
void irtStartTest(void)
{
...
#endif
	{
		...
		if (src_fd != -1) {
			...
			if (dst_fd == -1) {
				pr_info("[IRT]dst fd -1\n");
				return;
			}
		} else {
			pr_info("[IRT]src fd -1\n");
			return;
		}
	}

...
void mdp_temp_buffer_init(void)
{
	...
	g_mdp_temp_buffer_info.client =
		ion_client_create(g_ion_device, "mdp");
}
...
bool mdp_temp_buffer_find_free_buffer(int32_t *p_index)
{
	...
	for (i = 0; i < MAX_TEMP_BUFFER_NODE; i++) {
		...
		if (g_mdp_temp_buffer_info.buffer_pool[i].state ==
			MDP_TEMP_BUFFER_STATE_FREE) {
			mutex_unlock(
				&g_mdp_temp_buffer_info.buffer_pool[i].lock);
			break;
		}
	...
	if (i == MAX_TEMP_BUFFER_NODE)
		...
		*p_index = i;
		mutex_lock(&g_mdp_temp_buffer_info.buffer_pool[i].lock);
		g_mdp_temp_buffer_info.buffer_pool[i].state =
			MDP_TEMP_BUFFER_STATE_USED_BY_IMGRESZ;
		mutex_unlock(&g_mdp_temp_buffer_info.buffer_pool[i].lock);
	}
...
enum MDP_TASK_STATUS mdp_temp_buffer_allocate_buffer(void)
{
	...
	for (i = 0; i < MAX_TEMP_BUFFER_NODE; i++) {
		...
		if (g_mdp_temp_buffer_info.buffer_pool[i].state ==
			MDP_TEMP_BUFFER_STATE_RELEASED) {
			mutex_unlock(
				&g_mdp_temp_buffer_info.buffer_pool[i].lock);
			need_allocate = true;
			break;
	...
	for (i = 0; i < MAX_TEMP_BUFFER_NODE; i++) {
		...
		g_mdp_temp_buffer_info.buffer_pool[i].handle =
			ion_alloc(g_mdp_temp_buffer_info.client,
				TEMP_BUFFER_SIZE, 0,
				ION_HEAP_TYPE_MULTIMEDIA, 0);
		...
		if (buffer_len != TEMP_BUFFER_SIZE) {
			MDP_ERR("allocated buffer size fail 0x%x %zu\n",
	...
	struct mdp_temp_buffer_struct **ppBuffer)
{
	...
	*ppBuffer = NULL;
	...
	add_wait_queue(
		&g_mdp_temp_buffer_info.mdp_temp_buffer_wait_buffer_free_queue,
		&wait);
	...
	struct mdp_temp_buffer_struct *pBuffer)
{
	...
	if (pBuffer == NULL)
		return MDP_TASK_STATUS_OK;
	do {
		buffer_index = (pBuffer -
			&g_mdp_temp_buffer_info.buffer_pool[0]);
		if (buffer_index >= MAX_TEMP_BUFFER_NODE || buffer_index < 0) {
			MDP_ERR("wrong temp_buffer:%p, buffer_pool:%p\n",
				pBuffer,
				&g_mdp_temp_buffer_info.buffer_pool[0]);
		...
		mutex_lock(
			&g_mdp_temp_buffer_info.buffer_pool[buffer_index].lock);
	...
	struct mdp_ion_struct *pIonHandle)
{
	...
	handle = ion_import_dma_buf_fd(g_mdp_temp_buffer_info.client, fd);
	...
	if (ion_kernel_ioctl(g_mdp_temp_buffer_info.client,
	...
	if (!isOneChannel) {
		/* 2 channel */
		#if 1
		handle2 = ion_import_dma_buf_fd(g_mdp_temp_buffer_info.client,
	...
	if (pBuffer->buffer_info.memory_type != DP_MEMORY_SECURE) {
		...
		if (isOneChannel) {
			pBuffer->y_buffer_address =
				mva + pBuffer->buffer_info.y_offset;
			if (pBuffer->buffer_info.cb_offset)
				pBuffer->u_buffer_address =
					mva + pBuffer->buffer_info.cb_offset;
			if (pBuffer->buffer_info.cr_offset)
				pBuffer->v_buffer_address =
					mva + pBuffer->buffer_info.cr_offset;
			if (pBuffer->buffer_info.ylen_offset)
				pBuffer->ufo_ylen_buffer_address =
					mva + pBuffer->buffer_info.ylen_offset;
			if (pBuffer->buffer_info.clen_offset)
				pBuffer->ufo_clen_buffer_address =
					mva + pBuffer->buffer_info.clen_offset;
		} else {
			pBuffer->y_buffer_address =
				mva + pBuffer->buffer_info.y_offset;
			pBuffer->u_buffer_address =
				mva2 + pBuffer->buffer_info.cb_offset;
			if (pBuffer->buffer_info.cr_offset)
				pBuffer->v_buffer_address =
					mva2 + pBuffer->buffer_info.cr_offset;
			if (pBuffer->buffer_info.ylen_offset)
				pBuffer->ufo_ylen_buffer_address =
					mva + pBuffer->buffer_info.ylen_offset;
			if (pBuffer->buffer_info.clen_offset)
				pBuffer->ufo_clen_buffer_address =
					mva2 + pBuffer->buffer_info.clen_offset;
		}
	} else {
		/* secure buffer */
	...
	struct mdp_ion_struct *pIonHandle)
{
	...
	handle = ion_import_dma_buf_fd(g_mdp_temp_buffer_info.client,
		(pBuffer->buffer_info.fd & 0x0000FFFF));
	if (IS_ERR_OR_NULL(handle)) {
		MDP_ERR("import ion handle failed:fd1[0x%x] client:%p\n",
			pBuffer->buffer_info.fd, g_mdp_temp_buffer_info.client);
		return MDP_TASK_STATUS_IMPORT_ION_FD_FAIL;
	...
	if (!isOneChannel) {
		handle2 = ion_import_dma_buf_fd(g_mdp_temp_buffer_info.client,
			(pBuffer->buffer_info.fd & 0xFFFF0000) >> 16);
		if (IS_ERR_OR_NULL(handle2)) {
			MDP_ERR("import ion failed:fd2[0x%x] client:%p\n",
				pBuffer->buffer_info.fd,
				g_mdp_temp_buffer_info.client);
			return MDP_TASK_STATUS_IMPORT_ION_FD_FAIL;
		}

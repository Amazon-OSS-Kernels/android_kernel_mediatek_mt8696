...
static struct ion_handle *irt_ion_import_handle(
...
static size_t irt_ion_phys_mmu_addr(struct ion_client *client,
...
static void irt_ion_free_handle(struct ion_client *client,
	struct ion_handle *handle)
{
	if (!client) {
		IRT_ERR("invalid ion client!\n");
	...
	struct irt_dma_config_param *config)
{
	...
	data->src_ion_handle = irt_ion_import_handle(
		data->client, config->src_fd);
	...
	data->dst_ion_handle = irt_ion_import_handle(
		data->client, config->dst_fd);
...
static int irt_trigger_sync_secure(struct irt_dma_config_param *irt_config)
{
	...
	ret = KREE_TeeServiceCall(data->irt_session,
		MDP_SERVICE_CALL_CMD_IRT_CONFIG, paramTypes, irt_param);
	if (ret != TZ_RESULT_SUCCESS)

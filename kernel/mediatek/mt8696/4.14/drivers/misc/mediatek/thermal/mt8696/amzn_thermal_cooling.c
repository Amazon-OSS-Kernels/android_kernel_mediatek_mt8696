...
static struct cpufreq_policy *amzn_cpu_policy;
...
static int amzn_register_cool_dev(struct amzn_cool_dev *cool)
{
	...
	//		return 0;
		amzn_cpu_policy = cpufreq_cpu_get(0);
		if (!amzn_cpu_policy) {
		...
		if (IS_ERR(cool->cooling_dev)) {
			int ret = PTR_ERR(cool->cooling_dev);
			pr_err("%s: Failed to register cpu cooling device %d\n", __func__, ret);
			cpufreq_cpu_put(amzn_cpu_policy);

...
#define THERMAL_IPI_LOG_TAG		"[Thermal/IPI]"
#define thermal_ipi_printk(fmt, args...)\
	pr_info(THERMAL_IPI_LOG_TAG fmt, ##args)
...
static int register_thermal_ipi(void)
{
	...
	ret = mtk_ipi_register(&mcupm_ipidev, CH_S_PLATFORM, NULL, NULL,
		(void *)&ack_data);
	if (ret != 0) {
		thermal_ipi_printk("%s error ret:%d\n", __func__, ret);
		return -1;
	}
#endif
	...
	unsigned int cmd, struct thermal_ipi_data *thermal_data)
{
	...
	case THERMAL_IPI_INIT_GRP6:
		...
		if (ret != 0)
			thermal_ipi_printk("send init cmd(%d) error ret:%d\n",
				cmd, ret);
		else if (ack_data < 0)
			thermal_ipi_printk("cmd(%d) return error(%d)\n",
				cmd, ack_data);
	...
	case THERMAL_IPI_LVTS_INIT_GRP1:
		thermal_data->cmd = cmd;
		thermal_ipi_printk("cmd(%d) lvts efuse to MCUPM (%d)\n",
				cmd, ack_data);
		...
		if (ret != 0)
			thermal_ipi_printk("sspm_ipi_send err cmd %d,ret:%d - %d\n",
					cmd, ret, ackData);
		else if (ackData < 0)
			thermal_ipi_printk("cmd(%d) return error(%d)\n",
				cmd, ackData);
	...
	case THERMAL_IPI_GET_TEMP:
		...
		ret = mtk_ipi_send_compl(&mcupm_ipidev, CH_S_PLATFORM,
			IPI_SEND_WAIT, thermal_data, THERMAL_SLOT_NUM, 2000);
		if (ret != 0)
			thermal_ipi_printk("send get_temp cmd(%d) error ret:%d\n",
				cmd, ret);
		else if (ack_data < 0)
			thermal_ipi_printk("cmd(%d) return error(%d)\n",
				cmd, ack_data);
	...
	case THERMAL_IPI_GET_BIG_FREQ_THRESHOLD:
		...
		if (ret != 0)
			thermal_ipi_printk("mtk_ipi_send_compl error ret:%d - %d\n",
					cmd, ret);
		else if (ackData < 0)
			thermal_ipi_printk("cmd(%d) return error(%d)\n",
				cmd, ackData);
	...
	case THERMAL_IPI_SET_DIS_THERMAL_THROTTLE:
		thermal_data->cmd = cmd;
		thermal_ipi_printk("cmd(%d) disable SSPM thermal throttle(%d)\n",
				cmd, ack_data);
	...
	case THERMAL_IPI_SUSPEND_RESUME_NOTIFY:
		thermal_data->cmd = cmd;
		//thermal_ipi_printk("cmd(%d) kernel suspend/resume(%d)\n",
		//		cmd, ack_data);
	...
	default:
		thermal_ipi_printk("cmd(%d) wrong!!\n", cmd);
		break;
	}
...
struct thermal_ipi_data *thermal_data, int *ackData)
{
	...
	case THERMAL_IPI_GET_ATM_GPU_LIMIT:
		...
		if ((ret != 0) || (ack_data < 0))
			thermal_ipi_printk("%s cmd %d ret %d ack %d\n",
	...
	default:
		thermal_ipi_printk("%s cmd %d err!\n", __func__, cmd);
		break;
	}
...
(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d %d", &arrayIndex, &bigCoreTj,
		...
		if ((arrayIndex >= BIG_CORE_THRESHOLD_ARRAY_SIZE) ||
			(arrayIndex < 0)) {
			thermal_ipi_printk("%s invalid array index: %s\n",
				__func__, desc);
	...
	thermal_ipi_printk("%s bad argument: %s\n", __func__, desc);
	return -EINVAL;

...
#ifdef CONFIG_ARM
		cmdq_err("channel request fail:%ld", PTR_ERR(client->chan));
#else
...
struct cmdq_pkt *cmdq_pkt_create(struct cmdq_client *client)
{
	...
	/* not support null client for this project */
	if (!client)
		return ERR_PTR(-EINVAL);
	...
	if (!pkt)
		return ERR_PTR(-ENOMEM);
	if (pkt->cur_pool.pool) {
		cmdq_err("create pool available: %p\n", pkt->cur_pool.pool);
		cmdq_err("cmplt size: %d\n", sizeof(struct completion));
	}
	memset(pkt, 0, sizeof(struct cmdq_pkt));
...
void cmdq_pkt_reset(struct cmdq_pkt *pkt)
{
	...
	u16 src_reg_idx, u32 mask)
{
	...
	if (addr < 0x20000) {
		cmdq_err("pkt:%p addr:%pa src_reg_idx:%#x mask:%#x thrd:%d",
	...
	u32 value, u32 mask)
{
	...
	if (addr < 0x20000) {
		cmdq_err("pkt:%p addr:%pa value:%#x mask:%#x thrd:%d",
			pkt, &addr, value, mask,
			cmdq_mbox_chan_id(client->chan));
		dump_stack();
	}
...
s32 cmdq_pkt_sleep(struct cmdq_pkt *pkt, u32 tick, u16 reg_gpr)
{
	...
	if (!pkt)
		return -EINVAL;
	cl = (struct cmdq_client *)pkt->cl;
	timeout_en = cmdq_mbox_get_base_pa(cl->chan) +
				CMDQ_TPR_TIMEOUT_EN;
...
s32 cmdq_pkt_finalize_loop(struct cmdq_pkt *pkt)
{
	...
	if (!pkt)
		return -EINVAL;
	client = (struct cmdq_client *)pkt->cl;
	id = cmdq_mbox_chan_id(client->chan);
...
static void cmdq_pkt_err_irq_dump(struct cmdq_pkt *pkt)
{
	...
	cmdq_util_err("pkt:%lx thread:%d pc:%pa",
		(unsigned long)pkt, thread_id, &pc);
	...
	if (inst) {
		...
		/* no inst available */
		cmdq_util_aee(mod,
			"%s(%s) instruction not available pc:%pa thread:%d",
...
void cmdq_pkt_err_dump_cb(struct cmdq_cb_data data)
{
	...
	gce_pa = cmdq_mbox_get_base_pa(client->chan);
	thread_id = cmdq_mbox_chan_id(client->chan);
	cmdq_util_dump_lock();
...
static int cmdq_pkt_wait_complete_loop(struct cmdq_pkt *pkt)
{
	...
	client = pkt->cl;
	item = pkt->flush_item;
	timeout_ms = cmdq_mbox_get_thread_timeout((void *)client->chan);

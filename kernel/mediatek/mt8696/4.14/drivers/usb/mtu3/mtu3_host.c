...
static void host_ports_num_get(struct ssusb_mtk *ssusb)
{
	...
	if (ssusb->support_u3_should && ssusb->u3_should_ports >= 0) {
		dev_info(ssusb->dev,
		   "host  u2_ports:%d, u3_ports:%d, u3_should_ports:%d\n",
		    ssusb->u2_ports, ssusb->u3_ports, ssusb->u3_should_ports);

		ssusb->u3_ports = MIN(ssusb->u3_ports, ssusb->u3_should_ports);
	} else

...
static const int ecc_strength_mt8518[] = {
...
static int nfiecc_wait_sector_decode_done(struct nfiecc *ecc, u32 sector)
{
	u32 secbit = BIT(sector);
	void *regs = ecc->res.regs;
	int ret, val;
...
static int nfiecc_wait_decode_done(struct nfiecc *ecc)
{
	...
	ret = nfiecc_wait_sector_decode_done(ecc, ecc->config.sectors - 1);
	if (ret)
		return ret;
...
static int nfiecc_get_decel(struct nfiecc *ecc,  u32 sector,
				u16 *decels, u32 *count)
{
	u32 err, offset, i;
	u32 loc;
				...
				u32 sectors)
{
	...
	for (i = start_sector; i < start_sector + sectors; i++) {
		...
		if (bitflips)
			pr_debug("bitflips:%d sect:%d start_sect:%d\n",
...
static struct nfiecc_caps nfiecc_caps_mt8518 = {
	.err_mask = 0x1f,
	.ecc_mode_shift = 5,
	.parity_bits = 14,
	.ecc_strength = ecc_strength_mt8518,
	.ecc_strength_num = 11,
};
...
struct nfiecc *nfiecc_init(struct nfiecc_resource *res)
{
	...
	ecc->wait_sector_done = nfiecc_wait_sector_decode_done;
	ecc->get_decel = nfiecc_get_decel;

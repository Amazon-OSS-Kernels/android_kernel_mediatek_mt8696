...
unsigned long cpufreq_cooling_get_level(unsigned int cpu, unsigned int freq)
{
	...
	list_for_each_entry(cpufreq_cdev, &cpufreq_cdev_list, node) {
		if (cpumask_test_cpu(cpu, cpufreq_cdev->policy->related_cpus)) {
			...
			pr_info("%s: cpu:%d freq:%d level:%ld\n", __func__, cpu, freq, level);
...
static int cpufreq_map_freq_to_state(struct thermal_cooling_device *cdev, unsigned long freq, unsigned long *state)
{

...
#define __ION_CACHE_OPTIMIZATION__

#ifdef __ION_CACHE_OPTIMIZATION__
#include <aee.h>
				 ...
				 int from_kernel)
{
	...
	if (!from_kernel) {
		...
		/* no need to check va from kernel */
	}
			       ...
			       int from_kernel)
{
	...
	if (sync_type < ION_CACHE_CLEAN_ALL) {
		/* By range operation */
		unsigned long start = (unsigned long)param->va;
		...
		if (sync_type < ION_CACHE_CLEAN_BY_RANGE_USE_VA &&
			...
			ret = __cache_sync_by_range(client, sync_type,
						    start, size, from_kernel);
#else
			ret = ion_cache_sync_kernel(start, size, sync_type);
#endif
		}

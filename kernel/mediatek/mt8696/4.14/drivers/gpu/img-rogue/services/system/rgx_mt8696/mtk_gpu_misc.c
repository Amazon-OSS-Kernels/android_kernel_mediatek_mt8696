...
static int freq_seq_show(struct seq_file *m, void *v)
{
	...
	mtk_get_gpu_memory_usage(&mem);

	seq_printf(m,
		"volt:%d, volt_sram %d, freq:%d, Loading %d, mem %d\n",
		volt, volt_sram, freq, loading, mem);
	...
	size_t count, loff_t *data)
{
	...
	unsigned long freq;
#ifdef PVR_DVFS
	struct devfreq *df = pvr_dvfs->sDVFSDevice.psDevFreq;
#endif
...
static int install_cmds(void)
{
	...
	if (pentry) {
		proc_create_data("freq", 0600, pentry, &freq_proc_fops, NULL);
		proc_create_data("regs", 0600, pentry, &regs_proc_fops, NULL);
		proc_create_data("dvfs", 0600, pentry, &dvfs_proc_fops, NULL);
	}
...
bool mtk_get_gpu_loading(unsigned int *pLoading)
{
	...
	dvfs_dev = &pvr_dvfs->sDVFSDevice;
	stat = &dvfs_dev->psDevFreq->last_status;
	snap_loading =
		(div_u64((u64)stat->busy_time * 100UL, (u64)stat->total_time));
...
static int gpu_max_power;
static int gpu_min_power;
int mt_gpufreq_get_max_power(void)
{
	return gpu_max_power;
}
EXPORT_SYMBOL(mt_gpufreq_get_max_power);

int mt_gpufreq_get_min_power(void)
{
...
void mtk_gpu_thermal_setup(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (i == 0)
			gpu_min_power = calc_power;
		gpu_max_power = calc_power;

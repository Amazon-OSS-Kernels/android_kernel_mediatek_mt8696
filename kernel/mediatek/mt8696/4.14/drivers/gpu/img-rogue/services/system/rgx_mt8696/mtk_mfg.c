...
static void mfg_power_on(void)
{
	...
	if (runtime_vgpu_ctrl)
		ret = mfg_regulator_enable(); /*FIXME: Process failure..*/
	...
	if (m) {
		...
		if (clk_pll && clk_mfg_sel) {
			...
			seq_printf(m, "Clk: %s %d, rate: %lu\n",
				...
				__clk_get_enable_count(clk_mfg_sel),
...
void mtk_mfg_set_freq(unsigned long freq, bool hopping)
{
	...
	/* Currently not using hopping, instead mux */
	if (pll_support_hopping && hopping) {
		unsigned long vco;
		/* Need to check div here, */
		vco = div64_u64((u64)freq * 4 * 0x4000UL, 26000000UL);
		mt_dfs_general_pll(FH_PLL1,  vco);
	}

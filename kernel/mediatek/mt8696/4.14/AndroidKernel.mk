...
ifneq ($(filter kara,$(TARGET_DEVICE)),)
KERNEL_ROOT_DIR := $(PWD)
...
ifneq ($(strip $(TARGET_NO_KERNEL)),true)
  ...
  ifeq ($(wildcard $(TARGET_PREBUILT_KERNEL)),)
    ...
    $(warning ================== KERNEL_ARCH =$(KERNEL_ARCH) KERNEL_TARGET_ARCH =$(KERNEL_TARGET_ARCH) TARGET_KERNEL_ARCH 0=$(TARGET_KERNEL_ARCH) TARGET_ARCH =$(TARGET_ARCH))    
    BUILT_KERNEL_TARGET := $(KERNEL_ZIMAGE_OUT).bin
    INSTALLED_KERNEL_TARGET := $(PRODUCT_OUT)/kernel
    TARGET_KERNEL_CONFIG := $(KERNEL_OUT)/.config
    KERNEL_HEADERS_INSTALL := $(KERNEL_OUT)/usr
    #KERNEL_CONFIG_FILE := $(KERNEL_DIR)/arch/arm64/configs/$(KERNEL_DEFCONFIG)
...
ifneq (yes,$(strip $(DISABLE_MTK_CONFIG_CHECK)))
	python device/mediatek/build/build_mt8696/tools/check_kernel_config.py -c $(MTK_TARGET_PROJECT_FOLDER)/ProjectConfig.mk -k $(KERNEL_CONFIG_FILE) -p $(MTK_PROJECT_NAME)
else
	-python device/mediatek/build/build_mt8696/tools/check_kernel_config.py -c $(MTK_TARGET_PROJECT_FOLDER)/ProjectConfig.mk -k $(KERNEL_CONFIG_FILE) -p $(MTK_PROJECT_NAME)
endif
...
ifneq (yes,$(strip $(DISABLE_MTK_CONFIG_CHECK)))
	python device/mediatek/build/build_mt8696/tools/check_kernel_config.py -c $(MTK_TARGET_PROJECT_FOLDER)/ProjectConfig.mk -k $(TARGET_KERNEL_CONFIG) -p $(MTK_PROJECT_NAME)
else
	-python device/mediatek/build/build_mt8696/tools/check_kernel_config.py -c $(MTK_TARGET_PROJECT_FOLDER)/ProjectConfig.mk -k $(TARGET_KERNEL_CONFIG) -p $(MTK_PROJECT_NAME)
endif

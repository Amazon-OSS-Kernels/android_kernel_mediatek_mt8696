/*
 * Mediatek EARC driver

 *
 * Author:	Yulan Zhang,  <yulan.zhang@mediatek.com>
...
struct obj_t {
	...
	unsigned long int log_fifo_size;

	/* Controls which events will cause notification handler */
	/* to be called */
	unsigned long int event_flags_mask;

	/* Event Flags status*/
	unsigned long int event_flags_status;
	...
	unsigned long int earc_hpd_toggle_time;
...
{
	EARC_LOG("cec_enable_arc: u1EnArc:%d, earc_poweron:%d\n", u1EnArc, earc_poweron);
...
static const struct soc_enum earc_enalbe_mode_enum = SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(earc_enable_modes),\
			earc_enable_modes);
...
static int earc_dir_probe(struct snd_soc_codec *codec)
{
	int ret = 0;
...
static const struct snd_soc_codec_driver earc_dir = {
...
void earc_hb_detect_hpd(int high)
{
	...
	if (high == TRUE) {
		...
		if (earc_test_value == 0x10) {
			EARC_LOG("deglitch reset reset reset reset!!");
			vWriteEarcMsk(COMMA_TOP_CONFIG_2,
...
void earc_audio_mute(int enable)
{
	...
	if (enable == 1) {
		earcmute = 0x1;
		EARC_POLL_LOG("earc audio mute!\n");
	} else if (enable == 0) {
		earcmute = 0x0;
		EARC_POLL_LOG("earc audio unmute!\n");
	}
}
...
void earc_poll_isr(unsigned long int n)
{
	int	u32IntSta;
	bool b_hpd_in;
	bool EarcValid;
	...
	if (u32IntSta & 0x2000000) {
		...
		EARC_LOG("audiofmt: %d; channo: %d; codectype: %d\n",
			audiofmt, channelno, codectype);
		/*Earc_statble_reg();*/
	...
	if ((csmute == 0) && (caldone == 1) && (stableunmute == 1)) {
		csmute = 1;
		if (hbdetdone == 1) {
			hbdetdone = 0;
			vWriteEarcMsk(COMMA_TOP_CONFIG_2,
...
void Earc_valid_signal(void)
{
	...
	if (((freqValue >= 0xAA) && (freqValue <= 0x1068)) &&
...
void Earc_Channel_Status(void)
{
	...
	u32ChsL1 = bReadEarc(AUDIO_RX_CHAN_LEFT_1);
	u32ChanStatus = ((u32ChsL1 & 0x38) >> 3) || (u32ChsL1 & 0x03);
...
static int earc_probe(struct platform_device *pdev)
{
	...
	return snd_soc_register_codec(&pdev->dev, &earc_dir,
			&earc_dai, 1);
}
